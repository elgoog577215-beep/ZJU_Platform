...
if ((0, semver_1.lt)(process_1.default.version, LOWEST_NODE_VERSION)) {
    base_log_js_1.default.error(
        `current node version: ${process_1.default.version}, node version cannot be lower than ${LOWEST_NODE_VERSION}`
    );
    (0, exit_js_1.exit)(-1);
}
...
function checkDaemonCli(cliArgv) {
    return (
        !cliArgv.stopDaemon && // --stop-daemon
    ...
    ); // daemon模式或未设置默认为daemon
}
async function startDaemonIfNeeded() {
    if (checkDaemonCli(cliOptions)) {
        await Promise.resolve()
            .then(() =>
                __importStar(require("../src/base/daemon/process/daemon-process-factory.js"))
            )
            .then((module) => {
                if (cliOptions.Xmx) {
