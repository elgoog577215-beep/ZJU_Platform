...
const apiUrl = (server, endpoint) =>
    `${normalizeServer(server)}${endpoint.startsWith("/api") ? endpoint : `/api${endpoint}`}`;
...
const requestJson = async ({ server, endpoint, token, method = "GET", body, formData }) => {
    ...
    if (!response.ok) {
        const detail =
            data?.error || data?.message || data?.errors?.[0]?.msg || `HTTP ${response.status}`;
