...
这里仅提供一级目录导航；详细模块、数据真源和运行链查看[系统架构](./docs/系统架构.md)。
...
生产部署由 [deploy.yml](./.github/workflows/deploy.yml) 在 `master` 推送后触发。部署链、运行边界和故障判断见[系统架构](./docs/系统架构.md)；本地构建与检查命令见上文。
