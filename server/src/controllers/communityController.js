...
const appendMaterialTypeFilter = (whereClauses, whereParams, materialType) => {
    ...
    if (materialType === "course") {
        whereClauses.push(
            `LOWER(COALESCE(NULLIF(TRIM(material_type), ''), 'other')) IN (${["course", ...LEGACY_COURSE_MATERIAL_TYPES].map(() => "?").join(", ")})`
        );
    ...
    whereClauses.push(
        `LOWER(COALESCE(NULLIF(TRIM(material_type), ''), 'other')) NOT IN (${["course", "ai", ...LEGACY_COURSE_MATERIAL_TYPES].map(() => "?").join(", ")})`
    );
...
const normalizeContentStatus = (
    value,
    { section, user = {}, role = user?.role || "user", intent = "submit" } = {}
) => {
...
const reportPostContent = async (req, res, next) => {
    try {
        ...
        const targetType = String(req.body.target_type || "post")
            .trim()
            .toLowerCase();
...
const deletePost = async (req, res, next) => {
    try {
        ...
        const post = await db.get(
            "SELECT id, author_id, status FROM community_posts WHERE id = ?",
            [id]
        );
...
const restorePost = async (req, res, next) => {
    try {
        ...
        const post = await db.get(
            'SELECT * FROM community_posts WHERE id = ? AND status = "deleted"',
            [id]
        );
        if (!post) {
            return res.status(404).json({ error: "Post not found" });
...
const listPosts = async (req, res, next) => {
    try {
        ...
        const rawSection = String(req.query.section || "")
            .trim()
            .toLowerCase();
        ...
        const status = String(req.query.status || "")
            .trim()
            .toLowerCase();
        const workflowStatus = String(req.query.workflow_status || req.query.review_status || "")
            .trim()
            .toLowerCase();
        ...
        const rawMaterialType = String(req.query.material_type || "")
            .trim()
            .toLowerCase();
        const materialType =
            section === "materials" && MATERIAL_TYPES.has(rawMaterialType) ? rawMaterialType : null;
        ...
        if (isAdmin) {
            if (
                workflowStatus &&
                workflowStatus !== "all" &&
                CONTENT_STATUSES.has(workflowStatus)
            ) {
                ...
                !validatePostStatus(section, status)
            ) {
                whereClauses.push("status = ?");
            ...
            ownAuthorScope &&
            ["all", "draft", "pending", "rejected", "approved"].includes(workflowStatus || status)
        ) {
        ...
        if (q.length >= 2) {
            const term = `%${q}%`;
            whereClauses.push(
                "(title LIKE ? OR content LIKE ? OR tags LIKE ? OR author_name LIKE ? OR material_course LIKE ? OR material_teacher LIKE ? OR material_semester LIKE ? OR material_type LIKE ?)"
            );
...
const createPost = async (req, res, next) => {
    try {
        ...
        const maxMembers =
            section === "team" && Number.isInteger(Number(maxMembersRaw))
        ...
        const materialCourse =
            section === "materials"
                ...
                : null;
        const materialTeacher =
            section === "materials"
                ? sanitizeShortText(mutableBody.material_teacher, 60) || null
                : null;
        const materialSemester =
            section === "materials"
                ? sanitizeShortText(mutableBody.material_semester, 40) || null
                : null;
        const materialType =
            section === "materials" ? normalizeMaterialType(mutableBody.material_type) : null;
    ...
    try {
        ...
        try {
            if (existing) {
                ...
                await db.run("INSERT INTO community_post_likes (post_id, user_id) VALUES (?, ?)", [
                    id,
                    userId,
                ]);
                liked = true;
            ...
            floors.map((item) => ({
                ...
                replies: (repliesMap.get(item.id) || [])
                    ...
                    .map((reply) => {
                        ...
                        const replyTarget = reply.reply_to_comment_id
                            ? commentById.get(reply.reply_to_comment_id)
                            : null;
                        return {
                            ...base,
                            reply_to_author: replyTarget
                                ? replyTarget.author || replyTarget.author_name || "匿名用户"
                                : null,
                        };
                    }),
            }))
...
const createPostComment = async (req, res, next) => {
    try {
        ...
        const post = await db.get(
            'SELECT id, author_id, title, section FROM community_posts WHERE id = ? AND status = "approved"',
            [id]
        );
        if (!post) {
            return res.status(404).json({ error: "Post not found" });
        ...
        const user = await db.get("SELECT username, nickname, avatar FROM users WHERE id = ?", [
            userId,
        ]);
        const rawParentId = req.body.parent_id ? parseInt(req.body.parent_id, 10) : null;
        const rawReplyToCommentId = req.body.reply_to_comment_id
            ? parseInt(req.body.reply_to_comment_id, 10)
            : null;
        let parentId = Number.isInteger(rawParentId) && rawParentId > 0 ? rawParentId : null;
        let replyToCommentId =
            Number.isInteger(rawReplyToCommentId) && rawReplyToCommentId > 0
                ? rawReplyToCommentId
                : null;
        let rootId = null;
...
const updatePostStatus = async (req, res, next) => {
    try {
        ...
        const status = String(req.body.status || "")
            ...
            .toLowerCase();
        const post = await db.get(
            'SELECT * FROM community_posts WHERE id = ? AND status = "approved"',
            [id]
        );
        if (!post) {
            return res.status(404).json({ error: "Post not found" });
...
const joinTeamPost = async (req, res, next) => {
    try {
        ...
        if (maxMembers > 0 && currentMembers >= maxMembers) {
            await db.run(
                'UPDATE community_posts SET post_status = "full", updated_at = datetime("now") WHERE id = ?',
                [id]
            );
...
const leaveTeamPost = async (req, res, next) => {
    try {
        ...
        if (post.author_id === userId)
            return res.status(400).json({ error: "Post owner cannot leave" });
...
const listGroups = async (req, res, next) => {
    try {
        const db = await getDb();
        const requestedReviewStatus = String(req.query.review_status || "approved")
            .trim()
            .toLowerCase();
        ...
        const rows = await db.all(
            `SELECT * FROM community_groups ${where} ORDER BY is_recommended DESC, sort_order DESC, id DESC`,
            params
        );
...
const updatePost = async (req, res, next) => {
    try {
        ...
        normalizeLinkagePayload(mutableBody, { strict: true });
        const title =
            mutableBody.title !== undefined
                ? sanitizeCommunityText(String(mutableBody.title || ""))
                : existing.title;
        const content =
            mutableBody.content !== undefined
        ...
        const postStatus = mutableBody.post_status
            ? String(mutableBody.post_status || "")
                  .trim()
                  .toLowerCase()
        ...
        const nextStatus =
            mutableBody.status !== undefined ||
            mutableBody.intent !== undefined ||
            mutableBody.submit_intent !== undefined
        ...
        const materialCourse =
            isMaterialPost && mutableBody.material_course !== undefined
                ? sanitizeShortText(mutableBody.material_course, 80) || null
                : existing.material_course || null;
        const materialTeacher =
            isMaterialPost && mutableBody.material_teacher !== undefined
                ? sanitizeShortText(mutableBody.material_teacher, 60) || null
                : existing.material_teacher || null;
        const materialSemester =
            isMaterialPost && mutableBody.material_semester !== undefined
                ? sanitizeShortText(mutableBody.material_semester, 40) || null
                : existing.material_semester || null;
        const materialType =
            isMaterialPost && mutableBody.material_type !== undefined
...
const getGroup = async (req, res, next) => {
    try {
        ...
        const viewer = viewerFromReq(req);
        const linked = await attachLinkedResources(db, serializeGroup(row), {
            includePrimaryTags: true,
            viewer,
        });
        res.json(linked);
...
const createGroup = async (req, res, next) => {
    try {
        ...
        if (qr_code_url && !normalizedQr)
            return res.status(400).json({ error: "Invalid QR code URL" });
        if (invite_link && !normalizedInvite)
...
const updateGroup = async (req, res, next) => {
    try {
        ...
        const normalizedPlatform =
            platform !== undefined ? normalizeGroupPlatform(platform) : group.platform;
        const normalizedQr =
            qr_code_url !== undefined ? normalizeHttpUrl(qr_code_url) : group.qr_code_url;
        const normalizedInvite =
            invite_link !== undefined ? normalizeHttpUrl(invite_link) : group.invite_link;
        const normalizedValidUntil =
            valid_until !== undefined ? normalizeValidUntil(valid_until) : group.valid_until;
        if (!normalizedPlatform) return res.status(400).json({ error: "Invalid platform type" });
        if (qr_code_url !== undefined && qr_code_url && !normalizedQr)
            return res.status(400).json({ error: "Invalid QR code URL" });
        if (invite_link !== undefined && invite_link && !normalizedInvite)
            return res.status(400).json({ error: "Invalid invite link URL" });
        if (valid_until !== undefined && valid_until && !normalizedValidUntil)
            return res.status(400).json({ error: "Invalid valid_until date" });
        const expiredFlag = deriveExpiredFlag(
            normalizedValidUntil,
...
const adminCommunityStats = async (req, res, next) => {
    try {
        ...
        const approvedPosts = await db.get(
            'SELECT COUNT(*) as count FROM community_posts WHERE status = "approved"'
        );
        const rejectedPosts = await db.get(
            'SELECT COUNT(*) as count FROM community_posts WHERE status = "rejected"'
        );
    ...
    try {
        ...
        await db.run(
            ...
            [
                newStatus,
                action === "reject"
                    ? sanitizeCommunityText(String(reason || "")).slice(0, 500) || null
                    : null,
                id,
            ]
        );
