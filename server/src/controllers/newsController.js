...
const buildExcerpt = (value = "") => String(value).replace(/\s+/g, " ").trim().slice(0, 180);
...
const getNews = async (req, res, next) => {
    try {
      ...
      `,
        ...
        if (item.status === "approved") {
            await db.run(
                "UPDATE news SET views_count = COALESCE(views_count, 0) + 1, hot_score = COALESCE(hot_score, 0) + 1 WHERE id = ?",
                [id]
            );
        }
...
const checkNewsSourceHealth = async (req, res, next) => {
    try {
        ...
        const { id } = req.params;
        const item = await db.get(
            "SELECT id, source_url FROM news WHERE id = ? AND deleted_at IS NULL",
            [id]
        );
        if (!item) {
            return res.status(404).json({ error: "News item not found" });
...
const updateNews = async (req, res, next) => {
    try {
        ...
        const userId = req.user?.id;
        const { id } = req.params;
        const existing = await db.get("SELECT * FROM news WHERE id = ? AND deleted_at IS NULL", [
            id,
        ]);
        ...
        const publisherProfileId =
            req.body.publisher_profile_id !== undefined
                ? await profileService.resolvePublisherProfileId(
                      db,
                      userId,
                      req.body.publisher_profile_id,
                      req.user?.role
      ...
      `,
            [
                ...
                payload.status === "pending" || payload.status === "approved"
                    ? null
                    : payload.rejection_reason || existing.rejection_reason || null,
                payload.related_article_ids,
                payload.related_post_ids,
                payload.related_news_ids,
                payload.related_group_ids,
...
const deleteNews = async (req, res, next) => {
    try {
        ...
        await db.run(
            "UPDATE news SET deleted_at = datetime('now'), updated_at = datetime('now') WHERE id = ?",
            [id]
        );
...
const restoreNews = async (req, res, next) => {
    try {
        ...
        const existing = await db.get(
            "SELECT * FROM news WHERE id = ? AND deleted_at IS NOT NULL",
            [id]
        );
        if (!existing) return res.status(404).json({ error: "News item not found" });
        if (req.user?.role !== "admin" && existing.uploader_id !== userId) {
...
const reviewNews = async (req, res, next) => {
    try {
        ...
        const reason = String(req.body.reason || req.body.rejection_reason || "")
            ...
            .slice(0, 500);
        await db.run(
            "UPDATE news SET status = ?, rejection_reason = ?, updated_at = datetime('now') WHERE id = ?",
...
const importNews = async (req, res, next) => {
    try {
        ...
        const sourceUrl = normalizeSourceUrl(
            String(req.body.source_url || req.body.url || "").trim()
        );
        ...
        const duplicate = await db.get(
            "SELECT id FROM news WHERE source_url = ? AND deleted_at IS NULL",
            [sourceUrl]
        );
        if (duplicate) {
