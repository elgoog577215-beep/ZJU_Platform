...
const registerFutureLearning = async (req, res, next) => {
    ...
    const { topic, name, age, gender, organization, email, phone, message } = validation.data;
...
const updateRegistration = async (req, res, next) => {
    ...
    try {
        const db = await getDb();
        const existing = await db.get("SELECT id FROM future_learning_registrations WHERE id = ?", [
            req.params.id,
        ]);
        if (!existing) {
       ...
       WHERE id = ?`,
        ...
        const updated = await db.get("SELECT * FROM future_learning_registrations WHERE id = ?", [
            req.params.id,
        ]);
        res.json(updated);
    } catch (error) {
...
const deleteRegistration = async (req, res, next) => {
    try {
        const db = await getDb();
        await db.run("DELETE FROM future_learning_registrations WHERE id = ?", [req.params.id]);
