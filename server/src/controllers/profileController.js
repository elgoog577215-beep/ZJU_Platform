...
const normalizeAdminProfilePayload = (body = {}, existing = {}) => {
    const requestedType = String(body.type || existing.type || "organization")
        .trim()
        .toLowerCase();
    ...
    const displayName = String(body.display_name || body.name || existing.display_name || "")
        .trim()
        .slice(0, 120);
    if (!displayName) {
    ...
    const requestedStatus = String(body.status || existing.status || "active")
        .trim()
        .toLowerCase();
    ...
    return {
        ...
        display_name_en:
            String(body.display_name_en ?? body.name_en ?? existing.display_name_en ?? "")
                .trim()
                .slice(0, 160) || null,
        avatar_url:
            String(body.avatar_url ?? existing.avatar_url ?? "")
                .trim()
                .slice(0, 1000) || null,
        logo_url:
            String(body.logo_url ?? existing.logo_url ?? "")
                .trim()
                .slice(0, 1000) || null,
        cover_url:
            String(body.cover_url ?? existing.cover_url ?? "")
                .trim()
                .slice(0, 1000) || null,
        bio:
            String(body.bio ?? existing.bio ?? "")
                .trim()
                .slice(0, 500) || null,
        description:
            String(body.description ?? existing.description ?? "")
                ...
                .slice(0, 1200) || null,
        description_en:
            String(body.description_en ?? existing.description_en ?? "")
                .trim()
                .slice(0, 1200) || null,
        cooperation_direction:
            String(body.cooperation_direction ?? existing.cooperation_direction ?? "")
                .trim()
                .slice(0, 500) || null,
        cooperation_direction_en:
            String(body.cooperation_direction_en ?? existing.cooperation_direction_en ?? "")
                .trim()
                .slice(0, 500) || null,
        link_url:
            String(body.link_url ?? existing.link_url ?? "")
                .trim()
                .slice(0, 1000) || null,
...
const normalizeEditableProfilePayload = (body = {}, existing = {}) => {
    ...
    const payload = {
        display_name: displayName,
        display_name_en:
            String(body.display_name_en ?? existing.display_name_en ?? "")
                .trim()
                .slice(0, 160) || null,
        avatar_url:
            String(body.avatar_url ?? existing.avatar_url ?? "")
...
const listProfiles = async (req, res, next) => {
    try {
        ...
        if (type) {
            ...
            clauses.push(
                `p.type IN (${Array.from(DIRECTORY_PROFILE_TYPES)
                    .map(() => "?")
                    .join(",")})`
            );
...
const getProfileFeed = async (req, res, next) => {
    try {
         ...
         LIMIT ?`,
            ...
            rows.forEach((row) =>
                items.push(
                    feedItem(row, "event", {
                        relation:
                            row.publisher_profile_id === profile.id ? "published" : "organized",
                    })
                )
            );
        }
        ...
        const sorted = items
            .sort(
                (left, right) =>
                    new Date(right.date || right.created_at || 0) -
                    new Date(left.date || left.created_at || 0)
            )
...
const updateOwnProfile = async (req, res, next) => {
    try {
       ...
       WHERE id = ?`,
        ...
        await syncEcosystemPartnerFromProfile(
            db,
            updated,
            {
                ...updated,
    ...
    try {
       ...
       LIMIT 200`
        ...
        const profiles = await Promise.all(
            rows.map(async (row) =>
                profileService.serializeProfile(row, await profileService.listAliases(db, row.id))
            )
        );
...
const deleteAdminProfileMember = async (req, res, next) => {
    try {
        ...
        const profile = await profileService.findProfileById(db, profileId);
        if (!profile) return res.status(404).json({ error: "Profile not found" });
        await db.run(`DELETE FROM profile_members WHERE profile_id = ? AND user_id = ?`, [
            profileId,
            userId,
        ]);
