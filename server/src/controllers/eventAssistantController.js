    ...
    try {
        ...
        if (clarificationAnswer && typeof clarificationAnswer !== "string") {
            throw createAssistantError(
                ...
                "Clarification answer must be a string.",
                400
            );
        }

        if (
            typeof clarificationAnswer === "string" &&
            clarificationAnswer.trim().length > MAX_CLARIFICATION_LENGTH
        ) {
            throw createAssistantError(
                "EVENT_ASSISTANT_BAD_REQUEST",
                "Clarification answer is too long.",
                400
            );
        }
...
const updateEventAssistantPreferences = async (req, res) => {
    try {
        ...
        const savedPreference = await userProfileService.updateEventAssistantPreference(
            db,
            req.user.id,
            preference
        );
