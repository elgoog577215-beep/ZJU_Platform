...
const restoreOwnHandler = (table) => async (req, res, next) => {
    try {
        ...
        const item = await db.get(`SELECT id, uploader_id, deleted_at FROM ${table} WHERE id = ?`, [
            id,
        ]);
        if (!item) {
...
const updateHandler = (table, fields) => async (req, res, next) => {
    try {
        ...
        if (
            table === "events" &&
            (req.body.organizer_profile_id !== undefined || req.body.organizer !== undefined)
        ) {
            ...
            if (req.user.role === "admin") {
                await db.run(
                    ...
                    [
                        ...
                        id,
                        "permanent_delete",
                        "Admin permanently deleted resource",
                    ]
                );
            }
...
const getAllHandler =
    ...
    async (req, res, next) => {
        try {
            ...
            if (!isAdmin) {
                const ownUploaderScope = !!(
                    userId &&
                    effectiveUploaderId &&
                    Number(userId) === Number(effectiveUploaderId)
                );
                ...
                filterableFields.forEach((field) => {
                    if (req.query[field]) {
                        if (field === "target_audience") {
                            const collegeScopeFilter = buildEventCollegeScopeFilter(
                                req.query[field]
                            );
            ...
            if (table !== "events" && tagsQuery && tagsQuery.trim() !== "") {
                const tagsList = tagsQuery
                    .split(",")
                    .map((t) => t.trim())
                    .filter(Boolean);
                if (tagsList.length > 0) {
                    // Optimize tag matching to avoid substring matches (e.g. 'art' matching 'smart')
                    const tagConditions = tagsList
                        .map(() => "(tags = ? OR tags LIKE ? OR tags LIKE ? OR tags LIKE ?)")
                        .join(" OR ");
...
const updateStatus = (table) => async (req, res, next) => {
    try {
        ...
        await db.run(`UPDATE ${table} SET status = ?, rejection_reason = ? WHERE id = ?`, [
            status,
            reason || null,
            id,
        ]);
...
const getEventDistinctOptions = async (req, res, next) => {
    try {
        ...
        const [locationRows, organizerRows, audienceRows, sourceCollegeRows, noticeTypeRows] =
            await Promise.all(
        ...
        if (table === "events") {
            ...
            if (tags.length > 0) {
                const tagConditions = tags
                    .map(() => "(tags LIKE ? OR tags LIKE ? OR tags LIKE ? OR tags LIKE ?)")
                    .join(" OR ");
