...
const registerEvent = async (req, res, next) => {
    try {
        ...
        if (existing) {
            ...
            // Register
            await db.run("INSERT INTO event_registrations (event_id, user_id) VALUES (?, ?)", [
                id,
                userId,
            ]);
    ...
    try {
        ...
        if (req.user?.role === "admin") {
            const existingEvent = await db.get(
                "SELECT COALESCE(views, 0) as views FROM events WHERE id = ?",
                [id]
            );
        ...
        const event = await db.get(
            "SELECT id, COALESCE(views, 0) as views FROM events WHERE id = ?",
            [id]
        );
        if (!event) {
            return res.status(404).json({ error: "Event not found" });
