...
const extractWorkStoryFields = (body, options = {}) => {
    ...
    return {
        ...
        storyFileUrl,
        publicConsent:
            body.public_consent === undefined && body.publicConsent === undefined
                ? 1
...
const makeUniqueSlug = async (db, input, excludeId = null) => {
    ...
    while (true) {
        const existing = excludeId
            ? await db.get("SELECT id FROM competitions WHERE slug = ? AND id != ?", [
                  candidate,
                  excludeId,
              ])
       ...
       WHERE id = ? AND deleted_at IS NULL`,
            [
                ...
                req.body.event_date !== undefined || req.body.eventDate !== undefined
                    ? nullableText(req.body.event_date || req.body.eventDate, 80)
                    : existing.event_date,
                req.body.cover_image !== undefined || req.body.coverImage !== undefined
                    ? coverImage
                    : existing.cover_image,
                req.body.status !== undefined
                    ? normalizeCompetitionStatus(req.body.status, existing.status)
                    : existing.status,
                id,
            ]
        );
        ...
        if (req.body.is_featured !== undefined) {
            if (toBooleanInt(req.body.is_featured)) {
                ...
                await db.run(
                    'UPDATE competitions SET is_featured = 0, updated_at = datetime("now") WHERE id = ?',
                    [id]
                );
            }
        }
...
const setFeaturedCompetition = async (db, id) => {
    ...
    try {
        await db.exec("BEGIN TRANSACTION");
        await db.run(
            'UPDATE competitions SET is_featured = 0, updated_at = datetime("now") WHERE deleted_at IS NULL'
        );
        await db.run(
    ...
    try {
        ...
        try {
            ...
            await createAuditLog(
                ...
                id,
                "soft_delete",
                "Admin soft deleted competition outcome bundle"
            );
            await db.exec("COMMIT");
        } catch (error) {
            await db.exec("ROLLBACK");
...
const createAdminMedia = async (req, res, next) => {
    try {
        ...
        if (!MEDIA_TYPES.has(type)) return sendBadRequest(res, "请选择有效的成果类型");
        if (!title) return sendBadRequest(res, "请填写标题");
        if (!isSafeAssetUrl(url, { allowExternal: true }))
            return sendBadRequest(res, "请填写有效的素材地址或上传文件");
...
const updateAdminMedia = async (req, res, next) => {
    try {
        ...
        const id = toInteger(req.params.id, 0);
        const existing = await db.get(
            "SELECT * FROM competition_media WHERE id = ? AND deleted_at IS NULL",
            [id]
        );
        if (!existing) return res.status(404).json({ error: "素材不存在" });

        const competitionId =
            req.body.competition_id !== undefined || req.body.competitionId !== undefined
                ? toInteger(
                      req.body.competition_id || req.body.competitionId,
        ...
        const reviewedBy =
            status !== existing.status && (status === "approved" || status === "rejected")
                ? req.user.id
...
const deleteAdminMedia = async (req, res, next) => {
    try {
        ...
        const id = toInteger(req.params.id, 0);
        const existing = await db.get(
            "SELECT id FROM competition_media WHERE id = ? AND deleted_at IS NULL",
            [id]
        );
        if (!existing) return res.status(404).json({ error: "素材不存在" });

        await db.run(
            'UPDATE competition_media SET deleted_at = datetime("now"), updated_at = datetime("now") WHERE id = ?',
            [id]
        );
    ...
    try {
       ...
       WHERE id = ?`,
        ...
        await notifyReviewResult({
            ...
            reason,
            adminId: req.user.id,
            resourceType: "competition_media",
        });
...
const updateAdminWork = async (req, res, next) => {
    try {
        ...
        const title = req.body.title !== undefined ? trimText(req.body.title, 140) : existing.title;
        const author =
            req.body.author !== undefined ? trimText(req.body.author, 140) : existing.author;
        const summary =
            req.body.summary !== undefined ? trimText(req.body.summary, 1200) : existing.summary;
        const gitUrl =
       ...
       WHERE id = ? AND deleted_at IS NULL`,
            [
                ...
                coverUrl,
                req.body.honor_title !== undefined || req.body.honorTitle !== undefined
                    ? story.honorTitle
                    : existing.honor_title,
                ...
                req.body.story_file_url !== undefined || req.body.storyFileUrl !== undefined
                    ? story.storyFileUrl
                    : existing.story_file_url,
                req.body.public_consent !== undefined || req.body.publicConsent !== undefined
                    ? story.publicConsent
                    : existing.public_consent,
                req.body.sort_order !== undefined
                    ? toInteger(req.body.sort_order, 0)
                    : existing.sort_order,
                status,
...
const deleteAdminWork = async (req, res, next) => {
    try {
        ...
        const id = toInteger(req.params.id, 0);
        const existing = await db.get(
            "SELECT id FROM competition_works WHERE id = ? AND deleted_at IS NULL",
            [id]
        );
        if (!existing) return res.status(404).json({ error: "作品不存在" });

        await db.run(
            'UPDATE competition_works SET deleted_at = datetime("now"), updated_at = datetime("now") WHERE id = ?',
            [id]
        );
    ...
    try {
       ...
       WHERE id = ?`,
        ...
        await notifyReviewResult({
            ...
            reason,
            adminId: req.user.id,
            resourceType: "competition_works",
        });
        return res.json({ success: true, id, status, reason });
    } catch (error) {
        return next(error);
