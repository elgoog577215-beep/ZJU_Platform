...
class DatabasePool {
    ...
    async backup(backupPath) {
        ...
        const backupFile =
            backupPath || path.join(path.dirname(DB_CONFIG.filename), `backup-${timestamp}.sqlite`);
    ...
    async getStats() {
        ...
        const dbSize = fs.existsSync(DB_CONFIG.filename) ? fs.statSync(DB_CONFIG.filename).size : 0;
