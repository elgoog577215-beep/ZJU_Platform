...
async function runMigrations(db) {
    ...
    try {
        ...
        if (userColumns.length > 0) {
            ...
            if (!userColumns.includes("review_permission")) {
                await db.exec(
                    `ALTER TABLE users ADD COLUMN review_permission TEXT DEFAULT 'normal'`
                );
    ...
    `);
        ...
        const profileCardColumnAdds = [
            [
                "card_type",
                `ALTER TABLE user_profile_cards ADD COLUMN card_type TEXT DEFAULT 'other'`,
            ],
            ...
            [
                "aspect_ratio",
                `ALTER TABLE user_profile_cards ADD COLUMN aspect_ratio TEXT DEFAULT 'wide'`,
            ],
        ];
    ...
    `);
        ...
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_site_visit_events_date_key ON site_visit_events(date_key)`
        );
    ...
    `);
        ...
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_event_view_events_event_id ON event_view_events(event_id)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_event_view_events_visitor_key ON event_view_events(visitor_key)`
        );
    ...
    `);
        ...
        if (registrationColumns.length > 0 && !registrationColumns.includes("created_at")) {
            await db.exec(
                `ALTER TABLE event_registrations ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP`
            );
    ...
    `);
        ...
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_posts_section_status_created ON community_posts(section, status, created_at DESC)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_posts_author ON community_posts(author_id)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_posts_likes ON community_posts(likes_count DESC, created_at DESC)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_posts_post_status ON community_posts(section, post_status, created_at DESC)`
        );
        await db.exec(
        ...
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_post_members_post ON community_post_members(post_id)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_post_members_user ON community_post_members(user_id)`
        );
    ...
    try {
        ...
        if (cpColumns.length > 0 && !cpColumns.includes("current_members")) {
            await db.exec(
                `ALTER TABLE community_posts ADD COLUMN current_members INTEGER DEFAULT 0`
            );
    ...
    try {
        ...
        if (groupColumns.length > 0 && !groupColumns.includes("review_status")) {
            await db.exec(
                `ALTER TABLE community_groups ADD COLUMN review_status TEXT DEFAULT 'approved'`
            );
        }
        if (groupColumns.length > 0 && !groupColumns.includes("is_recommended")) {
            await db.exec(
                `ALTER TABLE community_groups ADD COLUMN is_recommended INTEGER DEFAULT 0`
            );
        }
    ...
    `);
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_reports_post ON community_reports(post_id, created_at DESC)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_reports_reporter ON community_reports(reporter_id, created_at DESC)`
        );
        await db.exec(
            `CREATE UNIQUE INDEX IF NOT EXISTS idx_community_reports_unique_target ON community_reports(post_id, COALESCE(comment_id, 0), reporter_id, target_type)`
        );
    ...
    `);
        ...
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_news_hot_score ON news(status, is_pinned DESC, pin_weight DESC, hot_score DESC, created_at DESC)`
        );
    ...
    `);
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_metrics_type_date ON community_metrics_events(metric_type, date_key)`
        );
        await db.exec(
            `CREATE INDEX IF NOT EXISTS idx_community_metrics_source ON community_metrics_events(source_type, source_id, date_key)`
        );
    ...
    `);
        ...
        if ((partnerCount?.count || 0) === 0) {
            const defaults = [
                ...
                [
                    ...
                    "MiniMax",
                    "提供模型能力、技术资源与生态支持。",
                    "/images/partner-logos/minimax.png",
                    "/images/partner-logos/minimax-dark.png",
                    null,
                    10,
                ],
                [
                    ...
                    "ModelScope 魔搭社区",
                    "提供模型社区、技术资源与开发者生态支持。",
                    "/images/partner-logos/modelscope.png",
                    "/images/partner-logos/modelscope-dark.png",
                    null,
                    20,
                ],
                [
                    ...
                    "Bonjour",
                    "提供数字名片与合作传播支持。",
                    "/images/partner-logos/company-3.png",
                    "/images/partner-logos/company-3-dark.png",
                    null,
                    30,
                ],
                [
                    ...
                    "阿里云",
                    "提供云资源与技术基础设施支持。",
                    "/images/partner-logos/aliyun-cn.svg?v=2",
                    "/images/partner-logos/aliyun-cn-white.svg?v=2",
                    null,
                    40,
                ],
                [
                    ...
                    "Qoder",
                    "提供 AI 开发工具与工程实践支持。",
                    "/images/partner-logos/qoder.png",
                    "/images/partner-logos/qoder-dark.png",
                    null,
                    50,
                ],
                [
                    ...
                    "阶跃 StepFun",
                    "提供模型、平台与技术生态支持。",
                    "/images/partner-logos/stepfun.png",
                    "/images/partner-logos/stepfun-white.png",
                    null,
                    60,
                ],
            ];
        ...
        const organizationDefaults = [
            [
                ...
                "XLAB",
                "协同选手招募、志愿执行与赛后社群承接。",
                "Supports participant recruitment, volunteer coordination, and post-event community operations.",
                "活动组织 / 社群承接",
                "Event organization / Community operations",
                ["XLAB"],
                10,
            ],
            [
                ...
                "ZJUAI",
                "协同校园 AI 学习者与开发者社群连接。",
                "Connects campus AI learners and developer communities.",
                "AI 社群 / 开发者连接",
                "AI community / Developer connection",
                ["ZJUAI"],
                20,
            ],
            [
                ...
                "EAI",
                "协同活动运营、现场执行与实践人群组织。",
                "Supports event operations, on-site execution, and practitioner community organization.",
                "活动运营 / 现场执行",
            ...
            [
                ...
                "AIRA",
                "协同 AI 实践社群共建与项目交流。",
                "Supports AI practice community building and project exchange.",
                "AI 实践 / 项目交流",
                "AI practice / Project exchange",
                ["AIRA"],
                40,
            ],
            [
                ...
                "KAB",
                "协同创新创业人群组织与活动承接。",
                "Supports innovation and entrepreneurship groups, event organization, and campus outreach.",
                "创新创业 / 活动承接",
                "Innovation and entrepreneurship / Event support",
                ["KAB"],
                50,
            ],
            [
                "浙江大学本科生院",
                "ZJU Undergraduate School",
                "协同发布本科生培养、报名与校园成长相关活动信息。",
                "Supports undergraduate-facing activity distribution and student development opportunities.",
                "活动联动 / 信息触达",
                "Event coordination / Student reach",
                ["浙江大学本科生院", "本科生院"],
                110,
            ],
            [
                "浙江大学艺术与考古博物馆",
                "ZJU Museum of Art and Archaeology",
                "协同文化艺术、展览导览与博物馆教育活动触达。",
                "Supports cultural, exhibition, museum education, and guided activity outreach.",
                "文化艺术 / 展览共创",
                "Arts and culture / Exhibition co-creation",
                ["浙江大学艺术与考古博物馆", "艺术与考古博物馆"],
                120,
            ],
            [
                "浙江大学星辰汇",
                "ZJU Xingchenhui",
                "协同校园活动传播、青年成长与社群参与。",
                "Supports campus activity communication, youth development, and community participation.",
                "社群传播 / 青年成长",
                "Community communication / Youth development",
                ["浙江大学星辰汇", "星辰汇"],
                130,
            ],
            [
                "浙大出国交流资讯",
                "ZJU Global Exchange Info",
                "协同出国交流、国际项目与留学相关信息触达。",
                "Supports global exchange, international program, and study-abroad information outreach.",
                "国际交流 / 信息服务",
                "Global exchange / Information service",
                ["浙大出国交流资讯", "出国交流资讯"],
                140,
            ],
            [
                "浙江大学 CC98 论坛",
                "ZJU CC98 Forum",
                "协同校园社区讨论、活动扩散与学生信息互助。",
                "Supports campus forum discussion, activity distribution, and peer information exchange.",
                "社区扩散 / 学生互助",
                "Community distribution / Student support",
                ["浙江大学 CC98 论坛", "CC98", "浙江大学CC98论坛"],
                150,
            ],
            [
                "浙江大学红十字会",
                "ZJU Red Cross",
                "协同公益实践、志愿服务与健康安全相关活动。",
                "Supports public service, volunteering, health, and safety activities.",
                "公益实践 / 志愿服务",
            ...
            [
                "浙江大学图书馆",
                "ZJU Library",
                "协同信息素养、阅读推广与学习资源相关活动。",
                "Supports information literacy, reading promotion, and learning resource activities.",
                "学习资源 / 信息素养",
            ...
            [
                "浙大体育与艺术",
                "ZJU Sports and Arts",
                "协同体育艺术、校园美育与综合素质活动传播。",
                "Supports sports, arts, aesthetic education, and holistic development activities.",
                "体育艺术 / 素质拓展",
            ...
            [
                ...
                "ZJU Life",
                "协同校园生活服务、学生资讯与活动提醒。",
                "Supports campus life services, student information, and activity reminders.",
                "校园生活 / 信息触达",
                "Campus life / Information outreach",
                ["浙大生活"],
                190,
            ],
            [
                ...
                "ZJU Student Union",
                "协同学生权益、校园活动与组织动员。",
                "Supports student affairs, campus events, and student mobilization.",
                "学生组织 / 活动执行",
                "Student organization / Event execution",
                ["浙江大学学生会", "学生会"],
                200,
            ],
            [
                ...
                "ZJU Qiushi College",
                "协同书院育人、学业发展与校园活动触达。",
                "Supports college education, academic development, and campus activity outreach.",
                "书院协同 / 学生成长",
                "College coordination / Student growth",
                ["浙江大学求是学院", "求是学院"],
                210,
            ],
            [
                "浙江大学团委",
                "ZJU Youth League Committee",
                "协同团学活动、青年发展与志愿实践信息。",
                "Supports youth league activities, youth development, and volunteer practice information.",
                "团学活动 / 青年发展",
                "Youth league activities / Youth development",
                ["浙江大学团委", "团委"],
                220,
            ],
            [
                "浙大素拓 ZJUST",
                "ZJUST",
                "协同素质拓展、第二课堂与综合成长活动触达。",
                "Supports holistic development, second-classroom, and growth activities.",
                "素质拓展 / 第二课堂",
                "Holistic development / Second classroom",
                ["浙大素拓 ZJUST", "ZJUST", "浙大素拓"],
                230,
            ],
            [
                "浙大微学工",
                "ZJU Student Affairs",
                "协同学工资讯、学生服务与校园通知触达。",
                "Supports student affairs information, services, and campus notices.",
                "学工资讯 / 学生服务",
            ...
            [
                ...
                "Lantian College",
                "协同学园育人、学生发展与活动信息触达。",
                "Supports college community development, student growth, and activity outreach.",
                "学园协同 / 学生成长",
                "College coordination / Student growth",
                ["蓝田学园"],
                250,
            ...
            [
                "蓝田青年",
                "Lantian Youth",
                "协同蓝田学园青年活动、志愿实践与信息传播。",
                "Supports Lantian youth activities, volunteer practice, and information distribution.",
                "青年活动 / 志愿实践",
                "Youth activities / Volunteer practice",
                ["蓝田青年"],
                260,
            ],
            [
                "求是学院丹阳青溪学园",
                "Danyang Qingxi College",
                "协同学园社区、书院活动与学生成长信息。",
                "Supports college community activities and student development information.",
                "学园社区 / 活动共创",
                "College community / Activity co-creation",
                ["求是学院丹阳青溪学园", "丹阳青溪学园"],
                270,
            ],
            [
                ...
                "Yunfeng News",
                "协同学园资讯、校园活动与学生服务传播。",
                "Supports college information, campus activities, and student service communication.",
                "学园资讯 / 活动传播",
                "College news / Activity communication",
                ["云峰微讯"],
                280,
            ],
            [
                "浙大竺院人",
                "ZJU Chu Kochen College",
                "协同竺院资讯、荣誉教育与学生发展活动。",
                "Supports Chu Kochen College information, honors education, and student development activities.",
                "荣誉教育 / 学生成长",
                "Honors education / Student growth",
                ["浙大竺院人", "竺院人", "竺可桢学院"],
                290,
            ],
        ];
        ...
        const scopeMigration = await db.get("SELECT value FROM settings WHERE key = ? LIMIT 1", [
            scopeMigrationKey,
        ]);
        if (!scopeMigration) {
