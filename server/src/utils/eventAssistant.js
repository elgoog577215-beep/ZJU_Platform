...
const FEEDBACK_REASON_DEFINITIONS = [
    {
        value: "not_relevant",
        label: "不相关",
        patterns: [/不相关/, /不适合/, /irrelevant/i, /not relevant/i],
    },
    {
        value: "time_mismatch",
        label: "时间不合适",
        patterns: [/时间不合适/, /时间冲突/, /没时间/, /time mismatch/i, /wrong time/i],
    },
    {
        value: "location_mismatch",
        label: "地点不合适",
        patterns: [/地点不合适/, /校区不合适/, /太远/, /location mismatch/i, /wrong location/i],
    },
    {
        value: "benefit_mismatch",
        label: "收益不符合",
        patterns: [
            /收益不符合/,
            /收益不匹配/,
...
const uniqueTextArray = (value, maxItems = 12, itemMaxLength = 80) => {
    const list = Array.isArray(value) ? value : sanitizeText(value, 1000).split(/[,，、;；\s\/|]+/);
    return unique(
        list.map((item) => sanitizeText(String(item || ""), itemMaxLength)).filter(Boolean)
    ).slice(0, maxItems);
};
...
const normalizeSearchText = (...values) =>
    values.map((value) => sanitizeText(value, 1000).toLowerCase()).join(" ");
...
const includesAnyPhrase = (haystack, needles) =>
    needles.some((needle) => includesPhrase(haystack, needle));

const isPersonalizationSignal = (signal) =>
    /行动证据|收藏|报名|正反馈|负反馈/.test(String(signal || ""));

const pickPersonalizationSignals = (signals = []) => signals.filter(isPersonalizationSignal);

const getBenefitMatch = (item, benefit) => {
    const text = buildCandidateSemanticText(item);
    if (benefit === "score") {
        return getComprehensiveEvaluationSignal(item.event) ||
            includesAnyPhrase(text, BENEFIT_ALIASES.score)
            ? "综测/加分"
            : "";
    }
...
const getComprehensiveEvaluationSignal = (event) => {
    ...
    const hasKeyword = BENEFIT_ALIASES.score.some((keyword) =>
        allText.includes(keyword.toLowerCase())
    );

    if (!hasKeyword) return "";
    if (scoreText && !isNumericRatingText(scoreText))
        return `含综测信息：${sanitizeText(scoreText, 36)}`;
    return "活动信息提到综测/加分";
};

const hasExplicitTime = (value) =>
    typeof value === "string" && /^\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}/.test(value.trim());
...
const classifyEventScope = (event, now = new Date()) => {
    ...
    const ongoingEnd = hasTimedEnd ? resolvedEnd : getEndOfDay(resolvedEnd);
...
const detectOrganizerTerms = (text) => {
    ...
    const detected = terms.filter((item) => {
        ...
        if (value.includes("科学与技术") && normalized.includes(value.replace("科学与技术", "")))
            return true;
        return false;
    });
...
const parseAssistantIntent = ({ query, clarificationAnswer }) => {
    ...
    const rawTokens = splitTokens(combined)
        ...
        .filter(
            (token) => !["活动", "推荐", "帮我", "想找", "一个", "一些", "有没有"].includes(token)
        );
    const topics = unique([
...
const normalizeAiIntent = (rawIntent = {}, fallbackIntent) => {
    ...
    const normalized = {
        ...
        topics: cleanTopicTerms(
            [...uniqueTextArray(rawIntent.topics, 12), ...fallbackIntent.topics],
            {
                campuses: [...uniqueTextArray(rawIntent.campuses, 8), ...fallbackIntent.campuses],
                audiences: [
        ...
        clarificationOptions: uniqueTextArray(
            rawIntent.clarification_options || rawIntent.options,
            4,
            80
        ),
                ...
                content: JSON.stringify(
                    {
                        ...
                        outputContract: {
                            ...
                            date_constraints: [
                                "today/tomorrow/this_week/weekend/specific date terms",
                            ],
                            format: "online/offline/hybrid/empty string",
                            hard_constraints: [
                                "explicit must-have constraints from the user, Chinese short phrases",
                            ],
...
const validateModelResult = (rawResult, context) => {
    ...
    if (rawResult.type === "clarify") {
        ...
        if (!question) {
            throw createAssistantError(
                "EVENT_ASSISTANT_MODEL_INVALID",
                "Clarification response did not include a valid question.",
                502
            );
        }
    ...
    if (!Array.isArray(rawResult.recommendations)) {
        throw createAssistantError(
            "EVENT_ASSISTANT_MODEL_INVALID",
            "Recommendation response did not include a recommendations array.",
            502
        );
    }
    ...
    if (recommendations.length < min) {
        throw createAssistantError(
            "EVENT_ASSISTANT_MODEL_INVALID",
            "Recommendation response did not include enough valid candidate IDs.",
            502
        );
    }
...
const summarizeWeightedCounts = (counts, limit = 8) =>
...
const buildActionEvidence = (
    ...
    return {
        ...
        negativeEventIds: unique([...negativeEventIds, ...negativeActionEventIds]).slice(0, 40),
...
const buildProfileSummary = (profile) => {
    ...
    if (profile.explicit.interestTags?.length)
        signals.push(`显式兴趣：${profile.explicit.interestTags.slice(0, 4).join("、")}`);
    if (profile.userSystem?.profileStatusLabel)
        signals.push(`个人状态：${profile.userSystem.profileStatusLabel}`);
    if (profile.userSystem?.tags?.length)
        signals.push(`个人名片标签：${profile.userSystem.tags.slice(0, 3).join("、")}`);
    if (profile.learned.categories?.length)
        signals.push(
            `历史偏好：${profile.learned.categories
                .slice(0, 3)
    ...
    if (profile.actionEvidence?.positiveCategories?.length) {
        signals.push(
            `行动证据偏好：${profile.actionEvidence.positiveCategories
                .slice(0, 3)
                .map((item) => CATEGORY_LABELS[item.value] || item.value)
                .join("、")}`
        );
    }
    ...
    if (profile.actionEvidence?.negativeCategories?.length) {
        signals.push(
            `近期负反馈：${profile.actionEvidence.negativeCategories
                .slice(0, 2)
                .map((item) => CATEGORY_LABELS[item.value] || item.value)
                .join("、")}`
        );
    ...
    if (profile.actionEvidence?.negativeReasons?.length) {
        signals.push(
            `负反馈原因：${profile.actionEvidence.negativeReasons
                .slice(0, 2)
                .map((item) => getFeedbackReasonLabel(item.value) || item.value)
                .join("、")}`
        );
    }
    if (profile.memory?.length)
        signals.push(
            `助手记忆：${profile.memory
                .slice(0, 2)
...
const getFeedbackReasonPenalty = ({ event, text, profile, intent }) => {
    ...
    addReasonPenalty(
        "time_mismatch",
        hasEventSpecificReason && eventReason === "time_mismatch",
        8,
        "时间不合适"
    );
    addReasonPenalty(
        "location_mismatch",
        Boolean(intent.campuses?.length) &&
            !scoreTextMatch(text, intent.campuses, 1, () => "").total,
        8,
        "地点不合适"
    );
    addReasonPenalty(
        ...
        Boolean(intent.benefits?.length) &&
            !intent.benefits.some((benefit) => {
                ...
                if (benefit === "volunteer_time")
                    return Boolean(sanitizeText(event.volunteer_time, 80));
    ...
    addReasonPenalty(
        "already_joined",
        profile.actionEvidence?.registeredEventIds?.includes(Number(event.id)),
        16,
        "已参加过"
    );
...
const scoreEvent = (event, intent, profile, scope, now = new Date()) => {
    ...
    const audienceMatch = scoreTextMatch(
        text,
        intent.audiences,
        12,
        (value) => `面向对象包含 ${value}`
    ...
    const positiveEvidence = getActionEvidenceCategory(
        profile,
        eventCategory,
        "positiveCategories"
    );
    if (positiveEvidence) {
        ...
        score += boost;
        signals.unshift(
            `行动证据显示你更常选择 ${CATEGORY_LABELS[eventCategory] || eventCategory}`
        );
    }

    const negativeEvidence = getActionEvidenceCategory(
        profile,
        eventCategory,
        "negativeCategories"
    );
    ...
    const prioritizedSignals = [
        ...signals.filter((signal) =>
            /综测|加分|志愿|收益|技能|作品集|主题|关键词|AI topic|score|skill|volunteer/i.test(
                String(signal)
            )
        ),
        ...signals.filter((signal) =>
            /校区|地点|主办方|学院|面向对象|形式|日期|时间|campus|organizer|audience|format|date/i.test(
                String(signal)
            )
        ),
        ...signals.filter((signal) =>
            /行动证据|收藏|报名|负反馈|profile|favorite|registration|feedback/i.test(String(signal))
        ),
        ...signals,
    ];
    ...
    const visibleSignals =
        personalizedSignal && !limitedSignals.some(isPersonalizationSignal)
...
const rankCandidates = (candidates, intent, profile, scope, now = new Date()) =>
    candidates
...
const scoreAiProfileCandidate = (rankedItem, intent, aiProfile) => {
    ...
    if (aiProfile) {
        if (intent.categories.includes(aiProfile.category)) {
            score += 26;
            signals.push(
                `AI画像类型匹配 ${CATEGORY_LABELS[aiProfile.category] || aiProfile.category}`
            );
        }
...
const buildCandidateSemanticText = (item) =>
    normalizeSearchText(
        item.event?.title,
        item.event?.description,
        item.event?.content,
...
const getFallbackIntentBoost = (item, intent = {}) => {
    ...
    const topicMatch = scoreTextMatch(
        ...
        intent.topics || [],
        12,
        (value) => `备用排序：主题匹配「${value}」`
    );
    boost += topicMatch.total;
    signals.push(...topicMatch.signals.slice(0, 3));

    const campusMatch = scoreTextMatch(
        text,
        intent.campuses || [],
        14,
        (value) => `备用排序：地点匹配 ${value}`
    );
...
const getEventTimeConstraintMatch = (item, dateConstraints = [], now = new Date()) => {
    ...
    const matched = normalizedTerms.some((term) => {
        ...
        if (["tomorrow", "明天"].includes(term))
            return start >= tomorrowStart && start <= tomorrowEnd;
        if (["this_week", "本周", "这周", "近几天"].includes(term))
            return start >= nowStart && start <= weekEnd;
        if (["weekend", "周末", "星期六", "星期日"].includes(term)) return day === 0 || day === 6;
        const dateMatch = term.match(/(\d{1,2})[./月-](\d{1,2})/);
        if (dateMatch) {
            return (
                start.getMonth() + 1 === Number(dateMatch[1]) &&
                start.getDate() === Number(dateMatch[2])
            );
        }
        return false;
    });
...
const getRankThreshold = (intent, scope) => {
    ...
    if (intent.categories.length === 0 && intent.topics.length <= 2 && intent.benefits.length === 0)
        return 6;
    return 10;
};
...
const buildReasoningTrace = ({
    ...
    const actionEvidenceSummary = {
        ...
        negativeActionCount:
            Number(actionTypeCounts.unfavorite || 0) +
    ...
    const actionEvidenceUsed = recommendations.some((item) =>
        (item.matchSignals || item.candidate?.signals || []).some(
            (signal) =>
                String(signal).includes("行动证据") ||
                String(signal).includes("收藏") ||
                String(signal).includes("报名") ||
                String(signal).includes("负反馈")
        )
    );
    ...
    const hasHardConstraintDiagnostics = recommendations.some(
        (item) =>
    ...
    if (!intent.dateConstraints?.length && !intent.timePreference)
        weakOrMissing.push("时间偏好不明确");
    ...
    return {
        ...
        scoringFactors: {
            lifecycle: true,
            hardConstraints:
                hasIntentHardConstraint ||
                hasHardConstraintDiagnostics ||
                /匹配|硬约束|校区|地点|面向对象|主办方|学院|收益|形式|constraint|campus|organizer|audience|benefit|format/i.test(
                    signalText
                ),
            topicCategory:
                hasIntentTopicCategory ||
                /主题|类型|关键词|AI画像|topic|category|keyword|profile/i.test(signalText),
            userProfile:
                /你|画像|兴趣|收藏|报名|行动证据|profile|interest|favorite|registration|action evidence/i.test(
                    signalText
                ),
...
const buildDecisionHint = ({ item, index, recommendations }) => {
    ...
    if (index === 0 && recommendations.length > 1) {
        ...
        const betterHardConstraints =
            Number(item.diagnostics?.hardConstraintScore || 0) >=
            Number(next.diagnostics?.hardConstraintScore || 0);
...
const buildDecisionSupport = ({ item, index, recommendations, matched, missing, uncertainty }) => {
    ...
    const preferencePrompt =
        missing.length || uncertainty.length
            ? `如果要更准，可以补充${unique([...missing, ...uncertainty])
                  .slice(0, 2)
                  .join("、")}。`
            : "";
...
const buildOpportunityMatch = ({ item, index, recommendations, reasoningTrace }) => {
    ...
    const priorityMatched = [
        ...matchedSignals.filter((signal) =>
            /收益|主题|关键词|类型|AI topic|topic|skill|social|volunteer/i.test(String(signal))
        ),
        ...matchedSignals.filter((signal) =>
            /校区|地点|主办方|学院|面向对象|形式|日期|时间|campus|organizer|audience|format|date/i.test(
                String(signal)
    ...
    const uncertainty = unique([
        ...
        ...(item.aiProfile?.status && item.aiProfile.status !== "ready"
            ? ["活动画像可能不完整"]
            : []),
    ]).slice(0, 4);
    const feedbackSignals = matched.filter((signal) =>
        /负反馈|行动证据|收藏|报名/.test(String(signal))
    );
...
const attachOpportunityMatches = ({ recommendations, reasoningTrace }) =>
    recommendations.map((item, index, list) => ({
        ...item,
...
const buildIntentSummary = (intent, profile) => {
    ...
    if (intent.benefits.length)
        parts.push(`收益：${intent.benefits.map(getBenefitLabel).join("、")}`);
...
const maybeRememberPreference = async (db, userId, intent, rememberPreference) => {
    ...
    const content = sanitizeText(
        [
            intent.categories.length
                ? `类型偏好：${intent.categories.map((item) => CATEGORY_LABELS[item] || item).join("、")}`
                : "",
            intent.benefits.length
                ? `收益偏好：${intent.benefits.map(getBenefitLabel).join("、")}`
                : "",
            ...
            messages: [
                {
                    role: "system",
                    content:
                        "你是浙江大学活动推荐助手的文案层。只能根据服务端给你的候选活动和匹配信号输出 JSON，不得新增活动、链接、时间或未提供的事实。",
                },
                {
                    role: "user",
                    content: JSON.stringify(
                        buildModelPrompt({ intent, profile, rankedItems }),
                        null,
                        2
                    ),
                },
...
const runInjectedModelTurn = async ({
    ...
    let parsedResult;
    try {
        const modelOutput = normalizeModelRunnerOutput(
            await modelRunner({
                intent,
                scope: scopeInfo.scope,
        ...
        if (
            !["EVENT_ASSISTANT_MODEL_INVALID", "EVENT_ASSISTANT_MODEL_EMPTY"].includes(error?.code)
        ) {
            throw error;
        }
...
const buildAiRerankPrompt = ({ intent, profile, candidates, recommendationCount }) => ({
    ...
    candidates: candidates.map((item) => ({
        ...
        actionEvidence: {
            positiveCategoryWeight:
                getActionEvidenceCategory(profile, item.category, "positiveCategories")?.weight ||
                0,
            negativeCategoryWeight:
                getActionEvidenceCategory(profile, item.category, "negativeCategories")?.weight ||
                0,
            categoryActionWeight:
                getActionEvidenceCategory(profile, item.category, "positiveCategories")?.weight ||
                0,
            priorViewDetail: Boolean(
                profile.actionEvidence?.viewedEventIds?.includes(Number(item.event.id))
            ),
            priorFavoriteAction: Boolean(
                profile.actionEvidence?.recommendationFavoriteEventIds?.includes(
                    Number(item.event.id)
                )
            ),
            priorRegisterAction: Boolean(
                profile.actionEvidence?.recommendationRegisterEventIds?.includes(
                    Number(item.event.id)
                )
            ),
            priorNegativeAction: Boolean(
                profile.actionEvidence?.negativeActionEventIds?.includes(Number(item.event.id))
            ),
            priorPositiveAction: Boolean(
                profile.actionEvidence?.positiveEventIds?.includes(Number(item.event.id))
            ),
            priorNegativeFeedback: Boolean(
                profile.actionEvidence?.negativeEventIds?.includes(Number(item.event.id))
            ),
        },
        deterministicSignals: item.signals,
...
const normalizeRerankResult = (rawResult, candidateMap) => {
    if (!rawResult || typeof rawResult !== "object" || Array.isArray(rawResult)) {
        throw createAssistantError(
            "EVENT_ASSISTANT_MODEL_INVALID",
            "Model returned an invalid rerank object.",
            502
        );
    }
    ...
    if (!Array.isArray(rawResult.recommendations)) {
        throw createAssistantError(
            "EVENT_ASSISTANT_MODEL_INVALID",
            "Model rerank did not include recommendations.",
            502
        );
    }
    ...
    if (recommendations.length === 0) {
        throw createAssistantError(
            "EVENT_ASSISTANT_MODEL_INVALID",
            "Model rerank did not include any valid candidate IDs.",
            502
        );
    }
    ...
    return {
        ...
        reasoningTrace:
            rawResult.reasoning_trace && typeof rawResult.reasoning_trace === "object"
                ? {
                      rankingBasis: uniqueTextArray(
                          rawResult.reasoning_trace.ranking_basis ||
                              rawResult.reasoning_trace.rankingBasis,
                          6,
                          80
                      ),
                      uncertainty: uniqueTextArray(rawResult.reasoning_trace.uncertainty, 4, 80),
                      actionEvidenceUsed: Boolean(
                          rawResult.reasoning_trace.action_evidence_used ||
                          rawResult.reasoning_trace.actionEvidenceUsed
                      ),
                  }
                : null,
...
const completeRerankRecommendations = ({ recommendations = [], candidates = [] }) => {
    ...
    for (const candidate of fallbackCandidates) {
        ...
        next.push({
            ...
            confidence: Math.max(
                0.52,
                Math.min(0.82, (candidate.aiScore ?? candidate.score ?? 0) / 150 + 0.42)
            ),
...
const mergeRecommendationSignals = ({
    ...
    if (personalized.length && !limited.some(isPersonalizationSignal)) {
        return unique([...limited.slice(0, Math.max(0, limit - 1)), personalized[0]]).slice(
            0,
            limit
        );
    }
...
const rerankCandidatesWithModel = async ({ db, intent, profile, candidates, modelRunner }) => {
        ...
        result = await aiRuntime.callJson(db, {
            ...
            messages: [
                modelRequest.messages[0],
                {
                    ...
                    content: JSON.stringify({
                        instruction:
                            "Retry. Return compact valid JSON only. Rank by hardConstraintScore first, then semantic relevance.",
                        ...
                        candidates: candidates.map((item) => ({
                            ...
                            summary:
                                item.aiProfile?.summary ||
                                sanitizeText(item.event.description, 120),
                        })),
...
const buildAiRecommendationResponse = ({
    ...
    const candidateMap = new Map(candidates.map((item) => [Number(item.event.id), item]));
    const sortedByHardConstraints = [...candidates].sort(
        (left, right) =>
            (right.hardConstraint?.score || 0) - (left.hardConstraint?.score || 0) ||
            right.aiScore - left.aiScore ||
            compareByAscendingDate(left.event, right.event)
    ...
    if (
        ...
        const existingIndex = selected.findIndex(
            (item) => Number(item.id) === Number(best.event.id)
        );
        const promoted =
            existingIndex >= 0
    ...
    const backendCompletedRecommendationCount = recommendations.filter(
        (item) => item.diagnostics?.backendCompleted
    ).length;
    const feedbackLearningUsed = recommendations.some(
        (item) => item.opportunityMatch?.feedbackLearning?.used
    );
    ...
    return {
        ...
        reasoningTrace: {
            ...
            uncertainty: rerank.reasoningTrace?.uncertainty?.length
                ...
                : reasoningTrace.weakOrMissing,
            actionEvidenceUsed: Boolean(
                rerank.reasoningTrace?.actionEvidenceUsed || reasoningTrace.actionEvidenceUsed
            ),
    ...
    const ranked = candidates
        .map((item) => {
            ...
            return {
                ...
                signals: unique([...fallback.signals, ...(item.signals || [])]).slice(0, 8),
            };
        })
        .sort(
            (left, right) =>
    ...
    return {
        ...
        reasoningTrace: {
            ...
            actionEvidenceUsed: ranked.some((item) =>
                (item.signals || []).some(
                    (signal) =>
                        String(signal).includes("行动证据") || String(signal).includes("负反馈")
                )
            ),
        },
...
const buildFallbackRecommendationResponse = ({
    ...
    return {
        ...response,
        recommendationMode: usedHistoricalFallback
            ? "historical_fallback_ai_attempted"
            : "fallback_ai_attempted",
        warnings: unique([
            ...(response.warnings || []),
...
const buildRecommendationResponse = async ({
    ...
    const polish = await polishWithModel(
        ...
        intent,
        profile,
        selected.slice(0, MAX_MODEL_CANDIDATES)
    );
...
const buildClarificationResponse = ({
    ...
    const provisionalRecommendations = buildProvisionalClarificationRecommendations(
        grouped,
        intent,
        profile,
        now
...
const runUnifiedEventAssistantTurn = async ({
    ...
    const [profile, grouped] = await Promise.all([
        timeEventAssistantStep(timings, "profileLoadMs", () =>
            services.profile.load(db, userId, visitorKey)
        ),
        timeEventAssistantStep(timings, "candidateLoadMs", () =>
    ...
    let parsedIntent;
    try {
        parsedIntent = await timeEventAssistantStep(timings, "intentMs", () =>
            services.intent.parseWithModel({
                db,
                query,
                clarificationAnswer,
                profile,
    ...
    const remembered = await timeEventAssistantStep(timings, "preferenceMemoryMs", () =>
    ...
    if (
        ...
        return withPerformance(
            services.response.clarification({
                intent,
                profile,
                grouped,
                coverage,
    ...
    const pool = await timeEventAssistantStep(timings, "candidatePoolMs", () =>
        services.retrieval.buildCandidatePool({
            db,
            grouped,
            intent,
            profile,
    ...
    if (intentFailure && pool.candidates.length > 0) {
        return withPerformance(
            services.response.fallbackRecommendation({
                ...
                modelStatuses: [intentModelStatus, ...pool.modelStatuses],
    ...
    if (pool.candidates.length === 0) {
        return withPerformance({
            ...
            modelStatus: {
                ...
                profileStats: pool.profileStats,
                message:
                    "AI understood the request, but no candidate events were available for reranking.",
            },
        });
    }
    ...
    let rerank;
    try {
        rerank = await timeEventAssistantStep(timings, "rerankMs", () =>
            services.ranking.rerankWithModel({
                db,
                intent,
                profile,
                candidates: pool.candidates,
    ...
    return withPerformance(
        services.response.aiRecommendation({
            rerank,
            candidates: pool.candidates,
            intent,
            profile,
...
const recordEventAssistantRun = async (db, response = {}, userId = null) => {
    ...
    try {
        ...
        const recommendedEventRanks = recommendations
            ...
            .slice(0, MAX_RECOMMENDATIONS);
        const recommendedCategories = unique(
            recommendations
                .map((item) => item?.event?.category || item?.aiProfile?.category || "")
                .filter(Boolean)
        ).slice(0, 8);
        const profileStatuses = unique(
            recommendations.map((item) => item?.aiProfile?.status || "").filter(Boolean)
        ).slice(0, 6);
        ...
        const averageHardConstraintRatio = hardConstraintRatios.length
            ? Number(
                  (
                      hardConstraintRatios.reduce((sum, value) => sum + value, 0) /
                      hardConstraintRatios.length
                  ).toFixed(3)
              )
            : 1;
        const opportunityMissingCount = recommendations.reduce(
            (sum, item) => sum + Number(item.opportunityMatch?.missing?.length || 0),
            0
        );
        const opportunityMatchedCount = recommendations.reduce(
            (sum, item) => sum + Number(item.opportunityMatch?.matched?.length || 0),
            0
        );
        const backendCompletedRecommendationCount = recommendations.filter(
            (item) => item.diagnostics?.backendCompleted
        ).length;
        const opportunityFeedbackLearningUsed = recommendations.some(
            (item) => item.opportunityMatch?.feedbackLearning?.used
        );
        const decisionSupportNextActionCount = recommendations.filter(
            (item) => item.opportunityMatch?.decisionSupport?.nextAction
        ).length;
        const decisionSupportTradeoffCount = recommendations.reduce(
            (sum, item) =>
                sum + Number(item.opportunityMatch?.decisionSupport?.tradeoffs?.length || 0),
            0
        );
        const decisionSupportWatchoutCount = recommendations.reduce(
            (sum, item) =>
                sum + Number(item.opportunityMatch?.decisionSupport?.watchouts?.length || 0),
            0
        );
        const decisionSupportPreferencePromptCount = recommendations.filter(
            (item) => item.opportunityMatch?.decisionSupport?.preferencePrompt
        ).length;
      ...
      `,
            [
                ...
                JSON.stringify({
                    ...
                    actionEvidenceSourceCount: Number(
                        reasoningTrace.actionEvidenceSummary?.sourceCount || 0
                    ),
                    viewDetailEvidenceCount: Number(
                        reasoningTrace.actionEvidenceSummary?.viewDetailCount || 0
                    ),
                    favoriteActionEvidenceCount: Number(
                        reasoningTrace.actionEvidenceSummary?.favoriteActionCount || 0
                    ),
                    registerActionEvidenceCount: Number(
                        reasoningTrace.actionEvidenceSummary?.registerActionCount || 0
                    ),
                    negativeActionEvidenceCount: Number(
                        reasoningTrace.actionEvidenceSummary?.negativeActionCount || 0
                    ),
                    ...
                    clarificationOptionCount: Array.isArray(response.clarificationOptions)
                        ? response.clarificationOptions.length
                        : 0,
                    provisionalRecommendationCount: provisionalRecommendations.length,
                    weakSignalCount: Array.isArray(reasoningTrace.weakOrMissing)
                        ? reasoningTrace.weakOrMissing.length
                        : 0,
...
const validateRecommendationActionAttribution = async ({
    ...
    if (!row) {
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Assistant run does not exist.",
            400
        );
    }
    if (row.module && row.module !== "event_recommendation") {
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Assistant run does not belong to event recommendations.",
            400
        );
    }
    ...
    if (recommendedEventIds.length > 0 && !recommendedEventIds.includes(eventId)) {
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Event was not recommended by this assistant run.",
            400
        );
    }
    ...
    if (rankRecord && rankRecord.rank !== recommendationRank) {
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Recommendation rank does not match this assistant run.",
            400
        );
    }
    if (
        ...
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Recommendation rank is outside this assistant run.",
            400
        );
    }
};
...
const recordEventAssistantDecisionAction = async ({
    ...
    if (!EVENT_ASSISTANT_DECISION_ACTION_TYPES.has(normalizedActionType)) {
        throw createAssistantError(
            "EVENT_ASSISTANT_BAD_REQUEST",
            "Valid decision action is required.",
            400
        );
    }
...
const recordEventAssistantFeedback = async ({
    ...
    if (!userId) {
        throw createAssistantError(
            "EVENT_ASSISTANT_AUTH_REQUIRED",
            "Login is required for feedback.",
            401
        );
    }
    if (!Number.isInteger(normalizedEventId)) {
