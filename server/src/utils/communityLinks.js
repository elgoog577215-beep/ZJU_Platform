...
const parseRawIdTokens = (value, { strict = false, fieldName = "linkage ids" } = {}) => {
    ...
    if (typeof value !== "string") {
        if (strict)
            throw createValidationError(
                `Invalid ${fieldName}: must be an array or comma-separated string`
            );
        return [];
    }
    ...
    try {
        ...
        if (strict)
            throw createValidationError(`Invalid ${fieldName}: JSON value must be an array`);
        return [];
    } catch (_error) {
...
const normalizePrimaryTags = (value, { strict = false } = {}) => {
    ...
    if (!Array.isArray(value) && typeof value !== "string") {
        if (strict)
            throw createValidationError(
                "Invalid primary_tags: must be an array or comma-separated string"
            );
        return null;
    }
    ...
    const tags = [...new Set(raw.map((item) => String(item || "").trim()).filter(Boolean))].slice(
        0,
        MAX_PRIMARY_TAGS
    );
...
const normalizeLinkagePayload = (
    body = {},
    { includePrimaryTags = false, strict = false } = {}
) => {
...
const attachLinkedResources = async (
    db,
    item,
    { includePrimaryTags = false, viewer = null } = {}
) => {
    if (!item) return item;
