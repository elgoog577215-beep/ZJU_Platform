...
const detectMapValues = (query, dictionary) =>
...
const parseGlobalSearchQuery = (query, options = {}) => {
    ...
    const modules = unique(
        Object.entries(MODULE_KEYWORDS)
            .filter(([, aliases]) => includesAny(lowered, aliases))
            .map(([key]) => key)
    );
    const mediaTypes = unique(
        Object.entries(MEDIA_TYPE_KEYWORDS)
            .filter(([, aliases]) => includesAny(lowered, aliases))
            .map(([key]) => key)
    );
    const benefits = unique(
        Object.entries(BENEFIT_KEYWORDS)
            .filter(([, aliases]) => includesAny(lowered, aliases))
            .map(([key]) => key)
    );
...
const getExpandedEventKeywords = (parsed) =>
    unique([
        ...parsed.keywords,
...
const buildIndexedResultLink = (row) => {
    ...
    if (row.resource_type === "post") {
        const section =
            Array.isArray(facets.sections) && facets.sections[0] ? facets.sections[0] : "help";
...
const buildIndexedResultMeta = (row) => {
    ...
    if (row.resource_type === "event") {
        ...
        parts.push(
            (facets.sections || []).find((item) => SECTION_LABELS[item]) ||
                (facets.sections || [])[0]
        );
...
const searchEvents = async (db, parsed, limit = DEFAULT_LIMITS.events) => {
    ...
    if (parsed.campuses.length) {
        const campusClauses = parsed.campuses.flatMap(() => [
            "e.location LIKE ?",
            "e.description LIKE ?",
            "e.content LIKE ?",
        ]);
        ...
        parsed.campuses.forEach((campus) =>
            params.push(buildLike(campus), buildLike(campus), buildLike(campus))
        );
    }

    if (parsed.audiences.length) {
        const audienceClauses = parsed.audiences.flatMap(() => [
            "e.target_audience LIKE ?",
            "e.description LIKE ?",
        ]);
        where.push(`(${audienceClauses.join(" OR ")})`);
        parsed.audiences.forEach((audience) =>
            params.push(buildLike(audience), buildLike(audience))
        );
    }
    ...
    return rows.map((row) => {
        ...
        if (parsed.categories.includes(row.category))
            reasons.push(`活动类型：${getCategoryLabel(row.category) || row.category}`);
        if (!reasons.length) {
        ...
        parsed.campuses.forEach((campus) => {
            if (includesAny(`${row.location} ${row.description}`, [campus]))
                reasons.push(`校区：${campus}`);
        });
        parsed.audiences.forEach((audience) => {
            if (includesAny(`${row.target_audience} ${row.description}`, [audience]))
                reasons.push(`面向：${audience}`);
        });
        if (parsed.benefits.includes("score") && row.score)
            reasons.push(`含综测/加分：${sanitizeText(row.score, 32)}`);
        if (parsed.benefits.includes("volunteer_time") && row.volunteer_time)
            reasons.push(`含志愿时长：${sanitizeText(row.volunteer_time, 32)}`);
        if (parsed.timeRange) reasons.push(`时间：${parsed.timeRange.label}`);
        if (!reasons.length && eventKeywords.length)
            reasons.push(`关键词匹配：${eventKeywords.slice(0, 2).join("、")}`);
        ...
        return createResult({
            ...
            date: row.date,
            meta: [getCategoryLabel(row.category) || row.category, row.location, row.organizer]
                .filter(Boolean)
                .join(" · "),
...
const searchCommunity = async (db, parsed, limit = DEFAULT_LIMITS.community) => {
    ...
    const postKeywordClause = addKeywordConditions(
        ["p.title", "p.content", "p.tags", "p.section", "p.author_name"],
        keywords,
        postParams
    );
    ...
    const groupKeywordClause = addKeywordConditions(
        ["g.name", "g.description", "g.category", "g.platform", "g.primary_tags"],
        keywords,
        groupParams
    );
    ...
    const newsKeywordClause = addKeywordConditions(
        ["n.title", "n.excerpt", "n.content", "n.source_name"],
        keywords,
        newsParams
    );
    ...
    const articleResults = articles.map((row) =>
        createResult({
            id: row.id,
            type: "article",
            group: "community",
            ...
            match_reasons: [
                ...
                parsed.keywords.length
                    ? `关键词匹配：${parsed.keywords.slice(0, 2).join("、")}`
                    : "内容匹配",
            ],
        })
    );

    const postResults = posts.map((row) =>
        createResult({
            id: row.id,
            type: "post",
            group: "community",
            ...
            date: row.created_at,
            meta: [SECTION_LABELS[row.section] || "社区帖子", row.author_name]
                .filter(Boolean)
                .join(" · "),
            score: Number(row.likes_count || 0) + Number(row.comments_count || 0),
            match_reasons: [
                SECTION_LABELS[row.section] || "社区帖子",
                parsed.keywords.length
                    ? `关键词匹配：${parsed.keywords.slice(0, 2).join("、")}`
                    : "内容匹配",
            ],
    ...
    const groupResults = groups.map((row) =>
        createResult({
            id: row.id,
            type: "group",
            group: "community",
            ...
            meta: ["社群", row.platform, row.member_count ? `${row.member_count} 人` : ""]
                .filter(Boolean)
                .join(" · "),
    ...
    const newsResults = news.map((row) =>
        createResult({
            id: row.id,
            type: "news",
            group: "community",
...
const searchMedia = async (db, parsed, limit = DEFAULT_LIMITS.media) => {
    ...
    const photoResults = photos.map((row) =>
        createResult({
            id: row.id,
            type: "photo",
            group: "media",
    ...
    const videoResults = videos.map((row) =>
        createResult({
            id: row.id,
            type: "video",
            group: "media",
...
const buildLegacyResults = (results) =>
    results.map((item) => ({
        id: item.id,
        title: item.title,
        type: item.type,
...
const searchGlobalContent = async (db, query, options = {}) => {
    ...
    const searches = await Promise.all([
        ...
        !selectedModules || selectedModules.has("media")
            ...
            : [],
        resourceSearchIndexService
            .searchResourceIndex(db, parsed, {
