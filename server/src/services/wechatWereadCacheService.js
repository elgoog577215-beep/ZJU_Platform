...
const { localizeWechatArticleImages, classifyWechatArticleContent } = require("./wechatMpAdminService");
...
const readFeed = async (id, root) => {
    try {
        return JSON.parse(await fs.readFile(path.join(root, `${normalizeFeedId(id)}.json`), "utf8"));
    } catch (error) {
...
const fetchArticles = async ({ feedId, root = cacheRoot() } = {}) => {
    ...
    const articles = (feed.articles || []).filter((a) => isTrustedArticleLink(a.link))
        .map((a) => ({ ...a, create_time: a.published_at || "", time_text: a.published_at || "",
            collector_content_status: a.content_status, content_status: "not_fetched" }));
    return { articles, total: articles.length, checked_at: feed.checked_at };
};
const fetchArticleContent = async ({ feedId, url, root = cacheRoot(), localize = localizeWechatArticleImages } = {}) => {
    const feed = await readFeed(feedId, root);
    const article = feed.articles.find((a) => a.link === url && a.content_status === "ready");
    if (!article || !/^[a-f0-9]{64}\.json$/.test(article.body_file || "")) throw new Error("微信读书正文尚未就绪");
    const body = JSON.parse(await fs.readFile(path.join(root, "bodies", article.body_file), "utf8"));
    const html = sanitizeHtml(body.content_html);
    // Preserve extracted images even when downloading is paused for disk pressure.
    let result = { title: article.title, author: feed.name, url, coverImage: article.cover,
        contentText: body.content_text || "", contentHtml: html, images: body.images || [] };
    ...
    return { ...result, content_status: policy.imageOnly ? "image_only" : result.contentText ? "fetched" : "empty" };
};
