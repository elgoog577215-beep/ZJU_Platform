...
const AGENT_DEFINITIONS = [
    {
        ...
        promptTemplates: [
            {
                ...
                version: "v1",
                purpose:
                    "Parse user goal, campus, time, benefits, category, and clarification needs.",
            },
            {
                id: "event_profile",
                version: "v1",
        ...
        qualityBacklog: {
            auto_update:
                "Schedule generated spec/profile refresh after event catalog, prompt contract, or profile-version changes.",
            safety_cost:
                "Expose per-agent latency, failure rate, and token-budget trend signals in the admin console.",
        },
        maturity: {
    ...
    },
    {
        ...
        promptTemplates: [
            {
                ...
                version: "v1",
                purpose:
                    "Produce suitability, project direction, preparation plan, risk, and event-day strategy.",
            },
        ],
        reasoningChain: [
        ...
        outputContracts: ["hackathon_coach"],
        validation: [
        ...
        qualityBacklog: {
            standard_libraries:
                "Move hackathon rules, event-day strategy cards, and track vocabulary into a versioned standard library.",
            memory_feedback:
                "Summarize anonymous question categories and coaching outcomes for product tuning without storing raw prompts.",
            observability:
                "Expose aggregate coaching intent categories, confidence buckets, and fallback rate in the admin overview.",
            auto_update:
                "Version the hackathon context-card index separately from code and show the active version in generated specs.",
            safety_cost:
                "Add task-level timeout and token ceilings for hackathon coaching calls in the runtime policy registry.",
        },
        maturity: {
            prompt_templates: "complete",
            reasoning_chain: "complete",
    ...
    },
    {
        ...
        promptTemplates: [
            {
                ...
                version: "v1",
                purpose:
                    "Extract title, dates, location, organizer, audience, category, content, and metadata.",
            },
        ],
        reasoningChain: [
        ...
        outputContracts: ["parsed_event_payload"],
        validation: [
        ...
        qualityBacklog: {
            context_index:
                "Persist bounded parse-cache metadata and confidence summaries so repeated URLs do not require full re-analysis.",
            memory_feedback:
                "Aggregate parse failure categories and admin corrections for prompt and scraper tuning.",
            observability:
                "Show parser confidence, warnings, provider, and model status directly in the admin form.",
            auto_update:
                "Refresh parser prompt context automatically when event catalog or academic-calendar context changes.",
            safety_cost:
                "Expose parser timeout, scrape failure, model failure, and fallback rates in the admin overview.",
        },
        maturity: {
            prompt_templates: "complete",
            reasoning_chain: "complete",
    ...
    },
    {
        ...
        promptTemplates: [
            {
                ...
                version: "v1",
                purpose:
                    "Review low-confidence or ambiguous governance suggestions against standard catalogs and recent admin decisions.",
            },
        ],
        reasoningChain: [
        ...
        outputContracts: ["governance_scan", "event_governance_review", "governance_apply_summary"],
        validation: [
        ...
        qualityBacklog: {
            context_index:
                "Expose changed-field coverage and recent ambiguous-case distribution in the governance workspace.",
            memory_feedback:
                "Use recent applied and skipped admin decisions to adjust deterministic confidence before model review.",
            auto_update:
                "Regenerate governance guidance when catalog aliases, whitelist fields, or confidence policy changes.",
        },
        maturity: {
            prompt_templates: "complete",
            reasoning_chain: "complete",
    ...
    {
        id: "global_ai_search",
        title: "Global AI Search Agent",
        agentClass: "search_retrieval",
        productEntrance: "Global search palette",
        api: ["GET /api/search"],
        frontend: ["src/components/SearchPalette.jsx", "src/components/MediaLibrary.jsx"],
        backend: [
        ...
        promptTemplates: [
            {
                id: "global_search_query_parser",
                version: "v1-rule",
                purpose:
                    "Map query terms to modules, event categories, campuses, audiences, benefits, media type, and time range without model dependency.",
            },
        ],
        reasoningChain: [
        ...
        outputContracts: ["parsed_query", "groups", "results", "legacy", "index", "match_reasons"],
        validation: [
        ...
        autoUpdateTriggers: [
            "Global search result contract changes",
            "New public content modules",
        ...
        relatedAgents: [
            {
                id: "event_recommendation",
                relationship: "parallel_user_assistant",
                reason: "Global search finds resources across the site; event recommendation uses user profiles and action evidence for personalized activity suggestions.",
            },
            {
                id: "event_governance",
        ...
        qualityBacklog: {
            prompt_templates:
                "Replace rule-only parser with a bounded structured model parser when evaluation cases are available.",
            context_index:
                "Improve resource_search_index recall with model embeddings and scheduled refresh cadence.",
            memory_feedback:
                "Aggregate anonymous no-result and click-through signals for search quality tuning.",
            evaluation_checks:
                "Add deterministic fixture checks for events, community, media, no-result, and module-only queries.",
            observability:
                "Expose search group counts and no-result rates in the admin AI overview.",
            auto_update:
                "Regenerate search guidance when searchable modules or index schemas change.",
        },
        maturity: {
    ...
    },
    {
        ...
        contextIndexes: ["ai_model_configs", "task runtime policy registry"],
        outputContracts: ["callJson result", "modelStatus"],
        validation: [
        ...
        qualityBacklog: {
            prompt_templates:
                "Version shared quality and JSON repair prompt templates with explicit task owners.",
            standard_libraries:
                "Move runtime provider rules, task ceilings, and repair policy into a documented standard library.",
            memory_feedback:
                "Persist longer-term runtime telemetry trend windows and provider health history.",
            auto_update:
                "Regenerate runtime policy documentation when task policy or provider priority changes.",
        },
        maturity: {
            prompt_templates: "partial",
            reasoning_chain: "complete",
    ...
    },
    {
        ...
        promptTemplates: [
            {
                ...
                version: "v1",
                purpose:
                    "Extract summary, category, topics, campus, organizer, audience, benefits, and confidence.",
            },
        ],
        reasoningChain: [
        ...
        contextIndexes: ["event_ai_profiles", "ai_assistant_runs refresh summaries"],
        outputContracts: ["event_ai_profile"],
        validation: [
        ...
        qualityBacklog: {
            memory_feedback:
                "Track which profile issue reasons later correlate with poor recommendation feedback.",
            auto_update:
                "Schedule profile refresh after large event imports and catalog/profile-version changes.",
            safety_cost:
                "Add embedding-compatible profile fields without changing current consumers or increasing live request latency.",
        },
        maturity: {
            prompt_templates: "complete",
            reasoning_chain: "complete",
...
const scoreMaturity = (maturity = {}) => {
    ...
    const score = MATURITY_DIMENSIONS.reduce(
        (sum, dimension) => sum + (statusWeight[maturity[dimension.id]] || 0),
        0
    );
    return {
        score: clampScore(score / total),
        completeCount: MATURITY_DIMENSIONS.filter(
            (dimension) => maturity[dimension.id] === "complete"
        ).length,
        partialCount: MATURITY_DIMENSIONS.filter(
            (dimension) => maturity[dimension.id] === "partial"
        ).length,
        plannedCount: MATURITY_DIMENSIONS.filter(
            (dimension) => maturity[dimension.id] === "planned"
        ).length,
        missingCount: MATURITY_DIMENSIONS.filter(
            (dimension) => !maturity[dimension.id] || maturity[dimension.id] === "missing"
        ).length,
    };
};

const buildQualityProfile = (agent) =>
...
const getHighPriorityGaps = (agents) =>
...
const buildNextIterationPlan = (agents) =>
    getHighPriorityGaps(agents)
        .slice(0, 8)
        .map((gap, index) => {
            const agent = agents.find((item) => item.id === gap.agentId);
            const firstImprovement =
                agent?.nextImprovements?.[0] || `Improve ${gap.dimensionLabel}.`;
            return {
                order: index + 1,
...
const getPartialMaturityGaps = (agents) =>
    agents
        .flatMap((agent) => {
            const gaps = [];
            for (const dimension of MATURITY_DIMENSIONS) {
...
const buildContinuousImprovementPlan = (agents, health = {}) =>
    getPartialMaturityGaps(agents)
        .sort((left, right) => {
            ...
            return (
                (runtimeIssuePriority[leftRuntime] ?? 10) -
                    (runtimeIssuePriority[rightRuntime] ?? 10) ||
                left.priority - right.priority ||
                left.agentTitle.localeCompare(right.agentTitle) ||
                left.dimensionLabel.localeCompare(right.dimensionLabel)
            );
...
const resolveAgentStatus = (agent, health = {}) => {
    if (agent.id === "event_recommendation")
        return Number(health.eventCount || 0) > 0 ? "live" : "attention";
    if (agent.id === "event_governance")
        return Number(health.uncategorizedEventCount || 0) > 0 ? "attention" : "ready";
    if (agent.id === "model_config_runtime")
        return Number(health.enabledModelConfigCount || 0) > 0 ? "ready" : "attention";
...
const buildAgentMetrics = (agent, health = {}) => {
    ...
    if (agent.id === "event_recommendation") {
        return [
            ...
            { label: "Runs", value: health.recommendationRunCount || 0 },
            {
                ...
                value: health.recommendationActionEvidenceStatus || "NO_RECOMMENDATION",
            },
            {
                label: "Action rate",
                value: `${Math.round((health.recommendationActionRate || 0) * 100)}%`,
            },
            ...base,
        ];
    }
    ...
    if (agent.id === "global_ai_search") {
        return [
            ...
            {
                label: "Coverage",
                value: `${Math.round((health.globalSearchIndexCoverageRatio || 0) * 100)}%`,
            },
    ...
    if (agent.id === "event_profile_index") {
        return [
            {
                label: "Coverage",
                value: `${Math.round((health.eventAiProfileCoverageRatio || 0) * 100)}%`,
            },
    ...
    if (agent.id === "hackathon_coach") {
        return [{ label: "Runs", value: health.hackathonRunCount || 0 }, ...base];
    }
    if (agent.id === "wechat_event_parser") {
        return [{ label: "Parse runs", value: health.wechatParseRunCount || 0 }, ...base];
    }
    return base;
};

const buildOverviewModules = (health = {}) =>
    getAgentDefinitions().map((agent) => ({
        id: agent.id,
        title: agent.title,
...
const getAgentSystemOverview = (health = {}) => {
    ...
    return {
        ...
        summary: {
            ...
            liveAgentCount: buildOverviewModules(health).filter((agent) => agent.status === "live")
                .length,
        },
        ...
        qualityProfiles: Object.fromEntries(
            agents.map((agent) => [agent.id, agent.qualityProfile])
        ),
...
const validateAgentRegistry = () => {
    ...
    for (const agent of AGENT_DEFINITIONS) {
        ...
        if (
            ...
            (!Array.isArray(agent.relatedAgents) || agent.relatedAgents.length === 0)
        ) {
        ...
        for (const key of Object.keys(agent.maturity || {})) {
            if (!dimensionIds.has(key))
                errors.push(`${agent.id} has unknown maturity dimension: ${key}`);
        }
    ...
    for (const agent of AGENT_DEFINITIONS) {
        for (const relatedAgent of agent.relatedAgents || []) {
            if (!relatedAgent.id) {
                ...
                errors.push(
                    `${agent.id}.relatedAgents references unknown agent: ${relatedAgent.id}`
                );
            }
            ...
            if (!relatedAgent.relationship) {
                errors.push(
                    `${agent.id}.relatedAgents.${relatedAgent.id} is missing relationship.`
                );
            }
...
const buildAgentSpecMarkdown = (health = {}) => {
        ...
        lines.push(
            ...
            agent.promptTemplates.length
                ? agent.promptTemplates
                      .map(
                          (template) =>
                              `- ${template.id} (${template.version}): ${template.purpose}`
                      )
                      .join("\n")
                : "- None yet",
            "",
    ...
    lines.push(
        ...
        "## Next Iteration Plan",
        "",
        ...(overview.nextIterationPlan.length > 0
            ? overview.nextIterationPlan
            : overview.continuousImprovementPlan
        ).flatMap((item) => [
