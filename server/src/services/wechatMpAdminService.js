...
const TOKEN_HEALTH_INVALID_TEXT =
    /invalid\s*session|session\s*(?:expired|invalid)|登录态|登录超时|重新登录|token\s*(?:is\s*)?invalid/i;
...
const extractAuthenticatedToken = (url) => {
    ...
    if (
        ["/cgi-bin/login", "/cgi-bin/bizlogin", "/cgi-bin/scanloginqrcode"].includes(
            parsed.pathname
        )
    ) {
        return "";
...
const normalizePacingOptions = (options = {}) => {
    ...
    const pagePauseRangeSeconds = normalizeDelayRangeSeconds(
        rawPagePauseRange,
        DEFAULT_PAGE_PAUSE_RANGE_SECONDS
    );
    return {
        queryDelayRangeSeconds: normalizeDelayRangeSeconds(
            rawQueryDelayRange,
            DEFAULT_QUERY_DELAY_RANGE_SECONDS
        ),
        contentDelayRangeSeconds: normalizeDelayRangeSeconds(
            rawContentDelayRange,
            DEFAULT_CONTENT_DELAY_RANGE_SECONDS
        ),
...
const waitDelayRange = async (range, runtime = {}) =>
    waitSeconds(randomSecondsInRange(range, runtime.random || Math.random), runtime);
...
const assertBrowserRuntimeReady = () => {
    ...
    if (!status.chromium_installed) {
        const error = new Error(
            "微信 MP 扫码登录强依赖 Playwright Chromium，请先运行 `npx playwright install chromium`。"
        );
...
const setTokenHealth = (credentials, status, { reason = "", message = "" } = {}) => {
    tokenHealthCache = {
...
const tokenHealthSignal = (value) => {
    ...
    const message =
        value?.message ||
        value?.response?.data?.base_resp?.err_msg ||
        value?.response?.data?.errmsg ||
        "";
    const body =
    ...
    return {
        expired:
            status === 401 ||
            TOKEN_HEALTH_INVALID_CODES.has(String(ret)) ||
            TOKEN_HEALTH_INVALID_TEXT.test(`${message} ${body}`),
...
const mpGetJson = async ({ path: apiPath, params, referer }) => {
    ...
    try {
        ...
        const wrapped = new Error(
            redactCredentials(error.response?.data?.errmsg || error.message, credentials)
        );
    ...
    for (const candidate of candidates) {
        ...
        if (
            ...
            (nickname === normalizedQuery ||
                ...
                normalizedQuery.includes(nickname))
        ) {
            return candidate;
        }
...
const parseMpArticlesPayload = (payload) => {
    ...
    for (const publishItem of publishList) {
        const publishInfo =
            decodeJsonValue(publishItem.publish_info || publishItem.publish_info_str) || {};
...
const extractArticleBody = (html) => {
    ...
    contentRoot.find("img").each((_index, element) => {
        uniquePush(
            images,
            trustedArticleImageUrl($(element).attr("data-src") || $(element).attr("src") || "")
        );
    });
...
const localizeWechatArticleImages = async (
    articleBody,
    { downloader = downloadWeChatImage } = {}
) => {
    ...
    if (contentHtml) {
        ...
        for (const element of imageElements) {
            const $image = $(element);
            const originalUrl = normalizeHttpsUrl(
...
const fetchArticleContents = async ({ articles = [], urls = [], pacing, runtime } = {}) => {
    const pacingOptions = pacingFromOptions(pacing);
    const inputs = [
        ...(Array.isArray(articles) ? articles : []),
        ...(Array.isArray(urls) ? urls : []),
    ]
        .map(normalizeContentInput)
