...
const normalizeText = (value) => trimText(value, 500).toLowerCase().replace(/\s+/g, "");
...
const loadCandidateEvents = (db, limit = 300) =>
    db.all(
...
const classifyEventForProfile = (event, profile, aliases) => {
    ...
    const contentText = [event.title, event.description, event.link, event.source_url]
        .filter(Boolean)
        .join("\n");
...
const applyEventAttributionMigration = async (db, options = {}) => {
    ...
    const candidates = Array.isArray(options.candidates)
        ? options.candidates
              ...
              .filter((candidate) => candidate.eventId && candidate.targetProfileId)
        : [];
