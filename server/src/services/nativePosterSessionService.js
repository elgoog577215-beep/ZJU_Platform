...
const decodePngDataUrl = (imageData) => {
  ...
  if (!match) {
    throw createHttpError('Poster image must be a PNG data URL', 400, 'NATIVE_POSTER_INVALID_IMAGE');
  }
...
const getNativePosterSession = async (sessionId, token) => {
  ...
  if (!session || session.token !== String(token || '')) {
    throw createHttpError('Native poster session not found', 404, 'NATIVE_POSTER_SESSION_NOT_FOUND');
  }
