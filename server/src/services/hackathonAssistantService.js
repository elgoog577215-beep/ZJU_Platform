...
const QUICK_CONTEXT_CARD_IDS = ["format", "tools", "judging", "deliverables"];

const DEFAULT_HACKATHON_PARTNERS = ["MiniMax", "阿里云 Qoder", "Bonjour", "魔搭", "阶跃星辰"];
...
const buildHackathonProfile = (settings = {}, ecosystemPartnerNames = null) => {
    const partnerNames = Array.isArray(ecosystemPartnerNames)
        ...
        : [...parsePartnerList(settings.hackathon_partners), ...DEFAULT_HACKATHON_PARTNERS];
    ...
    return {
        ...
        description:
            toText(settings.hackathon_desc, 500) ||
...
const buildContextCards = (profile) => [
    ...
    },
    {
        ...
        content:
            "允许使用 AI 开发工具。更推荐把 AI 用在需求拆解、代码生成、调试、测试、文案和数据处理上，而不是只做一个包装页面。",
    },
    {
        id: "judging",
        title: "评审重点",
    ...
    },
    {
        ...
        content:
            "新手更适合做范围小但闭环完整的工具，例如资料整理、活动推荐、简历润色、校园服务问答或自动化表单助手。",
    },
    {
        id: "builder",
        title: "工程路径",
        ...
        content:
            "有工程基础的参赛者可以做带数据、登录状态、AI 调用和结果保存的完整小应用，注意减少技术栈数量。",
    },
    {
        id: "product",
        title: "产品路径",
        ...
        content:
            "偏产品/设计的参赛者可以把目标放在场景选择、交互闭环、提示词体验和高质量样例上，用 AI 工具补齐实现。",
    },
    {
        id: "event_day",
        title: "现场策略",
        ...
        content:
            "现场建议先冻结范围，再做最短主流程，最后补样例、错误状态和演示数据。不要把时间耗在大而全的架构上。",
    },
    {
        id: "risk_scope",
        title: "范围风险",
        ...
        content:
            "最大风险是范围过大。建议只保留一个核心用户、一个关键输入、一个关键输出和一个可复用 AI 能力。",
    },
    {
        id: "registration",
        title: "报名信息",
...
const detectIntent = (query) => {
    ...
    if (
        ["适合", "能参加", "要不要", "可以参加", "水平"].some((item) => includesKeyword(text, item))
    ) {
        goals.push("suitability");
    }
    if (
        ["准备", "怎么做", "计划", "当天", "5小时", "时间"].some((item) =>
            includesKeyword(text, item)
        )
    ) {
        goals.push("preparation");
    }
    if (
        ["工具", "codex", "cursor", "claude", "trae", "qoder", "api"].some((item) =>
            includesKeyword(text, item)
        )
    ) {
        goals.push("tooling");
    ...
    if (
        ["不会前端", "前端不强", "不会代码", "零基础", "新手"].some((item) =>
            includesKeyword(text, item)
        )
    ) {
    ...
    if (
        ["python", "后端", "数据库", "接口", "react", "全栈"].some((item) =>
            includesKeyword(text, item)
        )
    ) {
    ...
    if (
        ["codex", "cursor", "claude", "trae", "qoder"].some((item) => includesKeyword(text, item))
    ) {
...
const scoreCard = (card, query, intent) => {
    ...
    if (intent.profileSignals.includes("beginner_or_non_frontend") && card.id === "beginner")
        score += 5;
...
const selectContextCards = (cards, query, intent) =>
    cards
...
const compactContextCards = (cards) =>
    cards.map((card) => ({
        id: card.id,
        title: card.title,
...
const buildFallbackResponse = ({
    ...
    return {
        ...
        warnings: ["本轮大模型没有完成可靠结构化输出，当前结果使用本地赛事画像和策略索引生成。"],
        modelStatus: {
            used: false,
            fallbackUsed: true,
...
const normalizeIntent = (rawIntent, fallbackIntent) => ({
    primaryGoal:
        toText(rawIntent?.primaryGoal || rawIntent?.primary_goal, 80) || fallbackIntent.primaryGoal,
    goals: unique([
        ...normalizeArray(rawIntent?.goals, 8).map((item) => toText(item, 80)),
        ...fallbackIntent.goals,
    ]).slice(0, 8),
    profileSignals: unique([
        ...normalizeArray(rawIntent?.profileSignals || rawIntent?.profile_signals, 8).map((item) =>
            toText(item, 80)
        ),
...
const normalizeModelResponse = ({
    ...
    recommendation: {
        ...
        fitScore: clampNumber(
            raw.recommendation?.fitScore || raw.recommendation?.fit_score,
            0,
            100,
            fallback.recommendation.fitScore
        ),
        rationale: toText(raw.recommendation?.rationale, 320) || fallback.recommendation.rationale,
        nextAction:
            toText(raw.recommendation?.nextAction || raw.recommendation?.next_action, 220) ||
            fallback.recommendation.nextAction,
    },
    prepPlan: normalizePrepPlan(raw.prepPlan || raw.prep_plan, fallback),
    strategy: {
        eventDay:
            toText(raw.strategy?.eventDay || raw.strategy?.event_day, 260) ||
    ...
    warnings: normalizeArray(raw.warnings, 4)
        .map((item) => toText(item, 160))
        .filter(Boolean),
    modelStatus: {
        ...(modelStatus || {}),
        used: true,
...
const buildOutputContract = () => ({
    ...
    sources: [{ id: "one of provided context card ids" }],
...
const runHackathonAssistant = async ({
    ...
    try {
        const taskPayload = {
            ...
            userPrompt:
                "Return only summary, recommendation, sources, and confidence. Keep the answer compact.",
        };
        let result;
        try {
            ...
            result = await runStructuredTask(db, {
                ...
                timeout: 30000,
                userPrompt:
                    "Retry once. Return only compact valid JSON with summary, recommendation, sources, confidence.",
            });
            result.modelStatus = {
                ...(result.modelStatus || {}),
                fallbackAttempts: [
