       ...
       WHERE wi.openid = ?`,
        ...
        if (!existingIdentity) {
            ...
            throw createPublicError(
                409,
                "Please sign in with your website account and bind WeChat first",
                "WECHAT_NOT_BOUND"
            );
        }
    ...
    try {
       ...
       WHERE ticket_hash = ?`,
        ...
        if (
            ...
            throw createPublicError(
                400,
                "WeChat binding ticket is invalid or expired",
                "WECHAT_BIND_TICKET_INVALID"
            );
        }
       ...
       WHERE openid = ?`,
        ...
        if (
            existingOpenidIdentity &&
            Number(existingOpenidIdentity.user_id) !== Number(bindTicket.user_id)
        ) {
            await db.exec("COMMIT");
            transactionOpen = false;
            throw createPublicError(
                409,
                "This WeChat account is already bound to another account",
       ...
       LIMIT 1`,
        ...
        if (existingUserIdentity && existingUserIdentity.openid !== session.openid) {
            ...
            throw createPublicError(
                409,
                "This website account already has a bound WeChat account",
                "WECHAT_USER_ALREADY_BOUND"
            );
        }
