...
const resolveLocalCover = async (coverUrl) => {
  ...
  if (!pathname.startsWith('/uploads/') || pathname.includes('\0') || pathname.includes('..')) return null;
...
const buildCoverFallback = (title) => Buffer.from(`
...
const renderProjectShareCard = async (project) => {
  ...
  const coverLayer = cover
    ? await sharp(cover).rotate().resize(WIDTH, COVER_HEIGHT, { fit: 'cover', position: 'north' }).png().toBuffer()
    : buildCoverFallback(project.title);

  return sharp({
    create: { width: WIDTH, height: HEIGHT, channels: 4, background: { r: 5, g: 11, b: 24, alpha: 1 } },
  })
    .composite([
