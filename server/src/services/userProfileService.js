...
const uniqueTextArray = (value, maxItems = 16, itemMaxLength = 60) => {
    const list = Array.isArray(value) ? value : sanitizeText(value, 1200).split(/[,，、;；\s/|]+/);
...
const normalizePreferencePayload = (payload = {}) => ({
    ...
    availability: sanitizeText(payload.availability, 160),
...
const loadUserProfileFoundation = async (db, userId) => {
    ...
    return {
        ...
        explicit: {
            ...
            availability: sanitizeText(preference?.availability, 160),
            interestTags,
...
const loadRecommendationProfileFoundation = async (db, userId) =>
    loadUserProfileFoundation(db, userId);
