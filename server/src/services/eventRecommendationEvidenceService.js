...
const toSqliteTimestamp = (value) => value.toISOString().slice(0, 19).replace("T", " ");
        ...
        stats.recommendationCount > 0
            ? Math.min(
                  ...
                  0.45 +
                      stats.positive * 0.18 +
                      stats.negative * 0.16 +
                      Math.min(stats.recommendationCount, 5) * 0.04
              )
            : 0.25;
    const actionRate =
        stats.recommendationCount > 0
...
const getRecommendationActionEvidence = async (db, options = {}) => {
    ...
    for (const run of runs) {
        ...
        const recommendedIds = Array.isArray(summary.recommendedEventIds)
            ? summary.recommendedEventIds
            : [];
    ...
    if (eventIds.size === 0) {
        ...
        return {
            ...
            nextAdjustment:
                "Keep collecting recommendation telemetry before changing ranking policy.",
        };
    }

    const eventPlaceholders = [...eventIds].map(() => "?").join(",");
    const userFilter =
        userIds.size > 0 ? `AND user_id IN (${[...userIds].map(() => "?").join(",")})` : "";
