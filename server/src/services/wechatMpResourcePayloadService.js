...
const stripHtml = (value) => toText(value).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
...
const renderTextBlock = (block) => {
  ...
  if (block.style === 'quote') return `<blockquote>${escapeHtml(text.replace(/^>\s*/, ''))}</blockquote>`;
  if (block.style === 'list') {
...
const buildContentHtml = (blocks = [], { sourceUrl = '' } = {}) => {
  ...
  if (sourceUrl) {
    ...
    parts.push(`<p><a href="${href}" target="_blank" rel="noopener noreferrer">阅读原文</a></p>`);
  }
...
const buildRecord = ({ article = {}, content = {} } = {}) => {
  ...
  const cover = toText(content.coverImage || content.cover || article.cover || article.coverImage || images[0]);

  return {
    title,
    account: toText(content.author || article.author || article.account),
    sourceUrl,
    publishedAt: toText(article.published_at || article.time_text || article.create_time || content.published_at),
...
const buildEventPayload = ({ article = {}, content = {}, status = DEFAULT_EVENT_STATUS } = {}) => {
  ...
  const blocks = buildContentBlocks(record.contentText, record.images);
  const contentHtml = buildContentHtml(blocks, { sourceUrl: record.sourceUrl });
  const excerpt = buildExcerpt(record.summary || record.contentText || stripHtml(contentHtml), 220);
