...
const buildRuntimeSuggestedAction = (
    status,
    { fallbackRate, errorCount, avgDurationMs, retryCount }
) => {
    if (status === "NO_DATA")
        return "Collect runtime samples before making optimization decisions.";
    if (status === "blocked")
        return "Inspect provider health, model key status, and fallback path before increasing traffic.";
    if (errorCount > 0) return "Review recent errors and add a focused golden case for this Agent.";
    if (fallbackRate >= 0.3)
        return "Reduce fallback rate by improving prompt contract, model reliability, or context index coverage.";
    if (avgDurationMs >= 6000)
        return "Review token budget, candidate size, and timeout policy for this Agent.";
...
const finalizeRuntimeAggregate = (agentId, aggregate) => {
    ...
    const modelUsedRate =
        aggregate.runCount > 0 ? aggregate.modelUsedCount / aggregate.runCount : 0;
    const fallbackRate = aggregate.runCount > 0 ? aggregate.fallbackCount / aggregate.runCount : 0;
    const avgDurationMs =
        aggregate.telemetryTaskCount > 0
...
const getAgentRuntimeHealthOverview = async (db, agentIds = []) => {
    ...
    `);
        ...
        for (const row of rows) {
            ...
            const runtimeTelemetry =
                summary.runtimeTelemetry || summary.modelReview?.runtimeTelemetry || {};

            aggregate.runCount += 1;
            if (summary.modelUsed === true || summary.modelReview?.used === true)
                aggregate.modelUsedCount += 1;
            if (summary.fallbackUsed === true || summary.modelReview?.fallbackUsed === true)
            ...
            if (
                ...
                aggregate.recentError =
                    aggregate.recentError ||
                    toText(summary.errorCode || summary.error || row.status, 180);
            }
        ...
        return Object.fromEntries(
            agentIds.map((agentId) => [agentId, emptyAgentRuntimeHealth(agentId)])
        );
    }
    ...
    return Object.fromEntries(
        agentIds.map((agentId) => [
            agentId,
            finalizeRuntimeAggregate(agentId, aggregates.get(agentId)),
        ])
    );
};

const buildModelHealthOverview = (
    configs = [],
    runtimeTelemetryOverview = emptyRuntimeTelemetryOverview(),
    agentRuntimeHealth = {}
) => {
    const enabledConfigs = configs.filter((config) => config.enabled);
    const healthyConfigs = configs.filter((config) => config.last_status === "ok");
    const errorConfigs = configs.filter(
        (config) => config.last_status && config.last_status !== "ok"
    );
    ...
    if (enabledConfigs.length === 0) {
        ...
        status = "blocked";
        suggestedAction =
            "Test model keys, inspect provider errors, and keep deterministic fallbacks available.";
    } else if (errorConfigs.length > 0 || degradedAgents > 0 || retryCount >= 3) {
        status = "degraded";
        suggestedAction =
            "Inspect failing configs, lower priority for unstable providers, or rotate keys.";
...
const buildSuggestionId = (eventId, field, suggestedValue) =>
    `${eventId}:${field}:${Buffer.from(String(suggestedValue || ""))
        .toString("base64url")
        .slice(0, 24)}`;
...
const reviewGovernanceSuggestionsWithModel = async (
    db,
    { events, suggestions, memory, modelRunner }
) => {
    ...
    try {
        ...
        const reviewByKey = new Map(
            reviews.map((review) => [`${review.eventId}:${review.field}`, review])
        );
        ...
        const reviewed = suggestions.map((suggestion) => {
            ...
            if (!normalizedReview || normalizedReview.accepted === false) {
                return {
                    ...
                    reason:
                        normalizedReview?.reason ||
                        `${suggestion.reason} Model review did not accept this change.`,
            ...
            return {
                ...
                confidence: Number(
                    Math.max(suggestion.confidence, normalizedReview.confidence).toFixed(2)
                ),
        ...
        return {
            ...
            memorySignals: Array.isArray(result.parsed.memorySignals)
                ? result.parsed.memorySignals
                : [],
            warnings: Array.isArray(result.parsed.warnings)
                ? result.parsed.warnings.map((item) => toText(item, 160))
                : [],
        };
    } catch (error) {
        return {
            ...
            memorySignals: [],
            warnings: [
                "Model governance review failed; deterministic governance suggestions were used.",
            ],
        };
    }
};
...
const getAssistantOverview = async (db) => {
    const [
        ...
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM events WHERE deleted_at IS NULL AND (category IS NULL OR TRIM(category) = '')"
        ),
        ...
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'event_governance'"
        ),
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'event_recommendation'"
        ),
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'hackathon_coach'"
        ),
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'wechat_event_parser'"
        ),
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'event_profile_index'"
        ),
        safeCount(
            db,
            "SELECT COUNT(*) AS count FROM ai_assistant_runs WHERE module = 'global_search_index'"
        ),
        ...
        getAgentRuntimeHealthOverview(
            db,
            aiAgentRegistryService.getAgentDefinitions().map((agent) => agent.id)
        ),
    ]);
    ...
    const enabledConfigs = configs.filter((config) => config.enabled);
    const healthyConfigs = configs.filter((config) => config.last_status === "ok");
    const modelHealth = buildModelHealthOverview(
        configs,
        runtimeTelemetryOverview,
        agentRuntimeHealth
    );
    ...
    const health = {
        ...
        runtimeHealthSummary: {
            ...
            noDataCount: Object.values(agentRuntimeHealth || {}).filter(
                (item) => item.status === "NO_DATA"
            ).length,
            watchCount: Object.values(agentRuntimeHealth || {}).filter(
                (item) => item.status === "watch"
            ).length,
            degradedCount: Object.values(agentRuntimeHealth || {}).filter(
                (item) => item.status === "degraded"
            ).length,
            blockedCount: Object.values(agentRuntimeHealth || {}).filter(
...
const scanEventGovernance = async (db, options = {}) => {
    ...
    const ruleSuggestions = events.flatMap((event) =>
        buildEventSuggestions(event, Math.min(minConfidence, GOVERNANCE_MODEL_REVIEW_THRESHOLD))
    );
    ...
    try {
        for (const row of rows) {
            ...
            if (!GOVERNANCE_FIELDS.has(field) || Number(row.confidence || 0) < minConfidence) {
                ...
                await db.run("UPDATE ai_event_governance_suggestions SET status = ? WHERE id = ?", [
                    "skipped",
                    row.id,
                ]);
                continue;
            ...
            await db.run(`UPDATE events SET "${field}" = ? WHERE id = ?`, [
                toText(row.new_value, 1000),
                row.event_id,
            ]);
            await db.run(
                `
