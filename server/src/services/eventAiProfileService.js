...
const uniqueTextArray = (value, maxItems = 12, itemMaxLength = 60) => {
    ...
    return [...new Set(list.map((item) => toText(item, itemMaxLength)).filter(Boolean))].slice(
        0,
        maxItems
    );
};
...
const normalizeProfile = (profile = {}, event = {}) => {
    const category =
        normalizeEventCategory(profile.category) ||
    ...
    const timePreferenceTerms = uniqueTextArray(
        profile.time_preference_terms || profile.timeTerms,
        8
    );
    const summary =
        toText(profile.summary, 260) || toText(event.description, 180) || toText(event.title, 120);
...
const buildRuleProfile = (event = {}) => {
    ...
    const benefitTerms = [
        ...
        /实习|就业|简历|offer|招聘|internship|career|recruitment|resume/i.test(text)
            ? "就业/实习"
            : "",
    ].filter(Boolean);
...
const getProfileRow = async (db, eventId) =>
    db.get("SELECT * FROM event_ai_profiles WHERE event_id = ?", [eventId]);
        ...
        if (options.persistFallback === false) {
            return {
                profile: serializeProfileRow(
                    buildProfileRow(event, fallback, {
                        status: "fallback",
...
const refreshEventProfileIndex = async (db, options = {}) => {
    ...
    const staleEvents = rows
        .filter((event) => {
            if (force) return true;
            return (
                !event.profile_source_hash ||
