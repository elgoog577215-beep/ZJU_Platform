...
const syncPartnerProfile = async (db, partnerId) => {
    const partner = await db.get(`SELECT * FROM ecosystem_partners WHERE id = ?`, [partnerId]);
    ...
    if (profile?.id) {
       ...
       WHERE profile_id = ?
        ...
        await db
            .run(`UPDATE ecosystem_partners SET profile_id = ? WHERE id = ?`, [
                profile.id,
                partner.id,
            ])
            .catch(() => {});
        ...
        const aliases = uniqueTextArray(
            [...parseJsonArray(partner.event_organizer_aliases), partner.name, partner.name_en],
            30,
            160
        );
...
const findProfileById = (db, id) =>
    db.get(`SELECT * FROM profiles WHERE id = ? AND deleted_at IS NULL`, [id]);

const findProfileByHandle = (db, handle) =>
    db.get(`SELECT * FROM profiles WHERE handle = ? AND deleted_at IS NULL`, [
        String(handle || "").trim(),
    ]);
