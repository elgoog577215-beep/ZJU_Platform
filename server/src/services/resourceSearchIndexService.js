...
const { getCategoryLabel, normalizeEventCategory } = require("./eventIntelligenceService");
...
const uniqueTextArray = (value, maxItems = 16, itemMaxLength = 80) => {
    ...
    return [...new Set(items.map((item) => toText(item, itemMaxLength)).filter(Boolean))].slice(
        0,
        maxItems
    );
};

const flattenFacetValues = (facets = {}) =>
...
const stableHashInt = (value) =>
    crypto.createHash("sha256").update(String(value)).digest().readUInt32BE(0);
...
const normalizeResourceTypes = (types) => {
    const rawItems = Array.isArray(types) ? types : String(types || "").split(",");
...
const buildSourceHash = (resource) =>
    crypto
        .createHash("sha256")
        .update(
            [
...
const buildEventIndexPayload = (resource) => {
    ...
    const category =
        normalizeEventCategory(resource.profile_category || resource.category) ||
        toText(resource.category, 60);
    ...
    return {
        ...
        summary: toText(
            resource.profile_summary || resource.description || resource.content || resource.title,
            260
        ),
        contentText,
        ...
        keywordTerms: uniqueTextArray(
            [...flattenFacetValues(facets), ...getTopTokens(contentText, 16)],
            24
        ),
        popularityScore: Number(resource.views || 0) + Number(resource.likes || 0),
        qualityScore: clamp(
            0.45 + (resource.profile_summary ? 0.2 : 0) + (profileTopics.length ? 0.15 : 0),
            0.3,
            0.95
        ),
    };
...
const buildCommunityIndexPayload = (resource) => {
    ...
    const facets = {
        ...
        topics: uniqueTextArray(
            [resource.tags, resource.primary_tags, resource.category, resource.title],
            16
        ),
    ...
    return {
        title:
            toText(resource.title || resource.name, 180) ||
            RESOURCE_TYPE_LABELS[resource.resource_type],
        summary: toText(
            resource.excerpt || resource.description || resource.content || blockText,
            260
        ),
        contentText,
        imageUrl: toText(resource.image_url, 500),
        ...
        popularityScore:
            Number(resource.likes || resource.likes_count || 0) +
            Number(resource.comments_count || 0) +
            Number(resource.views_count || resource.hot_score || 0),
        qualityScore: clamp(
            0.42 +
                (resource.excerpt || resource.description ? 0.12 : 0) +
                (resource.tags || resource.primary_tags ? 0.1 : 0),
            0.3,
            0.9
        ),
    };
...
const buildMediaIndexPayload = (resource) => {
    ...
    return {
        ...
        qualityScore: clamp(
            0.4 + (resource.tags ? 0.1 : 0) + (resource.gameDescription ? 0.1 : 0),
            0.3,
            0.85
        ),
    };
...
const loadIndexableResources = async (db, options = {}) => {
    ...
    return rows.filter((row) => row && RESOURCE_TYPE_GROUPS[row.resource_type]);
};
...
const pruneUnavailableResources = async (db, types) => {
    ...
    if (selectedTypes.has("event")) {
        pruned += await deleteUnavailable(
            db,
            "event",
            "events",
            "e",
            "e.deleted_at IS NULL AND e.status = 'approved'"
        );
    }
    if (selectedTypes.has("article")) {
        pruned += await deleteUnavailable(
            ...
            "articles",
            "a",
            "a.deleted_at IS NULL AND a.status = 'approved'"
        );
    }
    ...
    if (selectedTypes.has("group")) {
        pruned += await deleteUnavailable(
            ...
            "community_groups",
            "g",
            "g.review_status = 'approved' AND COALESCE(g.is_expired, 0) = 0"
        );
    }
    if (selectedTypes.has("news")) {
        pruned += await deleteUnavailable(
            ...
            "news",
            "n",
            "n.deleted_at IS NULL AND n.status = 'approved'"
        );
    }
    if (selectedTypes.has("photo")) {
        pruned += await deleteUnavailable(
            ...
            "photos",
            "p",
            "p.deleted_at IS NULL AND p.status = 'approved'"
        );
    }
    if (selectedTypes.has("video")) {
        pruned += await deleteUnavailable(
            ...
            "videos",
            "v",
            "v.deleted_at IS NULL AND v.status = 'approved'"
        );
    }
    return pruned;
};
    ...
    const entries = await Promise.all(
        [
            [
                ...
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM events WHERE deleted_at IS NULL AND status = 'approved'"
                ),
            ],
            [
                "article",
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM articles WHERE deleted_at IS NULL AND status = 'approved'"
                ),
            ],
            [
                "post",
            ...
            [
                ...
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM community_groups WHERE review_status = 'approved' AND COALESCE(is_expired, 0) = 0"
                ),
            ],
            [
                "news",
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM news WHERE deleted_at IS NULL AND status = 'approved'"
                ),
            ],
            [
                "photo",
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM photos WHERE deleted_at IS NULL AND status = 'approved'"
                ),
            ],
            [
                "video",
                safeGet(
                    db,
                    "SELECT COUNT(*) AS count FROM videos WHERE deleted_at IS NULL AND status = 'approved'"
                ),
            ],
...
const buildGroupFilter = (groups = []) => {
    const normalized = uniqueTextArray(groups, 3).filter((group) =>
        ["events", "community", "media"].includes(group)
    );
...
const loadSearchableIndexRows = async (db, groups = [], limit = 500) => {
  ...
  `;
    const params = selects
        .flatMap((select) => select.params)
        .concat([normalizeLimit(limit, 500, 2000)]);
...
const scoreIndexRow = (row, parsed, queryVector) => {
    ...
    if (
        ...
        !isDateWithinRange(row.resource_date, parsed.timeRange)
    ) {
        return null;
    }
    if (
    ...
    const keywordHits = (parsed.keywords || []).filter(
        (keyword) => containsAny(searchableText, [keyword]) || intersects([keyword], keywords)
    );
    ...
    if (parsed.categories?.length) {
        ...
        const matched = parsed.categories.filter(
            (category) =>
        ...
        if (matched.length) {
            ...
            reasons.push(
                `索引分类：${matched
                    .map((item) => getCategoryLabel(item) || item)
                    .slice(0, 2)
                    .join("、")}`
            );
        }
    ...
    const campusHits = (parsed.campuses || []).filter((campus) =>
        containsAny(searchableText, [campus])
    );
    ...
    const audienceHits = (parsed.audiences || []).filter((audience) =>
        containsAny(searchableText, [audience])
    );
    ...
    const benefitHits = (parsed.benefits || []).filter((benefit) =>
        containsAny(searchableText, [benefit])
    );
...
const searchResourceIndex = async (db, parsed, options = {}) => {
    ...
    const rows = await loadSearchableIndexRows(db, selectedGroups, options.poolLimit || 500);
