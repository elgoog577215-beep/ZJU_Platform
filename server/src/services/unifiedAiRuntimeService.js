...
const attachQualityInstruction = (messages = []) => {
    ...
    if (firstSystemIndex < 0) {
        return [{ role: "system", content: QUALITY_INSTRUCTION }, ...messages];
    }
    ...
    return messages.map((message, index) => {
        ...
        return {
            ...message,
            content: [contentToText(message.content), QUALITY_INSTRUCTION]
                .filter(Boolean)
                .join("\n\n"),
        };
    });
...
const readUsage = (data = {}) => {
    ...
    const totalTokens =
        Number(usage.total_tokens ?? usage.totalTokens ?? 0) || promptTokens + completionTokens;
...
const summarizeModelStatusTelemetry = (statusLike) => {
    ...
    summary.avgDurationMs =
        summary.taskCount > 0 ? Math.round(summary.totalDurationMs / summary.taskCount) : 0;
...
const extractJsonObject = (content) => {
    ...
    const fencedMatch =
        content.match(/```json\s*([\s\S]*?)\s*```/i) || content.match(/```\s*([\s\S]*?)\s*```/i);
...
const mergeAttemptLists = (...lists) => lists.flat().filter(Boolean);
...
const callJson = async (db, { task, messages, temperature, maxTokens, timeout, modelRunner }) => {
    ...
    try {
        ...
        if (error.code === "AI_RUNTIME_EMPTY_CONTENT") {
            ...
            try {
                const alternateResult = await callChatCompletionWithFailover(db, modelPayload, {
