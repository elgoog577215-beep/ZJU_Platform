...
test("WeRead cache preserves unknown publication time and pending article metadata", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(root, `${feedId}.json`), JSON.stringify({ articles: [
            { link: "https://mp.weixin.qq.com/s/abc", content_status: "ready", published_at: "" },
...
test("WeRead cache localizes the collected body without fetching the MP backend", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(root, "MP_WXS_123.json"), JSON.stringify({ name: "测试", articles: [
            { link: url, title: "推文", content_status: "ready", body_file: name }
        ] }));
        await fs.writeFile(path.join(root, "bodies", name), JSON.stringify({
            content_text: "有效正文".repeat(100), content_html: '<p>正文</p><script>bad()</script>', images: []
        }));
        let calls = 0;
        const body = await service.fetchArticleContent({ feedId: "MP_WXS_123", url, root,
            localize: async (value) => { calls++; return value; } });
