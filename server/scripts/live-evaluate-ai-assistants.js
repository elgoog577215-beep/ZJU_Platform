...
const seedSettings = async (db) => {
    const settings = [
        ...
        [
            "hackathon_desc",
            "在限定时间内独立完成一个可运行的 AI 应用，允许使用 Codex、Cursor、Qoder 等 AI 工具。",
        ],
...
const seedBaseData = async (db, { profiles = true } = {}) => {
    await db.run(
        ...
        ["live_eval_user", "Live Eval User", "计算机学院", 20]
    );
    await db.run(
        `
...
const getCaseFilter = () => {
    const raw =
        process.env.AI_LIVE_EVAL_CASES ||
        process.argv.find((arg) => arg.startsWith("--cases="))?.slice("--cases=".length) ||
        "";
    return raw
        .split(",")
        .map((item) => item.trim())
        ...
        messages: [
            ...
            },
            {
                ...
                content: JSON.stringify({
                    instruction:
                        '返回严格 JSON：{"status":"ok","agent":"runtime","next":"evaluate"}',
                }),
            },
        ],
        temperature: 0,
...
const evaluateEventRecommendation = async (db) => {
    ...
    const checks = [
        makeCheck(
            ...
            result.type === "recommend" && result.recommendations.length >= 2
        ),
        makeCheck(
            "model rerank used",
            result.modelStatus?.used === true &&
                result.modelStatus?.tasks?.includes("event_recommendation_rerank")
        ),
        makeCheck(
            "top result is CS AI workshop",
            top?.event?.id === 1,
            `top=${top?.event?.title || "none"}`
        ),
        makeCheck(
            "reasons are grounded",
            (result.recommendations || []).every((item) =>
                /AI|紫金港|综测|项目|计算机|工作坊|路演|竞赛/.test(
                    `${item.reason} ${item.matchSignals?.join(" ")}`
                )
            )
        ),
        makeCheck(
            "no music distraction",
            !titles.slice(0, 3).some((title) => title.includes("音乐"))
        ),
...
const evaluateEventClarification = async (db) => {
    ...
    const checks = [
        ...
        makeCheck(
            "has options or provisional results",
            (result.clarificationOptions?.length || 0) > 0 || provisionalCount > 0
        ),
...
const evaluateHackathonCoach = async (db) => {
    ...
    const checks = [
        ...
        makeCheck(
            "model used",
            result.modelStatus?.used === true && result.modelStatus?.fallbackUsed !== true
        ),
        ...
        makeCheck(
            ...
            result.risks?.some(
                (item) =>
                    item.severity === "high" ||
                    /范围|时间|前端|跑/.test(`${item.risk} ${item.mitigation}`)
            )
        ),
...
const evaluateWechatParser = async (db) => {
    ...
    const checks = [
        ...
        makeCheck(
            "standard category",
            ["lecture", "competition", "other"].includes(parsed.category)
        ),
...
const evaluateGovernance = async (db) => {
    ...
    const checks = [
        ...
        makeCheck(
            ...
            scan.suggestions.every((item) => allowedFields.has(item.field))
        ),
        makeCheck(
            "confidence normalized",
            scan.suggestions.every(
                (item) => Number(item.confidence) >= 0 && Number(item.confidence) <= 1
            )
        ),
    ...
    try {
        ...
        const checks = [
            ...
            makeCheck(
                ...
                rows.every((row) => row.summary && row.category && row.profile_json)
            ),
...
const classifyError = (error) => {
    ...
    if (error.response?.status === 429 || /429|rate|quota|limit/i.test(error.message || ""))
        return "provider";
    if (/429|rate|quota|limit|exceeded/i.test(JSON.stringify(error.attempts || [])))
        return "provider";
...
const suggestFix = (result) => {
    ...
    if (category === "contract")
        return "优先检查输出 JSON contract、字段别名、空输出重试和 JSON repair。";
    if (category === "safety") return "优先检查白名单校验、候选 ID 校验、字段越权和幻觉事实过滤。";
    if (category === "provider") return "优先检查供应商限流、超时、重试节奏和小样本过滤。";
    if (category === "infrastructure")
        return "优先检查模型配置、数据库配置、密钥解密和本地后端环境。";
    return "优先检查提示词约束、候选上下文质量、排序阈值和用户可读理由。";
};

const hasProviderLimit = (value) =>
    /429|rate|quota|limit|exceeded/i.test(JSON.stringify(value || {}));
...
const main = async () => {
    ...
    const selectedCases = filter.length
        ? cases.filter(
              (testCase) => filter.includes(testCase.id) || filter.includes(testCase.agent)
          )
        : cases;
    ...
    try {
        console.log(
            `[live-eval] Running ${selectedCases.length} case(s) with ${modelConfigs[0]?.model || "configured model"}...`
        );
        for (const testCase of selectedCases) {
            ...
            results.push(result);
            console.log(
                `${result.pass ? "PASS" : "FAIL"} score=${result.score} duration=${result.durationMs}ms`
            );
        }
    } finally {
        await db.close();
