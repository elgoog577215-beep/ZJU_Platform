...
async function main() {
  ...
  `);
    ...
    const incomplete = eventsWithTags.filter(
        (event) => !validCategories.has(String(event.category || "").trim())
    );
    ...
    if (incomplete.length > 0) {
        ...
        incomplete.slice(0, 80).forEach((event) => {
            console.log(
                `  - #${event.id} ${event.title || "(untitled)"} category=${event.category || "(empty)"} tags=${event.tags}`
            );
        });
    ...
    try {
        ...
        console.log(
            `Rebuilt tag dictionary from non-event content: ${Object.keys(tagCounts).length} tags.`
        );
    } catch (error) {
