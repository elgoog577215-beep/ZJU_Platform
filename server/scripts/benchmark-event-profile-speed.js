...
const main = async () => {
    ...
    try {
        ...
        if (elapsedMs >= expectedSerialMs * 0.7) {
            throw new Error(
                `Event profile preparation is still near-serial: ${elapsedMs}ms for ${PROFILE_COUNT} profiles.`
            );
        }
        if (elapsedMs > maxExpectedConcurrentMs) {
            throw new Error(
                `Event profile preparation exceeded speed budget: ${elapsedMs}ms > ${maxExpectedConcurrentMs}ms.`
            );
        }
        if (result.stats.generated !== PROFILE_COUNT) {
            throw new Error(
                `Expected ${PROFILE_COUNT} generated profiles, got ${result.stats.generated}.`
            );
        }
