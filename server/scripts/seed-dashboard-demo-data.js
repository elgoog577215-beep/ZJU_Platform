...
const distribute = (total, bucketCount, seed = 0) => {
    ...
    const weights = Array.from(
        { length: bucketCount },
        (_, index) => EVENT_WEIGHTS[index] || Math.max(0.08, 1 / bucketCount)
    );
...
async function seedEventAnalytics(db, datedProfile) {
    ...
    for (const day of datedProfile) {
        ...
        const registrationDistribution = distribute(
            day.registrations,
            events.length,
            day.dayIndex + 1
        );
        ...
        for (let eventIndex = 0; eventIndex < events.length; eventIndex += 1) {
           ...
           VALUES (?, ?, ?, ?)`,
                    [
                        ...
                        visitorKey,
                        day.dateKey,
                        timestampFor(day.dateKey, index + eventIndex),
                    ]
                );
                eventViews += 1;
            }
...
async function main() {
    ...
    const [userCount, siteViews7d, eventViews7d, registrations7d] = await Promise.all([
        ...
        db.get("SELECT COUNT(*) as count FROM site_visit_events WHERE date_key >= ?", [
            lastSevenStart,
        ]),
        db.get("SELECT COUNT(*) as count FROM event_view_events WHERE date_key >= ?", [
            lastSevenStart,
        ]),
        db.get("SELECT COUNT(*) as count FROM event_registrations WHERE created_at >= ?", [
            `${lastSevenStart} 00:00:00`,
        ]),
    ]);
