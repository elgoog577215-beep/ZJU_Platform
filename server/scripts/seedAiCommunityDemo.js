...
const helpPosts = [
    [
        "求助：PyTorch 分布式训练在 Windows 上卡住",
        "训练在 init_process_group 阶段阻塞，怀疑端口配置冲突。",
        "PyTorch,分布式,Windows",
        "open",
    ],
    [
        "已解决：RAG 检索召回率低",
        "把 chunk 从 1000 改到 300 后召回显著提升。",
        "RAG,检索,已解决",
        "solved",
    ],
    [
        "求助：Agent 调度异常回环",
        "多 Agent 工作流中 reviewer 节点偶发死循环。",
        "Agent,LangGraph,调度",
        "open",
    ],
];
...
const helpComments = [
    [
        "求助：PyTorch 分布式训练在 Windows 上卡住",
        "先确认 MASTER_ADDR 和 MASTER_PORT 是否被占用。",
        false,
    ],
    [
        "求助：PyTorch 分布式训练在 Windows 上卡住",
        "建议加 `NCCL_P2P_DISABLE=1` 做对照实验。",
        false,
    ],
...
const groups = [
    ...
    [
        ...
        "剪枝量化蒸馏实践",
        "qq",
        "https://example.com/groups/compress",
        63,
        "技术交流",
        "pending",
        0,
    ...
    [
        ...
        "用于展示过期卡片样式",
        "discord",
        "https://example.com/groups/expired",
        21,
        "测试",
        "approved",
        0,
    ...
    [
        ...
        "用于展示驳回备注",
        "telegram",
        "https://example.com/groups/rejected",
        8,
        "测试",
        "rejected",
        0,
...
const newsRows = [
    [
        ...
        "汇总本周重点活动、讲座与协作任务。",
        "tuotuzju.com",
        "https://tuotuzju.com/news/weekly-01",
        "manual",
        168,
        1,
        12,
    ...
    [
        ...
        "本周末将举办大模型工程化专题讲座。",
        "zju.edu.cn",
        "https://www.zju.edu.cn/lecture/ai-engineering",
        "external",
        96,
        0,
        0,
    ...
    [
        ...
        "实验教学周期间开放时段调整。",
        "lab.zju.edu.cn",
        "https://lab.zju.edu.cn/notice/gpu-schedule",
        "external",
        82,
        0,
        0,
    ...
    [
        "导入草稿示例：LLM Benchmark 新榜单",
        "用于测试导入后编辑确认流程。",
        "用于测试导入后编辑确认流程。",
        "benchmark.example.com",
        "https://benchmark.example.com/llm/rankings",
        "external",
        20,
        0,
        0,
    ...
    [
        ...
        "用于测试 pending 审核流。",
        "maker.example.com",
        "https://maker.example.com/ai-camp",
        "manual",
        35,
        0,
        0,
    ...
    [
        "驳回示例：失效来源新闻",
        "用于测试失效来源 fallback 提示。",
        "用于测试失效来源 fallback 提示。",
        "legacy.example.org",
        "https://legacy.example.org/unreachable-news",
        "external",
        10,
        0,
        0,
...
async function upsertHelpPosts(db, adminId) {
    for (const [title, content, tags, postStatus] of helpPosts) {
        const exists = await db.get(
            `SELECT id FROM community_posts WHERE section='help' AND title = ?`,
            [title]
        );
        if (exists) continue;
        await db.run(
...
async function run() {
    ...
    if (!adminId) {
        throw new Error(
            "未找到管理员用户，请先执行 `npm --prefix server run seed` 初始化基础账号。"
        );
    }
