...
const successRunner = async ({ task, messages }) => {
    ...
    return {
        summary:
            "You are a good fit if you keep the scope small and ship one complete AI assistant workflow.",
        intent: {
        ...
        recommendation: {
            ...
            fitScore: 88,
            rationale:
                "The user can use AI tools and worries about frontend, so a low-UI, high-AI-value tool is the safest path.",
        ...
        prepPlan: [
            {
                ...
                detail: "Choose one campus productivity problem.",
            },
            {
                step: 2,
                title: "Build the main flow",
                detail: "Complete input, AI processing, and result display.",
            },
...
const main = async () => {
    ...
    try {
        ...
        assert(
            runRows.length === 2,
            "Expected hackathon runs to be recorded for success and fallback."
        );
        ...
        assert(
            summaries[0].runtimeTelemetry.taskCount === 1,
            "Expected success run to record runtime telemetry."
        );
        assert(
        ...
        assert(
            summaries[1].fallbackUsed === true,
            "Expected fallback run to record fallback usage."
        );
        assert(
            summaries[1].runtimeTelemetry.taskCount === 0,
            "Fallback run without model result should have empty telemetry."
        );
        assert(
            !runRows[0].summary_json.includes("Codex"),
