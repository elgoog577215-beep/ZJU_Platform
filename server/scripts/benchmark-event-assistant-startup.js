...
const createDelayedDb = ({ profileDelayMs = 120, candidateDelayMs = 120 } = {}) => ({
    async get(sql) {
...
const main = async () => {
    ...
    if (elapsedMs >= startupSerialMs) {
        throw new Error(
            `Assistant startup appears serial: elapsed=${elapsedMs}ms serial=${startupSerialMs}ms`
        );
    }
    ...
    if (elapsedMs > startupParallelBudgetMs) {
        throw new Error(
            `Assistant startup exceeded parallel budget: elapsed=${elapsedMs}ms budget=${startupParallelBudgetMs}ms`
        );
    }
