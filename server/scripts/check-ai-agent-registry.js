...
const main = () => {
    ...
    for (const agent of agents) {
        ...
        assert(
            agent.outputContracts.length >= 1,
            `${agent.id} needs at least one output contract.`
        );
        ...
        assert(
            Array.isArray(agent.qualityProfile) &&
                agent.qualityProfile.length === MATURITY_DIMENSIONS.length,
        ...
        for (const dimension of agent.qualityProfile) {
            ...
            assert(
                dimension.status,
                `${agent.id} quality profile ${dimension.id} is missing status.`
            );
            assert(
                dimension.nextStep,
                `${agent.id} quality profile ${dimension.id} is missing nextStep.`
            );
        }
        assert(
            agent.maturityScore.score >= 0.55,
            `${agent.id} maturity is too low for production tracking.`
        );
    }
    ...
    assert(
        eventAgent.promptTemplates.some(
            (template) => template.id === "event_recommendation_rerank"
        ),
    ...
    assert(
        eventAgent.contextIndexes.some(
            (item) =>
                String(item).includes("favorites") || String(item).includes("event_registrations")
        ),
    ...
    assert(
        globalSearchAgent.contextIndexes.includes(
    ...
    const overview = getAgentSystemOverview({
        ...
        agentRuntimeHealth: {
            event_recommendation: {
                ...
                suggestedAction:
                    "Reduce fallback rate by improving prompt contract, model reliability, or context index coverage.",
            },
            model_config_runtime: {
                status: "watch",
                sampleSize: 2,
    ...
    assert(
        overview.summary.agentCount === requiredAgents.length,
        "Overview should expose all registered agents."
    );
    assert(
        overview.summary.averageMaturity > 0.7,
        "Average agent maturity should stay above the first rollout floor."
    );
    assert(
        overview.modules.some((module) => module.id === "model_config_runtime"),
        "Overview should include runtime module."
    );
    assert(
        overview.summary.highPriorityGapCount === 0,
        "High-priority maturity gaps should be cleared."
    );
    assert(
        overview.summary.partialGapCount > 0,
        "Partial maturity gaps should remain visible for polishing."
    );
    assert(
        !overview.partialGaps.some(
            (gap) => gap.agentId === "event_profile_index" && gap.dimensionId === "observability"
        ),
        "Event profile index observability should be complete after exposing stale and missing profile signals."
    );
    assert(
        !overview.partialGaps.some(
            (gap) => gap.agentId === "model_config_runtime" && gap.dimensionId === "observability"
        ),
    ...
    assert(
        globalSearchModule.metrics.some(
            (metric) => metric.label === "Indexed" && metric.value === 22
        ),
        "Global AI search metrics should expose resource index coverage."
    );
    assert(
        overview.continuousImprovementPlan.length > 0,
        "Overview should expose partial-gap improvement plan."
    );
    assert(
    ...
    assert(
        overview.collaborationMap?.some(
            (item) =>
    ...
    assert(
        overview.modules.some(
            (module) =>
                module.id === "event_recommendation" && module.runtimeHealth?.fallbackRate === 0.5
        ),
    ...
    assert(
        spec.includes("#### Quality Profile"),
        "Generated spec should include quality profile sections."
    );
    assert(
        spec.includes("#### Related Agents"),
        "Generated spec should include related agent sections."
    );
    assert(
        spec.includes("## Runtime Health"),
        "Generated spec should include runtime health section."
    );
    assert(
        spec.includes("#### Runtime Observability"),
        "Generated spec should include per-agent runtime observability sections."
    );
    assert(
        spec.includes("Circuit breaker recommendation"),
        "Generated spec should include circuit breaker recommendation."
    );
    assert(
    ...
    for (const dimension of MATURITY_DIMENSIONS) {
        assert(
            spec.includes(dimension.label),
            `Generated spec should mention dimension ${dimension.label}.`
        );
    }
