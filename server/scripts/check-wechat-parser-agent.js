...
const main = async () => {
    ...
    try {
        ...
        assert(
            !persistedText.includes("mp.weixin.qq.com"),
            "Run summaries must not store raw WeChat URLs."
        );
        assert(
            !persistedText.includes("never be persisted"),
            "Run summaries must not store raw article content."
        );
