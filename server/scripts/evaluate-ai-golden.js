...
const seedData = async (db) => {
    await db.run(
        "INSERT INTO users (id, username, nickname, organization, age) VALUES (1, ?, ?, ?, ?)",
        ["golden_user", "Golden User", "College of Computer Science", 20]
    );
    await db.run(
        ...
        [
            ...
            "Year 2",
            "Zijingang",
            "AI,product,volunteer",
            "competition,lecture,volunteer",
            "score,volunteer_time",
            "offline",
        ]
    );
    ...
    const events = [
        {
            ...
            description:
                "Hands-on AI agent product workshop for students who want to build a project demo. Comprehensive score is available.",
        ...
        },
        {
            ...
            description:
                "AI hackathon competition briefing with team demo, pitch and score policy.",
        ...
        },
        {
            ...
            location: "Zijingang",
            description:
                "Student teams join an AI startup competition roadshow with final pitch and score recognition.",
...
const goldenModelRunner = async ({ task, messages }) => {
    ...
    if (task === "event_recommendation_intent") {
        ...
        if (/活动推荐|推荐活动|找活动|some events/i.test(query)) {
            return {
                query_summary:
                    "User broadly asks for activity recommendations but has not specified topic, campus, benefit, or time.",
                topics: [],
                campuses: [],
                organizers: [],
        ...
        return {
            query_summary:
                "User wants a Zijingang AI event with comprehensive score and practical project value.",
    ...
    if (task === "event_profile") {
        ...
        return {
            ...
            rationale:
                "Profile is extracted from title, description, location, organizer and benefit fields.",
        };
    }
    ...
    if (task === "event_recommendation_rerank") {
        ...
        assert(
            candidates.some(
                (candidate) =>
                    ...
                    typeof candidate.actionEvidence.positiveCategoryWeight === "number"
            ),
        ...
        assert(
            candidates.every(
                (candidate) =>
                    candidate.actionEvidence &&
                    typeof candidate.actionEvidence.priorViewDetail === "boolean"
            ),
        ...
        return {
            summary:
                "Ranked by AI topic fit, practical project value, Zijingang offline access, and score benefit.",
            reasoning_trace: {
                ranking_basis: ["AI topic fit", "Zijingang offline access", "score benefit"],
                uncertainty: ["exact date flexibility is inferred"],
                action_evidence_used: scored.some(
                    ({ candidate }) =>
            ...
            recommendations: scored.slice(0, 3).map(({ candidate }, index) => ({
                ...
                matched_signals: [
                    /AI|hackathon|agent/i.test(`${candidate.title} ${candidate.description}`)
                        ? "AI topic"
                        : "partial topic fit",
                    /Zijingang/i.test(candidate.location || "")
                        ? "Zijingang"
                        : "location is less direct",
                    /score/i.test(candidate.score || candidate.description || "")
                        ? "score benefit"
                        : "no explicit score",
                ],
            })),
    ...
    if (task === "hackathon_ai_coach") {
        const payload = extractPayload(messages);
        assert(
            payload.contextCards.length > 0,
            "Hackathon golden sample should pass context cards."
        );
        return {
            summary:
                "You should aim for one narrow AI workflow that can run end to end in the room, not a broad platform.",
            intent: {
            ...
            recommendation: {
                ...
                fitScore: 88,
                rationale:
                    "The user has AI-tool ability but worries about frontend stability, so a small workflow has the best completion odds.",
            ...
            prepPlan: [
                {
                    step: 1,
                    title: "Freeze scope",
                    detail: "Pick one user, one input and one output before touching UI.",
                },
                {
                    step: 2,
                    title: "Build main path",
                    detail: "Make the input, AI call, validation and result display work first.",
                },
                {
                    step: 3,
                    title: "Prepare demo data",
                    detail: "Use two stable sample inputs so the final presentation does not depend on luck.",
                },
            ],
            strategy: {
                eventDay:
                    "Spend 45 minutes on scope, 3 hours on the main path, and the rest on fallback and demo polish.",
                tooling:
                    "Use AI tools for scaffolding, debugging and concise copy, but keep product judgment manual.",
                delivery: "Show a working loop and explain what the model contributes.",
            },
            risks: [
                {
                    risk: "Scope too large",
                    mitigation: "Cut every feature except the main AI loop.",
                    severity: "high",
                },
                {
                    risk: "Unstable model output",
                    mitigation: "Validate JSON and keep fixed demo examples.",
                    severity: "medium",
                },
            ],
            sources: payload.contextCards.slice(0, 3).map((card) => ({ id: card.id })),
            suggestedQuestions: [
                "How should I split the 5 hours?",
    ...
    if (task === "wechat_event_parse") {
        return {
            ...
            description:
                "AI hackathon competition for all students with demo, pitch and comprehensive score.",
            content:
                "<h3>Event Introduction</h3><p>Participants build an AI demo and present the result.</p>",
            date_reasoning:
                "The article explicitly says May 20, 19:00-21:00; use the current academic year.",
...
const assertNoInventedIds = (recommendations) => {
    for (const item of recommendations) {
        assert(
            ...
            "Recommendation should use a numeric provided event id."
        );
        assert(
            item.event?.id === item.id,
            "Recommendation event id should match the selected candidate."
        );
    }
};
...
const assertOpportunityMatch = (item, label = "Recommendation") => {
    assert(
        Array.isArray(item.opportunityMatch?.matched),
        `${label} should expose opportunity matched signals.`
    );
    assert(
        Array.isArray(item.opportunityMatch?.missing),
        `${label} should expose opportunity missing signals.`
    );
    assert(
        Array.isArray(item.opportunityMatch?.uncertainty),
        `${label} should expose opportunity uncertainty signals.`
    );
    assertUsefulText(item.opportunityMatch?.decisionHint, `${label} decision hint`, 12);
    assertUsefulText(
        item.opportunityMatch?.decisionSupport?.nextAction,
        `${label} next action`,
        12
    );
    assert(
        Array.isArray(item.opportunityMatch?.decisionSupport?.tradeoffs),
        `${label} should expose decision tradeoffs.`
    );
    assert(
        Array.isArray(item.opportunityMatch?.decisionSupport?.fitFor),
        `${label} should expose decision fit reasons.`
    );
    assert(
        Array.isArray(item.opportunityMatch?.decisionSupport?.watchouts),
        `${label} should expose decision watchouts.`
    );
};
...
const evaluateEventRecommendation = async (db) => {
    ...
    assert(
        /AI|Hackathon|Agent/i.test(result.recommendations[0].event.title),
        "Top event should match AI intent."
    );
    assert(
        result.recommendations[0].confidence >= 0.85,
        "Top event should have strong confidence."
    );
    assert(
        result.recommendations[0].matchSignals.length >= 2,
        "Top event should expose matched signals."
    );
    assertUsefulText(result.recommendations[0].reason, "Top recommendation reason");
    assert(
        result.reasoningTrace?.intentConfidence > 0,
        "Recommendation should expose intent confidence in reasoning trace."
    );
    assert(
        result.reasoningTrace?.candidateCount >= result.recommendations.length,
        "Recommendation trace should expose candidate count."
    );
    assert(
        Array.isArray(result.reasoningTrace?.rankingBasis) &&
            result.reasoningTrace.rankingBasis.length >= 1,
        "Recommendation trace should expose ranking basis."
    );
    assert(
        Array.isArray(result.reasoningTrace?.uncertainty),
        "Recommendation trace should expose uncertainty signals."
    );
    assert(
        result.reasoningTrace?.scoringFactors?.hardConstraints === true,
        "Recommendation trace should expose structured hard-constraint scoring factors."
    );
    assert(
        result.reasoningTrace?.scoringFactors?.topicCategory === true,
        "Recommendation trace should expose structured topic/category scoring factors."
    );
    assert(
        Number.isInteger(Number(result.assistantRunId)),
        "Recommendation should expose assistant run id for decision-action attribution."
    );
    assert(
        typeof result.recommendations[0].diagnostics?.deterministicScore === "number",
        "Recommendation should expose deterministic score diagnostics."
    );
    assert(
        typeof result.recommendations[0].diagnostics?.semanticScore === "number",
        "Recommendation should expose semantic score diagnostics."
    );
    assert(
        typeof result.recommendations[0].diagnostics?.hardConstraintRatio === "number",
        "Recommendation should expose hard-constraint ratio diagnostics."
    );
    assertOpportunityMatch(result.recommendations[0], "Top recommendation");
    assert(
        result.reasoningTrace?.opportunityStage === "trusted_decision_loop_v1",
        "Recommendation trace should expose opportunity matching stage."
    );
    assert(
        result.modelStatus.tasks.includes("event_recommendation_rerank"),
        "Recommendation should use rerank task."
    );
    assert(
        result.modelStatus.profileStats.generated >= 1,
        "Recommendation should use event profile generation."
    );
    ...
    assert(
        Array.isArray(runSummary.recommendedEventIds) && runSummary.recommendedEventIds.length >= 1,
        "Recommendation run should store bounded event evidence anchors."
    );
    assert(
        runSummary.averageConfidence >= 0.7,
        "Recommendation run should store average confidence."
    );
    assert(
        runSummary.opportunityStage === "trusted_decision_loop_v1",
        "Recommendation run should store opportunity stage."
    );
    assert(
        typeof runSummary.averageHardConstraintRatio === "number",
        "Recommendation run should store hard-constraint ratio."
    );
    assert(
        Array.isArray(runSummary.recommendedEventRanks) &&
            runSummary.recommendedEventRanks.length >= 1,
        "Recommendation run should store event-rank attribution anchors."
    );
    assert(
        runSummary.recommendedEventRanks[0].eventId === result.recommendations[0].event.id,
        "Recommendation run should store the top event-rank anchor."
    );
    assert(
        runSummary.recommendedEventRanks[0].rank === result.recommendations[0].rank,
        "Recommendation run should store the top recommendation rank."
    );
    assert(
        typeof runSummary.opportunityMatchedCount === "number" &&
            runSummary.opportunityMatchedCount >= 1,
        "Recommendation run should store opportunity matched count."
    );
    assert(
        typeof runSummary.opportunityMissingCount === "number",
        "Recommendation run should store opportunity missing count."
    );
    assert(
        typeof runSummary.durationMs === "number" && runSummary.durationMs >= 0,
        "Recommendation run should store total assistant duration."
    );
    assert(
        typeof runSummary.performance?.candidateLoadMs === "number",
        "Recommendation run should store candidate load duration."
    );
    assert(
        typeof runSummary.performance?.candidatePoolMs === "number",
        "Recommendation run should store candidate pool duration."
    );
    assert(
        typeof runSummary.performance?.rerankMs === "number",
        "Recommendation run should store rerank duration."
    );
    assert(
        typeof runSummary.decisionSupportNextActionCount === "number" &&
            runSummary.decisionSupportNextActionCount >= 1,
        "Recommendation run should store decision-support next-action count."
    );
    assert(
        typeof runSummary.decisionSupportTradeoffCount === "number" &&
            runSummary.decisionSupportTradeoffCount >= 1,
        "Recommendation run should store decision-support tradeoff count."
    );
    assert(
        typeof runSummary.decisionSupportWatchoutCount === "number",
        "Recommendation run should store decision-support watchout count."
    );
    assert(
        !run.summary_json.includes("Find me a Zijingang AI activity"),
        "Recommendation run should avoid raw query text."
    );
    ...
    await recordEventAssistantDecisionAction({
        ...
        metadata: {
            nextAction:
                result.recommendations[0].opportunityMatch?.decisionSupport?.nextAction || "",
        },
    });
    ...
    assert(
        actionRows.some((row) => row.action_type === "view_detail"),
        "Decision action log should store view_detail."
    );
    assert(
        actionRows.some((row) => row.action_type === "feedback_up"),
        "Decision action log should store feedback action."
    );
    ...
    assert(
        ...
        "Decision action attribution should reject events outside the assistant run."
    );
    ...
    assert(
        invalidRankRejected,
        "Decision action attribution should reject impossible recommendation rank."
    );
...
const evaluateEventRecommendationQueryMatrix = async (db) => {
    const cases = [
        {
            ...
            expected: /AI Agent|AI Hackathon|AI Startup/i,
            matched: /综测|score/i,
        },
        {
            query: "I want an offline AI project workshop at Zijingang with score credit.",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
            matched: /skill|AI topic|综测|score/i,
        },
        {
            query: "Recommend an AI agent event for all students near Zijingang.",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "Any practical AI demo activity this week with comprehensive score?",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "Find a College of Computer Science AI activity at Zijingang.",
            expected: /AI Agent/i,
        },
        {
            query: "I need a hands-on agent workshop, preferably offline and score-bearing.",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "Show me AI activities that help build a project demo.",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "What Zijingang activity combines AI, product practice, and score value?",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "Recommend a near-term AI lecture for students who want to build demos.",
            expected: /AI Agent/i,
        },
        {
            query: "Find an AI campus event with score recognition and offline participation.",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "我想找紫金港这周有综测的 AI 活动。",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
            matched: /综测|加分|score/i,
        },
        { query: "有没有计算机学院办的智能体线下活动？", expected: /AI Agent/i },
        {
            query: "帮我找能做项目 demo 的大模型活动。",
            expected: /AI Agent|AI Hackathon|AI Startup/i,
        },
        {
            query: "我想参加 AI 创业或路演类活动，最好有加分。",
            expected: /AI Hackathon|AI Startup/i,
        },
        {
            query: "推荐一个适合做作品集的 AI 实践活动。",
            expected: /AI Agent|AI Startup/i,
            matched: /技能成长/,
            intentSummary: /收益：.*技能成长/,
        },
        {
            query: "想找有志愿时长的公益服务活动。",
            expected: /Volunteer Service Orientation/i,
            matched: /志愿时长|volunteer/i,
        },
        {
            query: "紫金港附近有没有志愿时长活动？",
            expected: /Volunteer Service Orientation/i,
            matched: /志愿时长|volunteer/i,
        },
        ...
        {
            query: "想找玉泉的音乐或艺术活动放松一下。",
            expected: /Classical Music Sharing Session/i,
            matched: /social|音乐|艺术|partial topic/i,
        },
        {
            query: "推荐一个轻松社交的文体活动。",
            expected: /Classical Music Sharing Session/i,
            matched: /社交放松|音乐|艺术|partial topic/i,
            intentSummary: /收益：.*社交放松/,
        },
    ...
    for (const item of cases) {
        ...
        const ok =
            ...
            (!item.intentSummary ||
                item.intentSummary.test((result.understoodIntent?.understood || []).join(" "))) &&
    ...
    assert(
        failures.length === 0,
        `Recommendation query matrix failed: ${JSON.stringify(failures, null, 2)}`
    );
...
const evaluateIntentLocalConstraintRetention = async (db) => {
    ...
    assert(
        result.type === "recommend",
        "Sparse intent output should still return recommendations."
    );
    assert(
        top?.event?.organizer === "College of Computer Science",
        "Local organizer constraint should survive sparse model intent output."
    );
    assert(
        top.diagnostics?.hardConstraintScore > 0,
        "Retained local constraints should contribute to hard-constraint score."
    );
    assertOpportunityMatch(top, "Chinese sparse-intent recommendation");
    assert(
        top.matchSignals.some(
            (signal) =>
                signal.includes("紫金港") || signal.includes("Zijingang") || signal.includes("地点")
        ),
...
const evaluateNegativeFeedbackReasonLearning = async (db) => {
    ...
    assert(
        result.type === "recommend",
        "Negative feedback reason learning should still return recommendations."
    );
    assert(
        result.reasoningTrace?.feedbackLearning?.negativeReasons?.some(
            (item) => item.value === "time_mismatch"
        ),
...
const evaluateLocalBenefitAliasBoundaries = async (db) => {
    ...
    assert(
        result.type === "recommend",
        "Benefit alias boundary check should return recommendations."
    );
    assert(
        /AI Agent|AI Hackathon|AI Startup/i.test(top?.event?.title || ""),
        "Participation wording should not redirect to art/social events."
    );
    assert(
        !top.opportunityMatch?.matched?.some((signal) =>
            /收益匹配：social|social/i.test(String(signal))
        ),
...
const evaluateRerankBackendCompletionAndGuardrail = async (db) => {
    const underReturningRunner = async ({ task, messages }) => {
        if (task === "event_recommendation_rerank") {
            ...
            const candidates = payload.candidates || [];
            const weak =
                candidates.find((candidate) => /Music|Campus Life/i.test(candidate.title || "")) ||
                candidates[candidates.length - 1];
            return {
                summary:
                    "The model returned too few candidates and placed a weaker candidate first.",
                reasoning_trace: {
    ...
    assert(
        result.type === "recommend",
        "Partial rerank output should still return recommendations."
    );
    assert(
        result.recommendations.length >= 3,
        "Backend should complete partial model rerank results."
    );
    assert(
        top?.event?.organizer === "College of Computer Science",
        "Hard-constraint guardrail should promote the best constrained candidate."
    );
    assert(
...
const evaluateRecommendationActionEvidence = async (db) => {
    await db.run("INSERT INTO favorites (user_id, item_id, item_type) VALUES (?, ?, ?)", [
        1,
        2,
        "event",
    ]);
    await db.run("INSERT INTO event_registrations (user_id, event_id) VALUES (?, ?)", [1, 2]);
    await db.run(
        "INSERT INTO event_recommendation_feedback (user_id, event_id, feedback, query, reason) VALUES (?, ?, ?, ?, ?)",
    ...
    assert(
        ["OBSERVED", "PARTIALLY_OBSERVED"].includes(evidence.status),
        "Recommendation evidence should observe positive action."
    );
...
const evaluateSmartClarification = async (db) => {
    ...
    assert(
        result.type === "clarify",
        "Broad recommendation query should trigger smart clarification."
    );
    assertUsefulText(result.question, "Clarification question", 10);
    assert(
        Array.isArray(result.clarificationOptions) && result.clarificationOptions.length >= 2,
        "Clarification should expose bounded options."
    );
    assert(
        Array.isArray(result.provisionalRecommendations) &&
            result.provisionalRecommendations.length >= 1,
        "Clarification should expose provisional recommendations."
    );
    assert(
        result.reasoningTrace?.clarificationSuggested === true,
        "Clarification should expose trace flag."
    );
    assert(
        result.reasoningTrace?.candidateCount >= result.provisionalRecommendations.length,
        "Clarification trace should expose candidate count."
    );
...
const evaluateRecommendationActionEvidenceRanking = async (db) => {
    ...
    assert(
        result.type === "recommend",
        "Action evidence ranking should still return recommendations."
    );
    assert(
        result.recommendations[0].event.id === 1,
        "Positive action evidence should lift the matching AI workshop above a downvoted competition."
    );
    assert(
...
const evaluateRecommendationActionLearning = async (db) => {
    const staleRun = await db.run(
     ...
     VALUES (?, ?, ?, ?, ?)`,
        [
            ...
            "completed",
            1,
            JSON.stringify({ seed: "action-learning" }),
        ]
    );
    await recordEventAssistantDecisionAction({
        db,
    ...
    assert(
        result.type === "recommend",
        "Recommendation action learning should still return recommendations."
    );
    assert(
        result.reasoningTrace?.actionEvidenceUsed === true,
        "Recommendation should mark action learning evidence as used."
    );
    assert(
        result.recommendations[0].matchSignals.some((signal) => signal.includes("行动证据")),
    ...
    const runSummary = JSON.parse(run.summary_json || "{}");
    assert(
        runSummary.actionEvidenceUsed === true,
        "Run summary should record action evidence usage."
    );
    assert(
        runSummary.actionEvidenceSourceCount >= 3,
        "Run summary should count recommendation action evidence sources."
    );
    assert(
        runSummary.viewDetailEvidenceCount >= 1,
        "Run summary should count view detail evidence."
    );
    assert(
        runSummary.favoriteActionEvidenceCount >= 1,
        "Run summary should count favorite action evidence."
    );
    assert(
        runSummary.negativeActionEvidenceCount >= 1,
        "Run summary should count negative action evidence."
    );
    assert(
        !run.summary_json.includes("PRIVATE_METADATA_SHOULD_NOT_APPEAR_IN_SUMMARY"),
        "Run summary should avoid raw action metadata."
    );
    assert(
        !run.summary_json.includes("Find me a practical AI project workshop"),
        "Run summary should avoid raw query text."
    );
...
const evaluateAnonymousRecommendationActionLearning = async (db) => {
    ...
    assert(
        result.type === "recommend",
        "Anonymous action learning should still return recommendations."
    );
    assert(
        result.reasoningTrace?.actionEvidenceUsed === true,
        "Anonymous visitor action evidence should be used for recommendations."
    );
    assert(
        result.reasoningTrace?.actionEvidenceSummary?.sourceCount >= 2,
        "Anonymous visitor action evidence should count recent actions."
    );
    assert(
        result.recommendations[0].matchSignals.some((signal) =>
            /\u884c\u52a8\u8bc1\u636e|\u6536\u85cf|\u62a5\u540d|action evidence/i.test(
                String(signal)
            )
        ),
...
const evaluateEventRecommendationFallbackPerformance = async (db) => {
    ...
    assert(
        ...
        "Fallback recommendation should keep useful candidates."
    );
    assert(
        /AI|Hackathon|Agent/i.test(result.recommendations[0].event.title),
        "Fallback top event should preserve AI intent."
    );
    assert(
        result.modelStatus.fallbackUsed === true,
        "Fallback recommendation should expose model fallback status."
    );
    assert(
        result.modelStatus.profileStats.transient >= 1,
        "Fallback recommendation should use transient request profiles."
    );
    assert(
        after.count === before.count,
        "Transient request profiles should not be written into the profile index."
    );
    assertNoInventedIds(result.recommendations);
...
const evaluateHackathonCoach = async (db) => {
    const result = await runHackathonAssistant({
        ...
        participantProfile: {
            skills: ["AI tools", "Python"],
            concerns: ["frontend stability", "time limit"],
        },
        modelRunner: goldenModelRunner,
    });
    ...
    assert(
        ...
        "Hackathon coach should identify high risk."
    );
    assert(result.sources.length > 0, "Hackathon coach should cite context cards.");
    assert(
        result.modelStatus.tasks.includes("hackathon_ai_coach"),
        "Hackathon coach should expose model task."
    );
...
const evaluateWechatParser = async (db) => {
    const parsed = await parseWithLLM(
        {
            ...
            content:
                "May 20, 19:00-21:00, Zijingang Innovation Space. AI hackathon challenge, demo pitch, comprehensive score 0.5, open to all students.",
            coverImage: "",
        },
        {
            db,
    ...
    assert(
        parsed.target_audience === "全校",
        "WeChat parser should normalize English audience to the standard catalog."
    );
    assert(parsed.tags.length === 0, "WeChat parser should retire generated tags.");
    assert(
        parsed.aiMeta.runtimeTelemetry.taskCount === 1,
        "WeChat parser should expose runtime telemetry."
    );
...
const evaluateGovernance = async (db) => {
    ...
    assert(
        scan.summary.modelReview.used === true,
        "Governance should use model review on ambiguous candidates."
    );
    assert(
        scan.summary.modelReview.runtimeTelemetry.taskCount === 1,
        "Governance should expose model telemetry."
    );
    assert(
        scan.summary.suggestionCount >= 1,
        "Governance should keep clearly supported suggestions."
    );
    assert(
        scan.suggestions.some(
            (item) =>
                item.field === "category" &&
                item.suggestedValue === "competition" &&
                item.confidence >= 0.72
        ),
        "Governance should accept clearly supported competition suggestions."
    );
    assert(
        scan.suggestions.every((item) => ["category", "target_audience"].includes(item.field)),
        "Governance should only use whitelisted fields."
    );
    assert(
        scan.suggestions.every(
            (item) => item.field !== "category" || STANDARD_CATEGORIES.has(item.suggestedValue)
        ),
        "Governance category suggestions should be standard values."
    );
    assert(
        scan.suggestions.every(
            (item) => !String(item.source).includes("model_rejected") || item.confidence < 0.45
        ),
...
const main = async () => {
    ...
    try {
        ...
        const rerankBackendCompletionAndGuardrail =
            await evaluateRerankBackendCompletionAndGuardrail(db);
        ...
        const recommendationActionEvidenceRanking =
            await evaluateRecommendationActionEvidenceRanking(db);
        const recommendationActionLearning = await evaluateRecommendationActionLearning(db);
        const anonymousRecommendationActionLearning =
            await evaluateAnonymousRecommendationActionLearning(db);
        const eventRecommendationFallbackPerformance =
            await evaluateEventRecommendationFallbackPerformance(db);
        ...
        const runs = await db.all(
            "SELECT module, action, summary_json FROM ai_assistant_runs ORDER BY id ASC"
        );
        const persisted = JSON.stringify(runs);
        assert(
            !persisted.includes("Find me a Zijingang AI activity"),
            "Golden summaries should not store raw recommendation query."
        );
        assert(
            !persisted.includes("May 20, 19:00-21:00"),
            "Golden summaries should not store raw WeChat content."
        );
