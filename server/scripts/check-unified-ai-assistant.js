...
const sampleEvents = [
    ...
    },
    {
        ...
        tags: "",
        description:
            "\u56f4\u7ed5\u672a\u6765\u5b66\u4e60\u3001\u6821\u56ed\u5de5\u5177\u548c AI \u521b\u610f\u8fdb\u884c\u4ea4\u6d41\u5206\u4eab\u3002",
        content:
            "\u542b\u5c0f\u7ec4\u8ba8\u8bba\u548c\u7ecf\u9a8c\u5206\u4eab\uff0c\u4e0d\u6d89\u53ca\u6bd4\u8d5b\u6216\u5fd7\u613f\u670d\u52a1\u3002",
        organizer: "\u5b66\u751f\u793e\u56e2",
        location: "\u7d2b\u91d1\u6e2f",
        target_audience: "\u5168\u6821\u5e08\u751f",
    },
...
const runMainFlowCheck = async () => {
    ...
    const governanceReviewRunner = async ({ task, messages }) => {
        ...
        return {
            reviews: payload.candidates.map((candidate) => {
                const isAmbiguousFutureShare = candidate.event.title.includes(
                    "\u672a\u6765\u79d1\u6280"
                );
                return {
                    ...
                    accepted: true,
                    suggestedValue: isAmbiguousFutureShare
                        ? "other"
                        : candidate.ruleSuggestion.suggestedValue,
    ...
    assert(
        overview.health.eventAiProfileCount === 0,
        "Overview should expose event AI profile coverage."
    );
    ...
    assert(
        scan.summary.highConfidenceCount >= 3,
        "Seeded data should produce high confidence suggestions."
    );
    assert(
        governanceReviewCalls === 1,
        "Governance scan should invoke model review for ambiguous candidates."
    );
    ...
    assert(
        afterApply[3].category === "exchange",
        "Canonical exchange category should stay stable."
    );
    assert(
        afterApply[4].category === "other",
        "Model-reviewed ambiguous event should be normalized conservatively."
    );
    ...
    assert(
        runs.some((run) => run.action === "scan"),
        "Scan run should be recorded."
    );
    assert(
        runs.some((run) => run.action === "apply"),
        "Apply run should be recorded."
    );
    assert(
        stored.some((suggestion) => suggestion.status === "applied"),
        "Applied suggestions should be recorded."
    );
    assert(
        memory.decisionCount >= 1,
        "Applied admin decisions should be available as governance memory."
    );
...
const runConflictCheck = async () => {
    ...
    const categorySuggestion = scan.suggestions.find(
        (suggestion) => suggestion.field === "category"
    );
    ...
    assert(
        finalEvent.category === "manual_review",
        "Conflict apply must not overwrite current value."
    );
