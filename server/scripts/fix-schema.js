...
const DB_PATH = process.env.DATABASE_FILE || path.join(__dirname, "..", "database.sqlite");
...
function preflight() {
    if (!fs.existsSync(DB_PATH)) {
        throw new Error(
            `DB not found at ${DB_PATH}. Set DATABASE_FILE env or run from correct directory.`
        );
    }
...
async function main() {
    ...
    if (pre.freeBytes !== null) {
        console.log(
            `      Free disk: ${formatBytes(pre.freeBytes)} (need ≥ ${formatBytes(pre.dbSize * 2)})`
        );
    }
    ...
    if (pre.hasWal || pre.hasShm) {
        console.log(
            `      ⚠️  Active WAL/SHM detected. Recommendation: stop the app server ` +
    ...
    if (Object.keys(willAdd).length === 0) {
        console.log(
            "      ✓ All expected columns already present. Migrations may still run " +
    ...
    if (isDryRun) {
        ...
        console.log(
            "      The actual ALTER statements are chosen by runMigrations() at apply " +
    ...
    for (const t of bak.targets) {
        ...
        console.log(
            `      ${path.basename(t.src)} → ${path.relative(process.cwd(), t.dst)} (${formatBytes(size)})`
        );
    }
    ...
    const anyIssue =
        rowDiffs.length > 0 || missing.length > 0 || after.integrity.toLowerCase() !== "ok";
    if (anyIssue) {
