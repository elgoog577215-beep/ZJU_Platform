...
const { loadScopedCandidates, runEventAssistantTurn } = require("../src/utils/eventAssistant");
            ...
            return {
                ...
                recommendations: candidates
                    ...
                    .map((candidate, index) => ({
                        ...
                        matched_signals: candidate.deterministicSignals?.slice(0, 3) || [
                            "候选活动匹配",
                        ],
                    })),
                reasoning_trace: {
...
const main = async () => {
    ...
    try {
        ...
        try {
            ...
            assert(
                emptyUpcoming.type === "empty",
                "Controlled current DB copy should produce an empty response."
            );
            assert(
                emptyUpcoming.scope === "upcoming",
                "Controlled current DB copy should stay in upcoming scope."
            );
            assert(
                emptyUpcoming.emptyReason === "no_upcoming",
                "Controlled current DB copy should report no_upcoming."
            );
            assert(
                emptyUpcoming.canExpandScope === true,
                "Controlled current DB copy should allow explicit scope expansion."
            );
            console.log(
                "✓ 4.1 A controlled copy of the current DB returns a strict no-upcoming empty state."
            );
        } finally {
    ...
    if (fs.existsSync(backupDbPath)) {
        ...
        try {
            ...
            assert(
                recommendUpcoming.type === "recommend",
                "Backup DB should produce recommendations."
            );
            assert(
                recommendUpcoming.scope === "upcoming",
                "Backup DB should recommend from upcoming scope first."
            );
            assert(
                recommendUpcoming.recommendations.length >= 1,
                "Backup DB should return at least one recommendation."
            );
            console.log(
                "✓ 4.2 Backup DB can produce recommendations from real upcoming candidates."
            );
            ...
            assert(
                fallbackRecommend.type === "recommend",
                "Invalid model IDs should fall back to usable recommendations."
            );
            assert(
            ...
            assert(
                ...
                "First ambiguous turn should allow a clarification."
            );
            ...
            assert(
                limitedClarify.type !== "clarify",
                "Second clarification attempt should not ask again."
            );
      ...
      `,
        ...
        assert(
            !upcomingCandidates.some(
                (event) => event.title === "待审核未来活动" || event.title === "已删除未来活动"
            ),
