...
const normalizeFavoriteItemTypes = async (db) => {
    ...
    for (const [from, to] of Object.entries(ITEM_TYPE_MAP)) {
        const result = await db.run(`UPDATE favorites SET item_type = ? WHERE item_type = ?`, [
            to,
            from,
        ]);
        if (result?.changes) {
    ...
    for (const { table, itemType } of RESOURCE_TABLES) {
      ...
      `,
        ...
        const mismatched = drift.filter(
            (row) => Number(row.stored || 0) !== Number(row.actual || 0)
        );
        console.log(
            `  ${table} (${itemType}): total ${drift.length}, drifted ${mismatched.length}`
        );
        if (mismatched.length) {
