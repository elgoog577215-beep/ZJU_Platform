...
const stressModelRunner = async ({ task, messages }) => {
    ...
    if (task === "event_profile") {
        ...
        return {
            ...
            category: isAi
                ? String(event.title).includes("黑客松")
                    ? "competition"
                    : "lecture"
    ...
    if (task === "event_recommendation_rerank") {
        ...
        return {
            ...
            recommendations: ranked.slice(0, 3).map((candidate, index) => ({
                ...
                matched_signals: [
                    /AI|Agent|黑客松/.test(`${candidate.title} ${candidate.description}`)
                        ? "AI 主题"
                        : "校园活动",
    ...
    if (task === "event_governance_review") {
        ...
        return {
            ...
            memorySignals: [
                {
                    pattern: "AI 实践、Demo、路演活动优先归入 competition 或 lecture。",
                    weight: 0.8,
                },
            ],
            warnings: [],
...
const stressEventRecommendation = async (db) => {
    ...
    assert(
        ...
        "Event recommendation should include multiple ranked options."
    );
    assert(
        result.recommendations[0].confidence >= 0.85,
        "Top recommendation should carry strong confidence."
    );
    assert(
        /AI|黑客松|Agent/.test(result.recommendations[0].event.title),
        "Top recommendation should match AI intent."
    );
    for (const item of result.recommendations) {
        assertUsefulText(item.reason, "Recommendation reason");
        assert(
            Array.isArray(item.matchSignals) && item.matchSignals.length > 0,
            "Recommendation should expose match signals."
        );
    }
    assert(
    ...
    assert(
        ...
        "Recommendation should use model rerank."
    );
    assert(
    ...
    assert(
        ...
        "Recommendation should exercise profile indexing."
    );
...
const stressHackathonCoach = async (db) => {
    ...
    assert(
        result.recommendation.fitScore >= 70,
        "Hackathon fit score should be normalized and useful."
    );
    ...
    assert(
        ...
        "Hackathon coach should identify high-impact risks."
    );
...
const stressWechatParser = async (db) => {
    ...
    assert(
        parsed.category === "competition",
        "WeChat parser should normalize to a standard category."
    );
...
const stressGovernance = async (db) => {
    ...
    assert(
        ...
        "Governance should use model review for ambiguous candidates."
    );
    assert(
        scan.summary.modelReview.runtimeTelemetry.taskCount === 1,
        "Governance should expose runtime telemetry."
    );
    assert(
        scan.suggestions.every((suggestion) =>
            ["category", "target_audience"].includes(suggestion.field)
        ),
...
const stressProfileIndex = async (db) => {
    ...
    assert(
        refresh.summary.runtimeTelemetry.taskCount >= 4,
        "Profile refresh should aggregate telemetry."
    );
};
...
const stressRuntimeQuality = async (db) => {
    ...
    const result = await aiRuntime.callJson(db, {
        ...
        modelRunner: async ({ messages }) => {
            sawQualityInstruction = messages.some((message) =>
                String(message.content || "").includes("Quality requirements")
            );
    ...
    assert(
        result.modelStatus.telemetry.promptTokensEstimate > 0,
        "Runtime should record telemetry."
    );
};
...
const main = async () => {
    ...
    try {
        ...
        assertNoRawLeak(
            runs,
            ["这周我想在紫金港参加", "5月20日19:00-21:00"],
            "assistant run summaries"
        );

        const overview = await assistantService.getAssistantOverview(db);
        assert(
            overview.health.runtimeTelemetryTaskCount >= 6,
            "Overview should aggregate runtime telemetry after stress."
        );
        assert(
        ...
        assert(
            typeof overview.health.agentRuntimeHealth.event_recommendation.fallbackRate ===
                "number",
