...
首页以“安静的科技导航”为方向，面向大学生和技术人群的日常查找任务。沿用品牌 Logo 与全站主题切换；仅首页使用森林绿中性色，不改变其他模块。

- 深色：底色 `#101815`、链接表面 `#16211c`、正文 `#e9efeb`、辅助文字 `#a3b3a9`、强调色 `#acd1bb`。浅色：底色 `#f7f9f7`、表面 `#eef2ef`、正文 `#1c3025`、辅助文字 `#57685d`、强调色 `#285e41`。
- 品牌栏、简短标题与搜索、常用直达、用途分类、网址网格、生态入口按序排列。第一排优先 AI 助手、模型数据和 AI 开发实验。
- 桌面三个分类并列，组间留白、组内紧凑；平板两列，手机单列。链接名称可换行，辅助说明单行省略且通过链接可访问名称保留完整用途。
- 标题用系统无衬线，链接 12–13px、辅助说明 11px，常规字重 400–550。低对比表面区分入口，正文保持可读对比。
