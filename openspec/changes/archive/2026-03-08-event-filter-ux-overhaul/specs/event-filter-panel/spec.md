...
`Dropdown` 组件的触发按钮和下拉菜单选项 SHALL 完整显示选项文字，不因容器宽度不足而截断图标或文字。长文字 SHALL 在按钮内以省略号截断（文字本身 truncate），但图标 SHALL 始终完整显示。
...
- **THEN** 每个选项的文字 SHALL 完整显示（允许换行）或以省略号截断，但勾选图标 SHALL 始终可见
