...
## 1. 后端 · 数据与实体
- [x] 1.1 `runMigrations.js` 新增 `project_cards` 表 + 索引（status/progress/created, user_id, likes/created）
- [x] 1.2 新增 `projectCardController.js`：create / update / delete（作者校验）
- [x] 1.3 list（q/progress/need/page，只回 published）/ detail（views++，联系方式按登录态裁剪）
- [x] 1.4 创建限流（customRateLimit，仿社区帖）+ repo_url https 校验 + 正文转义
- [x] 1.5 图片上传复用现有 cover 上传基建（JPG/PNG/WebP·5MB）
- [x] 1.6 路由接入 `api.js`（鉴权中间件 + 限流）

## 2. 后端 · 注册式复用（每处加一项）
- [x] 2.1 `favoriteController.js`：`FAVORITE_RESOURCE_META['project']` + `resolveFavoriteTarget` + likes 重算表名
- [x] 2.2 `notificationController.js`：`RESOURCE_TYPE_LABEL['project'] = '项目'`
- [x] 2.3 `userController.js getUserResources`：聚合 `project_cards`（type:'project'，访客只见 published，本人见草稿）
- [x] 2.4 举报：扩展 reports 支持 `target_type='project'` + 管理员下架（status=removed）

## 3. 前端 · 页面（由 /lab/projects 预览产品化）
- [x] 3.1 `ProjectPlaza.jsx`（`/projects`）：header/搜索/筛选/网格 + 读 `?id=` 开详情 + fromFavorites 返回 + 跟随 uiMode 主题
- [x] 3.2 `ProjectDetailModal.jsx`：画廊+缩略图 / 长正文 / 概要数据 / 需求 / 技术 / 联系(登录可见) / 看仓库 / 收藏；桌面弹窗+移动全屏+useBackClose
- [x] 3.3 `ProjectCreateEdit.jsx`（`/projects/new`、`/projects/:id/edit`）：表单 + 实时预览 + 存草稿/发布
- [x] 3.4 空状态 / 未登录可看引导登录 / 无封面占位

## 4. 前端 · 接入现有体系
- [x] 4.1 `App.jsx`：新增 `/projects*` 路由；移除临时 `/lab/projects` + 删 `ProjectPlazaPreview.jsx`
- [x] 4.2 `Navbar.jsx` / `MobileNavbar.jsx`：「项目广场」入口（非 AI 社区下）
...
- [x] 4.8 机会雷达筛选优化：开放招募入口筛任意需求项目，并与具体需求筛选互斥
