...
About 的“资源与合作”区域使用学院、技术企业、行业企业、资本、社团五块叙事卡片，与正式支持方分类一一对应；“查看全部支持方”进入未筛选的 `/about/partners`。新名录页采用 Read 模式：首屏说明支持网络、提供五类切换和搜索，正文按真实数据展示支持方，而不是再做一页品牌宣传。
...
名录页首屏不再只依靠大标题制造冲击力，而是把五类支持方、各类真实数量和总量组成可点击的网络图。中心节点返回全部支持方，外围节点直接切换对应分类；连线只表达这五类共同支撑生态的结构，不增加虚构合作关系。该结构复用 About 的冷色线条、巨大标题和公共实验场语汇，并在移动端保持同一信息顺序与完整可操作性。

### 8. 分类卡片通过深链和共享元素进入详情

About 的五类卡片分别进入 `/about/partners?category=<category>`，详情页以 query 作为当前分类真源；在详情内切换分类使用 replace 更新 query，避免筛选行为阻断浏览器返回 About。支持 View Transitions 的浏览器把被点击卡片与详情页对应网络节点作为同一共享元素，其他内容使用现有页面过渡；返回入口使用同一关系反向过渡。组件预加载保证跨路由动画不会先落到加载屏。`prefers-reduced-motion`、不支持 View Transitions 或低性能环境直接使用现有导航与轻量入场，不添加新依赖。
...
- [五类卡片比原四类叙事增加一个入口] → 用真实数量决定桌面网格面积，压缩重复说明；移动端保持单一阅读顺序，不缩小为不可读卡片。
- [Logo 尺寸差异造成视觉跳动] → 使用固定标志容器和 `object-contain`，无 Logo 时使用名称缩写，不生成假 Logo。
- [网络图被误读为具体合作关系图] → 连线只从五类节点汇聚到“全部支持方”中心，不绘制支持方之间的关系，也不展示未经确认的强弱、层级或流向。
