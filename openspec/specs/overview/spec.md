...
保证开发者与 AI Agent 能从少量、职责明确且可随项目更新的文档理解拓途浙享，并把项目事实、项目规则、专项资料与历史材料分开维护。
...
- **THEN** 能确定项目边界、执行与验收规则、文档职责、沉淀方式和 Git 收束方式

### Requirement: 当前事实按职责拆分

系统 SHALL 使用以下当前真源，并禁止建立重复概览或平行规则入口：

- `docs/产品蓝图.md`：产品定义、生态闭环、领域边界和长期建设顺序。
- `docs/产品状态.md`：当前成熟度、活动 OpenSpec、缺口、风险和主攻位置。
- `docs/技术架构.md`：当前代码结构、数据真源、运行链、兼容和部署边界。
- `AGENTS.md`：本项目长期有效的项目边界、执行规则、验收案例和沉淀方式。
- `openspec/changes/` 与正式 spec：具体功能正在采用或已经稳定的合同。
