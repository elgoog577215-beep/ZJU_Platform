# project-engagement Specification

## Purpose
项目名片可被收藏（=点赞，复用 `favorites`）并计入热度，被收藏时通知发起人（复用通知系统）；公开内容支持举报与管理员下架。（由 add-project-plaza 同步）
...
When a user favorites another user's project card, the project's owner SHALL receive a notification of type `favorite`, reusing the existing notification mechanism (identical to posts). Project-related notifications SHALL be clickable and navigate to the project detail.
...
- **AND** clicking it navigates to `/projects?id={card id}`.
