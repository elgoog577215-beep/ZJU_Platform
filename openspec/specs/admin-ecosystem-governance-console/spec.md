# admin-ecosystem-governance-console Specification

## Purpose
把管理员后台组织为面向生态运营的治理控制台，使内容与审核、主体与关系、项目运营、AI 能力和系统审计各有稳定入口、状态与权限边界，并保持历史 URL、现有业务合同和移动端访问方式可兼容。
...
### Requirement: Every admin workspace prioritizes its primary body

后台所有右侧管理工作区 SHALL 使用一致、紧凑的信息层级，让管理员进入模块后立即看到可判断、可操作的主体内容。

#### Scenario: 管理员打开任一现有管理模块

- **WHEN** 管理员从左侧导航进入总览、内容与审核、主体与关系、生态项目运营、AI 能力治理或系统与审计中的任一现有模块
- **THEN** 页面按紧凑页面头、必要状态或筛选、表格/列表/表单正文的顺序呈现
- **AND** 页面头不使用独立卡片包装，不通过大段说明文字解释布局
- **AND** 同类数字集中为单行状态，不拆成装饰性指标卡阵列
- **AND** 内容章节默认由留白和分隔线组织，不层层嵌套圆角面板
- **AND** 只有容易误解的指标、不可逆操作、异常状态或证据不足场景保留解释和强调容器
- **AND** 页面重排不改变既有 API、权限、URL tab、数据合同或业务状态机。
