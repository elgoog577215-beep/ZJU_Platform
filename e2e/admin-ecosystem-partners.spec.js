...
const installAdminPartnerMocks = async (page, onUpdate) => {
    ...
    await page.route("**/api/**", async (route) => {
        ...
        if (path === "/admin/ecosystem-partners/901" && request.method() === "PUT") {
...
test("admin ecosystem partners editor submits organization metadata fields", async ({ page }) => {
    ...
    await expect(page.getByRole("table").getByText("学生活动共创与校园传播")).toBeVisible();
    ...
    await page.getByTestId("ecosystem-partner-name-en-input").fill("ZJU Student Union Updated");
    await page
