    ...
    content_blocks: JSON.stringify([
        {
            type: "text",
            style: "paragraph",
            text: "A short digest for builders with enough content to open detail.",
        },
    ]),
...
const installCommunityMocks = async (page, currentNewsItem = newsItem) => {
    ...
    await page.route("**/api/news/901", (route) => route.fulfill({ json: currentNewsItem }));
    ...
    await page.route("**/api/articles?**", (route) =>
        route.fulfill({
            json: { data: [], pagination: { total: 0, page: 1, limit: 3, totalPages: 0 } },
        })
    );
...
test("mobile profile tab opens auth modal for signed-out users", async ({ page }) => {
    ...
    await expect.poll(() => page.evaluate(() => window.location.pathname)).toBe("/events");

    await page.goto("/me");
    await expect(page.getByRole("button", { name: "我的" })).not.toHaveAttribute(
        "aria-current",
        "page"
    );
});
...
test("mobile news tab opens detail fullscreen and closes with browser back", async ({ page }) => {
    ...
    await expect(page.getByRole("tab", { name: /新闻热点/ })).toHaveAttribute(
        "aria-selected",
        "true"
    );
    await page.getByRole("button", { name: /Campus AI News/ }).click();
