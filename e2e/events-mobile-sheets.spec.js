...
test.describe("events mobile sheets", () => {
    test("mobile college sheet only shows college filtering and restores page interaction", async ({
        ...
        await expect(filterDialog.getByRole("button", { name: "讲座", exact: true })).toHaveCount(
            0
        );

        await filterDialog.getByLabel("搜索学院或学园").fill("计算机");
        await filterDialog.getByRole("button", { name: "计算机科学与技术学院" }).click();
    ...
    test("mobile filter and sort sheets close with browser back", async ({ page }) => {
        await setupRoutes(page);
        await page.setViewportSize(mobileViewport);
        await page.goto("/events");
