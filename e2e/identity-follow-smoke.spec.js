...
async function approveResource(apiRequest, resourceTable, id) {
    ...
    const resp = await apiRequest.put(`${BASE_API}/${resourceTable}/${id}/status`, {
...
async function PrimeAuthStorage(page, user) {
    ...
    await page.evaluate(
        ({ token, id, username }) => {
            ...
            localStorage.setItem("user", JSON.stringify({ id, username, role: "user" }));
            localStorage.setItem("auth_token", token);
            localStorage.setItem("auth_user", JSON.stringify({ id, username, role: "user" }));
        },
...
test.describe("identity & follow notifications smoke", () => {
    ...
    // ---------------------------------------------------------------------------
    test("11.2 nickname collision returns 409 (API)", async ({ request: apiRequest }) => {
    ...
    test.fixme("11.2b nickname collision surfaces toast in settings UI", async () => {});
    ...
    test("11.3 follow triggers new_content notification after publish", async ({
        ...
        await expect
            .poll(
                ...
                {
                    message: "expected a new_content notification mentioning publisher + title",
                    timeout: 90_000,
        ...
        await expect
            .poll(
                async () => {
                    const payload = await fetchNotifications(apiRequest, fan);
                    const list = payload?.data || payload?.notifications || [];
                    const match = list.find((n) => JSON.stringify(n).includes(helpTitle));
    ...
    test("11.5 self-follow rejected with 400 (POST and DELETE)", async ({
        ...
        expect(JSON.stringify(postBody)).toMatch(/不能关注自己|Cannot follow yourself/i);

        const delResp = await toggleFollow(apiRequest, user, user.id, "DELETE");
        expect(delResp.status()).toBe(400);
        const delBody = await delResp.json();
        expect(JSON.stringify(delBody)).toMatch(/不能关注自己|Cannot follow yourself/i);
    });
    ...
    test.fixme("11.7 profile tab + scroll memory (UI-only; future smoke)", async () => {});
});
