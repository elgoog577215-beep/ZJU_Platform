    ...
    recommendations: [
        {
            ...
            opportunityMatch: {
                ...
                decisionHint:
                    "优先推荐「AI Agent Product Workshop」，因为它更完整满足你明确提出的条件。",
                decisionSupport: {
...
const setupRoutes = async (page) => {
    ...
    await page.route(`**/api/events/${event.id}`, (route) => route.fulfill({ json: event }));
    await page.route("**/api/events/*/view", (route) => route.fulfill({ json: { views: 12 } }));
...
test.describe("event assistant flow", () => {
    test("desktop assistant shows a simplified recommendation, feedback reasons, and opens detail", async ({
        page,
    }) => {
        ...
        await expect
            ...
            .toBeTruthy();
        const favoriteRequest = actionRequests.find((request) => request.actionType === "favorite");
