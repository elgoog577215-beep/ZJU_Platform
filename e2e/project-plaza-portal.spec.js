...
test("project plaza mobile detail uses body portal and restores scroll lock", async ({ page }) => {
...
test("project poster download opens native save page inside mini program webview", async ({
    page,
}) => {
    await installProjectMocks(page);
    await page.setViewportSize(mobileViewport);
    ...
    await page.route("**/api/native-poster-sessions", async (route) => {
        ...
        await route.fulfill({
            status: 201,
            json: {
                sessionId: "poster-session-1",
                token: "poster-token",
                fileName: "tuotu-project-Campus-AI-Builder.png",
                imageUrl:
                    "https://tuotuzju.com/api/native-poster-sessions/poster-session-1/image?token=poster-token",
    ...
    await expect
        ...
        .toBe(true);
    await expect
        .poll(() => page.evaluate(() => window.__miniNavigateUrls || []))
        .toContainEqual(expect.stringContaining("/pages/native-poster-save/index?"));
    await expect
...
test("project poster share opens native image share page inside mini program webview", async ({
    page,
}) => {
    await installProjectMocks(page);
    await page.setViewportSize(mobileViewport);
    ...
    await page.route("**/api/native-poster-sessions", async (route) => {
        ...
        await route.fulfill({
            ...
            json: {
                ...
                fileName: "tuotu-project-Campus-AI-Builder.png",
                imageUrl:
                    "https://tuotuzju.com/api/native-poster-sessions/poster-session-share/image?token=poster-token-share",
                expiresAt: "2026-07-07T12:00:00.000Z",
                expiresIn: 600,
            },
        });
