...
Page({
  ...
  onLoad(options) {
    ...
    const projectPath = normalizePath(
      rawPath || (explicitProjectId ? `/projects?id=${encodeURIComponent(explicitProjectId)}` : "/projects"),
    );
  ...
  goBack() {
    const pages = typeof globalThis.getCurrentPages === "function" ? globalThis.getCurrentPages() : [];
    if (pages.length > 1) {
