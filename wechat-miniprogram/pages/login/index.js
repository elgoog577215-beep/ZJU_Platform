...
const LOGIN_COPY = {
  kicker: "WECHAT LOGIN",
  title: "微信账号登录",
  description: "使用已绑定的微信身份完成登录后，将自动回到拓途浙享网页，并复用网站账号与内容权限。",
...
Page({
  ...
  openWebViewWithBindResult(result) {
    let targetPath = appendQueryParam(this.data.redirect || DEFAULT_PATH, "wechat_bind", result);
    targetPath = appendQueryParam(targetPath, "miniapp", "1");
