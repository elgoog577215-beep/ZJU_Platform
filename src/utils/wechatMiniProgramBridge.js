...
const hasMiniProgramNavigator = () =>
    typeof window !== "undefined" && Boolean(window.wx?.miniProgram?.navigateTo);

const hasMiniProgramMessenger = () =>
    typeof window !== "undefined" && Boolean(window.wx?.miniProgram?.postMessage);
...
export const buildWechatNativePosterSaveBridgeUrl = ({
    imageUrl,
    fileName,
    returnPath = "/projects",
    action = "save",
...
export const sharePosterViaNativeMiniProgram = async (payload = {}) =>
    navigateToMiniProgramPage(
        buildWechatNativePosterSaveBridgeUrl({
            ...payload,
            action: "share",
        })
    );
