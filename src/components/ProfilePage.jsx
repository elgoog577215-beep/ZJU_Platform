...
const canEditProfile = (profile, currentUser) => {
    ...
    if (profile.owner_user_id && String(profile.owner_user_id) === String(currentUser.id))
        return true;
...
const ProfileMark = ({ profile, isDayMode, displayName }) => {
    ...
    if (src) {
        return (
            <div
                className={`flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-[8px] border md:h-24 md:w-24 ${
    ...
    return (
        <div
            className={`flex h-20 w-20 shrink-0 items-center justify-center rounded-[8px] border text-2xl font-black md:h-24 md:w-24 ${
                isDayMode
...
const localizedProfileText = (profile, language, enField, zhField) => {
    ...
    return isEnglish
        ? profile?.[enField] || profile?.[zhField] || ""
        : profile?.[zhField] || profile?.[enField] || "";
};
...
const FeedCard = ({ item, isDayMode, t }) => {
    ...
    const content = (
        <article
            className={`group grid min-h-[148px] grid-cols-[108px_1fr] overflow-hidden rounded-[8px] border transition-colors md:grid-cols-[148px_1fr] ${
                isDayMode
        ...
        >
            <div
                className={`relative min-h-full ${isDayMode ? "bg-slate-100" : "bg-white/[0.04]"}`}
            >
                {item.cover ? (
                    <img
                        src={item.cover}
                    ...
                    <div className="flex h-full min-h-[148px] items-center justify-center">
                        <FileText
                            className={isDayMode ? "text-slate-400" : "text-white/35"}
                            size={28}
                        />
                    </div>
                )}
            ...
            <div className="flex min-w-0 flex-col p-3 md:p-4">
                <div className="mb-2 flex items-center gap-2">
                    <span
                        className={`rounded-[4px] border px-2 py-1 text-[11px] font-black uppercase ${
                            isDayMode
                ...
                <h3
                    className={`line-clamp-2 text-base font-black leading-6 md:text-lg ${
                ...
                <div
                    className={`mt-auto flex items-center gap-3 pt-3 text-xs font-semibold ${
    ...
    return (
        <Link
            to={target}
            className="block focus:outline-none focus-visible:ring-4 focus-visible:ring-sky-300/30"
        >
            {content}
        </Link>
    );
...
const ProfilePage = ({ forcedHandle = null }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        api.get(`/profiles/${handle}`, { signal: controller.signal })
            .then((response) => {
    ...
    const description =
        localizedProfileText(profile, language, "description_en", "description") ||
    ...
    const profileCardTags = useMemo(
        () =>
            uniqueText(
                (profileCard?.tags || []).map((tag) => tag.label),
                5
            ),
        [profileCard?.tags]
    );
    ...
    const heroTags = useMemo(() => {
        ...
        return uniqueText(
            [metaLabel, profile?.verified ? t("profiles.page.verified") : "", ...profileAliasTags],
            5
        );
    }, [
        metaLabel,
        ...
        try {
            ...
            setPasswordError(
                error.response?.data?.error ||
                    validationMessage ||
                    error.message ||
                    t("profiles.page.password_failed")
            );
        } finally {
    ...
    if (error || !profile) {
        return (
            <div
                className={`mx-auto flex min-h-[70vh] max-w-3xl flex-col items-center justify-center px-4 pb-[calc(env(safe-area-inset-bottom)+6rem)] pt-[calc(env(safe-area-inset-top)+5.25rem)] text-center md:pb-12 md:pt-28 ${
    ...
    return (
        <section
            className={`min-h-screen px-4 pb-[calc(env(safe-area-inset-bottom)+6rem)] pt-[calc(env(safe-area-inset-top)+5.25rem)] md:px-8 md:pb-12 md:pt-28 ${
                isDayMode ? "bg-slate-50 text-slate-950" : "text-white"
            }`}
        >
            <SEO
                title={`${displayName} - ${t("profiles.page.title_suffix")}`}
            ...
            <div className="mx-auto w-full max-w-6xl">
                ...
                >
                    ...
                    <div className="grid gap-5 p-4 md:grid-cols-[auto_1fr_auto] md:gap-6 md:p-6">
                        ...
                        <div className="min-w-0">
                            <div className="mb-3 flex flex-wrap items-center gap-2">
                                <span
                                    className={`inline-flex items-center gap-1.5 rounded-[4px] border px-2.5 py-1 text-xs font-black ${
                                        isDayMode
                                ...
                                {profileCardStatusLabel ? (
                                    <span
                                        className={`inline-flex items-center rounded-[4px] border px-2.5 py-1 text-xs font-black ${
                                            isDayMode
                            ...
                            <div
                                className={`mt-4 inline-flex max-w-full items-center rounded-[4px] border px-2.5 py-1 text-[11px] font-black uppercase tracking-[0.12em] ${
                                    isDayMode
                        ...
                        <div className="flex flex-wrap gap-2 md:flex-col md:items-end">
                            <button
                                ...
                                aria-label={t("profiles.page.share_card_aria", {
                                    name: displayName,
                                })}
                            >
                                <Share2 size={16} />
                            ...
                            {editable ? (
                                <button
                                    ...
                                    className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-[6px] border px-4 text-sm font-bold ${
                                        isDayMode
                                            ? "border-sky-200 bg-sky-50 text-sky-700 hover:bg-white"
                    ...
                    >
                        {[
                            ...
                            [
                                t("profiles.page.stats_status"),
                                profile.verified
                                    ? t("profiles.page.verified")
                                    : t("profiles.page.normal"),
                            ],
                        ].map(([label, value]) => (
                            <div
                                key={label}
                            ...
                            >
                                <div
                                    className={`text-xs font-black uppercase ${isDayMode ? "text-slate-400" : "text-slate-500"}`}
                                >
                                    {label}
                                </div>
                ...
                {editing
                    ? (() => {
                          const primaryImageField =
                              profile.type === "person" ? "avatar_url" : "logo_url";
                          const primaryImageLabel =
                              profile.type === "person"
                                  ? t("profiles.page.avatar")
                                  : t("profiles.page.logo");
                          return (
                              <form
                                  ...
                                  className={`mt-4 rounded-[8px] border p-4 md:p-5 ${
                                      isDayMode
                                          ...
                                          : "border-sky-300/15 bg-white/[0.04]"
                                  }`}
                              >
                                  <div className="mb-4 flex items-center justify-between gap-3">
                                      <div>
                                          <h2 className="text-lg font-black">
                                              {t("profiles.page.edit_title")}
                                          </h2>
                                          <p
                                              className={`mt-1 text-sm ${isDayMode ? "text-slate-500" : "text-slate-400"}`}
                                          >
                                  ...
                                  {editError ? (
                                      <div
                                          className={`mb-4 rounded-[6px] border px-3 py-2 text-sm font-semibold ${
                                              isDayMode
                                  ...
                                  <div className="grid gap-4 md:grid-cols-2">
                                      <label
                                          className={`grid gap-2 text-sm font-bold ${labelClass}`}
                                      >
                                      ...
                                      <label
                                          className={`grid gap-2 text-sm font-bold ${labelClass}`}
                                      >
                                          {t("profiles.page.primary_image_url", {
                                              label: primaryImageLabel,
                                          })}
                                          <div className="flex gap-2">
                                              ...
                                              />
                                              <span
                                                  className={`relative inline-flex min-h-11 shrink-0 cursor-pointer items-center gap-2 rounded-[6px] border px-3 text-sm font-black ${
                                                      isDayMode
                                                          ? "border-slate-200 bg-slate-50 text-slate-700"
                                                          : "border-white/10 bg-white/[0.05] text-slate-100"
                                                  }`}
                                              >
                                                  {uploadingField === primaryImageField ? (
                                      ...
                                      >
                                          ...
                                          <input
                                              ...
                                              onChange={(event) =>
                                                  updateEditField("link_url", event.target.value)
                                              }
                                              className={`h-11 rounded-[6px] border px-3 outline-none ${inputClass}`}
                                              placeholder="https://..."
                                              maxLength={1000}
                                      ...
                                      >
                                          <div className="mb-4 flex items-center gap-2">
                                              <KeyRound
                                                  size={17}
                                                  className={
                                                      isDayMode ? "text-sky-600" : "text-sky-200"
                                                  }
                                              />
                                              <div>
                                                  <h3 className="text-base font-black">
                                                      {t("profiles.page.password_title")}
                                                  </h3>
                                                  <p
                                                      className={`mt-0.5 text-xs ${isDayMode ? "text-slate-500" : "text-slate-400"}`}
                                                  >
                                          ...
                                          {passwordError ? (
                                              <div
                                                  className={`mb-3 rounded-[6px] border px-3 py-2 text-sm font-semibold ${
                                                      isDayMode
                                                          ? "border-red-200 bg-red-50 text-red-700"
                                                          : "border-red-400/25 bg-red-500/10 text-red-100"
                                                  }`}
                                      ...
                                      <button
                                          ...
                                          className={`inline-flex min-h-11 items-center gap-2 rounded-[6px] px-4 text-sm font-black disabled:cursor-not-allowed disabled:opacity-55 ${
                                              isDayMode
                                                  ...
                                                  : "bg-sky-300 text-slate-950 hover:bg-white"
                                          }`}
                                      >
                                          {saving ? (
                ...
                <div className="mt-4">
                    {feedLoading ? (
                        <div
                            className={`flex min-h-56 items-center justify-center rounded-[8px] border ${
                                isDayMode
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            {feed.map((item) => (
                                <FeedCard
                                    key={`${item.type}-${item.id}-${item.relation || "published"}`}
                                    item={item}
                                    isDayMode={isDayMode}
                                    t={t}
                                />
                        ...
                        <div
                            className={`flex min-h-64 flex-col items-center justify-center rounded-[8px] border border-dashed px-4 text-center ${
                                isDayMode
                        ...
                        >
                            <Grid3X3 size={34} />
                            <h2
                                className={`mt-4 text-lg font-black ${isDayMode ? "text-slate-900" : "text-white"}`}
                            >
                ...
                {pagination.totalPages > 1 ? (
                    ...
                    >
                        {t("profiles.page.page_indicator", {
