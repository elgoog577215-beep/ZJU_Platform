...
const STATUS_CONFIG = {
    // help statuses
    open: {
        label: "community.post_status_open",
        bg: "bg-amber-500/15",
        text: "text-amber-400",
    ...
    solved: {
        label: "community.post_status_solved",
        bg: "bg-emerald-500/15",
        text: "text-emerald-400",
        border: "border-emerald-500/20",
        dayBg: "bg-emerald-50",
        dayText: "text-emerald-600",
        dayBorder: "border-emerald-200",
    },
    closed: {
        label: "community.post_status_closed",
        bg: "bg-gray-500/15",
        text: "text-gray-400",
    ...
    // team statuses
    recruiting: {
        label: "community.post_status_recruiting",
        bg: "bg-violet-500/15",
        text: "text-violet-400",
        border: "border-violet-500/20",
        dayBg: "bg-violet-50",
        dayText: "text-violet-600",
        dayBorder: "border-violet-200",
    },
    full: {
        label: "community.post_status_full",
        bg: "bg-blue-500/15",
        text: "text-blue-400",
        border: "border-blue-500/20",
        dayBg: "bg-blue-50",
        dayText: "text-blue-600",
        dayBorder: "border-blue-200",
    },
    published: {
        label: "community.post_status_published",
        bg: "bg-emerald-500/15",
        text: "text-emerald-400",
        border: "border-emerald-500/20",
        dayBg: "bg-emerald-50",
    ...
    draft: {
        label: "community.status_draft",
        bg: "bg-slate-500/15",
        text: "text-slate-300",
        border: "border-slate-500/20",
        dayBg: "bg-slate-100",
        dayText: "text-slate-600",
        dayBorder: "border-slate-200",
    },
    pending: {
        label: "community.post_status_pending",
        bg: "bg-yellow-500/15",
        text: "text-yellow-400",
        border: "border-yellow-500/20",
        dayBg: "bg-yellow-50",
        dayText: "text-yellow-600",
        dayBorder: "border-yellow-200",
    },
    rejected: {
        label: "community.status_rejected",
        bg: "bg-rose-500/15",
        text: "text-rose-300",
        border: "border-rose-500/20",
        dayBg: "bg-rose-50",
        dayText: "text-rose-600",
...
const normalizeMaterialType = (value) => {
    ...
    return ["course", "ai", "other"].includes(type) ? type : type;
};
...
const PostCard = memo(({ post, index, onClick, canAnimate, isDayMode }) => {
    ...
    const visibleStatus =
        post.review_status && post.review_status !== "approved" ? post.review_status : post.status;
    ...
    const progress =
        isTeam && post.max_members
            ? Math.min((post.current_members || 0) / post.max_members, 1)
            : 0;
    const materialMeta = isMaterials
        ? [
              ...
              post.material_type
                  ? t(
                        `community.material_type_${normalizeMaterialType(post.material_type)}`,
                        post.material_type
                    )
                  : "",
          ]
        ...
        >
            <div className="flex-1 space-y-2 md:space-y-2.5">
                ...
                <div className="flex items-center gap-2">
                    <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-[11px] font-semibold border ${isDayMode ? `${statusCfg.dayBg} ${statusCfg.dayText} ${statusCfg.dayBorder}` : `${statusCfg.bg} ${statusCfg.text} ${statusCfg.border}`}`}
                    >
                        {t(statusCfg.label)}
                    </span>
                    <span
                        className={`text-xs font-mono ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                    >
                ...
                {/* Title */}
                <h3
                    className={`text-base font-bold md:text-xl ${titleHover} transition-colors leading-snug ${isDayMode ? "text-slate-900" : "text-white"}`}
                >
                    {post.title}
                </h3>
                ...
                {/* Excerpt */}
                <p
                    className={`line-clamp-2 text-[13px] leading-5 md:text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                >
                ...
                {isTeam && post.max_members && (
                    <div className="space-y-2 pt-1">
                        <div
                            className={`flex items-center gap-4 text-xs ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                        >
                            {post.deadline && (
                            ...
                            <span className="flex items-center gap-1">
                                <Users size={12} />
                                {post.current_members || 0}/{post.max_members}{" "}
                                {t("community.post_members_unit", "人")}
                            </span>
                        </div>
                        {/* Progress bar */}
                        <div
                            className={`w-full h-1.5 rounded-sm overflow-hidden ${isDayMode ? "bg-slate-100" : "bg-white/10"}`}
                        >
                            <div
                ...
                <div
                    className={`flex flex-wrap items-center gap-x-3 gap-y-1 pt-1 text-xs ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                >
