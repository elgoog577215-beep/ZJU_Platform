        ...
        <div className="hidden md:block">
            ...
            >
                ...
                <span className="min-w-0 flex-1">
                    <span
                        className={`flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.22em] ${isDayMode ? "text-cyan-700" : "text-cyan-200"}`}
                    >
                        <Sparkles className="h-3.5 w-3.5" />
                        {label}
                    </span>
                    <span
                        className={`mt-1 block truncate text-base font-black ${isDayMode ? "text-slate-900" : "text-white"}`}
                    >
                        {placeholder}
                    </span>
                </span>
                <span
                    className={`hidden items-center gap-2 text-xs font-bold lg:flex ${isDayMode ? "text-slate-400" : "text-white/42"}`}
                >
                    {hint}
                    <span
                        className={`inline-flex items-center gap-1 border px-2 py-1 font-mono ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-white/[0.04]"}`}
                    >
