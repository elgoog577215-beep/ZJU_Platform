...
const Dropdown = ({
    ...
    return (
        <div
            ...
            ref={containerRef}
            style={{ zIndex: isOpen && !isSheet ? 50 : 1 }}
        >
            <button
                type="button"
                ...
                <div className="flex items-center gap-3 min-w-0">
                    {Icon && (
                        <Icon
                            size={18}
                            aria-hidden="true"
                            className={`shrink-0 transition-colors ${hasSelection ? (isDayMode ? "text-cyan-700" : "text-cyan-200") : isDayMode ? "text-slate-500 group-hover:text-cyan-600" : "text-slate-400 group-hover:text-cyan-100"}`}
                        />
                    )}
                    <span
                        className={`text-sm font-medium truncate min-w-0 flex-1 text-left ${hasSelection ? (isDayMode ? "text-slate-900" : "text-white") : isDayMode ? "text-slate-500" : "text-gray-400"}`}
                    >
                ...
                {isOpen && (
                    ...
                    >
                        <div
                            id={listboxId}
                            role="listbox"
                            className={`${isSheet ? "max-h-48" : "max-h-72"} overflow-y-auto dropdown-scrollbar p-1.5 space-y-0.5`}
                        >
                            {options.map((option) => (
                                <button
                                    key={option.value}
