        ...
        >
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 flex flex-col justify-center space-y-3">
                    ...
                    <h3
                        className={`text-2xl font-bold group-hover:text-blue-400 transition-colors ${isDayMode ? "text-slate-900" : "text-white"}`}
                    >
                        {article.title}
                    </h3>
                    ...
                    <div className="pt-2 flex items-center justify-end gap-3 mt-auto">
                        <FavoriteButton
                            ...
                            className={`p-2 rounded-md transition-colors hover:text-blue-500 ${isDayMode ? "hover:bg-blue-50 text-slate-500" : "hover:bg-white/10 text-gray-400"}`}
                            onToggle={(f, l) => onToggleFavorite(article.id, f, l)}
                        />
                        <div
                            className={`p-2 rounded-md group-hover:bg-blue-500 group-hover:text-white transition-all duration-300 ${isDayMode ? "bg-blue-50 text-blue-500" : "bg-white/5"}`}
                        >
                            <ArrowRight
                                size={18}
        ...
        <CommunityDetailModal
            ...
            headerContent={
                feed.selectedItem && (
                    <>
                        <div
                            className={`flex items-center gap-3 font-bold text-lg md:text-xl uppercase tracking-[0.2em] mb-4 ${isDayMode ? "text-blue-500" : "text-blue-300 drop-shadow-lg"}`}
                        >
                            <span>{feed.selectedItem.date}</span>
                        </div>
                        <h2
                            className={`text-4xl md:text-6xl font-black leading-[0.95] tracking-tight font-serif ${isDayMode ? "text-slate-900" : "text-white drop-shadow-2xl"}`}
                        >
            ...
            authorBar={
                feed.selectedItem && (
                    <FavoriteButton
                        ...
                        className={`p-3 rounded-md transition-all border ${isDayMode ? "bg-white/85 hover:bg-red-50 text-slate-700 border-slate-200/80" : "bg-white/5 hover:bg-red-500/20 text-white border border-white/10"}`}
                        onToggle={(f, l) => feed.handleToggleFavorite(feed.selectedItem.id, f, l)}
                    />
                )
            }
    ...
    return (
        <CommunityFeedPanel
            ...
            renderSkeleton={(i) => (
                <div
                    key={i}
                    className={`backdrop-blur-xl border rounded-lg p-6 animate-pulse flex flex-col md:flex-row gap-6 ${isDayMode ? "bg-white/82 border-slate-200/80" : "bg-white/[0.04] border-white/5"}`}
                >
                    <div
                        className={`w-full md:w-48 h-48 md:h-32 rounded-md shrink-0 ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                    />
