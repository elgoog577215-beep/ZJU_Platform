...
const HomeFeed = () => {
    ...
    const isLoaded =
        featuredContent && featuredContent.photos && Array.isArray(featuredContent.photos);
        ...
        <section className="relative w-full min-h-screen lg:h-screen lg:overflow-hidden pt-20 pb-4 px-4 md:px-8 flex flex-col">
            ...
            >
                ...
                <div className="lg:col-span-8 h-full flex flex-col gap-4 overflow-hidden">
                    ...
                    <motion.div
                        variants={item}
                        className="flex-1 min-h-[300px] lg:min-h-0 overflow-hidden"
                    >
                        <ErrorBoundary variant="inline" silent>
                            {feedMainEvent && (
                                <DashboardEvent event={feedMainEvent} onClick={setActiveEvent} />
                            )}
                        </ErrorBoundary>
                    </motion.div>
                ...
                <motion.div
                    variants={item}
                    className="lg:col-span-4 h-[600px] lg:h-full overflow-hidden"
                >
                            ...
                            >
                                <div className="text-center p-8 bg-[#111] border border-white/10 rounded-2xl">
                                    <p className="text-red-400 mb-4">
                                        {t("common.error_loading_viewer", "加载查看器失败")}
                                    </p>
                                    <button
                                        onClick={() => setActivePhoto(null)}
                        ...
                        >
                            <div
                                ...
                                style={
                                    activeEvent.image
                                        ? {
                                              backgroundImage: `url(${getHighResUrl(activeEvent.image)})`,
                                          }
                                        : {}
                                }
                            >
                                ...
                                <button
                                    onClick={() => setActiveEvent(null)}
                                    className="absolute top-6 right-6 p-2 bg-black/50 text-white rounded-full hover:bg-white/20 z-10"
                                >
                                    <X size={24} />
                                </button>
                            ...
                            <div className="p-8 md:p-12">
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                        __html: DOMPurify.sanitize(
                                            activeEvent.content || activeEvent.description
                                        ),
                                    }}
                                />
                                ...
                                <div className="mt-8 pt-8 border-t border-white/10 flex flex-wrap gap-4">
                                    <Link
                                        to={`/events?id=${activeEvent.id}`}
                                        className="px-6 py-3 bg-indigo-500 text-white rounded-lg font-bold hover:bg-indigo-400 transition-colors flex items-center gap-2"
                                    >
