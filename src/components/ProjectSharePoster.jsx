...
const ProjectSharePoster = ({ project, onClose, variant = "playful" }) => {
    ...
    const shareText = String(
        intro || t("project_share_poster.share_text", "看看这个项目广场里的校园项目")
    ).slice(0, 120);
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            toast.error(
                t("project_share_poster.download_failed", "海报生成失败，请检查图片是否可访问")
            );
        } finally {
            setBusy(null);
        }
    ...
    const handleNativeShare = async () => {
        ...
        if (isMiniProgramWebView()) {
            ...
            try {
                ...
                toast.success(
                    t(
                        "project_share_poster.use_top_right_share",
                        "请点击右上角 ···，选择转发给朋友"
                    )
                );
            } catch {
        ...
        >
            <div className="ppp-poster-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="ppp-poster-preview">
                    <div className="ppp-poster-card" ref={posterRef}>
                        <div className="ppp-poster-cover">
                            {coverUrl ? (
                                <img
                                    className="ppp-poster-cover-img"
                                    src={coverUrl}
                                    alt=""
                            ...
                            <div className="ppp-poster-topbar">
                                <div className="ppp-poster-site">
                                    ...
                                    <div>
                                        ...
                                        <span>
                                            {t("project_share_poster.site_subtitle", "项目广场")}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div className="ppp-poster-main">
                            ...
                            <div className="ppp-poster-meta-row">
                                <div className="ppp-poster-owner">
                                    <div className="ppp-poster-avatar">
                                        {ownerAvatar ? (
                                            <img src={ownerAvatar} alt="" crossOrigin="anonymous" />
                                        ) : (
                                    ...
                                    <div className="ppp-poster-owner-text">
                                        <span>
                                            {t("project_share_poster.owner_label", "发起人")}
                                        </span>
                                ...
                                <div className="ppp-poster-stats">
                                    <div>
                                        <strong>{formatCount(project?.likes)}</strong>
                                        <span>{t("project_share_poster.likes_label", "收藏")}</span>
                                    </div>
                                    <div>
                                        <strong>{formatCount(project?.views)}</strong>
                                        <span>{t("project_share_poster.views_label", "浏览")}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {allTags.length > 0 && (
                                <div className="ppp-poster-tags">
                                    {allTags.map((tag) => (
                                        <span
                                            className={`ppp-poster-tag ${tag.tone}`}
                                            key={`${tag.tone}-${tag.value}`}
                                        >
                                            {tag.value}
                                        </span>
                                    ))}
                            ...
                            <div className="ppp-poster-footer">
                                ...
                                <div className="ppp-poster-cta">
                                    <em>
                                        {t("project_share_poster.brand", "拓浙AI生态：项目广场")}
                                    </em>
                                    <strong>
                                        {t("project_share_poster.scan_title", "扫码查看项目详情")}
                                    </strong>
                                    <span>
                                        {t(
                                            "project_share_poster.scan_subtitle",
                                            "发现更多校园项目与实践伙伴"
                                        )}
                                    </span>
                                </div>
                                <div className="ppp-poster-qr">
                                    {qrDataUrl ? (
                                        <img
                                            src={qrDataUrl}
                                            alt={t("project_share_poster.qr_alt", "项目详情二维码")}
                                        />
                                    ) : (
                                        <span />
                                    )}
                ...
                <aside className="ppp-poster-side">
                    <span className="ppp-poster-side-kicker">
                        {t("project_share_poster.preview_label", "海报预览")}
                    </span>
                    <h3>{t("project_share_poster.dialog_title", "项目分享海报")}</h3>
                    <p>
                        {t(
                            "project_share_poster.dialog_desc",
                            "先确认画面，再下载 PNG 或复制项目链接。二维码会打开当前项目详情。"
                        )}
                    </p>
                    <div className="ppp-poster-actions">
                        ...
                        >
                            <Download size={16} />
                            {busy === "download"
                                ? t("project_share_poster.exporting", "生成中...")
                                : t("project_share_poster.download_png", "下载 PNG")}
                        </button>
                        <button
                    ...
                    <div className="ppp-poster-note">
                        {t(
                            "project_share_poster.privacy_note",
                            "海报不会显示联系方式，只展示公开项目信息。"
                        )}
                    </div>
                </aside>
            </div>
