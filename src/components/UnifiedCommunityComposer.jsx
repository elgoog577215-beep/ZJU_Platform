...
import {
    ...
    extractPlainText,
    parseComposerBlocks,
} from "./unifiedCommunityEditorUtils";
...
const normalizeImportedContentBlocks = (rawBlocks = []) => {
    ...
    return rawBlocks
        ...
        .map((block) => ({
            ...
            style: ["paragraph", "heading", "quote", "list", "code"].includes(block.style)
                ? block.style
                : "paragraph",
...
const UnifiedCommunityComposer = ({
    ...
    useEffect(() => {
        ...
        setBlocks(
            parsed.length
                ? parsed
                : deriveBlocksFromText(extractPlainText(initialData?.content || ""))
        );
        setPreviewOpen(false);
    }, [initialData, isOpen]);

    useEffect(() => {
        if (!isOpen || !config.supportsMaterialFields || materialType !== "course")
            return undefined;
    ...
    const handleBlockFileUpload = useCallback(
        async (blockId, file) => {
            if (!file) return;
    ...
    const availableBlocks = useMemo(() => {
        ...
        if (config.supportsCode)
            base.splice(4, 0, {
                type: "code",
                icon: Code2,
                label: t("community.block_code", "代码"),
            });
        return base;
    ...
    const cleanBlocks = useMemo(
        () =>
            blocks
                .filter((block) =>
                    block.type === "text" ? String(block.text || "").trim() : block.url
                )
                .map(({ file: _file, ...block }) => block),
        [blocks]
    );
    ...
    const normalizedMaterialCourse = useMemo(
        () => normalizeCourseName(materialCourse),
        [materialCourse]
    );
    const hasExactMaterialCourse = useMemo(
        () =>
            materialCourseOptions.some(
                (option) => normalizeCourseName(option.name) === normalizedMaterialCourse
            ),
    ...
    const buildPayload = useCallback(
        (status) => {
            const fallbackExcerpt =
                excerpt.trim() || plainContent.replace(/\s+/g, " ").slice(0, 140);
            ...
            if (normalizedBoardKey === "tech") {
                return {
                    ...
                    cover:
                        cover.trim() ||
                        cleanBlocks.find((block) => block.type === "image")?.url ||
                        "",
                    tags: tags.trim(),
    ...
    const submit = useCallback(
        async (status) => {
            ...
            try {
                ...
                toast.success(
                    status === "draft"
                        ? t("community.draft_saved", "草稿已保存")
                        : t("community.post_created", "发布成功")
                );
                onSuccess?.();
                onClose?.();
            } catch (error) {
                toast.error(
                    error?.response?.data?.error || t("community.post_create_failed", "发布失败")
                );
            } finally {
                setSubmitting(false);
            }
    ...
    return createPortal(
        <AnimatePresence>
            {isOpen && (
                ...
                >
                    <div
                        className={`community-composer-shell flex h-[100dvh] flex-col border ${shellCls}`}
                    >
                        <header
                            className={`community-composer-header flex items-center justify-between gap-3 border-b px-4 py-3 md:px-6 ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-neutral-950"}`}
                        >
                            <div className="min-w-0">
                                <p
                                    className={`community-composer-eyebrow text-[11px] font-black uppercase tracking-[0.22em] ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                                >
                        ...
                        <main className="community-composer-main grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[minmax(0,1fr)_360px]">
                                ...
                                <div className="community-composer-editor-inner mx-auto max-w-4xl space-y-5">
                                    <div className="community-composer-field space-y-2">
                                        <label className={labelCls}>
                                            {t("community.post_title_label", "标题")}
                                        </label>
                                        <input
                                            value={title}
                                            onChange={(e) => setTitle(e.target.value)}
                                            className={`community-composer-input ${inputCls} text-lg font-bold`}
                                            maxLength={120}
                                        />
                                    </div>
                                    ...
                                    <div className="community-composer-meta-grid grid grid-cols-1 gap-4 md:grid-cols-2">
                                        {config.supportsExcerpt && (
                                            <div className="community-composer-field space-y-2 md:col-span-2">
                                                <label className={labelCls}>
                                                    {t("community.excerpt", "摘要")}
                                                </label>
                                                <textarea
                                                    value={excerpt}
                                                    onChange={(e) => setExcerpt(e.target.value)}
                                                    rows={3}
                                                    className={`community-composer-input community-composer-short-textarea ${inputCls} resize-none`}
                                                />
                                            </div>
                                        )}
                                        ...
                                        {config.supportsSource && (
                                            <>
                                                ...
                                                <div className="community-composer-field space-y-2">
                                                    <label className={labelCls}>
                                                        {t("community.news_source_url", "来源链接")}
                                                    </label>
                                                    <input
                                                        value={sourceUrl}
                                                        onChange={(e) =>
                                            ...
                                            <>
                                                ...
                                                <div className="community-composer-mini-grid grid grid-cols-2 gap-3">
                                                    ...
                                                    <div className="community-composer-field space-y-2">
                                                        <label className={labelCls}>
                                                            {t(
                                                                "community.post_max_members_label",
                                                                "招募人数"
                                                            )}
                                                        </label>
                                                        <input
                                        ...
                                        {config.supportsMaterialFields && (
                                            <>
                                                <div className="community-composer-field space-y-2">
                                                    <label className={labelCls}>
                                                        {t("community.material_type", "资源分类")}
                                                    </label>
                                                    <select
                                                        value={materialType}
                                                        onChange={(e) =>
                                                ...
                                                <div className="community-composer-field space-y-2">
                                                    <label className={labelCls}>
                                                        {t("community.post_link_label", "资源链接")}
                                                    </label>
                                                    <input
                                                        value={link}
                                                        onChange={(e) => setLink(e.target.value)}
                                                        className={`community-composer-input ${inputCls}`}
                                                        placeholder={t(
                                                            "community.material_link_placeholder",
                                                            "可选，粘贴原文、网盘或工具链接"
                                                        )}
                                                    />
                                                </div>
                                                    ...
                                                    <>
                                                        <div className="community-composer-field space-y-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className={`community-composer-input ${inputCls}`}
                                                                placeholder={t(
                                                                    "community.material_course_placeholder",
                                                                    "可选，如：微积分 / 大学物理"
                                                                )}
                                                                maxLength={80}
                                                            />
                                                            <div
                                                                className={`rounded-lg border p-2 ${isDayMode ? "border-emerald-100 bg-emerald-50/60" : "border-emerald-400/15 bg-emerald-400/[0.06]"}`}
                                                            >
                                                                <div className="mb-1.5 flex items-center justify-between gap-2">
                                                                    <span
                                                                        className={`text-[11px] font-bold ${isDayMode ? "text-emerald-700" : "text-emerald-100"}`}
                                                                    >
                                                                        {t(
                                                                            "community.material_course_suggestions",
                                                                            "已有课程标签"
                                                                        )}
                                                                    </span>
                                                                    {materialCourseLoading ? (
                                                                        <Loader2
                                                                            size={12}
                                                                            className={`animate-spin ${isDayMode ? "text-emerald-600" : "text-emerald-200"}`}
                                                                        />
                                                                    ) : null}
                                                                </div>
                                                                <div className="flex flex-wrap gap-1.5">
                                                                    ...
                                                                    0 ? (
                                                                            ...
                                                                            (course) => {
                                                                                ...
                                                                                return (
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <span
                                                                                            className={`rounded px-1 py-0.5 text-[10px] ${isSelected ? "bg-white/20" : isDayMode ? "bg-emerald-50 text-emerald-600" : "bg-emerald-300/10 text-emerald-200"}`}
                                                                                        >
                                                                                            {
                                                                                                course.count
                                                                                            }
                                                                        ...
                                                                        <span
                                                                            className={`text-xs ${isDayMode ? "text-emerald-700/75" : "text-emerald-100/75"}`}
                                                                        >
                                                                            {t(
                                                                                "community.material_course_suggestions_empty",
                                                                                "还没有可复用课程，提交后会生成新标签"
                                                                            )}
                                                                        </span>
                                                                    )}
                                                                    {normalizedMaterialCourse &&
                                                                    !hasExactMaterialCourse ? (
                                                                        <button
                                                                            type="button"
                                                                            onClick={() =>
                                                                                setMaterialCourse(
                                                                        ...
                                                                        >
                                                                            {t(
                                                                                "community.material_course_create_label",
                                                                                {
                                                                                    course: normalizedMaterialCourse,
                                                                                }
                                                                            )}
                                                                        </button>
                                                                    ) : null}
                                                                ...
                                                                <p
                                                                    className={`mt-1.5 text-[11px] leading-4 ${isDayMode ? "text-emerald-700/75" : "text-emerald-100/70"}`}
                                                                >
                                                                    {t(
                                                                        "community.material_course_single_hint",
                                                                        "一份资料只能选择一个课程；新课程审核通过后会进入公共标签库。"
                                                                    )}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <div className="community-composer-field space-y-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className={`community-composer-input ${inputCls}`}
                                                                placeholder={t(
                                                                    "community.material_teacher_placeholder",
                                                                    "可选"
                                                                )}
                                                                maxLength={60}
                                                            />
                                                        ...
                                                        <div className="community-composer-field space-y-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className={`community-composer-input ${inputCls}`}
                                                                placeholder={t(
                                                                    "community.material_semester_placeholder",
                                                                    "如：2025-2026 秋冬"
                                                                )}
                                                                maxLength={40}
                                                            />
                                    ...
                                    <div className="community-composer-content space-y-3">
                                        <div className="flex flex-wrap items-center justify-between gap-3">
                                            <label className={labelCls}>
                                                {t("community.post_content_label", "内容")}
                                            </label>
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                >
                                                    ...
                                                    {t(
                                                        "community.post_import_document",
                                                        "导入文档"
                                                    )}
                                                </button>
                                            </div>
                                        ...
                                        <div className="community-composer-blocks space-y-3">
                                            {blocks.map((block, index) => (
                                                <div
                                                    key={block.id}
                                                    className={`community-composer-block rounded-lg border p-3 ${isDayMode ? "border-slate-200 bg-slate-50/70" : "border-white/10 bg-white/[0.025]"}`}
                                                >
                                                    <div className="mb-2 flex items-center justify-between gap-2">
                                                        <div
                                                            className={`flex items-center gap-1.5 text-xs font-semibold ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                                                        >
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                            <button
                                                                ...
                                                                disabled={index === 0}
                                                                className={`rounded-md border p-1 disabled:opacity-30 ${isDayMode ? "border-slate-200 text-slate-500" : "border-white/10 text-gray-400"}`}
                                                            >
                                                                <ArrowUp size={13} />
                                                            </button>
                                                            <button
                                                            ...
                                                            <button
                                                                ...
                                                                className="rounded-md border border-red-500/30 p-1 text-red-400 disabled:opacity-30"
                                                            >
                                                                <Trash2 size={13} />
                                                            </button>
                                                        </div>
                                                        ...
                                                        <>
                                                            ...
                                                            >
                                                                <option value="paragraph">
                                                                    {t(
                                                                        "community.block_text",
                                                                        "文字"
                                                                    )}
                                                                </option>
                                                                <option value="heading">
                                                                    {t(
                                                                        "community.block_heading",
                                                                        "标题"
                                                                    )}
                                                                </option>
                                                                <option value="quote">
                                                                    {t(
                                                                        "community.block_quote",
                                                                        "引用"
                                                                    )}
                                                                </option>
                                                                <option value="list">
                                                                    {t(
                                                                        "community.block_list",
                                                                        "列表"
                                                                    )}
                                                                </option>
                                                                {config.supportsCode && (
                                                        ...
                                                        <div className="space-y-2">
                                                            {!block.url ? (
                                                                <div className="relative">
                                                                    ...
                                                                    />
                                                                    <div
                                                                        className={`community-composer-file-drop flex h-24 items-center justify-center rounded-lg border-2 border-dashed text-xs ${isDayMode ? "border-slate-200 text-slate-500" : "border-white/15 text-gray-500"}`}
                                                                    >
                                                                        {uploadingBlockId ===
                                                                        block.id ? (
                                                                            <Loader2
                                                                ...
                                                                <>
                                                                    {block.type === "image" && (
                                                                        <img
                                                                            src={block.url}
                                                                            alt=""
                                                                            className="community-composer-media-preview max-h-64 w-full rounded-lg object-contain"
                                                                        />
                                                                    )}
                                                                    {block.type === "video" && (
                                                                        <video
                                                                            src={block.url}
                                                                            controls
                                                                            className="community-composer-media-preview max-h-64 w-full rounded-lg"
                                                                        />
                                                                    )}
                                                                    {block.type === "file" && (
                                                                        <div
                                                                            className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${isDayMode ? "border-slate-200 bg-white text-slate-700" : "border-white/10 bg-white/5 text-gray-200"}`}
                                                                        >
                                                                            <Paperclip size={15} />
                                                                            <span className="truncate">
                                                                                {block.name ||
                                                                                    t(
                                                                                        "community.block_file",
                                                                                        "附件"
                                                                                    )}
                                                                            </span>
                                                                        </div>
                                        ...
                                        <div className="community-composer-add-buttons flex flex-wrap gap-2">
                                            {availableBlocks.map(({ type, icon: Icon, label }) => (
                                                <button
                                                    ...
                                                    className={`community-composer-small-button inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold ${isDayMode ? "border-slate-200 text-slate-700 hover:bg-slate-50" : "border-white/10 text-gray-300 hover:bg-white/10"}`}
                                                >
                                                    <Plus size={13} />
                                                    <Icon size={13} />
                                                    {label}
                            ...
                            <aside
                                className={`community-composer-side min-h-0 overflow-y-auto border-t p-4 lg:border-l lg:border-t-0 ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-white/[0.025]"}`}
                            >
                                <div className="community-composer-side-inner space-y-4">
                                    {previewOpen ? (
                                        <div
                                            className={`community-composer-preview rounded-lg border p-4 ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-neutral-950"}`}
                                        >
                                            <p className={labelCls}>
                                                {t("community.preview", "预览")}
                                            </p>
                                            <h3 className="mt-2 text-xl font-black">
                                                {title || t("community.untitled", "未命名")}
                                            </h3>
                                            {excerpt && (
                                                <p
                                                    className={`mt-2 text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                                >
                                                    {excerpt}
                                                </p>
                                            )}
                                                ...
                                                {cleanBlocks.map((block) => (
                                                    <div key={block.id}>
                                                        ...
                                                        {block.type === "text" &&
                                                            block.style === "quote" && (
                                                                <blockquote
                                                                    className={`border-l-4 pl-3 ${isDayMode ? "border-slate-300 text-slate-600" : "border-white/20 text-gray-300"}`}
                                                                >
                                                                    {block.text}
                                                                </blockquote>
                                                            )}
                                    ...
                                    <div className="community-composer-field space-y-2">
                                        <label className={labelCls}>
                                            {t("community.post_tags_label", "标签")}
                                        </label>
                                        <input
                                            value={tags}
                                    ...
                                    <div className="community-composer-field space-y-2">
                                        <label className={labelCls}>
                                            {t("community.related_resources", "跨模块关联")}
                                        </label>
                                        <input
                                            ...
                                            className={`community-composer-input ${inputCls}`}
                                            placeholder={t(
                                                "community.related_articles_placeholder",
                                                "相关文章 ID"
                                            )}
                                        />
                                        <input
                                            ...
                                            className={`community-composer-input ${inputCls}`}
                                            placeholder={t(
                                                "community.related_posts_placeholder",
                                                "相关帖子 ID"
                                            )}
                                        />
                                        <input
                                            value={relatedNewsIds}
                                            onChange={(e) => setRelatedNewsIds(e.target.value)}
                                            className={`community-composer-input ${inputCls}`}
                                            placeholder={t(
                                                "community.related_news_placeholder",
                                                "相关新闻 ID"
                                            )}
                                        />
                                        <input
                                            value={relatedGroupIds}
                                            onChange={(e) => setRelatedGroupIds(e.target.value)}
                                            className={`community-composer-input ${inputCls}`}
                                            placeholder={t(
                                                "community.related_groups_placeholder",
                                                "相关社群 ID"
                                            )}
                                        />
                                    </div>
                        ...
                        <footer
                            className={`community-composer-footer flex flex-wrap items-center justify-end gap-2 border-t px-4 py-3 md:px-6 ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-neutral-950"}`}
                        >
                            <button
                                type="button"
                                onClick={() => submit("draft")}
                                disabled={submitting}
                                className={`community-composer-footer-button inline-flex min-h-[42px] items-center gap-2 rounded-lg border px-4 text-sm font-semibold disabled:opacity-50 ${isDayMode ? "border-slate-200 text-slate-700 hover:bg-slate-50" : "border-white/10 text-gray-200 hover:bg-white/10"}`}
                            >
                                <Save size={16} />
                            ...
                            <button
                                type="button"
                                onClick={() => submit(isAdmin ? "approved" : config.publishStatus)}
                                disabled={submitting}
                                className={`community-composer-footer-button inline-flex min-h-[42px] items-center gap-2 rounded-lg px-5 text-sm font-semibold disabled:opacity-50 ${isDayMode ? accent.button : accent.darkButton}`}
                            >
                                {submitting ? (
                                ...
                                {config.publishStatus === "approved" || isAdmin
                                    ? t("community.publish_now", "发布")
                                    : t("community.submit_for_review", "提交审核")}
                            </button>
                        </footer>
                    </div>
                </motion.div>
