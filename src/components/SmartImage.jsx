...
const SmartImage = ({
    ...
    const rawImageSrc =
        src && typeof src === "object" ? src.url || src.medium?.url || src.small?.url : src;
    ...
    const handleError = useCallback(() => {
        ...
        if (
            currentImage &&
            originalUploadSrc &&
            originalUploadSrc !== currentSrc &&
            !currentImage.dataset.originalUploadFallback
        ) {
    ...
    const [fallbackLabelKey, fallbackLabelText] =
        fallbackLabelByType[type] || fallbackLabelByType.generic;
    ...
    if (!imageSrc || error) {
        return (
            ...
            >
                <div
                    className={`absolute inset-0 ${isDayMode ? "bg-white/18" : "bg-slate-950/45"}`}
                />
                <Icon
                    size={iconSize}
                    aria-hidden="true"
                    className={`relative z-10 ${isDayMode ? "text-slate-400/70" : "text-white/55"}`}
                />
            </div>
        );
    }
    ...
    return (
        ...
        >
            {!loaded && (
                <div
                    className={`absolute inset-0 z-10 ${isDayMode ? "bg-slate-100" : "bg-slate-950/35"}`}
                >
                    {blurPlaceholder ? (
                        <img
                            src={blurPlaceholder}
                        ...
                        <div
                            className={`absolute inset-0 bg-gradient-to-r from-transparent animate-shimmer ${isDayMode ? "via-white/64" : "via-white/10"}`}
                        />
                    )}
                </div>
            )}
          ...
          `}
            ...
            {retryCount > 0 && !loaded && !error && (
                <div className="absolute inset-0 flex items-center justify-center">
                    <div
                        className={`w-6 h-6 rounded-full animate-spin ${isDayMode ? "border-2 border-slate-200 border-t-slate-500" : "border-2 border-white/30 border-t-white"}`}
                    />
                </div>
            )}
        </div>
