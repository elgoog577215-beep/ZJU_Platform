...
const CommunityDetailModal = ({
    ...
    const relatedGroups = useMemo(
        () => flattenLinkedResources(item?.linked_resources),
    ...
    const primaryJoinGroup =
        relatedGroupCandidates.find(
            (group) => group?.id && String(group.id) !== String(item?.id)
        ) || null;
        ...
        try {
            if (isMiniProgramWebView()) {
                try {
                    ...
                    console.error(
                        "Error opening native mini program share page:",
                        nativeShareError
                    );
    ...
    const renderTextBlock = (block, blockKey, headingId) => {
        if (block.style === "heading") {
            return (
                <h3
                    id={headingId}
                    className={`scroll-mt-24 text-xl font-black tracking-tight md:text-3xl ${th.textPrimary}`}
                >
                    {block.text}
                </h3>
            );
        ...
        if (block.style === "quote") {
            return (
                <blockquote
                    className={`border-l-4 py-3 pl-4 text-base italic leading-7 md:pl-5 md:text-lg md:leading-8 ${isDayMode ? "border-slate-300 text-slate-600 bg-slate-50" : "border-orange-400/50 text-gray-200 bg-orange-500/5"} rounded-r-lg`}
                >
        ...
        if (block.style === "list") {
            ...
            return (
                <ul
                    className={`space-y-2 pl-5 text-base leading-7 md:pl-6 md:text-lg md:leading-8 list-disc ${th.textContent}`}
                >
                    {items.map((line, idx) => (
        ...
        if (block.style === "code" || block.type === "code") {
            return (
                <div
                    className={`rounded-lg border overflow-hidden ${isDayMode ? "bg-white border-slate-200/80 shadow-[0_16px_34px_rgba(148,163,184,0.14)]" : "bg-white/[0.035] border-white/10"}`}
                >
                    <div
                        className={`px-4 py-2 text-xs uppercase tracking-[0.2em] ${isDayMode ? "bg-slate-50 text-slate-500" : "bg-white/5 text-gray-400"}`}
                    >
                        {block.language || "code"}
                    </div>
        ...
        return (
            <p
                className={`whitespace-pre-wrap break-words text-base leading-7 md:text-lg md:leading-8 ${th.textContent}`}
            >
                <LinkifiedText text={block.text} />
            </p>
        );
        ...
        return (
            ...
            >
                <div className="flex items-start gap-3">
                    <div
                        className={`mt-0.5 w-10 h-10 rounded-md flex items-center justify-center ${isDayMode ? "bg-slate-100 text-slate-600" : "bg-white/10 text-gray-300"}`}
                    >
                        <Icon size={18} />
                    </div>
                    <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                            <div className={`text-sm font-bold truncate ${th.textPrimary}`}>
                                {title}
                            </div>
                            {clickable && <ChevronRight size={16} className={th.textTertiary} />}
                        </div>
                        <div
                            className={`mt-1 text-[11px] uppercase tracking-[0.18em] ${th.textTertiary}`}
                        >
                            {meta.label}
                        </div>
                        {desc ? (
                            <p className={`mt-2 text-sm line-clamp-2 ${th.textSecondary}`}>
                                {desc}
                            </p>
                        ) : null}
                    </div>
                ...
                >
                    <div className="min-h-full">
                        <motion.div
                            ...
                            aria-modal="true"
                            aria-label={
                                item.title || item.name || t("community.detail", "内容详情")
                            }
                        ...
                        >
                            ...
                            >
                                {!coverImage && (
                                    <div
                                        className={`absolute inset-0 bg-gradient-to-br ${gradientFrom} ${th.gradientTo}`}
                                    />
                                )}
                                ...
                                >
                                    <X
                                        size={22}
                                        className="transition-transform duration-300 group-hover:rotate-90 md:size-6"
                                    />
                                </button>
                            ...
                            <div
                                className={`mx-auto grid max-w-7xl gap-6 px-4 pb-28 pt-4 sm:px-8 md:px-12 ${desktopAssistContent ? "lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start lg:pb-12" : "lg:max-w-5xl lg:pb-12"}`}
                            >
                                <div className="min-w-0">
                                    <div
                                        className={`mb-6 flex flex-col gap-3 border-b pb-4 md:mb-8 md:flex-row md:items-center md:justify-between md:gap-4 md:pb-6 ${th.borderSubtle}`}
                                    >
                                        <div
                                            role="button"
                                            ...
                                            aria-label={
                                                canGoProfile
                                                    ? t(
                                                          "community.view_author_profile",
                                                          "查看作者主页"
                                                      )
                                                    : t(
                                                          "community.anonymous_author_no_profile",
                                                          "匿名作者无主页"
                                                      )
                                            }
                                            onClick={
                                                canGoProfile ? handleAuthorNavigate : undefined
                                            }
                                            onKeyDown={
                                                canGoProfile ? handleAuthorKeyDown : undefined
                                            }
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0">
                                                <div
                                                    className={`text-sm font-bold ${th.textPrimary}`}
                                                >
                                                    {canGoProfile
                                                        ? item.author_name ||
                                                ...
                                                <div className={`text-xs ${th.textTertiary}`}>
                                                    {t("common.author")}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {tocItems.length >= 2 ? (
                                        <div
                                            className={`mb-6 rounded-lg border p-4 md:mb-8 md:p-5 ${isDayMode ? "bg-slate-50 border-slate-200" : "bg-white/[0.03] border-white/10"}`}
                                        >
                                            <div
                                                className={`text-xs uppercase tracking-[0.22em] mb-3 ${th.textTertiary}`}
                                            >
                                                目录
                                            </div>
                                    ...
                                    {primaryJoinGroup ? (
                                        <div
                                            className={`mb-8 rounded-lg border p-4 ${isDayMode ? "bg-slate-50 border-slate-200" : "bg-blue-500/10 border-blue-500/30"}`}
                                        >
                                            <div
                                                className={`text-xs uppercase tracking-[0.2em] mb-2 ${isDayMode ? "text-blue-700" : "text-blue-300"}`}
                                            >
                                                社群入口
                                            </div>
                                            <p
                                                className={`text-sm mb-3 ${isDayMode ? "text-slate-700" : "text-blue-100"}`}
                                            >
                                                当前内容有对应社群，可直接加入继续交流。
                                            </p>
                                            <button
                                    ...
                                    {contentBlocks.length > 0 ? (
                                        <div className="mb-8 space-y-5 md:mb-10 md:space-y-6">
                                            {contentBlocks.map((block, bIdx) => {
                                                const headingEntry = tocItems.find(
                                                    (entry) => entry.index === bIdx
                                                );
                                                    ...
                                                    <div key={block.id || `${block.type}-${bIdx}`}>
                                                        ...
                                                        {block.type === "image" && block.url ? (
                                                            <figure className="space-y-2">
                                                                <div
                                                                    className={`rounded-lg overflow-hidden border ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-white/[0.035]"}`}
                                                                >
                                                                    <img
                                                                        src={block.url}
                                                                        alt={block.caption || ""}
                                                                ...
                                                                {block.caption ? (
                                                                    <figcaption
                                                                        className={`text-sm ${th.textSecondary}`}
                                                                    >
                                                                        {block.caption}
                                                                    </figcaption>
                                                                ) : null}
                                                        ...
                                                        {block.type === "file" && block.url ? (
                                                            ...
                                                            >
                                                                <div
                                                                    className={`w-10 h-10 rounded-md flex items-center justify-center flex-shrink-0 ${isDayMode ? "bg-amber-100 text-amber-600" : "bg-amber-500/15 text-amber-400"}`}
                                                                >
                                                                    <Copy size={18} />
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                    <div
                                                                        className={`text-sm font-medium truncate ${th.textPrimary}`}
                                                                    >
                                                                        {block.name ||
                                                                            t(
                                                                                "community.file_attachment",
                                                                                "附件"
                                                                            )}
                                                                    </div>
                                                                    {block.size ? (
                                                                        <div
                                                                            className={`text-xs ${th.textTertiary}`}
                                                                        >
                                                                            {(
                                                                                block.size / 1024
                                                                            ).toFixed(1)}{" "}
