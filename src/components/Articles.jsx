...
const getFileTypeBadgeClass = (name = "", mime = "", isDayMode = false) => {
  ...
  if (normalized === "ZIP" || normalized === "RAR")
    return isDayMode
...
const ArticleCard = memo(
  ({ article, index, onClick, onToggleFavorite, canAnimate, isDayMode }) => {
      ...
      >
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col justify-center space-y-3">
            <div
              className={`flex items-center gap-3 text-xs font-mono ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
            >
            ...
            <h3
              className={`text-xl md:text-2xl font-bold group-hover:text-orange-400 transition-colors ${isDayMode ? "text-slate-900" : "text-white"}`}
            >
              {article.title}
            </h3>
            <p
              className={`line-clamp-2 ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
            >
              {article.excerpt}
            </p>
              ...
              <div className="flex items-center gap-3 ml-auto">
                <FavoriteButton
                  ...
                  onToggle={(favorited, likes) =>
                    onToggleFavorite(article.id, favorited, likes)
                  }
                />
                <div
                  className={`p-2 rounded-full group-hover:bg-orange-500 group-hover:text-black transition-all duration-300 ${isDayMode ? "bg-orange-50 text-orange-500" : "bg-white/5"}`}
                >
                  <ArrowRight
                    size={18}
...
const Articles = () => {
  ...
  const handleToggleFavorite = useCallback(
    (articleId, favorited, likes) => {
      setArticles((prev) =>
        prev.map((a) =>
          a.id === articleId
            ? { ...a, likes: likes !== undefined ? likes : a.likes, favorited }
            : a,
        ),
      );
                  ...
                  <div className="p-6 overflow-y-auto custom-scrollbar flex-1 min-h-0 space-y-6">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-3">
                        <h4
                          className={`text-sm font-semibold uppercase tracking-wider ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                        >
                          {t("common.tags", "标签")}
                        </h4>
        ...
        <div className="space-y-6">
          {isLoading && displayArticles.length === 0 ? (
            ...
            [...Array(5)].map((_, i) => (
              <div
                key={i}
                className={`backdrop-blur-xl border rounded-3xl p-6 animate-pulse flex flex-col md:flex-row gap-6 ${isDayMode ? "bg-white/82 border-slate-200/80" : "bg-[#1a1a1a]/40 border-white/5"}`}
              >
                <div
                  className={`w-full md:w-48 h-48 md:h-32 rounded-xl shrink-0 ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                />
                <div className="flex-1 space-y-4 py-2">
                  <div className="flex gap-3">
                    <div
                      className={`h-4 rounded w-24 ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                    />
                    <div
                      className={`h-4 rounded w-20 ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                    />
                  </div>
                  <div
                    className={`h-8 rounded w-3/4 ${isDayMode ? "bg-slate-100" : "bg-white/10"}`}
                  />
                  <div className="space-y-2">
                    <div
                      className={`h-4 rounded w-full ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                    />
                    <div
                      className={`h-4 rounded w-2/3 ${isDayMode ? "bg-slate-100" : "bg-white/5"}`}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4">
              ...
              <p
                className={`mb-6 text-lg ${isDayMode ? "text-slate-600" : "text-gray-300"}`}
              >
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4">
              <div
                className={`bg-gradient-to-br from-orange-500/10 to-red-500/10 rounded-3xl p-8 mb-6 border backdrop-blur-xl shadow-xl ${isDayMode ? "border-orange-100/80 bg-white/72" : "border-white/5"}`}
              >
              ...
              <p
                className={`text-center max-w-md ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
              >
                {t("articles.subtitle")}
              </p>
              {selectedTags.length > 0 && (
              ...
              <div className="min-h-full">
                ...
                >
                  ...
                  <div
                    ...
                    style={
                      selectedArticle.cover
                        ? { backgroundImage: `url(${selectedArticle.cover})` }
                        : {}
                    }
                  >
                    {!selectedArticle.cover && (
                    ...
                    <div
                      className={`absolute bottom-0 left-0 px-6 pt-6 pb-6 md:px-10 md:pt-10 md:pb-8 w-full z-20 pt-48 -mb-1 backdrop-blur-[2px] ${isDayMode ? "bg-gradient-to-t from-white via-white/92 to-transparent" : "bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/90 to-transparent"}`}
                    >
                      <div
                        className={`flex items-center gap-3 font-bold text-lg md:text-xl uppercase tracking-[0.2em] mb-4 opacity-100 ${isDayMode ? "text-orange-500" : "text-orange-300 drop-shadow-lg"}`}
                      >
                        <span>{selectedArticle.date}</span>
                      </div>
                      <h2
                        className={`text-4xl md:text-6xl font-black leading-[0.95] tracking-tight font-serif decoration-clone ${isDayMode ? "text-slate-900" : "text-white drop-shadow-2xl"}`}
                      >
                  ...
                  <div className="px-5 sm:px-8 md:px-12 pt-4 pb-12 max-w-5xl mx-auto">
                    <div
                      className={`flex items-center justify-between gap-3 mb-8 pb-8 border-b ${isDayMode ? "border-slate-200/80" : "border-white/5"}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center overflow-hidden ${isDayMode ? "bg-slate-100" : "bg-gray-700"}`}
                        >
                          <User
                            size={20}
                            className={
                      ...
                      <div className="space-y-6">
                        {selectedContentBlocks.map((block) => (
                          <div
                            key={
                              block.id ||
                              `${block.type}-${block.url || block.text || Math.random()}`
                            }
                            className={`space-y-3 rounded-2xl p-4 md:p-5 border ${isDayMode ? "bg-slate-50/80 border-slate-200/80" : "bg-white/[0.03] border-white/10"}`}
                          >
                            {block.type === "text" &&
                              (block.style === "heading" ? (
                                <h3
                                  className={`whitespace-pre-wrap leading-tight text-3xl md:text-4xl font-black tracking-tight ${isDayMode ? "text-slate-900" : "text-white"}`}
                                >
                                  {block.text}
                                </h3>
                              ) : block.style === "quote" ? (
                                <blockquote
                                  className={`whitespace-pre-wrap leading-8 text-lg border-l-4 pl-4 italic ${isDayMode ? "text-slate-600 border-orange-300" : "text-gray-300 border-orange-400/60"}`}
                                >
                                  {block.text}
                                </blockquote>
                              ) : block.style === "list" ? (
                                <ul
                                  className={`list-disc pl-8 space-y-2 leading-8 text-lg ${isDayMode ? "text-slate-700" : "text-gray-300"}`}
                                >
                                  {(block.text || "")
                                    .split("\n")
                                    .map((line) => line.trim())
                                ...
                                <p
                                  className={`whitespace-pre-wrap leading-8 text-lg ${isDayMode ? "text-slate-700" : "text-gray-300"}`}
                                >
                                  {block.text}
                                </p>
                              ))}
                            {block.type === "image" && block.url && (
                              <figure className="space-y-3">
                                <div
                                  className={`flex ${getImageAlignClass(block.align)}`}
                                >
                                  <div
                                    className={`${getImageWidthClass(block.width)} rounded-2xl overflow-hidden border ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-black/20"}`}
                                  >
                                    <img
                                      src={block.url}
                                      alt={
                                        block.caption || selectedArticle.title
                                      }
                                ...
                                {block.caption && (
                                  <figcaption
                                    className={`text-sm px-1 ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                  >
                                    {block.caption}
                                  </figcaption>
                                )}
                            ...
                            {block.type === "video" && block.url && (
                              <figure className="space-y-3">
                                <div
                                  className={`rounded-2xl overflow-hidden border ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-black/20"}`}
                                >
                                  <video
                                    controls
                                    src={block.url}
                            ...
                            {block.type === "file" && block.url && (
                              ...
                              >
                                <Paperclip size={14} />
                                <span
                                  className={`text-[10px] px-2 py-0.5 rounded-full border ${getFileTypeBadgeClass(block.name, block.mime, isDayMode)}`}
                                >
                                  {getFileTypeLabel(block.name, block.mime)}
                                </span>
                                <span>
                                  {block.name || t("common.attachment", "附件")}
                                </span>
                                {!!block.size && (
                                  <span
