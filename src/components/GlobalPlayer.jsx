...
const PlayerInfo = memo(
    ({
        ...
        return (
            <div
                className={`flex items-center gap-4 relative z-10 mb-4 backdrop-blur-md rounded-2xl p-3 border shadow-lg ${isDayMode ? "bg-white/86 border-slate-200/80 shadow-[0_14px_32px_rgba(148,163,184,0.16)]" : "bg-white/5 border-white/5"}`}
            >
                {/* Drag Handle Indicator */}
                <div
                    className={`w-1 h-8 rounded-full cursor-grab active:cursor-grabbing transition-colors ${isDayMode ? "bg-slate-200 hover:bg-slate-300" : "bg-white/10 hover:bg-white/20"}`}
                    aria-hidden="true"
                />
                ...
                <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 relative group shadow-lg">
                    <div
                        className={`absolute inset-0 bg-gradient-to-tr from-cyan-500/20 to-purple-500/20 opacity-0 group-hover:opacity-100 transition-opacity z-10`}
                    />
                    <img
                    ...
                    />
                    <div
                        className={`absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20 ${isDayMode ? "bg-slate-900/20" : "bg-black/40"}`}
                    >
                        <Music
                            size={16}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        <div className="truncate pr-2">
                            <h4
                                className={`font-bold text-sm truncate leading-tight mb-0.5 ${isDayMode ? "text-slate-900" : "text-white"}`}
                            >
                                {currentTrack.title}
                            </h4>
                            <p
                                className={`text-[10px] truncate uppercase tracking-wider ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                            >
                                {currentTrack.artist}
                            </p>
                        </div>
                        <button
                            ...
                            className={`p-1.5 -mr-1 rounded-full active:scale-95 transition-all ${isDayMode ? "text-slate-500 hover:text-slate-900 hover:bg-slate-100" : "text-gray-400 hover:text-white hover:bg-white/10"}`}
                        >
...
const ProgressBar = memo(({ progress, duration, onSeek, isDayMode, label }) => {
    ...
    return (
        <div className="flex items-center gap-2 mb-2 relative z-10 px-1">
            <span
                className={`text-[9px] font-mono w-6 text-right ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
            >
                {formatTrackTime(progress)}
            </span>
            <input
                type="range"
                min="0"
            ...
            />
            <span
                className={`text-[9px] font-mono w-6 ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
            >
                {formatTrackTime(safeDuration)}
            </span>
        </div>
    );
});
    ...
    return (
        <div className="flex items-center justify-center gap-4 relative z-10 group">
            <button
                ...
                className={`p-2.5 md:p-3 rounded-full transition-all hover:scale-110 active:scale-95 ${isDayMode ? "text-slate-700 hover:bg-slate-100" : "text-white hover:bg-white/10"}`}
            >
                <SkipBack
                    size={20}
            ...
            >
                {isPlaying ? (
                    ...
                    <Play
                        size={24}
                        className="w-6 h-6 md:w-7 md:h-7 fill-black ml-1"
                        aria-hidden="true"
                        focusable="false"
                    />
                )}
...
const GlobalPlayer = () => {
    ...
    if (!isMobile && (location.pathname === "/music" || location.pathname === "/articles"))
        return null;
    ...
    if (isMobile) {
        return (
            ...
            >
                <div
                    className={`relative overflow-hidden backdrop-blur-2xl border rounded-full p-2 pr-3 flex items-center justify-between pointer-events-auto ring-1 ${isDayMode ? "bg-white/92 border-slate-200/80 shadow-[0_12px_32px_rgba(148,163,184,0.24)] ring-slate-200/70" : "bg-black/60 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] ring-white/5"}`}
                >
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        >
                            ...
                            >
                                <div
                                    className={`w-1.5 h-1.5 rounded-full border ${isDayMode ? "bg-white border-slate-300/90" : "bg-black border-white/20"}`}
                                />
                            </div>
                        </div>
                        <div
                            className="flex flex-col overflow-hidden max-w-[min(34vw,150px)]"
                            aria-live="polite"
                        >
                            <span
                                className={`font-bold text-sm truncate ${isDayMode ? "text-slate-900" : "text-white"}`}
                            >
                                {currentTrack.title}
                            </span>
                            <span
                                className={`text-[10px] truncate ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                            >
                                {currentTrack.artist}
                            </span>
                        </div>
    ...
    return (
        ...
        >
            <div
                className={`backdrop-blur-3xl border rounded-[2rem] p-5 flex flex-col relative overflow-hidden ring-1 group transition-colors ${isDayMode ? "bg-white/92 border-slate-200/80 shadow-[0_16px_46px_rgba(148,163,184,0.24)] ring-slate-200/70 hover:border-cyan-200/80" : "bg-black/60 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] ring-white/5 hover:border-white/20"}`}
            >
                {/* Visualizer Background */}
                {isPlaying && (
                    <div
                        className="absolute bottom-0 left-0 w-full h-32 opacity-20 pointer-events-none z-0 mask-image-gradient-to-t mix-blend-screen"
                        aria-hidden="true"
                    >
