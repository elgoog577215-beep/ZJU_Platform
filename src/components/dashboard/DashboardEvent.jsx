...
const DashboardEvent = ({ event, onClick }) => {
    ...
    return (
        ...
        >
            ...
            {/* Date Badge */}
            <div
                className={`absolute top-6 left-6 flex flex-col items-center border rounded-xl p-3 transition-colors ${isDayMode ? "bg-white border-slate-200/80 group-hover:bg-slate-50" : "bg-white/10 border-white/20 group-hover:bg-white/20 backdrop-blur-md shadow-lg"}`}
            >
                <span className="text-xs font-bold text-red-400 uppercase tracking-wider">
                    {new Date(event.date).toLocaleString("default", { month: "short" })}
                </span>
                <span
                    className={`text-2xl font-bold font-serif ${isDayMode ? "text-slate-900" : "text-white"}`}
                >
            ...
            <div className="absolute bottom-0 left-0 w-full p-8">
                ...
                <h3
                    className={`text-4xl font-bold font-serif leading-tight mb-2 ${isDayMode ? "text-slate-900" : "text-white"}`}
                >
                    {event.title}
                </h3>
                <p
                    className={`max-w-lg line-clamp-2 ${isDayMode ? "text-slate-600" : "text-gray-300"}`}
                >
                    {event.location} • {t("common.view_details")}
                </p>
            </div>
        </div>
    );
