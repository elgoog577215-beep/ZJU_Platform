...
const actionLinks = [
    {
        label: "完整相册",
        detail: "开幕式、开发现场、颁奖、交流分组",
        icon: ImageIcon,
        to: "/gallery",
    },
    {
        label: "作品索引",
        detail: "获奖作品、Git 链接、经验分享",
        icon: Trophy,
        to: "/hackathon/works",
    },
...
const shouldUseTranslatedShowcaseCopy = (language = "zh") => String(language).startsWith("en");
...
const SectionHeader = ({ eyebrow, title, copy, icon: Icon, theme, align = "left" }) => (
    ...
    >
        ...
        {copy ? (
            <p
                className={`max-w-2xl text-sm font-semibold leading-7 sm:text-base sm:leading-8 ${theme.muted}`}
            >
                {copy}
            </p>
        ) : null}
...
const ShowcaseSectionFrame = ({
    ...
    >
        {!liteMode ? (
            <>
                <div
                    className="showcase-stage-bg pointer-events-none absolute inset-0 opacity-80"
                    aria-hidden="true"
                />
                <div
        ...
        <div
            className={`showcase-section-grid relative z-10 mx-auto grid min-w-0 w-full max-w-[2200px] ${
                compactFlow
                    ? "min-h-0 gap-6"
...
const ShowcaseMetricTile = ({ stat, theme, isDayMode }) => (
    <div
        className={`showcase-metric-tile group relative overflow-hidden border px-4 py-4 transition duration-300 hover:-translate-y-0.5 ${theme.surface}`}
    >
        <div
            className={`absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent to-transparent ${isDayMode ? "via-cyan-500/50" : "via-cyan-300/70"}`}
        />
        <div className="relative flex items-end gap-2">
            <span
                className={`font-mono text-4xl font-black leading-none tracking-normal sm:text-5xl ${theme.accent}`}
            >
        ...
        <p className={`relative mt-1 text-xs font-semibold leading-5 ${theme.soft}`}>
            {stat.detail}
        </p>
    </div>
);
...
const ShowcaseWorkCard = ({
    ...
    const badgeLabel =
        work.award || work.honorTitle || t("hackathon.showcase.works.fallback_award");
            ...
            >
                <img
                    src={getShowcaseImageUrl(
                        work.cover || (index % 2 === 0 ? HERO_IMAGE : SECONDARY_IMAGE),
                        featured ? 1200 : 640
                    )}
                    ...
                    sizes={
                        featured
                            ...
                            : "(min-width: 1280px) 22vw, 50vw"
                    }
                    onError={(event) =>
                        handleShowcaseImageError(
                            event,
                            index % 2 === 0 ? HERO_IMAGE : SECONDARY_IMAGE
                        )
                    }
                />
                <div
...
const HackathonShowcase = ({ template, onSectionChange }) => {
    ...
    const event = resolvedTemplate.event;
    const competitionSlug = resolvedTemplate.results.competitionSlug;
    const worksHref = `/hackathon/works?competition=${encodeURIComponent(competitionSlug)}`;
    const eventYear = getHackathonYear(event);
    const titleLines = splitHackathonTitle(event.title);
    const showcaseEventStats = useMemo(
        () => [
            ...event.highlights.slice(0, 3),
            {
                id: "prize",
                value: event.prizeValue,
                shortValue: event.prizeValue,
                unit: event.prizeUnit,
                label: t("hackathon.event.prize_pool", "奖金池"),
                detail: t("hackathon.showcase.stats.prize.detail", "以作品质量和落地潜力完成激励"),
            },
        ],
        [event, t]
    );
    ...
    const getScrollMetrics = useCallback(() => {
        ...
        if (compactFlow) {
            ...
            const clientHeight =
                window.innerHeight || documentElement?.clientHeight || container.clientHeight || 0;
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            scrollFrameRef.current = window.requestAnimationFrame(() => {
                ...
                const nextProgress =
                    scrollHeight > 0 ? (metrics.scrollTop / scrollHeight) * 100 : 0;
                setScrollProgress((previous) =>
                    Math.abs(previous - nextProgress) < 0.4 ? previous : nextProgress
                );
    ...
    const officialVideoUrl =
        officialVideo?.url ||
        officialVideo?.video ||
        officialVideo?.video_url ||
        officialVideo?.videoUrl ||
        officialVideo?.media_url ||
        officialVideo?.mediaUrl ||
        "";
    const officialVideoCoverSource =
        officialVideo?.cover_url ||
        officialVideo?.coverUrl ||
        officialVideo?.thumbnail ||
        officialVideo?.poster ||
        SECONDARY_IMAGE;
    ...
    const translatedEventStats = useMemo(
        () =>
            showcaseEventStats.map((stat) => ({
                ...stat,
                shortValue: stat.shortValue || stat.value,
            })),
        [showcaseEventStats]
    );
    ...
    const translatedActionLinks = useMemo(
        () =>
            actionLinks.map((action, index) => ({
                ...action,
                to: action.to === "/hackathon/works" ? worksHref : action.to,
    ...
    const galleryMoments = useMemo(() => {
        ...
        const dynamicMoments = stagePhotos.slice(0, 5).map((item, index) => ({
            ...
            label: useTranslatedCopy
                ...
                : item.type_label ||
                  item.category ||
                  t("hackathon.showcase.gallery.stage_photo", "赛场照片"),
            ...
            caption: useTranslatedCopy
                ...
                : item.description ||
                  ...
                  t(
                      "hackathon.showcase.gallery.dynamic_caption",
                      "来自画廊的黑客松成果照片，审核通过后自动同步到这里。"
                  ),
            image:
                item.cover_url ||
                item.url ||
                translatedMediaMoments[index % translatedMediaMoments.length].image,
        }));

        return [...dynamicMoments, ...translatedMediaMoments.slice(dynamicMoments.length)].slice(
            0,
            5
        );
    ...
    const showcaseWorks = useMemo(() => {
        ...
        return works.slice(0, 3).map((work, index) => ({
            ...
            award:
                work.award ||
                work.honor_title ||
                t("hackathon.showcase.works.fallback_award", "优秀作品"),
            title: work.title || t("hackathon.showcase.works.fallback_title", "未命名作品"),
            author:
                work.author ||
                work.uploader_name ||
                t("hackathon.showcase.works.fallback_author", "获奖成员"),
            ...
            honorTitle:
                ...
                work.honorTitle ||
                work.award ||
                t("hackathon.showcase.works.fallback_honor", "Top 20 获奖成员"),
        }));
    ...
    const supportLineup = useMemo(() => {
        const detailByCategory = {
            school: t(
                "hackathon.showcase.partners.detail.school",
                "场地资源、导师评审、校园传播与长期机制支持。"
            ),
            organization: t(
                "hackathon.showcase.partners.detail.organization",
                "选手招募、志愿执行、现场协同与赛后社群承接。"
            ),
            enterprise: t(
                "hackathon.showcase.partners.detail.enterprise",
                "模型、云资源、AI 工具、技术支持与合作方传播露出。"
            ),
        };
    ...
    const theme = isDayMode
        ? {
              ...
              navActive:
                  "border-cyan-500 bg-cyan-100 text-cyan-950 shadow-[0_10px_28px_rgba(8,145,178,0.18)]",
              navIdle:
                  "border-slate-200 bg-white/82 text-slate-600 hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-800",
        ...
        : {
              ...
              surfaceStrong:
                  "border-white/12 bg-[#0b1210]/92 shadow-[0_34px_120px_rgba(0,0,0,0.58)]",
              muted: "text-white/68",
              soft: "text-white/46",
              ...
              navShell:
                  "border-white/12 bg-[#07100f]/72 text-white shadow-[0_0_44px_rgba(103,232,249,0.10)]",
              navActive: "border-cyan-200 bg-cyan-300 text-slate-950",
              navIdle:
                  "border-white/12 bg-black/22 text-white/64 hover:border-cyan-300/60 hover:text-white",
    ...
    return (
        ...
        >
            <SEO
                title={`${event.title}比赛成果`}
                description={`${event.title}比赛成果页，集中呈现宣传片、赛场照片、优秀作品和活动成果。`}
                image={HERO_IMAGE}
            />
            <style>
                {`
        ...
        `}
            ...
            >
                <MotionDiv {...heroReveal} className="showcase-hero-copy min-w-0">
                    ...
                    <h1
                        data-showcase-title
                        className={`showcase-title mt-5 max-w-[1120px] font-black ${isDayMode ? "text-slate-950" : "text-white"}`}
                    >
                        {titleLines.map((line) => (
                            <span key={line} className="showcase-title-line">
                                {line}
                            </span>
                        ))}
                        <span
                            className={`showcase-title-line ${isDayMode ? "text-cyan-700" : "text-cyan-200"}`}
                        >
                            {t("hackathon.showcase.hero.result")}
                        </span>
                    </h1>
                    <p
                        className={`showcase-hero-desc mt-5 max-w-4xl text-base font-semibold leading-7 sm:text-lg sm:leading-8 lg:text-xl lg:leading-9 ${theme.muted}`}
                    >
                        {event.description} 这里集中呈现宣传片、现场照片、获奖作品和支持阵容。
                    </p>
                ...
                >
                    <div
                        className={`absolute right-4 top-4 z-20 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] sm:right-6 sm:top-6 sm:text-xs sm:tracking-[0.2em] ${theme.accent}`}
                    >
                        <span
                    ...
                    <div
                        className={`absolute left-4 top-4 z-20 border px-2.5 py-1.5 text-[10px] font-black uppercase sm:left-5 sm:top-5 sm:px-3 sm:py-2 sm:text-xs ${theme.chip}`}
                    >
                        {officialVideo?.title || "Official Film"}
                    </div>
                    <div
                        className={`showcase-film-inner relative h-full overflow-hidden border border-white/10 ${compactFlow ? "min-h-[16rem] sm:min-h-[28rem]" : "min-h-[clamp(16rem,46svh,24rem)] lg:min-h-0"}`}
                    >
                        <img
                            ...
                            style={{
                                filter: isDayMode
                                    ? "brightness(1.03) saturate(1.08) contrast(1.01)"
                                    : "brightness(0.86) saturate(1.12) contrast(1.03)",
                            }}
                            onError={(event) => {
                        ...
                        <div
                            className={`showcase-media-overlay absolute inset-0 transition-opacity duration-300 ${isVideoPlaying ? "opacity-0" : "opacity-100"} ${isDayMode ? "showcase-media-overlay-day" : ""}`}
                        />
                        {!isVideoPlaying && officialVideoUrl ? (
                            <button
                                ...
                                aria-label={
                                    officialVideoUrl
                                        ? t("hackathon.showcase.hero.play_film")
                        ...
                        {!isVideoPlaying ? (
                            <div
                                className={`showcase-film-caption absolute bottom-0 left-0 right-0 z-20 p-5 sm:p-7 ${isDayMode ? "bg-gradient-to-t from-slate-950/70 via-slate-950/32 to-transparent" : ""}`}
                            >
                                <p className="showcase-image-kicker text-xs font-black uppercase tracking-[0.18em] text-cyan-200">
                ...
                <div className="mx-auto w-full max-w-[1760px]">
                            ...
                            {supportLineup.map((group, index) => (
                                ...
                                >
                                    <div className="lg:min-h-[8rem]">
                                        ...
                                        <p
                                            className={`mt-3 text-sm font-semibold leading-6 ${theme.muted}`}
                                        >
                                            {group.detail}
                                        </p>
                                    </div>
                    ...
                    <div className="showcase-closing-actions mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-stretch">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className="min-w-0">
                                            ...
                                            <span
                                                className={`mt-0.5 block text-xs font-semibold leading-5 ${theme.soft}`}
                                            >
                                                {action.detail}
                                            </span>
                                        </span>
                        ...
                        <div className="grid gap-2 sm:grid-cols-2 lg:w-[16rem] lg:grid-cols-1">
                            ...
                            {resolvedTemplate.navigation.registrationVisible ? (
                                <Link
                                    to={`/hackathon?event=${encodeURIComponent(event.key)}&view=register`}
                                ...
                                <span
                                    aria-disabled="true"
                                    className={`inline-flex min-h-12 cursor-not-allowed items-center justify-center gap-2 border px-6 text-sm font-bold opacity-50 ${theme.secondaryButton}`}
                                >
                                    {t("hackathon.showcase.partners.back", "赛事报名页")} · 未开放
                                    <CalendarDays className="h-4 w-4" />
                                </span>
                            )}
                        </div>
