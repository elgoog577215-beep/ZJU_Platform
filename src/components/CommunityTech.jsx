...
const ArticleCard = memo(
    ({ article, index, onClick, onToggleFavorite, canAnimate, isDayMode, actionBar = null }) => {
        ...
        return (
            ...
            >
                <div
                    className={`pointer-events-none absolute inset-x-0 top-0 h-px ${isDayMode ? "bg-slate-200/80" : "bg-gradient-to-r from-transparent via-orange-500/20 to-transparent"}`}
                />
                {isDayMode ? null : null}
                <div className="flex gap-3 items-start md:gap-6">
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-center space-y-1.5 md:space-y-3">
                        <div
                            className={`flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] font-mono md:text-xs ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                        >
                            {article.author_name ? (
                                <>
                        ...
                        <h3
                            className={`line-clamp-2 text-base font-bold leading-snug transition-colors md:text-2xl md:leading-tight ${isDayMode ? "text-slate-900 group-hover:text-blue-700" : "text-white group-hover:text-orange-400"}`}
                        >
                            {article.title}
                        </h3>
                        <p
                            className={`hidden md:block line-clamp-2 text-[15px] leading-7 ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                        >
                            {article.excerpt}
                        </p>
                        <div className="mt-auto flex items-center justify-end gap-1.5 pt-1 md:gap-3 md:pt-2">
                            ...
                            />
                            <div
                                className={`inline-flex min-h-[40px] min-w-[40px] items-center justify-center rounded-md p-1.5 transition-all duration-300 md:min-h-0 md:min-w-0 md:p-2 ${isDayMode ? "bg-slate-100 text-slate-500 group-hover:bg-slate-950 group-hover:text-white" : "bg-white/5 group-hover:bg-orange-500 group-hover:text-black"}`}
                            >
                                <ArrowRight
                                    size={16}
...
const CommunityTech = ({ onNewPost, hideNewPostButton = false }) => {
    ...
    const panelItems = useMemo(
        () =>
            featuredArticle
                ? (feed.displayItems || []).filter((item) => item.id !== featuredArticle.id)
                : feed.displayItems || [],
    ...
    const contentBlocks = useMemo(
        () => parseContentBlocks(feed.selectedItem?.content_blocks),
        [feed.selectedItem?.content_blocks]
    );
    ...
    const handleToggleFeatured = useCallback(
        async (article) => {
            try {
                ...
                toast.success(
                    article.featured
                        ? t("community.featured_disabled", "已取消精选")
                        : t("community.featured_enabled", "已设为精选")
                );
                feed.handleRefresh();
            } catch (error) {
                toast.error(
                    error?.response?.data?.error ||
                        t("community.featured_update_failed", "更新精选状态失败")
                );
            }
        },
        [feed, t]
    ...
    const renderCard = (article, index, { canAnimate, isDayMode: currentDayMode }) => {
        const adminActionBar = isAdmin ? (
            ...
            >
                <Sparkles size={12} />
                {article.featured
                    ? t("community.unfeature_article", "取消精选")
                    : t("community.feature_article", "设为精选")}
            </button>
        ) : null;

        const actionBar = adminActionBar ? (
            <div className="flex items-center gap-2 flex-wrap justify-end">{adminActionBar}</div>
        ) : null;
    ...
    const featuredSection = featuredArticle ? (
        <div
            className={`mb-5 rounded-none border border-transparent bg-transparent p-0 shadow-none ring-0 backdrop-blur-0 md:rounded-lg md:border md:p-5 ${isDayMode ? "md:bg-white md:border-slate-200/80 md:shadow-[0_8px_22px_rgba(15,23,42,0.045)]" : "md:bg-gradient-to-br md:from-orange-500/10 md:via-white/[0.03] md:to-amber-500/10 md:border-orange-500/20"}`}
        >
            <div className="mb-3 flex items-center justify-between gap-3 px-1 md:mb-4 md:px-0">
                <div className="flex items-center gap-2">
                    <Sparkles
                        size={16}
                        className={isDayMode ? "text-blue-700" : "text-orange-300"}
                    />
                    <span
                        className={`text-xs uppercase tracking-[0.22em] ${isDayMode ? "text-blue-700" : "text-orange-300"}`}
                    >
                        {t("community.featured_article", "精选文章")}
                    </span>
                </div>
            </div>
            <ArticleCard
        ...
        <CommunityDetailModal
            ...
            headerContent={
                feed.selectedItem ? (
                    <>
                        <div
                            className={`flex items-center gap-3 font-bold text-lg md:text-xl uppercase tracking-[0.2em] mb-4 ${isDayMode ? "text-blue-700" : "text-orange-300 drop-shadow-lg"}`}
                        >
                            <span>{feed.selectedItem.date}</span>
                        </div>
                        <h2
                            className={`text-2xl font-black leading-tight tracking-tight md:text-6xl md:leading-[0.95] font-serif ${isDayMode ? "text-slate-900" : "text-white drop-shadow-2xl"}`}
                        >
            ...
            authorBar={
                feed.selectedItem ? (
                    <FavoriteButton
                        ...
                        onToggle={(favorited, likes) =>
                            feed.handleToggleFavorite(feed.selectedItem.id, favorited, likes)
                        }
                    />
                ) : null
            }
    ...
    return (
        <CommunityFeedPanel
            ...
            renderSkeleton={(index) => (
                <div
                    key={index}
                    className={`backdrop-blur-xl border rounded-lg p-5 animate-pulse flex flex-col md:flex-row gap-6 ${isDayMode ? "bg-white/60 border-white/75" : "bg-white/[0.04] border-white/5"}`}
                >
                    <div
                        className={`w-full md:w-48 h-48 md:h-32 rounded-md shrink-0 ${isDayMode ? "bg-white/70" : "bg-white/5"}`}
                    />
                    <div className="flex-1 space-y-4 py-2">
                        <div
                            className={`h-8 rounded w-3/4 ${isDayMode ? "bg-white/70" : "bg-white/10"}`}
                        />
                        <div
                            className={`h-4 rounded w-full ${isDayMode ? "bg-white/58" : "bg-white/5"}`}
                        />
                    </div>
                </div>
            )}
