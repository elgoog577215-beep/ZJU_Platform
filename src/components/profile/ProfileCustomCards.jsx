...
const getCardView = (card) => {
    ...
    const typeLabel =
        card.custom_type || CARD_TYPE_LABELS[card.card_type] || card.title || "自定义";
...
const ProfileCustomCards = ({ cards = [], isDayMode }) => {
    ...
    return (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-12">
            {cards.map((card) => {
                const { cover, link, typeLabel, description, cropX, cropY, cropWidth, cropHeight } =
                    getCardView(card);
                ...
                return (
                    ...
                    >
                        {cover ? (
                            ...
                            <div
                                className={`flex w-full items-center justify-center ${isDayMode ? "bg-slate-100 text-slate-400" : "bg-white/5 text-gray-500"}`}
                                style={{ aspectRatio: getAspectRatio(card.aspect_ratio) }}
                            >
                                {typeLabel}
                            </div>
                        ...
                        {!card.is_visible && (
                            <div
                                className={`px-3 py-2 text-xs ${isDayMode ? "text-amber-600" : "text-amber-300"}`}
                            >
                                已隐藏，仅自己可见
                            </div>
                        )}
