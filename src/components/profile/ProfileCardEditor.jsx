...
const getTypeLabel = (card) =>
    card.custom_type ||
    CARD_TYPE_OPTIONS.find(([value]) => value === card.card_type)?.[1] ||
    "自定义";
...
const SaveState = ({ state, isDayMode }) => (
    <span
        className={`text-xs font-bold ${state === "dirty" ? "text-amber-500" : state === "error" ? "text-rose-500" : isDayMode ? "text-slate-500" : "text-gray-500"}`}
    >
...
const ProfileCardEditor = ({ profileCard, isDayMode, onSaved }) => {
    ...
    const buttonClass =
        "inline-flex min-h-[36px] items-center justify-center rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white transition-colors hover:bg-indigo-700 disabled:opacity-50";
    ...
    const buildPayload = (nextDraft = draft) => ({
        ...
        tags: splitList(nextDraft.tagsText)
            .slice(0, 24)
    ...
    const handleCoverUpload = async (index, file) => {
        ...
        try {
            ...
            toast.error(
                error.response?.data?.message || error.response?.data?.error || "封面上传失败"
            );
        } finally {
    ...
    const handleCropMove = (event) => {
        ...
        let next = {
            crop_x: drag.crop.x,
            crop_y: drag.crop.y,
            crop_width: drag.crop.w,
            crop_height: drag.crop.h,
        };
    ...
    return (
        <div className="space-y-5">
            <div
                className={`rounded-2xl border p-4 ${isDayMode ? "border-slate-200 bg-slate-50/80" : "border-white/10 bg-black/20"}`}
            >
                ...
                <div className="space-y-3">
                    ...
                    <button
                        ...
                        disabled={saveStates.basics === "saving"}
                    >
                        保存简介
                    </button>
                </div>
            ...
            >
                ...
                <button
                    ...
                    disabled={saveStates.tags === "saving"}
                >
                    保存标签
                </button>
            </div>
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            onClick={() => {
                                setDraft((prev) => ({
                                    ...prev,
                                    social_links: [...prev.social_links, emptyLink()],
                                }));
                                markState("socials", "dirty");
                            }}
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                                <button
                                    ...
                                    onClick={() => {
                                        setDraft((prev) => ({
                                            ...prev,
                                            social_links: moveItem(prev.social_links, index, -1),
                                        }));
                                        markState("socials", "dirty");
                                    }}
                                    className="rounded-lg px-2 py-1 text-xs font-bold bg-black/10"
                                >
                                    <ArrowUp size={14} />
                                </button>
                                <button
                                ...
                                <button
                                    type="button"
                                    onClick={() =>
                                        updateLink(index, { is_visible: !link.is_visible })
                                    }
                                    className="rounded-lg px-2 py-1 text-xs font-bold bg-black/10"
                                >
                                ...
                                <button
                                    ...
                                    className="rounded-lg px-2 py-1 text-xs font-bold bg-rose-600 text-white"
                                >
                                    <Trash2 size={14} />
                                </button>
                            </div>
                ...
                <button
                    type="button"
                    onClick={() => saveSection("socials")}
                    className={`${buttonClass} mt-3`}
                    disabled={saveStates.socials === "saving"}
                >
                    保存社交链接
                </button>
            </div>
            ...
            >
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            type="button"
                            onClick={() => saveSection("cardsOrder")}
                            className={buttonClass}
                            disabled={saveStates.cardsOrder === "saving"}
                        >
                            保存卡片排序
                        </button>
                        <button
                    ...
                    {draft.cards.map((card, index) => (
                        ...
                        >
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <div
                                    className={`inline-flex items-center gap-2 text-xs font-bold ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                >
                                    <GripVertical size={15} /> 拖动调整顺序
                                </div>
                                <SaveState
                                    state={saveStates.cards[index] || "saved"}
                                    isDayMode={isDayMode}
                                />
                            </div>
                            <div className="grid grid-cols-1 gap-2 sm:grid-cols-[140px_1fr]">
                                <select
                                    ...
                                    onChange={(event) =>
                                        updateCard(index, { card_type: event.target.value })
                                    }
                                    className={inputClass}
                                >
                                ...
                                {card.card_type === "other" ? (
                                    <input
                                        value={card.custom_type}
                                        onChange={(event) =>
                                            updateCard(index, { custom_type: event.target.value })
                                        }
                                        className={inputClass}
                                        placeholder="自定义类型名称"
                                    />
                                    ...
                                    <input
                                        value={card.link_url}
                                        onChange={(event) =>
                                            updateCard(index, { link_url: event.target.value })
                                        }
                                        className={inputClass}
                                        placeholder="跳转链接，可选"
                                    />
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                                {ASPECT_RATIO_OPTIONS.map(([value, label]) => (
                                    <button
                                        key={value}
                                        type="button"
                                        onClick={() => updateCard(index, { aspect_ratio: value })}
                                        className={`rounded-lg px-3 py-1.5 text-xs font-bold ${card.aspect_ratio === value ? "bg-indigo-600 text-white" : isDayMode ? "bg-slate-100 text-slate-600" : "bg-white/10 text-gray-300"}`}
                                    >
                                        {label}
                                    </button>
                                ))}
                            ...
                            <div
                                className={`mt-2 overflow-hidden rounded-xl border ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-black/20"}`}
                            >
                                {card.cover_url ? (
                                    <div
                                        data-crop-box
                                        className="relative mx-auto w-full max-w-2xl overflow-hidden select-none"
                                        style={{ aspectRatio: getAspectRatio(card.aspect_ratio) }}
                                    >
                                        <img
                                            src={card.cover_url}
                                            alt="自定义卡片原图"
                                        ...
                                        <div
                                            ...
                                            onPointerDown={(event) =>
                                                startCropDrag(event, index, "move")
                                            }
                                        >
                                            <img
                                                src={card.cover_url}
                                            ...
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    aria-label="调整裁切范围"
                                                    onPointerDown={(event) =>
                                                        startCropDrag(event, index, mode)
                                                    }
                                                    className={`absolute h-4 w-4 rounded-full border-2 border-white bg-indigo-600 shadow ${positionClass}`}
                                                />
                                            ))}
                                            <div className="pointer-events-none absolute inset-x-0 bottom-1 text-center text-[10px] font-bold text-white drop-shadow">
                                                拖动中间移动，拖四角缩放
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        className={`flex aspect-video w-full items-center justify-center text-sm ${isDayMode ? "text-slate-400" : "text-gray-500"}`}
                                    >
                                        暂未上传封面
                                    </div>
                                )}
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                <label
                                    className={`inline-flex min-h-[36px] cursor-pointer items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${isDayMode ? "bg-slate-100 text-slate-700 hover:bg-slate-200" : "bg-white/10 text-white hover:bg-white/15"}`}
                                >
                                    <Upload size={14} />
                                    ...
                                    <input
                                        ...
                                        onChange={(event) =>
                                            handleCoverUpload(index, event.target.files?.[0])
                                        }
                                    />
                                </label>
                                {card.cover_url && (
                                    <button
                                        ...
                                        className={`inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-bold ${isDayMode ? "bg-slate-100 text-slate-700" : "bg-white/10 text-white"}`}
                                    >
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                <button
                                    ...
                                    onClick={() => {
                                        setDraft((prev) => ({
                                            ...prev,
                                            cards: moveItem(prev.cards, index, -1),
                                        }));
                                        markState("cardsOrder", "dirty");
                                    }}
                                    className="rounded-lg px-2 py-1 text-xs font-bold bg-black/10"
                                ...
                                <button
                                    type="button"
                                    onClick={() =>
                                        updateCard(index, { is_visible: !card.is_visible })
                                    }
                                    className="rounded-lg px-2 py-1 text-xs font-bold bg-black/10"
                                >
                                ...
                                <button
                                    type="button"
                                    onClick={() => saveSection("card", index)}
                                    className={`${buttonClass} ml-auto`}
                                    disabled={saveStates.cards[index] === "saving"}
                                >
                                    保存此卡片
                                </button>
                            </div>
