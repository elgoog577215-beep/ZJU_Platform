...
const resolveCompletionTarget = (item) =>
    item?.target && !isBroadTarget(item.target) ? item.target : targetForCompletionKey(item?.key);
...
const UserSystemOverview = ({ overview, loading, isDayMode, t, onOpenTarget }) => {
    ...
    const buttonClass =
        "inline-flex min-h-[38px] items-center justify-center gap-2 rounded-[6px] px-3 py-2 text-xs font-bold transition-colors";
    ...
    const reviewPermission =
        permission.reviewPermission || overview?.account?.review_permission || "normal";
    ...
    const nextActions = (
        overview?.nextActions?.length ? overview.nextActions : generatedActions
    ).slice(0, 6);
    ...
    return (
        <section className={`mb-6 rounded-[8px] border p-4 md:p-6 ${panelClass}`}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                {loading ? (
                    <div
                        className={`inline-flex items-center gap-2 rounded-[6px] border px-3 py-2 text-xs font-bold ${moduleClass} ${mutedText}`}
                    >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {topStats.map(({ key, icon: Icon, value, label, target, tone }) => (
                    ...
                    >
                        <div
                            className={`mb-3 flex h-10 w-10 items-center justify-center rounded-[6px] ${isDayMode ? "bg-white" : "bg-white/10"} ${tone}`}
                        >
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[1.05fr_1fr]">
                <div className={`rounded-[8px] border p-4 ${moduleClass}`}>
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <div
                            className={`text-xl font-black ${isDayMode ? "text-indigo-600" : "text-indigo-200"}`}
                        >
                            {percent}%
                        </div>
                    </div>
                    <div
                        className={`mt-4 h-2 overflow-hidden rounded-full ${isDayMode ? "bg-slate-200" : "bg-white/10"}`}
                    >
                        <div
                ...
                <div className={`rounded-[8px] border p-4 ${moduleClass}`}>
                    ...
                    {nextActions.length === 0 ? (
                        <p className={`text-sm ${mutedText}`}>
                            {t("user_profile.system_overview.actions.empty")}
                        </p>
                    ) : (
                        <div className="grid gap-2">
                            {nextActions.map((action) => (
                                ...
                                >
                                    <span>
                                        {t(
                                            `user_profile.system_overview.next_actions.${action.key}`
                                        )}
                                    </span>
                                    {action.count ? (
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_0.8fr]">
                <div className={`rounded-[8px] border p-4 ${moduleClass}`}>
                    ...
                    {managedOrganizations.length ? (
                        <div className="grid gap-3 md:grid-cols-2">
                            {managedOrganizations.slice(0, 6).map((profile) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div
                                            className={`flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-[6px] border ${
                                                isDayMode
                                        ...
                                        <div className="min-w-0">
                                            <div
                                                className={`truncate text-sm font-bold ${titleText}`}
                                            >
                                                {profile.display_name}
                                            </div>
                                            <div
                                                className={`mt-1 flex flex-wrap gap-1.5 text-[11px] font-bold ${mutedText}`}
                                            >
                                                <span>
                                                    {t(
                                                        `user_profile.system_overview.profile_types.${profile.type}`,
                                                        profile.type
                                                    )}
                                                </span>
                                                {profile.member_role ? (
                                                    <span>{profile.member_role}</span>
                                                ) : null}
                        ...
                        <div
                            className={`rounded-[8px] border border-dashed p-4 text-sm leading-6 ${isDayMode ? "border-slate-200 bg-white text-slate-600" : "border-white/10 bg-white/5 text-gray-300"}`}
                        >
                ...
                <div className={`rounded-[8px] border p-4 ${moduleClass}`}>
                    ...
                    {contentStatusItems.length > 0 ? (
                        <div className="grid grid-cols-2 gap-2">
                            {contentStatusItems.map((item) => (
                                <button
                                    ...
                                    onClick={() =>
                                        openTarget(
                                            item.key === "pending" ? "submissions" : "published"
                                        )
                                    }
                                ...
                                >
                                    <div className={`text-xl font-black ${titleText}`}>
                                        {item.value}
                                    </div>
                                    <div className={`mt-1 text-xs font-bold ${mutedText}`}>
                                        {t(
                                            `user_profile.system_overview.content_status.${item.key}`
                                        )}
                                    </div>
                                </button>
                            ))}
                        ...
                        <p className={`text-sm ${mutedText}`}>
                            {t("user_profile.system_overview.content_empty")}
                        </p>
                    )}
                </div>
            </div>
