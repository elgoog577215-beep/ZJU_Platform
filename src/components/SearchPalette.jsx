...
const SearchPalette = ({ initialOpen = false }) => {
                            ...
                            .filter(
                                (group) =>
                                    !["music", "podcast"].includes(
                                        String(group.type || group.key || "").toLowerCase()
                                    )
                            );
    ...
    const getIcon = (type) => {
        switch (type) {
            case "photo":
                return <ImageIcon size={16} aria-hidden="true" className="text-purple-400" />;
            case "music":
                return <Music size={16} aria-hidden="true" className="text-pink-400" />;
            case "video":
                return <Film size={16} aria-hidden="true" className="text-blue-400" />;
            case "article":
                return <FileText size={16} aria-hidden="true" className="text-yellow-400" />;
            case "event":
                return <Calendar size={16} aria-hidden="true" className="text-green-400" />;
            case "post":
                return <MessageSquare size={16} aria-hidden="true" className="text-orange-400" />;
            case "group":
                return <Users size={16} aria-hidden="true" className="text-cyan-400" />;
            case "news":
                return <FileText size={16} aria-hidden="true" className="text-sky-400" />;
            default:
                return <Search size={16} aria-hidden="true" />;
        }
    };

    const resolveResultIndex = (item) =>
        results.findIndex(
    ...
    const renderResultItem = (item) => {
        ...
        return (
            ...
            >
                <div
                    className={`w-10 h-10 rounded-[5px] overflow-hidden flex-shrink-0 border ${isDayMode ? "bg-white border-slate-200/80" : "bg-black/50 border-white/10"}`}
                >
                    {item.image ? (
                        <img
                ...
                <div className="min-w-0 flex-1 text-left">
                    <div className="flex min-w-0 items-center gap-2">
                        <h4
                            className={`truncate text-sm font-medium ${isSelected ? (isDayMode ? "text-slate-900" : "text-white") : isDayMode ? "text-slate-700" : "text-gray-300"}`}
                        >
                            {highlightTitle(item.title)}
                        </h4>
                        <span
                            className={`shrink-0 rounded-[4px] px-1.5 py-0.5 text-[10px] font-semibold ${isDayMode ? "bg-slate-100 text-slate-500" : "bg-white/5 text-gray-400"}`}
                        >
    ...
    const highlightTitle = (title) => {
        ...
        return title.split(new RegExp(`(${escaped})`, "gi")).map((part, index) =>
            part.toLowerCase() === normalizedQuery.toLowerCase() ? (
                <span key={index} className="text-indigo-400 bg-indigo-500/10 px-0.5 rounded">
                    {part}
                </span>
            ) : (
                part
    ...
    return (
        <AnimatePresence initial={false}>
            {isOpen && (
                <div
                    className="fixed inset-0 z-[100] flex items-start justify-center md:pt-[20vh] md:px-4"
                    role="dialog"
                    aria-modal="true"
                        ...
                        <div className="custom-scrollbar flex-1 overflow-y-auto p-3 pb-[env(safe-area-inset-bottom)]">
                            {loading ? (
                                ...
                                <div
                                    className={`p-8 text-center ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                                >
                                    {t("search.no_results", { query: normalizedQuery })}
                                </div>
                            ) : (
                                <div
