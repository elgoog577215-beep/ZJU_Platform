...
const MobileNavbar = () => {
  ...
  const menuItems = [
    ...
    ...(user
      ? [{ key: "profile", path: `/user/${user.id}`, icon: UserCircle }]
      : [{ key: "login", action: "login", icon: LogIn }]),
  ];
  ...
  return (
    <>
      <nav
        className={`fixed left-3 right-3 bottom-[max(env(safe-area-inset-bottom),10px)] sm:left-4 sm:right-4 z-[100] backdrop-blur-2xl border rounded-2xl md:hidden app-select-none ${isDayMode ? "bg-white/88 border-slate-200/80 shadow-[0_12px_28px_rgba(148,163,184,0.2)]" : "bg-[#1a1a1a]/82 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)]"}`}
      >
          ...
          {navItems.map((item) => {
            ...
            if (item.action) {
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`p-1.5 rounded-xl transition-all duration-300 ${showMenu && item.action === "menu" ? "bg-indigo-500/20 text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)]" : ""}`}
                    >
                      <Icon
                        size={22}
                        strokeWidth={
                          showMenu && item.action === "menu" ? 2.5 : 2
                        }
                      />
                    </div>
                    <span
                      className={`text-[10px] font-medium transition-all ${showMenu && item.action === "menu" ? "opacity-100 font-semibold" : "opacity-70"}`}
                    >
                      {item.key === "menu"
                        ? t("nav.more", "更多")
                        : item.key === "about"
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`p-1.5 rounded-xl transition-all duration-300 ${isActive && !showMenu ? "bg-indigo-500/20 text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)]" : ""}`}
                  >
                    <Icon
                      size={22}
                      strokeWidth={isActive && !showMenu ? 2.5 : 2}
                    />
                  </div>
                  <span
                    className={`text-[10px] font-medium transition-all ${isActive && !showMenu ? "opacity-100 font-semibold" : "opacity-70"}`}
                  >
                    {t(
                      `nav.${item.key}`,
      ...
      <AnimatePresence>
        {showMenu && (
          <motion.div
            ...
            className={`fixed inset-0 z-[100] backdrop-blur-xl md:hidden flex flex-col touch-pan-y ${isDayMode ? "bg-[#f8fafc]/92" : "bg-[#0a0a0a]/80"}`}
          >
            {/* Drag Handle */}
            <div
            ...
            >
              <div
                className={`w-12 h-1.5 rounded-full ${isDayMode ? "bg-slate-300/80" : "bg-white/20"}`}
              />
            </div>

            {/* Header */}
            <div
              className={`flex items-center justify-between px-5 pt-[calc(env(safe-area-inset-top)+8px)] pb-4 border-b backdrop-blur-3xl ${isDayMode ? "border-slate-200/80 bg-white/70" : "border-white/10 bg-black/20"}`}
            >
              <h2
                className={`text-2xl font-bold ${isDayMode ? "text-slate-900" : "text-white"}`}
              >
                {t("nav.more", "Menu")}
              </h2>
            ...
            {/* Grid Menu */}
            <div className="flex-1 overflow-y-auto p-5 pb-[calc(env(safe-area-inset-bottom)+24px)]">
                    ...
                    return (
                      ...
                      >
                        <item.icon
                          size={24}
                          className={
                            isDayMode ? "text-indigo-500" : "text-indigo-400"
                          }
                        />
                        <span
                          className={`font-medium text-xs ${isDayMode ? "text-slate-700" : "text-white"}`}
                        >
                          {item.key === "login"
                            ? t("auth.log_in")
                            : item.key === "profile"
