...
const TagFilter = ({
    ...
    const selectedCountLabel = t("common.selected_count", "已选 {{count}} 项", {
    ...
    const containerClasses =
        variant === "card"
            ? `${isDayMode ? "border-t border-slate-200/80 bg-transparent" : "border-t border-white/10 bg-transparent"} pt-4 md:pt-5`
            : "";

    return (
        <div className={`w-full ${className || ""}`}>
            <div
                className={`${containerClasses} ${isFetching ? "opacity-90 transition-opacity duration-150" : ""}`}
            >
                {!isSheetVariant && !isInlineVariant ? (
                    <div className="flex items-center justify-between mb-4 md:mb-5">
                        ...
                        >
                            <div
                                className={`p-2 sm:p-1.5 rounded-md ${variant === "card" ? (isDayMode ? "text-indigo-500" : "text-indigo-300") : isDayMode ? "bg-slate-100 text-slate-700 border border-slate-200/80" : "bg-white/10 text-white"}`}
                            >
                                <Filter size={isMobile ? 16 : 18} />
                            </div>
                            <span
                                className={`font-bold text-base md:text-lg tracking-wide ${isDayMode ? "text-slate-900" : "text-white"}`}
                            >
                        ...
                        {selectedTags.length > 0 && (
                            <button
                                type="button"
                                onClick={() => onChange([])}
                                className={`text-xs font-medium flex items-center gap-1 transition-colors px-3 py-2 sm:px-3 sm:py-1.5 rounded-md min-h-[44px] sm:min-h-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 ${isDayMode ? "text-slate-500 hover:bg-slate-50 hover:text-slate-900" : "text-gray-400 hover:bg-white/8 hover:text-white"}`}
                            >
                                <X size={12} />
                    ...
                    <div className="mb-3 pl-1 flex items-center justify-between gap-3">
                        <span
                            className={`text-sm font-bold ${isDayMode ? "text-slate-700" : "text-white/80"}`}
                        >
                            {t("common.filter_by_tags", "标签筛选")}
                        </span>
                        {selectedTags.length > 0 && (
                            <span
                                className={`text-xs ${isDayMode ? "text-indigo-600" : "text-indigo-300"}`}
                            >
                                {selectedCountLabel}
                            </span>
                        )}
                ...
                {selectedTags.length > 0 && !isSheetVariant && !isInlineVariant && (
                    <div
                        className={`mb-3 text-xs inline-flex items-center px-2.5 py-1 rounded-md border-b-2 ${isDayMode ? "border-indigo-500 bg-indigo-50/70 text-indigo-600" : "border-indigo-300 bg-indigo-500/10 text-indigo-300"}`}
                    >
                        {selectedCountLabel}
                    </div>
                )}
                {shouldShowSearch && !isInlineVariant && (
                    <div className="mb-3 relative">
                        ...
                        <input
                            ...
                            className={`w-full rounded-md border pl-9 pr-10 py-2.5 min-h-[44px] text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-indigo-400/70 ${isDayMode ? "bg-white/78 border-slate-200/80 text-slate-700 placeholder:text-slate-400 focus:border-indigo-300/80" : "bg-white/[0.035] border-white/10 text-white placeholder:text-gray-500 focus:border-indigo-400/40"}`}
                        />
                        {searchTerm && (
                            <button
                                type="button"
                ...
                <AnimatePresence>
                    {(!isMobile || !isMobileCollapsed || isSheetVariant || isInlineVariant) && (
                        <motion.div
                            initial={
                                isMobile && !isSheetVariant ? { height: 0, opacity: 0 } : false
                            }
                            animate={
                                isMobile && !isSheetVariant ? { height: "auto", opacity: 1 } : false
                            }
                        ...
                        >
                            <motion.div
                                layout
                                className={`flex flex-wrap ${isSheetVariant ? "gap-2.5" : isInlineVariant ? "gap-2" : "gap-2.5 md:gap-3"}`}
                            >
                                {displayedTags.map((tag) => (
                                    <motion.button
                                        ...
                                        className={`${isInlineVariant ? "min-h-[32px] rounded-full px-3 py-1.5 text-xs" : "min-h-[40px] rounded-md px-3.5 py-2 text-sm sm:min-h-0 hover:-translate-y-0.5"} font-medium transition-all border flex items-center gap-2 ${
                                            selectedTags.includes(tag.name)
                                                ? isDayMode
                                                    ? "bg-blue-50 text-blue-700 border-blue-200/80 shadow-none"
                                                    : "bg-indigo-500/20 text-indigo-100 border-indigo-400/35 shadow-none"
                                                : isDayMode
                                                  ? "bg-white/70 text-slate-500 border-slate-200/80 hover:bg-white hover:text-slate-900 hover:border-blue-200/80"
                                            ...
                                            <motion.div
                                                ...
                                                className={
                                                    isDayMode
                                                        ...
                                                        : "rounded-full bg-indigo-300/20 p-0.5"
                                                }
                                            >
                            ...
                            {filteredTags.length === 0 && (
                                <div
                                    className={`text-center py-4 text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                >
                                    {t("search.no_results", '没有找到关于 "{{query}}" 的结果', {
                                        query: searchTerm,
                                    })}
                                </div>
                            )}
                            ...
                            {filteredTags.length > initialLimit && (
                                <div
                                    className={`flex ${isInlineVariant ? "mt-1 justify-start" : "mt-3 justify-center md:mt-4"}`}
                                >
                                    <button
                                        type="button"
