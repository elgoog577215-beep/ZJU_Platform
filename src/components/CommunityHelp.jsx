...
const CommunityHelp = ({ onNewPost, hideNewPostButton = false, discussionMode = false }) => {
                ...
                feed.selectedItem && (
                    <>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            >
                                {feed.selectedItem.status === "solved"
                                    ? t("community.post_status_solved", "已解决")
                                    : t("community.post_status_open", "待答")}
                            </span>
                            <span
                                className={`text-sm font-mono ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                            >
                                {feed.selectedItem.created_at &&
                                    new Date(feed.selectedItem.created_at).toLocaleDateString(
                                        "zh-CN"
                                    )}
                            </span>
                        </div>
                        <h2
                            className={`text-2xl font-black leading-tight tracking-tight md:text-5xl font-serif ${isDayMode ? "text-slate-900" : "text-white drop-shadow-2xl"}`}
                        >
    ...
    return (
        <CommunityFeedPanel
            ...
            emptyTitle={
                discussionMode
                    ? t("community_learning.discuss_empty", "暂无讨论")
                    : t("community.help_empty", "暂无帖子")
            }
            emptyDesc={
                discussionMode
                    ? t(
            ...
            newPostLabel={
                discussionMode ? t("community_learning.discuss_new_post", "发起讨论") : undefined
            }
