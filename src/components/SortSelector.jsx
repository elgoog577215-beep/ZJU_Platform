                    ...
                    {options.map((option) => {
                        ...
                        return (
                            <button
                                key={option.value}
                                type="button"
                                aria-label={t("sort_filter.choose_option", "选择 {{label}}", {
                                    label: option.label,
                                })}
                                aria-pressed={active}
                                ...
                                className={`w-full min-h-[48px] flex items-center justify-between gap-3 px-4 py-4 rounded-lg border transition-all text-left focus:outline-none focus-visible:ring-2 ${isDayMode ? "focus-visible:ring-blue-300/60" : "focus-visible:ring-indigo-400/70"} ${
                                    active
                                        ? isDayMode
                                            ? "bg-blue-50 text-slate-950 border-blue-200 shadow-[0_4px_12px_rgba(37,99,235,0.08)]"
                            ...
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    <div
                                        className={`p-2 rounded-md ${active ? (isDayMode ? "bg-blue-50 text-blue-700" : "bg-indigo-500/20 text-indigo-300") : isDayMode ? "bg-slate-50 text-slate-500" : "bg-white/5 text-gray-400"}`}
                                    >
                                ...
                                >
                                    <Check
                                        size={18}
                                        aria-hidden="true"
                                        className={isDayMode ? "text-blue-700" : "text-indigo-300"}
                                    />
                                </div>
                            </button>
                        );
