...
const SPLASH_SEEN_KEY = "tuozhe_welcome_seen";
...
const HomeSplash = () => {
    ...
    const [hasEntered, setHasEntered] = useState(
        () => Boolean(location.search || location.hash) || hasSeenWelcome()
    );
    ...
    const enterPlatform = useCallback(() => {
        ...
        try {
            ...
            // The welcome still closes when browser storage is unavailable.
        }
        setHasEntered(true);
    }, []);
    ...
    }, [enterPlatform, hasEntered]);

    if (hasEntered) return <HomeDirectory />;
    ...
    return (
        <section className={pageClass} data-testid="home-welcome">
            <SEO
            ...
            <div className="absolute inset-x-0 bottom-[calc(var(--mobile-content-bottom-padding)+1.25rem)] md:bottom-[calc(env(safe-area-inset-bottom)+1.25rem)] z-30 flex justify-center px-4">
                <motion.button
                    type="button"
