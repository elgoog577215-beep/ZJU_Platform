...
import performanceMonitor, { performanceDebugStorageKey } from "../utils/performanceMonitor";
...
const formatSeconds = (value) => (value == null ? "N/A" : `${(value / 1000).toFixed(2)} s`);
...
const PerformancePanel = () => {
    ...
    useEffect(() => {
        ...
        const enabled = window.localStorage.getItem(performanceDebugStorageKey) === "true";
        ...
        >
            ...
            <div className="grid grid-cols-2 gap-3">
                {metricCards.map(({ label, icon: Icon, value, status }) => (
                    <div key={label} className={`rounded-xl border p-3 ${statusClassMap[status]}`}>
