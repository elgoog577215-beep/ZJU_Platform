...
export const TextSkeleton = ({ lines = 3, label = "" }) => {
    return (
        <div className="space-y-2" role="status" aria-live="polite" aria-label={label || undefined}>
            {Array.from({ length: lines }).map((_, i) => (
                <SkeletonItem key={i} className={`h-4 ${i === lines - 1 ? "w-2/3" : "w-full"}`} />
            ))}
        </div>
    );
};
            ...
            {Array.from({ length: rows }).map((_, rowIndex) => (
                <div key={rowIndex} className="flex gap-4 py-3">
                    {Array.from({ length: columns }).map((_, colIndex) => (
                        <SkeletonItem key={colIndex} className="h-5 flex-1 rounded-lg" />
                    ))}
                </div>
            ))}
        </div>
