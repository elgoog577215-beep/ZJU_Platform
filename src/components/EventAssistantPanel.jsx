...
const EventAssistantPanel = ({
    ...
    const emptyStateText = useMemo(() => {
        switch (assistantState?.emptyReason) {
            case "assistant_unreliable":
                return t(
                    "events.assistant.empty_unreliable",
                    "这次结果不够可靠，可以换个说法再试。"
                );
            case "clarification_limit_reached":
                return t(
                    "events.assistant.empty_after_clarify",
                    "这次补充后仍然没有稳定结果，可以换个说法再试。"
                );
            case "no_matches":
    ...
    const sendAssistantRequest = async (payload, nextOriginalQuery, nextClarificationAsked) => {
        ...
        try {
            ...
            setClarificationAsked(
                response.data?.type === "clarify" ? true : nextClarificationAsked
            );
            setInput("");
            setFeedbackMap({});
        } catch (error) {
        ...
        try {
            await api.post(
                "/events/assistant/action",
                {
                    ...
                    source:
                        variant === "fullscreen"
                            ? "event_assistant_mobile"
                            : "event_assistant_card",
                    visitorKey: getOrCreateSiteVisitorKey(),
                    metadata,
                },
                { silent: true }
    ...
    const submitFeedback = async (item, feedback, reasonValue = "", recommendationRank = null) => {
        ...
        try {
            const reasonLabel = feedbackReasonOptions.find(
            ...
            await api.post("/events/assistant/feedback", {
                ...
                source:
                    variant === "fullscreen" ? "event_assistant_mobile" : "event_assistant_card",
                reason:
                    feedback === "down" && reasonLabel
            ...
            if (error?.response?.status === 401) {
                toast.error(
                    t("events.assistant.toast.feedback_login_required", "登录后可以让助手记住反馈")
                );
            } else {
                toast.error(
                    t("events.assistant.toast.feedback_failed", "反馈记录失败，请稍后再试")
                );
            }
        }
    };
    ...
    const promptCardClass =
        isFullscreenVariant || isRailVariant
    ...
    const shellRadiusClass = isFullscreenVariant
        ? "rounded-none"
        : isRailVariant
          ? "rounded-[7px]"
          : "rounded-lg";
    ...
    const resetButtonSizeClass = isRailVariant
        ? "min-h-[36px] px-2.5 text-xs"
        : "min-h-[38px] px-3 text-sm";
    const submitButtonSizeClass = isRailVariant
                ...
                <div className={`relative ${panelPaddingClass}`}>
                    {isRailVariant ? (
                        ...
                        >
                            <div className="flex items-center justify-between gap-2.5">
                                <div className="flex min-w-0 items-center gap-2 text-left">
                                    <span
                                        className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border ${isDayMode ? "border-slate-200/80 bg-white text-blue-700" : "border-blue-300/15 bg-blue-400/10 text-blue-200"}`}
                                    >
                                        <Sparkles size={16} />
                                    </span>
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                            <div className="max-w-2xl text-left">
                                <span
                                    className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] ${chipClass}`}
                                >
                                    <Sparkles
                                        size={13}
                                        className={isDayMode ? "text-blue-600" : "text-cyan-300"}
                                    />
                                    {t("events.assistant.card_badge", "AI 活动助手")}
                                </span>
                                <h3
                                    className={`mt-3 text-xl font-bold tracking-normal sm:text-2xl ${textClass}`}
                                >
                                    {t("events.assistant.panel_title", "用一句话找活动")}
                                </h3>
                                <p className={`mt-2 text-sm leading-7 sm:text-base ${mutedClass}`}>
                                    {t(
                                        "events.assistant.panel_subtitle",
                                        "写主题、时间、地点或收益，我帮你挑出更合适的活动。"
                                    )}
                                </p>
                            </div>
                    ...
                    {isFullscreenVariant && !originalQuery && !assistantState ? (
                        <div className="mb-5 flex flex-col items-center pt-4 text-center">
                            <div
                                className={`mb-4 flex h-20 w-20 items-center justify-center rounded-full border ${isDayMode ? "border-indigo-100 bg-indigo-50 text-indigo-700" : "border-indigo-300/20 bg-indigo-500/18 text-indigo-100"}`}
                            >
                                <Sparkles size={34} />
                            </div>
                            <h3 className={`text-base font-black ${textClass}`}>
                                你好，我是你的活动助手
                            </h3>
                            ...
                            <div
                                className={`mt-4 rounded-full border px-3 py-1 text-[11px] ${controlChipClass}`}
                            >
                                你可以这样问
                            </div>
                        </div>
                    ...
                    <AnimatePresence mode="wait">
                        {assistantState && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: -8 }}
                                transition={{
                                    duration: isRailVariant ? 0.2 : 0.24,
                                    ease: [0.22, 1, 0.36, 1],
                                }}
                                className="mt-4"
                            >
                                <div
                                    className={`rounded-lg border ${resultPaddingClass} ${resultPanelClass}`}
                                >
                                    {assistantState.summary ? (
                                        <p
                                            className={`text-base font-semibold leading-7 ${textClass}`}
                                        >
                                            {assistantState.summary}
                                        </p>
                                    ) : null}

                                    {assistantState.recommendationMode === "historical_fallback" &&
                                    assistantState.coverage ? (
                                        <p className={`mt-2 text-sm leading-6 ${faintClass}`}>
                                            {t(
                                                "events.assistant.historical_fallback_note",
                                                "没有足够匹配的未来活动，下面是可关注的历史线索。"
                                            )}
                                        </p>
                                    ) : null}
                                    ...
                                    {(assistantState.warnings || []).length > 0 ? (
                                        <div
                                            className={`mt-3 rounded-lg border px-4 py-3 text-sm leading-6 ${isDayMode ? "bg-amber-50 text-amber-800 border-amber-200/80" : "bg-amber-400/10 text-amber-100 border-amber-300/20"}`}
                                        >
                                    ...
                                    {assistantState.type === "clarify" && (
                                        <div className="mt-2 max-w-2xl">
                                            ...
                                            <p
                                                className={`mt-2 text-lg font-semibold leading-8 ${textClass}`}
                                            >
                                    ...
                                    {assistantState.type === "empty" && (
                                        <div
                                            className={`mt-4 rounded-lg border px-5 py-6 text-center ${softPanelClass}`}
                                        >
                                            <p
                                                className={`text-base font-semibold leading-8 sm:text-lg ${textClass}`}
                                            >
                                                {emptyStateText}
                                            </p>
                                            <p className={`mt-2 text-sm leading-7 ${mutedClass}`}>
                                                {assistantState.canExpandScope
                                                    ? t(
                                                          "events.assistant.expand_hint",
                                                          "可以放宽到进行中或历史活动，看有没有后续可关注的线索。"
                                                      )
                                                    : t(
                                                          "events.assistant.empty_hint",
                                                          "你可以换一个主题、校区或收益条件再试。"
                                                      )}
                                            </p>
                                            ...
                                            {assistantState.canExpandScope && (
                                                ...
                                                >
                                                    <ArrowRight size={15} />
                                                    {t(
                                                        "events.assistant.expand_button",
                                                        "看看进行中或历史活动"
                                                    )}
                                                </button>
                                            )}
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <div className="flex items-start justify-between gap-3">
                                                                    <div className="min-w-0">
                                                                        {item.isHistorical ? (
                                                                            <span
                                                                                className={`mb-2 inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-semibold ${isDayMode ? "bg-amber-50 text-amber-700 border-amber-200/80" : "bg-amber-400/10 text-amber-200 border-amber-300/15"}`}
                                                                            >
                                                                                <Clock3 size={12} />
                                                                                {t(
                                                                                    "events.assistant.historical_event",
                                                                                    "历史活动"
                                                                                )}
                                                                            </span>
                                                                        ) : null}
                                                                        <p
                                                                            className={`${recommendationTitleClass} ${textClass}`}
                                                                        >
                                                                            {item.event.title}
                                                                        </p>
                                                                        <p
                                                                            className={`${recommendationReasonClass} ${mutedClass}`}
                                                                        >
                                                                            {item.reason}
                                                                        </p>
                                                                    </div>
                                                                    <span
                                                                        className={`inline-flex shrink-0 items-center justify-center gap-1 rounded-md border text-xs font-semibold transition-transform group-hover:translate-x-1 ${detailButtonClass} ${chipClass}`}
                                                                    >
                                                                        <span className="sm:hidden">
                                                                            {t(
                                                                                "events.assistant.view_details",
                                                                                "详情"
                                                                            )}
                                                                        </span>
                                                            ...
                                                            {decisionPreview?.nextAction ? (
                                                                <p
                                                                    className={`mt-3 flex items-start gap-2 text-sm leading-6 ${isDayMode ? "text-emerald-700" : "text-emerald-100"}`}
                                                                >
                                                                    <ArrowRight
                                                                        size={14}
                                                            ...
                                                            <div
                                                                className={`mt-4 flex flex-wrap items-center gap-2 text-xs ${faintClass}`}
                                                            >
                                                                {item.event.date && (
                                                                    <span
                                                                        className={`inline-flex items-center gap-1.5 rounded-md border ${metaChipClass} ${chipClass}`}
                                                                    >
                                                                        <Calendar size={13} />
                                                                        {formatEventDate(
                                                                            item.event.date
                                                            ...
                                                            <div
                                                                className={`${isRailVariant ? "mt-3 pt-2.5" : "mt-4 pt-3"} flex items-center justify-between border-t ${isDayMode ? "border-slate-200/70" : "border-white/10"}`}
                                                            >
                                                                <span
                                                                    className={`text-xs ${faintClass}`}
                                                                >
                                                                    {t(
                                                                        "events.assistant.feedback_question",
                                                                        "有用吗？"
                                                                    )}
                                                                </span>
                                                                <div className="flex items-center gap-2">
                                                                    <button
                                                                        type="button"
                                                                        aria-label={t(
                                                                            "events.assistant.feedback_up_aria",
                                                                            "推荐适合我"
                                                                        )}
                                                                        title={t(
                                                                            "events.assistant.feedback_up_title",
                                                                            "适合我"
                                                                        )}
                                                                        onClick={() =>
                                                                            submitFeedback(
                                                                    ...
                                                                    <button
                                                                        type="button"
                                                                        aria-label={t(
                                                                            "events.assistant.feedback_down_aria",
                                                                            "推荐不适合我"
                                                                        )}
                                                                        title={t(
                                                                            "events.assistant.feedback_down_title",
                                                                            "不适合我"
                                                                        )}
                                                                        onClick={() => {
                                                                            if (
                                                            ...
                                                            {feedbackMap[item.id] === "down" && (
                                                                <div
                                                                    className={`mt-3 rounded-lg border p-3 ${isDayMode ? "border-rose-100 bg-rose-50/60" : "border-rose-300/15 bg-rose-400/10"}`}
                                                                >
                                                                    <div className="flex flex-wrap gap-2">
                                                                        {feedbackReasonOptions.map(
                                                                            (option) => (
                                                                                <button
                                                                                    key={
                                                                                        option.value
