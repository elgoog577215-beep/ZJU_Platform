...
const ARTICLE_LEARNING_ROUTE = {
    65: {
        ...
        levelKey: "expert",
        order: 30,
        trackKey: "ai-science-automation",
        trackOrder: 30,
    },
    64: {
        chapterKey: "cases",
    ...
    59: {
        ...
        levelKey: "basic",
        order: 10,
        trackKey: "multi-agent-orchestration",
        trackOrder: 10,
    },
    58: {
        chapterKey: "prompt",
    ...
    52: {
        ...
        levelKey: "basic",
        order: 10,
        trackKey: "llm-parameters-reasoning",
        trackOrder: 10,
    },
    51: {
        chapterKey: "rag",
    ...
    39: {
        ...
        levelKey: "basic",
        order: 10,
        trackKey: "corpus-data-engineering",
        trackOrder: 10,
    },
    38: {
        chapterKey: "launch",
    ...
    33: {
        ...
        levelKey: "basic",
        order: 20,
        trackKey: "model-data-publishing",
        trackOrder: 10,
    },
    32: {
        chapterKey: "prompt",
...
const getProgressionItems = (currentArticle, trackItems) => {
    if (!currentArticle?.trackKey || !Array.isArray(trackItems) || trackItems.length <= 1)
        return [];
    ...
    const levelItems = LEVEL_SEQUENCE.map((levelKey) => {
        ...
        const currentInLevel = levelArticles.find(
...
const classifyLevel = (item) => {
    ...
    if (
        ["工程", "系统", "架构", "rag", "agent", "自动化", "评测", "向量库", "embedding"].some(
...
const LearningProgressionDock = ({ items, isDayMode, onOpen, t, variant = "footer" }) => {
        ...
        return (
            ...
            >
                <div className={`flex items-center justify-between gap-2 ${compact ? "" : "mb-3"}`}>
                    <span
                        className={`font-black ${compact ? "text-[11px]" : "text-xs"} uppercase tracking-[0.16em] opacity-75`}
                    >
                        {levelLabel}
                    </span>
                    {isCurrent ? (
                        <span
                            className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${isDayMode ? "bg-violet-100 text-violet-700" : "bg-orange-400/10 text-orange-200"}`}
                        >
                ...
                {!compact ? (
                    <>
                        <div className="line-clamp-2 font-bold leading-6">
                            {article?.title ||
                                t("community_learning.progression_missing", "暂无对应篇")}
                        </div>
                        {article?.excerpt ? (
                            <p
                                className={`mt-2 line-clamp-2 text-xs leading-5 ${isCurrent ? "opacity-75" : isDayMode ? "text-slate-600" : "text-gray-400"}`}
                            >
                                {article.excerpt}
                            </p>
                        ) : null}
    ...
    if (variant === "header") {
        ...
        return (
            <div
                className={`mt-4 rounded-lg border p-3 ${isDayMode ? "border-violet-200 bg-violet-50/70 text-violet-800" : "border-white/10 bg-white/[0.045] text-gray-200"}`}
            >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                        ...
                        <div
                            className={`mt-1 text-sm font-bold ${isDayMode ? "text-slate-950" : "text-white"}`}
                        >
    ...
    if (variant === "desktopRail") {
        return (
            <aside className="sticky top-24 hidden self-start lg:block">
                <div
                    className={`rounded-lg border p-3 ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-white/[0.04]"}`}
                >
                    <div
                        className={`mb-3 text-xs font-black uppercase tracking-[0.18em] ${isDayMode ? "text-violet-600" : "text-orange-300"}`}
                    >
    ...
    if (variant === "mobileBar") {
        return (
            <div
                className={`fixed inset-x-0 bottom-0 z-[120] border-t px-3 pb-[calc(env(safe-area-inset-bottom)+0.45rem)] pt-2 shadow-[0_-14px_34px_rgba(0,0,0,0.28)] lg:hidden ${
            ...
            >
                <div className="grid grid-cols-[auto_repeat(3,minmax(0,1fr))] items-center gap-1.5">
                    <div
                        className={`flex items-center gap-1 text-[11px] font-black uppercase tracking-[0.12em] ${isDayMode ? "text-violet-600" : "text-orange-300"}`}
                    >
                        <BookOpen size={13} />
    ...
    return (
        <section
            className={`mt-10 rounded-lg border p-4 md:p-5 ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-white/[0.04]"}`}
        >
            <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div
                        className={`text-xs font-black uppercase tracking-[0.2em] ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                    >
                        {t("community_learning.progression_kicker", "学习进阶")}
                    </div>
                    <h3
                        className={`mt-1 text-lg font-black ${isDayMode ? "text-slate-950" : "text-white"}`}
                    >
...
const LearningCard = ({ item, chapter, level, isDayMode, onOpen, t, language }) => {
    ...
    return (
        <button
            type="button"
            onClick={() => onOpen(item)}
            className={`group border-t py-5 text-left transition-colors md:p-6 ${
                isDayMode
        ...
        >
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <span
                    className={`inline-flex items-center gap-1.5 border-b px-1 py-1 text-xs font-bold ${isDayMode ? tone.rail : tone.nightRail}`}
                >
                    <Icon size={13} />
                ...
                {level ? (
                    <span
                        className={`px-1 py-1 text-xs font-bold ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                    >
            ...
            <h3
                className={`line-clamp-2 text-base font-black leading-snug md:text-xl ${isDayMode ? "text-slate-950" : "text-white"}`}
            >
                {item.title || t("community.untitled", "未命名")}
            </h3>
            <p
                className={`mt-2 line-clamp-3 text-sm leading-6 ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
            >
                {item.excerpt ||
                    t("community_learning.no_excerpt", "暂无摘要，打开后查看完整内容。")}
            </p>
            <div
                className={`mt-4 flex flex-wrap items-center gap-2 text-xs ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
            >
...
const FeaturedLearningCard = ({ item, chapter, level, isDayMode, onOpen, t, language }) => {
    ...
    return (
        <button
            type="button"
            onClick={() => onOpen(item)}
            className={`group grid w-full gap-4 border-t py-5 text-left transition-colors md:grid-cols-[minmax(0,1fr)_10rem] md:p-6 ${
                isDayMode
                    ? "border-slate-200 hover:bg-slate-50/60"
                    : "border-white/10 hover:bg-white/[0.025]"
            }`}
        >
            <div className="min-w-0">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span
                        className={`inline-flex items-center gap-1.5 border-b px-1 py-1 text-xs font-black ${isDayMode ? tone.rail : tone.nightRail}`}
                    >
                        <Sparkles size={13} />
                        {t("community_learning.first_read", "建议先读")}
                    </span>
                    <span
                        className={`inline-flex items-center gap-1.5 px-1 py-1 text-xs font-bold ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                    >
                        <Icon size={13} />
                        {t(level.titleKey, level.titleFallback)}
                    </span>
                    <span
                        className={`ml-auto text-xs ${isDayMode ? "text-slate-400" : "text-gray-500"}`}
                    >
                        {formatDate(item.sortDate, language)}
                    </span>
                </div>
                <h4
                    className={`line-clamp-2 text-xl font-black leading-tight md:text-2xl ${isDayMode ? "text-slate-950" : "text-white"}`}
                >
                    {item.title || t("community.untitled", "未命名")}
                </h4>
                <p
                    className={`mt-3 line-clamp-3 text-sm leading-7 ${isDayMode ? "text-slate-600" : "text-gray-300"}`}
                >
                    {item.excerpt ||
                        t("community_learning.no_excerpt", "暂无摘要，打开后查看完整内容。")}
                </p>
            ...
            <div
                className={`flex min-h-0 items-center justify-between gap-3 border-t bg-transparent pt-3 md:min-h-24 md:flex-col md:items-stretch md:border-0 md:pt-0 ${
                    isDayMode ? "border-slate-200" : "border-white/10"
                }`}
            >
                <span
                    className={`text-xs font-bold ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                >
                    {t(chapter?.titleKey, chapter?.titleFallback || "AI")}
                </span>
                <div
                    className={`flex items-center justify-between gap-3 text-xs md:mt-4 ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                >
                    <span className="inline-flex items-center gap-1">
                        <Clock3 size={12} />
                        {calculateReadingTime(item.content || item.excerpt, t)}
                    ...
                    <span
                        className={`inline-flex h-8 w-8 items-center justify-center transition-colors ${
                            isDayMode
                                ? "text-violet-700 group-hover:translate-x-0.5"
                                : "text-orange-200 group-hover:translate-x-0.5"
                        }`}
                    >
                        <ArrowRight
                            size={15}
...
const LearningArea = ({ isDayMode }) => {
    ...
    return (
        <div className="grid min-w-0 gap-3 md:gap-6 lg:grid-cols-[15rem_minmax(0,1fr)] lg:items-start lg:gap-8">
            <div
                        ...
                        return (
                            <button
                                ...
                                className={`inline-flex min-h-11 min-w-fit snap-start items-center gap-2 border-x-0 border-t-0 border-b-2 px-3 text-xs font-bold transition-colors ${
                                    active
                                        ? isDayMode
                                            ? "border-lime-700 text-slate-950"
                                            : "border-lime-200 text-white"
                                        : isDayMode
                            ...
                            >
                                ...
                                <span className="opacity-65">
                                    {articlesByChapter[chapter.key]?.length || 0}
                                </span>
                            </button>
                        );
                    })}
            ...
            <aside className="hidden lg:sticky lg:top-24 lg:block lg:self-start">
                <nav
                    className={`max-h-[calc(100vh-7rem)] overflow-y-auto border-r pr-5 ${isDayMode ? "border-slate-300/80" : "border-white/10"}`}
                    aria-label={t("community_learning.curriculum_label", "AI LEARNING PATH")}
                >
                    <div
                        className={`mb-3 px-1 text-[11px] font-black uppercase tracking-[0.22em] ${isDayMode ? "text-lime-800" : "text-lime-200"}`}
                    >
                    ...
                    <div className="space-y-1">
                        {CHAPTERS.map((chapter, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    aria-pressed={active}
                                    onClick={() => handleChapterChange(chapter.key)}
                                    className={`flex min-h-11 w-full items-center gap-2.5 border-l-2 px-3 py-1 text-left transition-colors ${
                                        active
                                            ? isDayMode
                                                ? "border-lime-700 bg-lime-700/[0.055] text-slate-950"
                                                : "border-lime-200 bg-lime-200/[0.045] text-white"
                                            : isDayMode
                                ...
                                >
                                    <span
                                        className={`flex h-7 w-7 shrink-0 items-center justify-center ${active ? (isDayMode ? "text-lime-800" : "text-lime-200") : "opacity-75"}`}
                                    >
                                        <Icon size={16} />
                                    </span>
                        ...
                        <div
                            className={`mt-2 border-t pt-2 ${isDayMode ? "border-slate-300/80" : "border-white/10"}`}
                        >
                            <button
                                type="button"
                                onClick={handleCompanionResources}
                                className={`flex min-h-11 w-full items-center gap-2.5 border-l-2 border-transparent px-3 py-1 text-left transition-colors ${
                                    isDayMode
                                        ? "text-slate-600 hover:text-slate-950"
                                        : "text-gray-400 hover:text-white"
                                }`}
                            >
                                <span className="flex h-7 w-7 shrink-0 items-center justify-center opacity-75">
                                    <FileStack size={16} />
                                </span>
                                <span className="min-w-0">
                                    <span className="block text-xs font-black uppercase tracking-[0.16em] opacity-60">
            ...
            <section className="min-w-0">
                <div className="mb-4 md:mb-5">
                    <h2
                        className={`flex items-center gap-2 text-2xl font-black md:text-3xl ${isDayMode ? "text-slate-950" : "text-white"}`}
                    >
                        <ActiveIcon
                            size={18}
                            className={isDayMode ? "text-lime-800" : "text-lime-200"}
                        />
                        {t(activeChapter.titleKey, activeChapter.titleFallback)}
                    </h2>
                    <p
                        className={`mt-1 max-w-3xl text-sm leading-6 ${isDayMode ? "text-slate-600" : "text-gray-300"}`}
                    >
                ...
                <CommunitySearchInput
                    ...
                    placeholder={t(
                        "community_learning.search_placeholder",
                        "搜索本章教程、笔记和资料"
                    )}
                    isDayMode={isDayMode}
                />
                ...
                <div className="mt-5 space-y-5">
                    {articleResource.loading && visibleArticlesCount === 0 ? (
                        ...
                        <div
                            className={`rounded-lg border p-5 ${isDayMode ? "border-rose-200 bg-rose-50 text-rose-700" : "border-rose-400/20 bg-rose-400/10 text-rose-100"}`}
                        >
                        ...
                        levelGroups.map((level) => {
                            ...
                            const chapter =
                                CHAPTERS.find((chapter) => chapter.key === activeChapterKey) ||
                                activeChapter;
                            ...
                            return (
                                <section
                                    ...
                                    className={`scroll-mt-28 border-t py-5 transition-colors duration-200 ${
                                        levelActive
                                            ? isDayMode
                                                ? "border-lime-700/35 bg-transparent"
                                                : "border-lime-200/25 bg-transparent"
                                            : isDayMode
                                ...
                                >
                                    <div className="mb-4 flex items-start justify-between gap-3 md:items-center">
                                        <div className="flex min-w-0 gap-3 md:items-center">
                                            <span
                                                className={`mt-1 h-3 w-3 shrink-0 rounded-full ring-4 ${
                                                    levelActive
                                                        ? isDayMode
                                                            ? "bg-lime-700 ring-lime-700/10"
                                                            : "bg-lime-200 ring-lime-200/10"
                                                        : isDayMode
                                            ...
                                            <div className="min-w-0 md:flex md:items-baseline md:gap-3">
                                                <h3
                                                    className={`text-xl font-black ${isDayMode ? "text-slate-950" : "text-white"}`}
                                                >
                                                    {levelTitle}
                                                </h3>
                                                <p
                                                    className={`mt-1 text-sm leading-6 md:mt-0 ${isDayMode ? "text-slate-600" : "text-gray-400"}`}
                                                >
                                        ...
                                        <span
                                            className={`inline-flex shrink-0 items-center justify-center pt-1 text-xs font-bold md:pt-0 ${
                                                isDayMode ? "text-lime-800" : "text-lime-200"
                                            }`}
                                        >
                                            {t("community_learning.level_count", {
                                    ...
                                    <div className="space-y-4">
                                        {firstItem ? (
                                            ...
                                            <div
                                                className={`border-t border-dashed py-4 text-sm md:border-0 md:py-0 ${
                                                    isDayMode
                                        ...
                                        {moreItems.length > 0 ? (
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between gap-3">
                                                    <h4
                                                        className={`text-sm font-black ${isDayMode ? "text-slate-700" : "text-gray-200"}`}
                                                    >
                                                        {t(
                                                            "community_learning.more_level_articles",
                                                            {
                                                                level: levelTitle,
                        ...
                        <div
                            className={`border-y py-14 text-center md:py-16 ${isDayMode ? "border-slate-300/80 text-slate-500" : "border-white/10 text-gray-400"}`}
                        >
                            <Search className="mx-auto mb-3 h-9 w-9 opacity-45" />
                            <p className="font-bold">
                                {t("community_learning.empty_title", "本章暂时没有匹配的学习资料")}
                            </p>
                            <p className="mt-1 text-sm">
                                {t(
                                    "community_learning.empty_desc",
                                    "已自动隐藏求助、组队、新闻和无关内容，只保留教程、笔记和学习资源。"
                                )}
                            </p>
                        </div>
                    )}
            ...
            {selectedArticle ? (
                <CommunityDetailModal
                    ...
                    headerContent={
                        <>
                            <div
                                className={`mb-3 text-xs font-black uppercase tracking-[0.2em] ${isDayMode ? chapterTone.accent : chapterTone.nightAccent}`}
                            >
                                {t(
                                    CHAPTERS.find(
                                        (chapter) => chapter.key === selectedArticle.chapterKey
                                    )?.titleKey,
                                    "AI 学习"
                                )}
                            </div>
                            <h2
                                className={`text-2xl font-black leading-tight md:text-5xl ${isDayMode ? "text-slate-950" : "text-white drop-shadow-2xl"}`}
                            >
...
const CommunityPosts = () => {
    ...
    return (
        <div className="space-y-4 md:space-y-7">
            <nav
                className={`-mx-3 grid grid-cols-3 border-b px-3 md:mx-0 md:flex md:px-0 ${
                    isDayMode ? "border-slate-200" : "border-white/10"
                }`}
                ...
                {AREAS.map((area) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex min-h-12 min-w-0 items-center justify-center gap-1.5 border-b-2 px-1.5 text-center text-xs font-bold transition-colors md:min-h-14 md:flex-none md:justify-start md:gap-2 md:px-5 md:py-2 md:text-left ${
                                active
                                    ? isDayMode
                                        ? "border-lime-700 text-slate-950"
                                        : "border-lime-200 text-white"
                        ...
                        >
                            <div className="flex items-center justify-between gap-3">
                                <span
                                    className={`flex h-6 w-6 items-center justify-center md:h-7 md:w-7 ${active ? (isDayMode ? "text-lime-800" : "text-lime-200") : "opacity-75"}`}
                                >
