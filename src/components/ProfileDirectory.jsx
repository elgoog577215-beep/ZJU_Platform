    ...
    return (
        ...
        >
            <SEO
                ...
                description={t("profiles.directory_desc", "浏览社团、学校、企业和组织主页")}
            />
            <main className="mx-auto max-w-6xl">
                <div className="mb-4 flex flex-col gap-3 md:mb-6 md:flex-row md:items-end md:justify-between md:gap-4">
                    <div>
                        <p
                            className={`text-[11px] font-black uppercase tracking-[0.18em] md:text-xs md:tracking-[0.22em] ${isDayMode ? "text-indigo-700" : "text-indigo-300"}`}
                        >
                            Profiles
                        </p>
                    ...
                    <div className="flex w-full flex-col gap-2 sm:flex-row md:w-auto">
                        <div className="relative min-w-0 flex-1 md:w-72">
                            <Search
                                className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 ${isDayMode ? "text-slate-400" : "text-white/40"}`}
                                size={16}
                            />
                            <input
                                value={query}
                ...
                {loading ? (
                    <div
                        className={`rounded-[6px] border p-5 text-center text-sm md:p-8 ${isDayMode ? "border-slate-200 bg-white text-slate-500" : "border-white/10 bg-white/[0.03] text-white/50"}`}
                    >
                        {t("common.loading")}
                    </div>
                ) : profiles.length === 0 ? (
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start gap-2.5 md:gap-3">
                                        <div
                                            className={`flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-[6px] border md:h-12 md:w-12 ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-white/5"}`}
                                        >
                                            {mark ? (
                                                <img
                                                    src={mark}
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p
                                                className={`mt-1 text-xs ${isDayMode ? "text-slate-500" : "text-white/45"}`}
                                            >
                                                {typeLabelFor(profile.type)} · @{profile.handle}
                                            </p>
                                            {profile.bio || profile.description ? (
                                                <p
                                                    className={`mt-2 line-clamp-2 text-xs leading-5 md:mt-3 md:text-sm md:leading-6 ${isDayMode ? "text-slate-600" : "text-white/65"}`}
                                                >
                                                    {profile.bio || profile.description}
                                                </p>
                                            ) : null}
