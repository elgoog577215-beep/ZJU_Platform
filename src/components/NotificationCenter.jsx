...
const buildNotificationTargetPath = (notification) => {
    ...
    if (notificationType === "new_content") {
        if (!resourceId || !resourceType) {
            console.warn("[Notification] Unknown resource type:", resourceType, "id:", resourceId);
            return null;
        }
...
const NotificationCenter = ({
    ...
    const handleMarkAllRead = async () => {
        try {
            ...
            toast.error(t("notifications.mark_all_read_failed", "全部标记已读失败"));
        }
    };
    ...
    const handleDelete = async (id, e) => {
        ...
        try {
            ...
            const nextUnreadCount =
                target && !isNotificationRead(target) ? Math.max(0, unreadCount - 1) : unreadCount;
    ...
    const openLabel =
        unreadCount > 0
            ? t("notifications.open_with_unread", "打开通知中心，{{count}} 条未读", {
                  count: unreadCount,
              })
        ...
        >
            <div
                className={`flex items-center justify-between p-4 border-b ${
                    isDayMode ? "border-slate-200/80 bg-slate-50/90" : "border-white/10 bg-white/5"
                }`}
            >
                <div>
                    <h3
    ...
    return (
        <div className="relative z-50" ref={dropdownRef}>
            ...
            >
                <motion.div
                    animate={
                        unreadCount > 0 && !reduceMotion ? { rotate: [0, -15, 15, -15, 15, 0] } : {}
                    }
                    transition={{
