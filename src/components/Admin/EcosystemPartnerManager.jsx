...
const isPublicVisible = (partner) => isCollected(partner) && isCorePartner(partner);
...
const EcosystemPartnerManager = () => {
    ...
    const syncProfileAfterPartnerSave = async (partner, partnerPayload) => {
        ...
        return {
            ...
            profile_type:
                response.data?.type || profileTypeForPartnerCategory(partnerPayload.category),
        };
    };
    ...
    const removeProfileMember = async (member) => {
        ...
        try {
            await api.delete(
                `/admin/profiles/${editingPartner.profile_id}/members/${member.user_id}`
            );
            await fetchProfileMembers(editingPartner.profile_id);
    ...
    const handleVisibilityToggle = async (partner) => {
        ...
        try {
            ...
            toast.success(
                nextVisible
                    ? t(
                          "admin.ecosystem_partners.scope.core_enabled_toast",
                          "已标为生态/比赛合作方"
                      )
    ...
    const renderProfileLink = (partner) => {
        ...
        return (
            <button
                type="button"
                onClick={() =>
                    window.open(`/org/${partner.profile_handle}`, "_blank", "noopener,noreferrer")
                }
        ...
        <>
            ...
            >
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
                    ...
                    <AdminMetricCard
                        label={t(
                            "admin.ecosystem_partners.metrics.core_display",
                            "生态/比赛合作方"
                        )}
                        value={stats.visible}
                        icon={Eye}
                            ...
                            <AdminTableShell minWidth={1240}>
                                <thead>
                                    <tr className="theme-admin-table-head border-b text-xs uppercase tracking-[0.2em]">
                                        ...
                                        <th className="p-4">
                                            {t(
                                                "admin.ecosystem_partners.scope.table_header",
                                                "展示层级"
                                            )}
                                        </th>
                ...
                <div className="grid max-h-[60dvh] gap-4 overflow-y-auto pr-1">
                    ...
                    <label className={`block text-sm font-semibold ${headingTextClass}`}>
                        ...
                        <div className="mt-2 flex overflow-hidden rounded-xl border border-white/10">
                            <span className={`flex items-center px-3 text-sm ${mutedTextClass}`}>
                                /org/
                            </span>
                            <input
                    ...
                    {editingPartner?.profile_id ? (
                        ...
                        >
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <div>
                                    <h4 className={`text-sm font-bold ${headingTextClass}`}>
                                        主体管理员
                                    </h4>
                                    <p className={`mt-1 text-xs ${mutedTextClass}`}>
                                        owner/admin/editor 可用该主体发布内容
                                    </p>
                                </div>
                                {membersLoading ? (
                                    <span className={`text-xs ${mutedTextClass}`}>加载中...</span>
                                ) : null}
                            </div>
                            ...
                            <div className="mt-3 grid gap-2">
                                {profileMembers.length === 0 ? (
                                    ...
                                    profileMembers.map((member) => (
                                        ...
                                        >
                                            <span
                                                className={`min-w-0 truncate text-sm ${headingTextClass}`}
                                            >
                                                #{member.user_id}{" "}
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <label className={`block text-sm font-semibold ${headingTextClass}`}>
                            ...
                            <input
                                ...
                                onChange={(event) =>
                                    updateForm("dark_logo_url", event.target.value)
                                }
                    ...
                    <div className="grid gap-2">
                        ...
                        >
                            {isCollected(form)
                                ...
                                : t(
                                      "admin.ecosystem_partners.scope.not_collected",
                                      "不参与活动聚合"
                                  )}
                        ...
                        >
                            {isPublicVisible(form)
                                ? t(
                                      "admin.ecosystem_partners.scope.core_display_full",
                                      "生态/比赛合作方"
                                  )
                                : t(
                                      "admin.ecosystem_partners.scope.not_core_display",
                                      "活动提供方"
                                  )}
