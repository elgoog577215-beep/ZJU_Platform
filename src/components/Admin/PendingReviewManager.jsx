...
const PendingReviewManager = () => {
    ...
    const submitReview = async () => {
        ...
        try {
            await Promise.all(
                confirmState.items.map((item) => reviewPendingItem(item, status, reason))
            );
            ...
            setSelected((previous) => previous.filter((key) => !acceptedKeys.has(key)));
            toast.success(
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-2 text-xs text-gray-500">
                                                ...
                                                {item.competition_title ? (
                                                    <span>比赛: {item.competition_title}</span>
                                                ) : null}
                                                {item.type !== "events" && item.tags ? (
