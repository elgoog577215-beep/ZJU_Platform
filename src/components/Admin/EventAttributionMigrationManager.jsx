...
const EventAttributionMigrationManager = () => {
    ...
    const organizationProfiles = useMemo(
        () =>
            profiles.filter(
                (profile) => ORG_TYPES.has(profile.type) && profile.status !== "archived"
            ),
        [profiles]
    );
    ...
    const visibleCandidates = useMemo(
        () =>
            candidates.filter(
                (candidate) => levelFilter === "all" || candidate.match_level === levelFilter
            ),
    ...
    useEffect(() => {
        ...
        api.get("/admin/profiles")
            ...
            .catch((error) => {
                if (!active) return;
                toast.error(
                    error.response?.data?.error || t("admin.attribution.load_profiles_failed")
                );
            })
            .finally(() => {
    ...
    const applySelected = async () => {
        ...
        const confirmed = window.confirm(
            t("admin.attribution.apply_confirm", {
                ...
                defaultValue:
                    "Apply attribution for {{count}} selected events? Original uploader and creation time will not be overwritten.",
            })
        );
        if (!confirmed) return;
    ...
    const badgeClass = (level) => {
        ...
        if (tone === "amber")
            return isDayMode ? "bg-amber-50 text-amber-700" : "bg-amber-500/10 text-amber-200";
        if (tone === "rose")
            return isDayMode ? "bg-rose-50 text-rose-700" : "bg-rose-500/10 text-rose-200";
    ...
    return (
        ...
        >
            <AdminInlineNote tone="warning">{t("admin.attribution.safety_note")}</AdminInlineNote>

            <div className="grid gap-3 md:grid-cols-4">
                <AdminMetricCard
                    label={t("admin.attribution.metrics.total")}
                    value={summary.total || 0}
                    icon={GitBranch}
                />
                <AdminMetricCard
                    label={t("admin.attribution.metrics.strong")}
                    value={summary.strong || 0}
                    icon={CheckCircle2}
                    tone="emerald"
                />
                <AdminMetricCard
                    label={t("admin.attribution.metrics.medium")}
                    value={summary.medium || 0}
                    icon={ShieldCheck}
                    tone="amber"
                />
                <AdminMetricCard
                    label={t("admin.attribution.metrics.conflict")}
                ...
                action={
                    <ToolbarGroup>
                        {["all", ...LEVELS].map((level) => (
                            ...
                            >
                                {level === "all"
                                    ? t("admin.attribution.levels.all")
                                    : chipLabel(level)}
                            </FilterChip>
                        ))}
                    </ToolbarGroup>
            ...
            >
                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <ToolbarGroup>
                        ...
                        >
                            {applying
                                ? t("admin.attribution.applying")
                                : t("admin.attribution.apply_selected")}
                        </AdminButton>
                    </ToolbarGroup>
                </div>
                ...
                {previewLoading ? (
                    ...
                    <AdminEmptyState
                        ...
                        title={
                            preview
                                ? t("admin.attribution.empty_title")
                                : t("admin.attribution.initial_title")
                        }
                        description={
                            preview
                                ? t("admin.attribution.empty_desc")
                                : t("admin.attribution.initial_desc")
                        }
                    />
                ) : (
                    <>
                            ...
                            <thead>
                                <tr>
                                    <th className="w-12 p-4">
                                        {t("admin.attribution.table.select")}
                                    </th>
                                    <th className="p-4">{t("admin.attribution.table.event")}</th>
                                    <th className="p-4">
                                        {t("admin.attribution.table.organizer")}
                                    </th>
                            ...
                            <tbody>
                                {visibleCandidates.map((candidate) => {
                                    ...
                                    return (
                                        <tr
                                            key={key}
                                            className="border-t border-[rgba(128,146,167,0.14)]"
                                        >
                                            <td className="p-4">
                                                <input
                                                    type="checkbox"
                                                    ...
                                                    aria-label={t(
                                                        "admin.attribution.select_candidate"
                                                    )}
                                                />
                                            </td>
                                            <td className="p-4">
                                            ...
                                            <td className="p-4">
                                                <AdminTableCellText>
                                                    {candidate.event?.organizer ||
                                                        t("admin.attribution.no_organizer")}
                                                </AdminTableCellText>
                                            </td>
                                            <td className="p-4">
                                                <span
                                                    className={`inline-flex rounded-[5px] px-2 py-1 text-xs font-semibold ${badgeClass(candidate.match_level)}`}
                                                >
                                            ...
                                            <td className="p-4">
                                                <div
                                                    className={`max-w-sm text-xs leading-5 ${subtleTextClass}`}
                                                >
                                                    {candidate.evidence}
                                                </div>
                                                <div
                                                    className={`mt-1 text-[11px] uppercase tracking-[0.12em] ${mutedTextClass}`}
                                                >
                                            ...
                                            <td className="p-4">
                                                <div
                                                    className={`text-xs leading-5 ${mutedTextClass}`}
                                                >
                                                    {t("admin.attribution.current_publisher", {
                                                        id:
                                                            candidate.event?.publisher_profile_id ||
                                                            "-",
                                                    ...
                                                    {t("admin.attribution.current_organizer", {
                                                        id:
                                                            candidate.event?.organizer_profile_id ||
                                                            "-",
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <span
                                                className={`shrink-0 rounded-[5px] px-2 py-1 text-xs font-semibold ${badgeClass(candidate.match_level)}`}
                                            >
                                                {chipLabel(candidate.match_level)}
                                            </span>
                                        </div>
