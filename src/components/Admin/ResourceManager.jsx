...
const ResourceManager = ({ title, apiEndpoint, type, icon: Icon }) => {
  ...
  const fetchItems = async (page = 1) => {
    ...
    try {
      ...
      const response = await api.get(`/${apiEndpoint}?page=${page}&limit=10&search=${search}&status=all&sort=${sort}&_t=${timestamp}`);
  ...
  const eventSortOptions = useMemo(() => ([
    { value: 'views', label: t('admin.resource_manager_ui.sort_views', '按访问量') },
    { value: 'registrations', label: t('admin.resource_manager_ui.sort_registrations', '按报名量') },
  ...
  const eventStats = useMemo(() => {
    ...
    return items.reduce((accumulator, item) => ({
      ...
      totalRegistrations: accumulator.totalRegistrations + Number(item.registration_count || 0),
      upcoming: accumulator.upcoming + (item.date && new Date(item.date) >= new Date(new Date().toDateString()) ? 1 : 0)
    }), {
      totalViews: 0,
  ...
  const tableColumnCount = 4 + (type === 'image' ? 1 : 0) + (type === 'audio' ? 1 : 0) + (isEventResource ? 4 : 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 bg-[#111] p-4 rounded-2xl border border-white/10">
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
              ...
              {isEventResource ? <p className="text-sm text-gray-400 mt-1">{t('admin.resource_manager_ui.event_manager_desc', '后台统一查看活动热度、报名和审核状态。')}</p> : null}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 flex-1">
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs uppercase tracking-[0.22em] text-gray-400">{t('admin.resource_manager_ui.views', '访问量')}</span>
                  <Eye size={16} className="text-indigo-400" />
                </div>
                <div className="mt-3 text-2xl font-bold text-white">{formatNumber(eventStats?.totalViews)}</div>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs uppercase tracking-[0.22em] text-gray-400">{t('admin.resource_manager_ui.registrations', '报名量')}</span>
                  <Users size={16} className="text-emerald-400" />
                </div>
                <div className="mt-3 text-2xl font-bold text-white">{formatNumber(eventStats?.totalRegistrations)}</div>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs uppercase tracking-[0.22em] text-gray-400">{t('admin.resource_manager_ui.upcoming_events', '待开始')}</span>
                  <CalendarDays size={16} className="text-amber-400" />
                </div>
                <div className="mt-3 text-2xl font-bold text-white">{formatNumber(eventStats?.upcoming)}</div>
              </div>
            </div>
      ...
      <div className="bg-[#111] rounded-2xl border border-white/10 overflow-hidden">
        {isEventResource ? (
          <div className="md:hidden divide-y divide-white/5">
            {loading ? (
              <div className="p-8 text-center text-gray-500">{t('admin.resource_manager_ui.loading')}</div>
            ) : items.length === 0 ? (
              <div className="p-8 text-center text-gray-500">{t('admin.no_items')}</div>
            ) : (
              items.map(item => (
                <div key={item.id} className="p-4 space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className={`shrink-0 px-2.5 py-1 rounded-full border text-xs font-semibold ${getStatusClassName(item.status)}`}>
                      {item.status || '—'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl bg-white/5 border border-white/5 p-3">
                      <div className="text-xs text-gray-400 mb-1">{t('admin.resource_manager_ui.event_date', '活动时间')}</div>
                      <div className="text-sm text-white">{formatDate(item.date)}</div>
                    </div>
                    <div className="rounded-xl bg-white/5 border border-white/5 p-3">
                      <div className="text-xs text-gray-400 mb-1">{t('admin.resource_manager_ui.views', '访问量')}</div>
                      <div className="text-sm text-white">{formatNumber(item.views)}</div>
                    </div>
                    <div className="rounded-xl bg-white/5 border border-white/5 p-3">
                      <div className="text-xs text-gray-400 mb-1">{t('admin.resource_manager_ui.registrations', '报名量')}</div>
                      <div className="text-sm text-white">{formatNumber(item.registration_count)}</div>
                    </div>
                    <div className="rounded-xl bg-white/5 border border-white/5 p-3">
                      <div className="text-xs text-gray-400 mb-1">{t('admin.fields.tags')}</div>
                      <div className="text-sm text-white line-clamp-2">{item.tags || '—'}</div>
                    </div>
                  </div>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full text-left border-collapse ${isEventResource ? 'hidden md:table' : ''}`}>
            <thead>
              <tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
                <th className="p-4">{t('admin.id')}</th>
                <th className="p-4">{t('admin.fields.title')}</th>
                {isEventResource ? <th className="p-4">{t('admin.resource_manager_ui.event_date', '活动时间')}</th> : null}
                {isEventResource ? <th className="p-4">{t('admin.resource_manager_ui.review_status', '审核状态')}</th> : null}
                {isEventResource ? <th className="p-4">{t('admin.resource_manager_ui.views', '访问量')}</th> : null}
                {isEventResource ? <th className="p-4">{t('admin.resource_manager_ui.registrations', '报名量')}</th> : null}
                {type === 'image' ? <th className="p-4">{t('admin.fields.preview')}</th> : null}
                {type === 'audio' ? <th className="p-4">{t('admin.fields.artist')}</th> : null}
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={tableColumnCount} className="p-8 text-center text-gray-500">
                ...
                items.map(item => (
                  <tr key={item.id} className="hover:bg-white/5 transition-colors">
                    ...
                    {isEventResource ? <td className="p-4 text-gray-300">{formatDate(item.date)}</td> : null}
                    {isEventResource ? (
                      <td className="p-4">
                        <span className={`inline-flex px-2.5 py-1 rounded-full border text-xs font-semibold ${getStatusClassName(item.status)}`}>
                          {item.status || '—'}
                        </span>
                      </td>
                    ) : null}
                    {isEventResource ? <td className="p-4 text-indigo-300 font-semibold">{formatNumber(item.views)}</td> : null}
                    {isEventResource ? <td className="p-4 text-emerald-300 font-semibold">{formatNumber(item.registration_count)}</td> : null}
                    {type === 'image' ? (
                      <td className="p-4">
                        <img src={item.url} alt={item.title} className="w-12 h-12 object-cover rounded-lg border border-white/10" />
                      </td>
                    ) : null}
                    {type === 'audio' ? <td className="p-4 text-gray-400">{item.artist}</td> : null}
                    <td className="p-4">
