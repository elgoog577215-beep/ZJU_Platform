...
const UserManager = () => {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#111] p-3 md:p-4 rounded-2xl border border-white/10">
        <h3 className="text-lg md:text-xl font-bold text-white flex items-center gap-2">
          <User size={24} className="text-indigo-400" />{" "}
          {t("admin.users_manage")}
        </h3>
        <div className="relative w-full md:w-64">
          <input
            ...
            className="w-full bg-black/50 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 min-h-[44px] text-white focus:outline-none focus:border-indigo-500"
          />
          <Search
      ...
      <div className="md:hidden space-y-3">
        {filteredUsers.length === 0 ? (
          <div className="p-8 text-center text-gray-500 bg-[#111] border border-white/10 rounded-2xl">
            {t("admin.no_items")}
          </div>
        ) : (
          filteredUsers.map((user) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-bold uppercase ${user.role === "admin" ? "bg-purple-500/20 text-purple-400" : "bg-gray-500/20 text-gray-400"}`}
                >
                  {user.role}
                </span>
              </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 border-b border-white/10 text-gray-400 text-xs uppercase tracking-wider">
                <th className="p-3 md:p-4">{t("admin.fields.username")}</th>
                <th className="p-3 md:p-4">{t("admin.fields.role")}</th>
                <th className="p-3 md:p-4">{t("admin.fields.created_at")}</th>
        ...
        {isModalOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-[#1a1a1a] border border-white/10 rounded-t-2xl md:rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 pb-[calc(env(safe-area-inset-bottom)+24px)] md:pb-6"
        ...
        {deleteConfirmation && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="w-full max-w-md bg-[#1a1a1a] border border-white/10 rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden p-6 pb-[calc(env(safe-area-inset-bottom)+24px)] md:pb-6"
            >
