...
const getAccountType = (user) => user?.account_type || (user?.organization_cr ? "organization" : "personal");
const getReviewPermission = (user) => user?.review_permission || (user?.role === "admin" ? "admin" : "normal");
...
const UserManager = () => {
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || t("admin.user_manager_ui.error_load_organizations"));
      setOrganizations([]);
    } finally {
  ...
  const filteredUsers = useMemo(
    () =>
      searchedUsers.filter((user) => {
        const roleMatches = roleFilter === "all" || getUserRoleGroup(user.role) === roleFilter;
        const permissionMatches = permissionFilter === "all" || getReviewPermission(user) === permissionFilter;
  ...
  const filteredOrganizations = useMemo(() => {
    ...
    return organizations.filter((organization) => {
      const searchableText = [
        ...
        ...(organization.members || []).map((member) => `${member.username} ${member.nickname} ${member.user_id}`),
      ]
        .filter(Boolean)
        .map(normalizeSearchText)
        .join(" ");
  ...
  const totalCounts = useMemo(
    () => ({
      ...
      organizationAccounts: users.filter((user) => getAccountType(user) === "organization").length,
      pendingContent: users.reduce((sum, user) => sum + (Number(user.pending_content_count) || 0), 0),
    }),
    [users],
  );
  ...
  const organizationStats = useMemo(
    () => ({
      ...
      core: organizations.filter((item) => item.partner?.partner_scope === "core_partner").length,
      activity: organizations.filter((item) => item.partner?.partner_scope === "activity_provider").length,
  ...
  const roleTabs = [
    { key: "all", label: t("admin.user_manager_ui.filter_all"), icon: Users, total: totalCounts.total },
    { key: "admin", label: t("admin.user_manager_ui.role_admin"), icon: ShieldCheck, total: totalCounts.admin },
    { key: "user", label: t("admin.user_manager_ui.role_user"), icon: User, total: totalCounts.user },
  ];

  const permissionTabs = [
    { key: "all", label: t("admin.user_manager_ui.permission_all"), total: users.length },
    { key: "normal", label: t("admin.user_manager_ui.review_permission.normal"), total: totalCounts.normal },
    { key: "trusted", label: t("admin.user_manager_ui.review_permission.trusted"), total: totalCounts.trusted },
    { key: "admin", label: t("admin.user_manager_ui.review_permission.admin"), total: totalCounts.admin },
  ];
  ...
  const reviewPermissionChanged = Boolean(editingUser) && newReviewPermission !== editingReviewPermission;
  ...
  const hasEditableChange =
    roleChanged || accountTypeChanged || reviewPermissionChanged || adminScopeChanged || passwordChanged;
        ...
        previous.map((user) =>
          user.id === editingUser.id
            ? {
                ...
                review_permission: payload.review_permission || user.review_permission,
  ...
  const badgeClass = (tone) => {
    const map = {
      ...
      emerald: isDayMode ? "border-emerald-500/[0.18] bg-emerald-500/10 text-emerald-700" : "border-emerald-500/20 bg-emerald-500/15 text-emerald-300",
      amber: isDayMode ? "border-amber-500/[0.18] bg-amber-500/10 text-amber-700" : "border-amber-500/20 bg-amber-500/15 text-amber-300",
      slate: isDayMode ? "border-slate-300/70 bg-slate-100 text-slate-600" : "border-white/10 bg-white/5 text-gray-300",
      sky: isDayMode ? "border-sky-500/[0.18] bg-sky-500/10 text-sky-700" : "border-sky-500/20 bg-sky-500/15 text-sky-300",
    };
    return map[tone] || map.slate;
  };

  const renderBadge = (children, tone = "slate", Icon = null) => (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeClass(tone)}`}>
  ...
  const renderRoleBadge = (role) => {
    ...
    return renderBadge(
      roleGroup === "admin" ? t("admin.user_manager_ui.role_admin") : t("admin.user_manager_ui.role_user"),
  ...
  const renderPermissionBadge = (user) => {
    ...
    const tone = permission === "admin" ? "violet" : permission === "trusted" ? "emerald" : "amber";
    return renderBadge(t(`admin.user_manager_ui.review_permission.${permission}`), tone, ShieldCheck);
  };
  ...
  const renderUserIdentity = (user) => (
    <div className="flex min-w-0 items-center gap-3">
      ...
      >
        {user.avatar ? (
          <img src={user.avatar} alt="" className="h-full w-full rounded-md object-cover" loading="lazy" />
        ) : (
      ...
      {filteredUsers.map((user) => (
        ...
        >
          ...
          <div className={`mt-3 grid grid-cols-2 gap-2 text-xs ${mutedTextClass}`}>
            ...
            <span className="text-right">{formatCreatedAt(user.created_at, currentLanguage)}</span>
            <span>{t("admin.user_manager_ui.org_count")}</span>
            <span className="text-right tabular-nums">{user.organization_profile_count || 0}</span>
            <span>{t("admin.user_manager_ui.pending_content")}</span>
            <span className="text-right tabular-nums">{user.pending_content_count || 0}</span>
          </div>
          ...
          <AdminTableShell minWidth={1120}>
            <thead>
              <tr className="theme-admin-table-head border-b text-xs uppercase tracking-[0.16em]">
                ...
                <th className="p-4">{t("admin.user_manager_ui.col_review_permission")}</th>
                ...
                <th className="p-4 text-right">{t("admin.user_manager_ui.col_actions")}</th>
              </tr>
            </thead>
            <tbody className="theme-admin-table-body divide-y">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="theme-admin-row">
                  ...
                  <td className="p-4 tabular-nums">{user.organization_profile_count || 0}</td>
                  <td className="p-4 tabular-nums">{user.pending_content_count || 0}</td>
                  <td className={`p-4 ${mutedTextClass}`}>{formatCreatedAt(user.created_at, currentLanguage)}</td>
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      <AdminIconButton onClick={() => handleEdit(user)} label={t("admin.user_manager_ui.edit_user")}>
                        <Edit2 size={16} />
                      </AdminIconButton>
                      <AdminIconButton onClick={() => setConfirmDeleteUser(user)} label={t("admin.user_manager_ui.delete_user")} tone="danger">
                        <Trash2 size={16} />
                      </AdminIconButton>
                    </div>
                  </td>
  ...
  const renderOrganizationsView = () => (
    <AdminPanel
      title={t("admin.user_manager_ui.organizations_title", { count: filteredOrganizations.length })}
        ...
        <AdminTableShell minWidth={1040}>
          ...
          <tbody className="theme-admin-table-body divide-y">
            {filteredOrganizations.map((organization) => {
              ...
              const scopeTone = scope === "core_partner" ? "emerald" : scope === "activity_provider" ? "sky" : "slate";
              return (
                  ...
                  <td className="p-4">
                    <div className="min-w-0">
                      ...
                      <AdminTableCellText className="mt-1 block max-w-[320px] truncate text-xs">
                        {organization.display_name_en || organization.handle}
                      </AdminTableCellText>
                    </div>
                  </td>
                  <td className="p-4">
                    {renderBadge(t(`admin.user_manager_ui.partner_scope.${scope}`), scopeTone, Building2)}
                  </td>
                  <td className="p-4">
                    <div className="space-y-1">
                      <AdminTableCellText>{organization.member_count || 0}</AdminTableCellText>
                      {(organization.members || []).slice(0, 2).map((member) => (
                        <div key={`${organization.id}-${member.user_id}`} className={`text-xs ${mutedTextClass}`}>
                          #{member.user_id} {member.nickname || member.username} · {member.member_role}
                        </div>
                      ))}
                    </div>
                  </td>
                  <td className="p-4 tabular-nums">{organization.event_count || 0}</td>
                  <td className="p-4 tabular-nums">{organization.content_summary?.total || 0}</td>
                  <td className="p-4">
                    <a
                      href={organization.handle ? `/org/${organization.handle}` : undefined}
                      target="_blank"
                      rel="noreferrer"
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {permissionRows.map(({ permission, users: permissionUsers }) => (
          <AdminPanel
            ...
            description={t(`admin.user_manager_ui.review_permission_desc.${permission}`)}
          >
            <div className={`mb-3 text-3xl font-black ${headingTextClass}`}>{permissionUsers.length}</div>
              ...
              {permissionUsers.slice(0, 8).map((user) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between gap-3 rounded-[8px] border px-3 py-2 text-left ${
                    isDayMode ? "border-slate-200 bg-white hover:border-indigo-200" : "border-white/10 bg-white/5 hover:border-white/20"
                  }`}
                >
                  <span className={`min-w-0 truncate text-sm font-semibold ${headingTextClass}`}>
                    {getDisplayName(user, t("admin.user_manager_ui.unnamed_user"))}
                  </span>
                  <span className={`shrink-0 text-xs ${mutedTextClass}`}>#{user.id}</span>
                </button>
              ))}
              {permissionUsers.length === 0 ? (
                <p className={`text-sm ${mutedTextClass}`}>{t("admin.user_manager_ui.permission_empty")}</p>
              ) : null}
            </div>
          </AdminPanel>
        ))}
  ...
  return (
    <>
      <AdminPageShell
        ...
        actions={
          <AdminButton tone="subtle" onClick={refreshAll} disabled={refreshing || organizationsLoading}>
            <RefreshCw size={16} className={refreshing || organizationsLoading ? "animate-spin" : ""} />
        ...
        toolbar={
          <AdminToolbar>
            ...
            <ToolbarGroup className="w-full xl:w-auto">
              {VIEW_KEYS.map((key) => (
                <FilterChip key={key} active={activeView === key} onClick={() => setActiveView(key)}>
      ...
      >
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
          <AdminMetricCard label={t("admin.user_manager_ui.metric_total")} value={totalCounts.total} icon={Users} />
          <AdminMetricCard label={t("admin.user_manager_ui.metric_admin")} value={totalCounts.admin} icon={ShieldCheck} tone="violet" />
          <AdminMetricCard label={t("admin.user_manager_ui.metric_trusted")} value={totalCounts.trusted} icon={ShieldCheck} tone="emerald" />
          <AdminMetricCard label={t("admin.user_manager_ui.metric_org_accounts")} value={totalCounts.organizationAccounts} icon={Building2} tone="indigo" />
          <AdminMetricCard label={t("admin.user_manager_ui.metric_organizations")} value={organizationStats.total} icon={Building2} tone="amber" />
          <AdminMetricCard label={t("admin.user_manager_ui.metric_pending")} value={totalCounts.pendingContent} icon={AlertTriangle} tone="violet" />
        </div>
            ...
            <div className="flex flex-col gap-3">
              <ToolbarGroup>
                {roleTabs.map(({ key, label, icon: Icon, total }) => (
                  <FilterChip key={key} active={roleFilter === key} onClick={() => setRoleFilter(key)}>
              ...
              <ToolbarGroup>
                {permissionTabs.map(({ key, label, total }) => (
                  <FilterChip key={key} active={permissionFilter === key} onClick={() => setPermissionFilter(key)}>
                    {label}
                    <span className="ml-2 text-xs tabular-nums">{total}</span>
                  </FilterChip>
                ))}
      ...
      >
        <div className="space-y-4">
          <div className={`rounded-[8px] border p-3 ${isDayMode ? "border-slate-200 bg-slate-50" : "border-white/10 bg-white/[0.03]"}`}>
            <div className="flex items-start justify-between gap-3">
            ...
            <p className={`mt-3 text-xs leading-5 ${mutedTextClass}`}>
              {t("admin.user_manager_ui.edit_target_note", {
                username: getDisplayName(editingUser, t("admin.user_manager_ui.unnamed_user")),
              })}
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <label htmlFor={EDIT_ROLE_INPUT_ID} className={`block text-sm font-medium ${subtleTextClass}`}>
            ...
            <label htmlFor={EDIT_ACCOUNT_TYPE_INPUT_ID} className={`block text-sm font-medium ${subtleTextClass}`}>
            ...
            <label htmlFor={EDIT_REVIEW_PERMISSION_INPUT_ID} className={`block text-sm font-medium ${subtleTextClass}`}>
            ...
            <label htmlFor={EDIT_ADMIN_SCOPE_INPUT_ID} className={`block text-sm font-medium ${subtleTextClass}`}>
          ...
          <div>
            <label htmlFor={EDIT_PASSWORD_INPUT_ID} className={`mb-2 flex items-center gap-2 text-sm font-medium ${subtleTextClass}`}>
              <LockKeyhole size={14} />
            ...
            />
            <p className={`mt-2 flex items-center gap-1.5 text-xs ${passwordTooShort ? (isDayMode ? "text-rose-700" : "text-rose-300") : mutedTextClass}`}>
              {passwordTooShort ? <AlertTriangle size={13} /> : <Key size={13} />}
              {passwordTooShort ? t("admin.user_manager_ui.password_too_short") : t("admin.user_manager_ui.password_hint")}
            </p>
          </div>

          <AdminInlineNote tone="warning">
            <span className="font-semibold">{t("admin.user_manager_ui.permission_scope_title")}</span>
            <span className="mt-1 block">{t("admin.user_manager_ui.permission_scope_desc")}</span>
          </AdminInlineNote>

          {!hasEditableChange ? (
            <p className={`text-xs ${mutedTextClass}`}>{t("admin.user_manager_ui.no_changes")}</p>
          ) : null}
        </div>
      </ConfirmDialog>
      ...
      <ConfirmDialog
        ...
        description={t("admin.user_manager_ui.delete_desc", {
          username: getDisplayName(confirmDeleteUser, t("admin.user_manager_ui.unnamed_user")),
        })}
