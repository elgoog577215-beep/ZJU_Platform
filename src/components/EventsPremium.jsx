        ...
        <div className="p-3 md:p-5 relative flex-1 flex flex-col min-w-0 h-full justify-center md:justify-start">
          ...
          {event.match_reasons && event.match_reasons.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2.5 md:mb-3">
              {event.match_reasons.map((reason, rIdx) => (
                <span
                  key={rIdx}
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] md:text-xs font-bold leading-none border uppercase tracking-wider ${isDayMode ? "bg-amber-50 text-amber-700 border-amber-200/80 shadow-[0_1px_2px_rgba(245,158,11,0.08)]" : "bg-amber-500/10 text-amber-300 border-amber-500/20 shadow-[inset_0_1px_0_rgba(245,158,11,0.1)]"}`}
                >
...
const EventsPremium = () => {
  ...
  useEffect(() => {
    ...
    api.post("/events/search", { query: debouncedSearch })
      .then((res) => {
      ...
      .catch((err) => {
        console.error("Local SQL search failed:", err);
        setSearchResults([]);
        setRelaxedMatch(false);
      })
      .finally(() => {
  ...
  const activeEvents = useMemo(() => {
    return debouncedSearch.trim() ? searchResults : displayEvents;
  }, [debouncedSearch, searchResults, displayEvents]);
...
END:VCALENDAR`;
  ...
  const handleToggleFavorite = useCallback(
    ...
    [setEvents, setSelectedEvent, setDisplayEvents, setSearchResults],
  );
  ...
  const handleOpenAssistantEvent = useCallback(
    (assistantEvent) => {
      ...
      const cachedEvent =
        activeEvents.find((event) => event.id === assistantEvent.id) ||
  ...
  return (
    <section className="pt-[calc(env(safe-area-inset-top)+76px)] pb-[calc(env(safe-area-inset-bottom)+96px)] md:pb-20 md:pt-24 px-4 md:px-8 relative overflow-hidden flex-grow">
      ...
      {error ? (
      ...
      ) : (loading || searchLoading) && activeEvents.length === 0 ? (
        ...
        <>
          {relaxedMatch && activeEvents.length > 0 && (
            <div className="flex items-center gap-3 p-4 mb-6 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 text-sm max-w-7xl mx-auto animate-fadeIn justify-center font-bold relative z-10">
              <AlertCircle size={18} className="shrink-0 animate-pulse text-amber-400" />
              <span>未找到完全匹配，已为你放宽时间条件</span>
            </div>
          )}
