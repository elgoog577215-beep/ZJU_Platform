...
const CommunityPostDetail = ({
    ...
    const sortedComments = useMemo(() => {
        ...
        const best = comments.find(
            (c) =>
                c.id === bestAnswerId ||
                (Array.isArray(c.replies) && c.replies.some((r) => r.id === bestAnswerId))
        );
            ...
            return api
                ...
                .then((res) => {
                    ...
                    const total = loaded.reduce(
                        (sum, floor) =>
                            sum + 1 + (Array.isArray(floor.replies) ? floor.replies.length : 0),
                        0
                    );
    ...
    const handleReport = useCallback(
        async ({ targetType, targetId = null }) => {
            if (!post?.id) return;
            if (!user) {
                toast.error(t("auth.signin_required"));
                return;
    ...
    const openReply = useCallback(
        (floor, target, asQuote = false) => {
            const safeText = String(target?.content || "")
                .replace(/\s+/g, " ")
                .trim()
                .slice(0, 120);
            setReplyContext({
    ...
    const renderCommentText = useCallback(
        (commentId, content, size = "base") => {
            ...
            return (
                <div>
                    <p
                        className={`${size === "small" ? "text-xs" : "text-sm"} leading-relaxed ${th.textContent} whitespace-pre-wrap break-words`}
                    >
                    ...
                    {shouldCollapse && (
                        ...
                        >
                            {expanded
                                ? t("community.collapse_text", "收起")
                                : t("community.expand_text", "展开全文")}
                        </button>
                    )}
                </div>
            );
    ...
    const floorsWithExtraReplies = useMemo(
        () =>
            (sortedComments || [])
                .filter((c) => Array.isArray(c.replies) && c.replies.length > 3)
                .map((c) => c.id),
        [sortedComments]
    );
    ...
    const toggleAllReplies = useCallback(() => {
        ...
        if (hasExpandedFloor) {
            const next = {};
            floorsWithExtraReplies.forEach((id) => {
                next[id] = false;
            });
        ...
        <>
            ...
            {showComments && (
                <div className={`border-t pt-8 ${th.borderSubtle}`}>
                    <h3
                        className={`text-lg font-bold mb-6 flex items-center gap-2 ${th.textPrimary}`}
                    >
                    ...
                    <div className={`flex gap-3 mb-8 p-4 rounded-lg border ${th.commentBg}`}>
                        <div
                            className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${isDayMode ? "bg-slate-200" : "bg-gray-700"}`}
                        >
                        ...
                        <div className="flex-1">
                            {replyContext && (
                                <div
                                    className={`mb-2 rounded-lg border px-3 py-2 flex items-start justify-between gap-3 ${isDayMode ? "bg-amber-50 border-amber-200" : "bg-amber-500/10 border-amber-500/20"}`}
                                >
                                    <div className="min-w-0">
                                        <p
                                            className={`text-xs font-semibold ${isDayMode ? "text-amber-700" : "text-amber-300"}`}
                                        >
                                            {replyContext.mode === "quote"
                                                ? t(
                                                      "community.quote_replying_to",
                                                      "正在引用 {{name}}",
                                                      { name: replyContext.author }
                                                  )
                                                : t("community.replying_to", "正在回复 {{name}}", {
                                                      name: replyContext.author,
                                                  })}
                                        </p>
                                        <p
                                            className={`text-xs truncate ${isDayMode ? "text-amber-600" : "text-amber-200/80"}`}
                                        >
                    ...
                    {loadingComments ? (
                        <div className="space-y-4">
                            {[...Array(3)].map((_, i) => (
                                <div
                                    key={i}
                                    className={`flex gap-3 p-4 rounded-lg animate-pulse ${isDayMode ? "bg-slate-50" : "bg-white/[0.02]"}`}
                                >
                                    <div
                                        className={`w-8 h-8 rounded-full flex-shrink-0 ${isDayMode ? "bg-slate-200" : "bg-white/10"}`}
                                    />
                                    <div className="flex-1 space-y-2">
                                        <div
                                            className={`h-3 w-24 rounded ${isDayMode ? "bg-slate-200" : "bg-white/10"}`}
                                        />
                        ...
                        <div
                            className={`rounded-lg border px-4 py-5 text-center ${isDayMode ? "bg-rose-50 border-rose-200 text-rose-700" : "bg-rose-500/10 border-rose-500/20 text-rose-300"}`}
                        >
                        ...
                        <div className="space-y-3">
                            {sortedComments.map((c) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div
                                            className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden ${isDayMode ? "bg-slate-200" : "bg-gray-700"}`}
                                        >
                                            {c.avatar ? (
                                                <img
                                                    src={c.avatar}
                                                ...
                                                <User size={14} className={th.textSecondary} />
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                <span
                                                    className={`text-sm font-semibold ${th.textPrimary}`}
                                                >
                                                    {c.author ||
                                                        c.author_name ||
                                                        t("common.anonymous", "匿名用户")}
                                                </span>
                                                <span className={`text-xs ${th.textTertiary}`}>
                                                    {c.created_at &&
                                                ...
                                                {isBest && (
                                                    <span
                                                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold ${isDayMode ? "bg-emerald-100 text-emerald-700" : "bg-emerald-500/20 text-emerald-400"}`}
                                                    >
                                                        <Award size={12} />
                                            ...
                                            {c.quote_snapshot && (
                                                <div
                                                    className={`mt-2 rounded-lg border px-2.5 py-2 text-xs break-words ${isDayMode ? "bg-slate-50 border-slate-200 text-slate-600" : "bg-white/[0.02] border-white/10 text-gray-400"}`}
                                                >
                                                    <span className="font-semibold">
                                                        @{c.quote_snapshot.author}:
                                                    </span>{" "}
                                                    {c.quote_snapshot.content}
                                                </div>
                                            )}
                                            ...
                                            {Array.isArray(c.replies) && c.replies.length > 0 && (
                                                <div
                                                    className={`community-reply-thread mt-3 pl-3 border-l space-y-2 ${isDayMode ? "border-slate-200" : "border-white/10"}`}
                                                >
                                                    <div
                                                        className={`text-[11px] ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                                    >
                                                        {t(
                                                            "community.reply_count",
                                                            "共 {{count}} 条二级回复",
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-2 mb-1">
                                                                    <span
                                                                        className={`text-xs font-semibold ${th.textPrimary}`}
                                                                    >
                                                                        {reply.author ||
                                                                            reply.author_name ||
                                                                            t(
                                                                    ...
                                                                    <span
                                                                        className={`text-[11px] ${th.textTertiary}`}
                                                                    >
                                                                        {reply.created_at &&
                                                                            new Date(
                                                                                reply.created_at
                                                                    ...
                                                                    {replyIsBest && (
                                                                        <span
                                                                            className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-semibold ${isDayMode ? "bg-emerald-100 text-emerald-700" : "bg-emerald-500/20 text-emerald-300"}`}
                                                                        >
                                                                            <Award size={10} />
                                                                            {t(
                                                                                "community.best_answer",
                                                                                "最佳答案"
                                                                            )}
                                                                        </span>
                                                                    )}
                                                                ...
                                                                {reply.quote_snapshot && (
                                                                    <div
                                                                        className={`mb-1 rounded-md border px-2 py-1 text-[11px] break-words ${isDayMode ? "bg-slate-50 border-slate-200 text-slate-600" : "bg-white/[0.02] border-white/10 text-gray-400"}`}
                                                                    >
                                                                        <span className="font-semibold">
                                                                            @
                                                                            {
                                                                ...
                                                                {reply.reply_to_author && (
                                                                    <div
                                                                        className={`mb-1 text-[11px] ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                                                    >
                                                                        {t(
                                                                            "community.reply_to",
                                                                            "回复"
                                                    ...
                                                    {c.replies.length > 3 && (
                                                        ...
                                                        >
                                                            {expandedFloors[c.id]
                                                                ? t(
                                                                      "community.collapse_replies",
                                                                      "收起回复"
                                                                  )
                                                                : t(
                                                                      "community.expand_replies",
                                                                      "展开 {{count}} 条回复",
                                                                      {
                                                                          count:
                                                                              c.replies.length - 3,
