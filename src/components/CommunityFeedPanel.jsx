...
const CommunityFeedPanel = ({
    ...
    ignoreActiveFiltersForEmptyState = false,
    ...
    const showFilteredEmptyState = hasActiveFilters && !ignoreActiveFiltersForEmptyState;
    ...
    const accentBtnClass = {
        amber: isDayMode
            ? "bg-white text-amber-700 border-amber-200 shadow-none"
            : "bg-amber-600 text-white border-amber-600",
        violet: isDayMode
            ? "bg-white text-violet-700 border-violet-200 shadow-none"
            : "bg-violet-600 text-white border-violet-600",
        blue: isDayMode
            ? "bg-white text-sky-700 border-sky-200 shadow-none"
            : "bg-blue-600 text-white border-blue-600",
        orange: isDayMode
            ? "bg-white text-orange-700 border-orange-200 shadow-none"
            : "bg-orange-600 text-white border-orange-600",
        green: isDayMode
            ? "bg-white text-emerald-700 border-emerald-200 shadow-none"
            : "bg-emerald-600 text-white border-emerald-600",
        slate: isDayMode
            ? "bg-white text-slate-700 border-slate-200 shadow-none"
    ...
    const accentTabClass = {
        ...
        blue: isDayMode
            ? "border-sky-500 bg-sky-50/70 text-sky-700"
            : "border-sky-300 bg-sky-400/10 text-sky-100",
        orange: isDayMode
            ? "border-orange-500 bg-orange-50/70 text-orange-700"
            : "border-orange-300 bg-orange-400/10 text-orange-100",
        green: isDayMode
    ...
    const statusControl = statusTabs ? (
        <div
            className={`scrollbar-none flex w-full max-w-full items-center overflow-x-auto sm:w-auto ${
                isLearningSurface
                            ...
                            : `min-h-10 min-w-fit border-b-2 px-3 text-xs font-semibold transition-colors whitespace-nowrap md:px-3 ${
                                  statusFilter === key
                                      ...
                                      : isDayMode
                                        ...
                                        : "border-transparent text-gray-400 hover:bg-white/8 hover:text-white"
                              }`
                    }
                >
                    {t(label)}
            ...
            <div className="space-y-4">
                {isLoading && displayItems.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center px-4 py-12 md:py-20">
                        ...
                        <p
                            className={`mb-5 text-center text-base md:mb-6 md:text-lg ${th.textContent}`}
                        >
                            {t("common.error_fetching_data")}
                        </p>
                        <button
                            onClick={handleRefresh}
                            className={`min-h-11 rounded-lg border px-8 text-sm font-medium transition-all hover:scale-105 active:scale-95 md:py-3 ${th.btnSecondary}`}
                        >
                            {t("common.retry")}
                        </button>
                    </div>
