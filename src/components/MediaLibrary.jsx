...
const getLocalizedMediaCategoryName = (name, isEnglish) => {
  if (!name) return "";
  return isEnglish ? mediaCategoryNameMap[name] || name : name;
};

const PhotoCard = memo(forwardRef(({ photo, index, onClick, onToggleFavorite, canAnimate, isDayMode, t, isEnglish }, ref) => (
  <motion.div
    ref={ref}
        ...
        <div className="flex justify-between items-end gap-2">
          <div className="min-w-0 flex-1">
            <h3 className="text-lg font-bold text-[rgba(255,255,255,0.96)] drop-shadow-[0_2px_10px_rgba(15,23,42,0.45)] line-clamp-2">
              {photo.title || t("media_library.untitled_photo", "未命名照片")}
            </h3>
            {photo.category_name ? (
              <p className="mt-1 text-xs font-medium text-white/72 line-clamp-1">
                {getLocalizedMediaCategoryName(photo.category_name, isEnglish)}
              </p>
            ) : null}
          </div>
...
const VideoCard = memo(({ video, index, onClick, onToggleFavorite, canAnimate, isDayMode, t, isEnglish }) => (
  <motion.div
      ...
      <div className="flex justify-between items-end gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="text-base md:text-lg font-bold text-[rgba(255,255,255,0.96)] drop-shadow-[0_2px_10px_rgba(15,23,42,0.5)] line-clamp-1">
            {video.title || t("media_library.untitled_video", "未命名视频")}
          </h3>
          <p className="mt-1 text-xs font-medium text-white/68 line-clamp-1">
            {getLocalizedMediaCategoryName(video.category_name, isEnglish) || t("media_library.uncategorized", "未分类")}
          </p>
        </div>
...
const LoadMoreButton = ({ onClick, loading, disabled, tone = "indigo", isDayMode, t, children }) => (
  <div className="flex items-center justify-center py-8">
    ...
    >
      {loading ? t("common.loading", "正在加载...") : children}
    </motion.button>
  </div>
);

const MediaCategoryRail = memo(({ categories, activeCategoryId, onChange, isDayMode, t, isEnglish }) => {
        ...
        >
          <div className="scrollbar-none flex min-w-0 items-center gap-1 overflow-x-auto p-1 pr-10 md:pr-1">
            ...
            >
              ...
              <span className="relative z-10 whitespace-nowrap">{t("common.all", "全部")}</span>
            </button>
            ...
            {categories.map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="relative z-10 whitespace-nowrap">{getLocalizedMediaCategoryName(category.name, isEnglish)}</span>
                </button>
              );
            })}
          </div>
...
const MediaLibrary = () => {
  ...
  const activeCategoryName = useMemo(() => {
    ...
    const name = categories.find((category) => String(category.id) === String(categoryId))?.name;
    return getLocalizedMediaCategoryName(name, isEnglish) || t("media_library.current_category", "当前分类");
  }, [categories, categoryId, isEnglish, t]);
                    ...
                    <div className="flex-1">
                      ...
                      <div className={`flex flex-wrap items-center gap-3 text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}>
                        <p className={`rect-chip px-3 py-1 ${isDayMode ? "bg-slate-100 border-slate-200/80 text-slate-600" : "bg-white/5 border-white/5"}`}>
                          {getLocalizedMediaCategoryName(selectedVideo.category_name, isEnglish) || t("media_library.uncategorized", "未分类")}
                        </p>
