                                ...
                                <div className="flex items-center gap-1.5 sm:gap-2">
                                    ...
                                    <div
                                        className={`hidden p-2 rounded-full backdrop-blur-md border sm:block ${
                                            isDayMode
                                                ? "bg-white/72 border-white/40 shadow-[0_12px_24px_rgba(15,23,42,0.18)]"
    ...
    ({
        ...
        ${isDayMode ? "focus-visible:ring-offset-white" : ""}`}
                ...
                {(video.created_at || video.date) &&
                    new Date() - new Date(video.created_at || video.date) <
                        7 * 24 * 60 * 60 * 1000 && (
                ...
                <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
                    <div
                        className={`rect-icon-button flex h-10 w-10 items-center justify-center border group-hover:scale-105 transition-transform duration-300 relative sm:h-14 sm:w-14 ${
                            isDayMode
                                ? "bg-white/84 text-slate-950 border-white/70"
                    ...
                    >
                        <Play
                            ...
                            className={`${isDayMode ? "text-slate-950" : "text-white"} ml-0.5 relative z-10 sm:ml-1 sm:h-[30px] sm:w-[30px]`}
                        />
                    </div>
                </div>
...
const EmptyState = ({ icon: Icon, title, description, accent = "indigo", isDayMode }) => (
    <div className="flex min-h-[178px] flex-col items-center justify-center px-4 py-8 text-center md:min-h-[260px] md:py-16">
        ...
        >
            <Icon
                size={36}
                className={`${isDayMode ? "text-slate-400" : accent === "pink" ? "text-pink-400 opacity-80" : "text-gray-500"} md:h-14 md:w-14`}
            />
        </div>
        <h3
            className={`mb-1.5 text-lg font-bold md:mb-2 md:text-2xl ${isDayMode ? "text-slate-900" : "text-white"}`}
        >
            {title}
        </h3>
        <p
    ...
    <div className="flex items-center justify-center py-5 md:py-8">
        <motion.button
            ...
            className={`rect-button-secondary min-h-[44px] px-6 py-2.5 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                isDayMode ? "hover:text-blue-700" : "text-white hover:border-white/20"
            }`}
        >
...
const MediaCategoryRail = memo(
    ({ categories, activeCategoryId, onChange, isDayMode, allLabel }) => {
...
const MediaLibrary = () => {
    ...
    const activeCategoryName = useMemo(() => {
        ...
        return (
            ...
            t("media_library.current_category", "当前分类")
        );
    ...
    const handlePhotoUpload = async (newItem) => {
        if (Array.isArray(newItem)) {
            for (const item of newItem) {
                await api.post("/photos", {
                    ...item,
                    category_id: item.category_id || categoryParam || null,
                });
            }
        ...
        await api.post("/photos", {
            ...newItem,
            category_id: newItem.category_id || categoryParam || null,
        });
        refreshPhotos({ clearCache: true });
    };
        ...
        (photoId, favorited, likes) => {
            setPhotos((prev) =>
                Array.isArray(prev)
                    ? prev.map((photo) =>
                          photo.id === photoId
                              ? { ...photo, favorited, likes: likes ?? photo.likes }
                              : photo
                      )
                    : prev
            );
    ...
    const handleToggleVideoFavorite = useCallback(
        (videoId, favorited, likes) => {
            setVideos((prev) =>
                Array.isArray(prev)
                    ? prev.map((video) =>
                          video.id === videoId
                              ? { ...video, favorited, likes: likes ?? video.likes }
                              : video
                      )
                    : prev
            );
    ...
    useEffect(() => {
        ...
        api.get(`/photos/${photoDeepLinkId}`, { signal: abortController.signal })
            .then((res) => {
    ...
    useEffect(() => {
        ...
        const abortController = new AbortController();
        api.get(`/videos/${videoDeepLinkId}`, { signal: abortController.signal })
            .then((res) => {
    ...
    const selectedPhoto =
        selectedPhotoIndex !== null ? displayPhotos[selectedPhotoIndex] : externalPhoto;
    ...
    return (
        <section className="day-page-theme day-page-theme-tech pt-[calc(env(safe-area-inset-top)+76px)] pb-6 md:py-16 px-4 md:px-6 relative overflow-x-hidden flex-grow">
            <SEO
                title={t("media_library.title", "影像库")}
                description={t("media_library.meta_desc", "按分类浏览现场照片与视频记录。")}
            />
            ...
            <div className="max-w-[1180px] 2xl:max-w-[1440px] w-full mx-auto relative z-10">
                ...
                {error ? (
                    ...
                    >
                        ...
                        >
                            {t("media_library.content_error", "影像内容加载失败，请稍后重试。")}
                        </p>
                        <button
                            type="button"
                            onClick={() => {
                ...
                <div className="grid gap-6 md:gap-10">
                    <section className="order-1 min-w-0">
                        <div className="mb-3 flex items-center justify-between gap-3 md:mb-4 md:gap-4">
                            <div>
                                <h2
                                    className={`text-lg font-semibold tracking-tight md:text-2xl ${isDayMode ? "text-slate-900" : "text-white"}`}
                                >
                                    {t("media_library.videos_title", "视频记录")}
                                </h2>
                                <p
                                    className={`mt-0.5 text-xs md:mt-1 md:text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                >
                                    {t(
                                        "media_library.videos_count",
                                        "{{visible}} / {{total}} 条视频",
                                        { visible: visibleVideoCount, total: totalVideoCount }
                                    )}
                                </p>
                            </div>
                            <button
                        ...
                        {videosLoading && displayVideos.length === 0 ? (
                                ...
                                {[...Array(4)].map((_, index) => (
                                    <div
                                        ...
                                        className={`rect-media-card relative h-20 animate-pulse overflow-hidden md:aspect-video md:h-auto ${
                                            isDayMode
                                                ? "bg-white/84 border-slate-200/80"
                            ...
                            <EmptyState
                                ...
                                description={t(
                                    "media_library.empty_videos_desc",
                                    "这个分类下还没有视频，上传时选择该分类即可出现在这里。"
                                )}
                                accent="pink"
                                isDayMode={isDayMode}
                            />
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 lg:gap-5">
                                {displayVideos.map((video, index) => (
                                    <VideoCard
                                        ...
                                        isDayMode={isDayMode}
                                        untitledLabel={t(
                                            "media_library.untitled_video",
                                            "未命名视频"
                                        )}
                                        uncategorizedLabel={t(
                                            "media_library.uncategorized",
                                            "未分类"
                                        )}
                    ...
                    <section className="order-2 min-w-0">
                        <div className="mb-3 flex items-center justify-between gap-3 md:mb-4 md:gap-4">
                            <div>
                                ...
                                >
                                    {t(
                                        "media_library.photos_count",
                                        "{{visible}} / {{total}} 张照片",
                                        { visible: visiblePhotoCount, total: totalPhotoCount }
                                    )}
                                </p>
                            </div>
                            <button
                        ...
                        {photosLoading && displayPhotos.length === 0 ? (
                            ...
                            <EmptyState
                                ...
                                description={t(
                                    "media_library.empty_photos_desc",
                                    "这个分类下还没有图片，上传时选择该分类即可出现在这里。"
                                )}
                                isDayMode={isDayMode}
                            />
                        ) : (
                                ...
                                <AnimatePresence mode="popLayout">
                                    {displayPhotos.map((photo, index) => (
                                        <PhotoCard
                                            ...
                                            isDayMode={isDayMode}
                                            untitledLabel={t(
                                                "media_library.untitled_photo",
                                                "未命名照片"
                                            )}
                                            openLabel={t("gallery.open_photo", {
                                                title:
                                                    photo.title ||
                                                    t("media_library.untitled_photo", "未命名照片"),
                                            })}
                                        />
                                    ))}
                                </AnimatePresence>
                        ...
                        {!error && displayPhotos.length > 0 && canLoadMorePhotos ? (
                            <LoadMoreButton
                                onClick={() =>
                                    setPhotoPage((prev) => Math.min(prev + 1, photoTotalPages))
                                }
                                loading={photosLoading}
                ...
                {selectedPhoto ? (
                    <Lightbox
                        ...
                        onLikeToggle={(favorited, likes) =>
                            handleTogglePhotoFavorite(selectedPhoto.id, favorited, likes)
                        }
                        onSelect={(photo) => {
                            const nextIndex = displayPhotos.findIndex(
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex-1">
                                            ...
                                            <div
                                                className={`flex flex-wrap items-center gap-3 text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                                            >
                                                <p
                                                ...
                                                >
                                                    {selectedVideo.category_name ||
                                                        t("media_library.uncategorized", "未分类")}
                                                </p>
                                        ...
                                        <FavoriteButton
                                            ...
                                            className={`rect-icon-button p-3 transition-colors shrink-0 ${
                                                isDayMode
                                                    ? "bg-white/90 hover:bg-slate-50 text-slate-700 border-slate-200/80"
                                                    : "bg-white/5 hover:bg-pink-500/20 border-white/10"
                                            }`}
                                            onToggle={(favorited, likes) =>
            ...
            {isMobileUploadOpen
                ? createPortal(
                      ...
                      >
                          ...
                          >
                              <h3
                                  className={`text-lg font-bold ${isDayMode ? "text-slate-950" : "text-white"}`}
                              >
                              ...
                              <div className="mt-4 grid grid-cols-2 gap-2">
                                  <button
                                      type="button"
                                      onClick={() => chooseUploadType("image")}
                                      className="rect-button-secondary min-h-[48px]"
                                  >
                                      {t("media_library.upload_photos", "上传照片")}
                                  </button>
                                  <button
                                      type="button"
                                      onClick={() => chooseUploadType("video")}
                                      className="rect-button-primary min-h-[48px] text-white"
                                  >
                              ...
                              <button
                                  type="button"
                                  onClick={() => setIsMobileUploadOpen(false)}
                                  className={`mt-3 w-full py-2 text-sm ${isDayMode ? "text-slate-500" : "text-gray-400"}`}
                              >
                                  {t("common.cancel", "取消")}
                              </button>
                          </div>
