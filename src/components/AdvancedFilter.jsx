...
const AdvancedFilter = ({
            ...
            try {
                ...
                const nextOptions = {
                    location: Array.isArray(data.location)
                        ? data.location.map((item) => ({ value: item, label: item }))
                        : [],
                    organizer: Array.isArray(data.organizer)
                        ? data.organizer.map((item) => ({ value: item, label: item }))
                        : [],
                    target_audience: Array.isArray(data.target_audience)
                        ? data.target_audience.map((item) => ({ value: item, label: item }))
                        : [],
                };
    ...
    const attributeFilterConfig = [
        {
            key: "organizer",
            icon: Building2,
            labelKey: "advanced_filter.organizer",
            allLabelKey: "advanced_filter.all_organizers",
            options: options.organizer,
        },
        {
            key: "location",
            icon: MapPin,
            labelKey: "advanced_filter.location",
            allLabelKey: "advanced_filter.all_locations",
            options: options.location,
        },
        {
            key: "target_audience",
            icon: Users,
            labelKey: "advanced_filter.target_audience",
            allLabelKey: "advanced_filter.all_target_audiences",
            options: options.target_audience,
        },
    ];
    ...
    return (
        <div className={`w-full relative z-20 ${className}`}>
            ...
            <div
                className={`relative ${containerClasses} ${isFetching ? "opacity-90 transition-opacity duration-150" : ""}`}
            >
                {!isSheetVariant && (
                    <div className="flex items-center justify-between mb-4 md:mb-6">
                        ...
                        >
                            ...
                            <h3
                                className={`text-lg font-bold tracking-wide ${isDayMode ? "text-slate-900" : "text-white"}`}
                            >
                                {t("advanced_filter.title")}
                            </h3>
                            {activeFilterCount > 0 && (
                                <span
                                    className={`text-xs px-2 py-0.5 rounded-md border ${isDayMode ? "bg-indigo-50 text-indigo-600 border-indigo-200/80" : "bg-indigo-500/15 text-indigo-300 border-indigo-500/30"}`}
                                >
                                    {t("common.selected_count", "已选 {{count}} 项", {
                ...
                <AnimatePresence>
                    {(!isMobile || !isCollapsed || isSheetVariant) && (
                        <motion.div
                            ...
                            className={
                                isMobile && !overflowVisible && !isSheetVariant
                                    ? "overflow-hidden"
                                    : ""
                            }
                        >
                            <div
                                className={`grid ${isSheetVariant ? "grid-cols-1 gap-3" : "grid-cols-2 xl:grid-cols-4 gap-3"} pb-1`}
                            >
                                {attributeFilterConfig.map(
                                    ({
                                        key,
                                        icon,
                                        labelKey,
