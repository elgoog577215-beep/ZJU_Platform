...
const CommunityTeam = ({ onNewPost, hideNewPostButton = false }) => {
    ...
    const handleJoin = async () => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.error || t("community.team_join_failed", "加入失败")
            );
        }
    };
    ...
    const renderDetail = () => (
        <CommunityPostDetail
            ...
            beforeContent={
                feed.selectedItem && (
                    <div
                        className={`mb-6 rounded-lg border p-4 ${isDayMode ? "border-violet-100 bg-violet-50/70" : "border-violet-500/20 bg-violet-500/10"}`}
                    >
                        <div
                            className={`flex flex-wrap items-center gap-4 text-sm ${isDayMode ? "text-slate-700" : "text-violet-100"}`}
                        >
                            {feed.selectedItem.deadline ? (
                            ...
                            {feed.selectedItem.max_members ? (
                                <span className="inline-flex items-center gap-1.5">
                                    <Users size={15} />
                                    {feed.selectedItem.current_members || 0}/
                                    {feed.selectedItem.max_members}
                                </span>
                            ) : null}
                        </div>
                        <button
                            type="button"
                            onClick={handleJoin}
                            className={`mt-3 rounded-lg px-4 py-2 text-sm font-semibold ${isDayMode ? "bg-violet-600 text-white hover:bg-violet-700" : "bg-violet-500 text-white hover:bg-violet-400"}`}
                        >
                ...
                feed.selectedItem && (
                    <>
                        <div className="mb-3 flex items-center gap-3">
                            <span
                                className={`inline-flex items-center rounded-md border px-3 py-1 text-xs font-semibold ${isDayMode ? "border-violet-200 bg-violet-50 text-violet-700" : "border-violet-500/20 bg-violet-500/15 text-violet-300"}`}
                            >
