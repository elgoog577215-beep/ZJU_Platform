...
const BOARD_OPTIONS = [
    {
        key: "all",
        labelKey: "user_profile.submissions.board_all",
        fallback: "全部版面",
        icon: FileText,
    },
...
const normalizeSubmission = (board, item) => ({
    ...
    businessStatus:
        board === "help" || board === "team" || board === "materials" ? item.status : "",
...
const UserCommunitySubmissions = ({ userId, isDayMode }) => {
            ...
            try {
                ...
                const results = await Promise.all(
                    boards.map(async (board) => {
                        ...
                        return normalizeListPayload(response.data).map((item) =>
                            normalizeSubmission(board, item)
                        );
                    })
                );
                setItems(
    ...
    const deleteItem = async (item) => {
        if (!window.confirm(t("user_profile.submissions.delete_confirm", "确认删除这条投稿吗？")))
            return;
        ...
        try {
            ...
            toast.error(
                err?.response?.data?.error ||
                    t("user_profile.submissions.delete_failed", "删除失败")
            );
        } finally {
            setActionId("");
        }
    ...
    const openInCommunity = (item) => {
        ...
        navigate(config.path(item.id), {
            state: { fromUserProfile: { userId, scrollY: 0, contentTab: item.board } },
        });
    };
        ...
        <div className="space-y-4">
            <div className={`rounded-2xl border p-4 md:p-5 ${panelClass}`}>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 className="text-xl font-bold">
                            {t("user_profile.submissions.title", "我的投稿")}
                        </h3>
                    </div>
                    <button
                        type="button"
                ...
                <div className="mt-4 grid gap-3">
                    <div
                        className={`scrollbar-none flex gap-1 overflow-x-auto rounded-xl border p-1 ${controlShell}`}
                    >
                    ...
                    <div className="grid gap-3 lg:grid-cols-[1fr_260px]">
                        ...
                        <div
                            className={`flex min-h-[40px] items-center gap-2 rounded-xl border px-3 ${isDayMode ? "border-slate-200 bg-white" : "border-white/10 bg-white/5"}`}
                        >
                            ...
                            <input
                                ...
                                placeholder={t(
                                    "user_profile.submissions.search_placeholder",
                                    "搜索投稿"
                                )}
            ...
            <div className="space-y-3">
                {loading ? (
                    <div
                        className={`flex items-center justify-center rounded-2xl border py-14 ${panelClass}`}
                    >
                    ...
                    <div
                        className={`rounded-2xl border border-dashed py-14 text-center ${isDayMode ? "border-slate-200 bg-white/70 text-slate-500" : "border-white/10 bg-white/[0.03] text-gray-400"}`}
                    >
                        <FileText className="mx-auto mb-3 h-10 w-10 opacity-30" />
                        <p className="font-bold">
                            {t("user_profile.submissions.empty_title", "暂无投稿")}
                        </p>
                    </div>
                ) : (
                            ...
                            >
                                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div className="min-w-0 flex-1">
                                        <div
                                            className={`mb-2 flex flex-wrap items-center gap-2 text-xs font-bold ${subtleText}`}
                                        >
                                            ...
                                            {item.sortDate ? (
                                                <span>
                                                    {formatDate(item.sortDate, i18n.language)}
                                                </span>
                                            ) : null}
                                        </div>
                                        <h4 className="line-clamp-2 text-lg font-bold">
                                            {item.title ||
                                                t(
                                                    "user_profile.submissions.untitled",
                                                    "未命名投稿"
                                                )}
                                        </h4>
                                        {item.excerpt || item.content ? (
                                            <p
                                                className={`mt-2 line-clamp-2 text-sm leading-6 ${subtleText}`}
                                            >
                                                {item.excerpt || item.content}
                                            </p>
                                        ) : null}
                                        ...
                                        item.rejectionReason ? (
                                            <p
                                                className={`mt-3 rounded-xl border px-3 py-2 text-xs leading-5 ${isDayMode ? "border-rose-200 bg-rose-50 text-rose-700" : "border-rose-400/20 bg-rose-500/10 text-rose-200"}`}
                                            >
                                                {t(
                                                    "user_profile.submissions.rejected_reason",
                                                    "驳回原因"
                                                )}
                                                ：{item.rejectionReason}
                                            </p>
