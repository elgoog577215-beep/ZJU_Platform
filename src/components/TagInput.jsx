...
const TagInput = ({ value = "", onChange, type }) => {
    ...
    const handleInputChange = (e) => {
        ...
        if (val.trim()) {
            const filtered = allTags.filter(
                (tag) =>
                    tag.name.toLowerCase().includes(val.toLowerCase()) && !tags.includes(tag.name)
            );
    ...
    return (
        <div className="relative">
            <div
                className={`flex flex-wrap gap-2 px-3 py-2 rounded-xl transition-all duration-200 min-h-[44px] items-center ${isDayMode ? "bg-white/92 border border-slate-200/80 focus-within:border-indigo-400/70 focus-within:bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.55)]" : "bg-white/5 border border-white/10 focus-within:border-indigo-500/50 focus-within:bg-white/10"}`}
            >
                <AnimatePresence>
                    {tags.map((tag, index) => (
                        <motion.span
            ...
            {allTags.length > 0 && tags.length < 5 && !showSuggestions && (
                <div className="mt-2 flex flex-wrap gap-2">
                    <span
                        className={`text-sm py-1 ${isDayMode ? "text-slate-500" : "text-gray-500"}`}
                    >
                        {t("upload.popular_tags")}
                    </span>
                    {allTags
                        .slice(0, 5)
