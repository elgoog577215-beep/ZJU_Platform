        ...
        .then((cacheNames) =>
            Promise.all(
                cacheNames
                    .filter(
                        (cacheName) =>
                            STALE_CHUNK_CACHE_NAMES.has(cacheName) ||
...
const MusicRedirect = () => {
    ...
    params.delete("id");
    params.delete("music");
    return <Navigate to={`/articles${params.toString() ? `?${params.toString()}` : ""}`} replace />;
};
...
const MeRedirect = () => {
    ...
    if (canOpenProfile) {
        const target = location.search
            ? `/user/${user.id}/center${location.search}`
            : `/user/${user.id}`;
        ...
        if (bindResult || bindError) {
            if (bindResult === "success") {
                ...
                window.dispatchEvent(
                    new CustomEvent("wechat-miniapp-bind-return", {
...
const AppContent = () => {
    ...
    return (
        <div
            ...
            } ${isEventsRoute ? "events-fullscreen-shell" : ""}`}
        >
            <ResourceHints />
                ...
                >
                    <Suspense fallback={<LoadingScreen />}>
                        <Routes location={location}>
                            ...
                            <Route
                                path="/community/help"
                                element={<Navigate to="/articles?postTab=help" replace />}
                            />
                            <Route
                                path="/community/tech"
                                element={<Navigate to="/articles?postTab=tech" replace />}
                            />
                            <Route
