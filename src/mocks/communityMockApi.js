...
const applySort = (items, sort) => {
    if (sort === "hot" || sort === "likes") {
        return [...items].sort(
            (a, b) => (b.likes_count || b.likes || 0) - (a.likes_count || a.likes || 0)
        );
    }
    if (sort === "oldest") {
...
const resolveGet = (url, config) => {
    ...
    if (path === "/community/material-courses") {
        ...
        mockStore.posts
            ...
            .forEach((post) => {
                ...
                if (
                    !current.latest_at ||
                    new Date(post.updated_at || post.created_at) > new Date(current.latest_at)
                ) {
    ...
    if (path === "/articles") {
        ...
        if (tags.length)
            rows = rows.filter((item) => tags.every((t) => String(item.tags || "").includes(t)));
        rows = applySort(rows, sort);
        return toResponse(paginate(rows, params.page, params.limit));
    }
    ...
    if (path === "/community/groups") {
        const status = String(params.review_status || "approved")
            .trim()
            .toLowerCase();
        const rows =
            status === "all"
        ...
        return toResponse(
            rows.sort((a, b) => b.is_recommended - a.is_recommended || b.sort_order - a.sort_order)
        );
    }

    if (path === "/news") {
        const status = String(params.status || "approved")
            .trim()
            .toLowerCase();
...
export const resolveCommunityMock = (method, url, configOrData) => {
    ...
    if (["post", "put", "delete"].includes(normalized))
        return resolveWrite(normalized, url, configOrData);
    return null;
};
