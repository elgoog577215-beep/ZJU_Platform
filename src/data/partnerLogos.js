...
export const defaultEcosystemPartners = [
  ...
  {
    id: "default-enterprise-getui",
    profile_handle: "partner-getui",
    category: "enterprise",
    name: "每日互动",
    name_en: "Getui",
    ...
    logo_url: "/images/partner-logos/getui.svg",
    dark_logo_url: "/images/partner-logos/getui-dark.svg",
  ...
  },
  {
    ...
    logo_url: "/images/partner-logos/qoder.svg?v=2",
    dark_logo_url: "/images/partner-logos/qoder-dark.svg?v=2",
    size: "h-5 sm:h-6 lg:h-7",
    sort_order: 50,
    enabled: true,
    featured: true,
...
export const normalizeEcosystemPartner = (partner = {}) => {
  ...
  return {
    ...
    id:
      partner.id ??
      `${partner.category || "partner"}-${partner.name || Math.random()}`,
    ...
    dark_logo_url:
      partner.dark_logo_url || partner.darkLogoUrl || partner.darkSrc || "",
    link_url: partner.link_url || partner.linkUrl || "",
    sort_order:
      Number.parseInt(partner.sort_order ?? partner.sortOrder ?? 0, 10) || 0,
    enabled: normalizeBooleanFlag(partner.enabled, true),
    featured: hasExplicitScope
      ? partnerScope === CORE_PARTNER_SCOPE
      : legacyFeatured,
...
export const sortEcosystemPartners = (partners = []) =>
  [...partners].sort((left, right) => {
    const categoryDiff =
      (categoryOrder.get(left.category) ?? 99) -
      (categoryOrder.get(right.category) ?? 99);
...
export const groupEcosystemPartners = (
  partners = [],
  { scope = CORE_PARTNER_SCOPE } = {},
) => {
...
export const getPartnerProfilePath = (partner = {}) => {
  const explicitHandle =
    partner.profile_handle || partner.profileHandle || partner.handle;
...
export const hackathonPartnerLogos =
  defaultEnterprisePartners.map(toLegacyLogo);
