...
const normalizeWeatherData = (data) => {
    ...
    return {
        ...data,
        temperature,
        weathercode: Number.isFinite(Number(data?.weathercode)) ? Number(data.weathercode) : 0,
        windspeed: Number.isFinite(Number(data?.windspeed)) ? Number(data.windspeed) : 0,
    ...
    useEffect(() => {
        ...
        const timeoutId = window.setTimeout(async () => {
            try {
                ...
                const nextWeather =
                    normalizeWeatherData(response.data?.current_weather) || buildFallbackWeather();
        ...
        try {
            ...
            const dedupedApiResults = apiResults.filter(
                (apiCity) =>
                    !localResults.some(
                        (localCity) =>
                            localCity.name === apiCity.name && localCity.admin1 === apiCity.admin1
                    )
            );
