...
export const useEcosystemPartners = () => {
    ...
    const supporterGroups = useMemo(
        () =>
            groupEcosystemSupporters(partners, { scope: CORE_PARTNER_SCOPE }).filter(
                (group) => group.partners.length > 0
            ),
        [partners]
    );
    ...
    const enterpriseLogoRows = useMemo(() => chunkPartners(enterpriseLogos, 3), [enterpriseLogos]);
    ...
    useEffect(() => {
        ...
        const channel =
            "BroadcastChannel" in window ? new BroadcastChannel(ECOSYSTEM_PARTNERS_CHANNEL) : null;
