...
- Action / proof：桌面首屏先显示简洁有冲击力的“学习社区”标题，再显示四个大型功能入口；选择后同一组入口通过共享布局动画收缩为半透明顶部栏目，返回位于左侧，页面自己的搜索与发布入口承接后续操作。
...
- Memorable moment：桌面大标题把四个知识入口压到视口中下部，选择后入口原位收拢为“左侧返回 + 四栏目 + 必要操作”的轻透命令栏；移动端保持紧凑入口和真实内容顺序。
